...
function _getAuxQrInst() {
  if (!_auxQrInst && window.Html5Qrcode) {
    ...
    try { _auxQrInst = new Html5Qrcode('_auxScanDiv', { verbose: false }); } catch(_) {}
  }
  return _auxQrInst;
}
...
function onCodigoEscaneado(codigo) {
  ...
  // ══ MODO INVENTARIO: doble-confirmación (exactitud > velocidad) ══════
  // Requiere leer el mismo código 2 veces → evita registrar un código falso
  if (_scannerModo === 'inventario') {
    if (_ultimoCodLeido === codLimpio) {
      _ultimoCodLeido = null;
      _lastScanTs = now;
      cerrarScanner();
      onCodigoInventario(codLimpio);
    } else {
      _ultimoCodLeido = codLimpio;
      setScannerStatus('✔ Confirmando… ' + codLimpio.slice(0, 22));
    }
    return;
  }
