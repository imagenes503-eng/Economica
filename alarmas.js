...
async function iniciarScanner() {
  ...
  try {
    ...
    // ── Estrategia: obtener lista de cámaras y priorizar la trasera ────
    // getCameras() antes de instanciar evita conflictos con getUserMedia
    let camId = null;
    try {
      const cameras = await Html5Qrcode.getCameras();
      if (cameras && cameras.length > 0) {
        // Buscar por label: "back", "environment", "rear", "posterior"
