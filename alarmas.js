...
// Recoge lecturas durante VOTE_MS y procesa la más frecuente
// Elimina falsas lecturas por frames borrosos
const VOTE_MS        = 400;   // 400ms @ 12fps = 4-5 frames → suficiente para mayoría
let _voteBuffer      = {};    // { codigo: conteo }
let _voteTimer       = null;  // timer para cerrar votación
let _voteActive      = false; // hay votación en curso
let _lastVoteCod     = '';    // anti-duplicado por frame
let _lastVoteTs      = 0;     // timestamp del último frame procesado
...
async function iniciarScanner() {
  // ── 1. Limpiar instancia previa ──────────────────────────────────────
  if (_html5Qr) {
    try { if (_scannerRunning) await _html5Qr.stop(); } catch(_) {}
  ...
  try {
    ...
    // ── 2. Formatos explícitos + hints de rendimiento ─────────────────
    // TRY_HARDER y ALSO_INVERTED están en Html5QrcodeScannerState/hints
    // En html5-qrcode 2.3.8 se pasan como `experimentalFeatures`
    const formats = [];
    if (window.Html5QrcodeSupportedFormats) {
      const F = Html5QrcodeSupportedFormats;
      const wanted = [
        'EAN_13','EAN_8','CODE_128','QR_CODE',
        'CODE_39','ITF','UPC_A','UPC_E',
        'AZTEC','DATA_MATRIX','PDF_417','MAXICODE',
        'RSS_14','RSS_EXPANDED'
      ];
      wanted.forEach(k => { if (F[k] !== undefined) formats.push(F[k]); });
    }

    // ── 3. Config: máxima velocidad + frame completo ─────────────────
    const config = {
      fps: 12,                        // 12 fps = ~80ms por frame — óptimo para móvil
      aspectRatio: 1.5,
      rememberLastUsedCamera: false,
      // Frame completo — sin qrbox para máximo rango y detección
    };
    if (formats.length) config.formatsToSupport = formats;

    // TRY_HARDER + ALSO_INVERTED via experimentalFeatures (2.3.8+)
    try {
      config.experimentalFeatures = {
        useBarCodeDetectorIfSupported: true  // API nativa del navegador si disponible
      };
    } catch(_) {}

    // ── 4. Cámara trasera con cache ──────────────────────────────────
    let camConstraint;
    if (_rearDeviceId) {
    ...
    // ── 5. Iniciar — con fallback a facingMode si deviceId falla ─────
    _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
    try {
      await _html5Qr.start(camConstraint, config, onCodigoEscaneado, () => {});
    } catch(_) {
      _rearDeviceId = null;
      try { _html5Qr.clear(); } catch(_) {}
      _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
      await _html5Qr.start({ facingMode: { ideal: 'environment' } }, config, onCodigoEscaneado, () => {});
    }

    _scannerRunning = true;
    setScannerStatus('📷 Apunta al código de barras…');

    // ── 6. Foco continuo post-apertura ───────────────────────────────
    setTimeout(async () => {
      try {
        const v = document.querySelector('#scannerRegion video');
    ...
    // ── 7. Ocultar controles extra de html5-qrcode ───────────────────
    setTimeout(() => {
      document.querySelectorAll(
        '#scannerRegion button:not([id]), #scannerRegion select, #scannerRegion img[alt="Info"]'
...
// ── Procesar el ganador de la votación ──────────────────────────────
function _procesarVotoGanador() {
  if (!_voteActive) return;
  _voteActive = false;
  clearTimeout(_voteTimer);
  _voteTimer = null;

  // Elegir el código con más votos
  let ganador = null, maxVotos = 0;
  for (const [cod, votos] of Object.entries(_voteBuffer)) {
    if (votos > maxVotos) { maxVotos = votos; ganador = cod; }
  }
  _voteBuffer = {};

  if (!ganador || _scannerCerrando) return;

  if (navigator.vibrate) navigator.vibrate([40, 20, 40]);

  // ── Modo inventario ────────────────────────────────────────────────
  if (_scannerModo === 'inventario') {
    onCodigoInventario(ganador);
    return;
  }

  // ── Modo venta: buscar producto ────────────────────────────────────
  let prod = productos.find(p => (p.cod || '').trim() === ganador);
  if (!prod) {
    prod = productos.find(p => {
      const c = (p.cod || '').trim();
      return c && c.replace(/^0+/, '') === ganador.replace(/^0+/, '');
    });
  }
  ...
  if (prod) {
    ...
    toast('❌ Código ' + ganador + ' no está en el inventario', true);
    const inp = document.getElementById('busVenta');
    if (inp) inp.value = ganador;
  }
}

// ── Recibir lectura de la cámara ─────────────────────────────────────
function onCodigoEscaneado(codigo) {
  ...
  // Anti-duplicado inmediato: mismo código en < 100ms = mismo frame procesado dos veces
  const now = Date.now();
  if (codLimpio === _lastVoteCod && now - _lastVoteTs < 100) return;
  _lastVoteCod = codLimpio;
  _lastVoteTs  = now;

  // Acumular votos para este código
  _voteBuffer[codLimpio] = (_voteBuffer[codLimpio] || 0) + 1;
  const votos = _voteBuffer[codLimpio];

  if (!_voteActive) {
    // Primera lectura: abrir ventana de votación
    _voteActive = true;
    setScannerStatus('📡 Leyendo…');
    _voteTimer = setTimeout(_procesarVotoGanador, VOTE_MS);
  }

  // Si el código lidera con 3+ votos → procesar ya sin esperar timeout
  if (votos >= 3) {
    _procesarVotoGanador();
  }
}
...
async function cerrarScanner() {
  ...
  _lastVoteTs  = 0;
  _scannerRunning = false;
  // Ocultar modal de inmediato sin esperar stop()
  const modal = document.getElementById('modalScanner');
  if (modal) modal.style.display = 'none';
  ocultarResultadoScanner();
  // Detener cámara en background
  const inst = _html5Qr;
  _html5Qr = null;
  if (inst) {
...
function onCodigoInventario(codigo) {
  ...
  const existe = productos.find(p =>
    (p.cod || '').trim() === codLimpio ||
    (p.cod || '').replace(/^0+/, '') === codLimpio.replace(/^0+/, '')
  );

  // 1. Cerrar scanner
  cerrarScanner();

  // 2. Escribir en el campo DESPUÉS de que el modal cierre (siguiente frame)
  requestAnimationFrame(() => {
    ...
    if (existe) {
      toast('⚠ Código ya existe en: ' + existe.nom, true);
    } else {
      toast('✓ Código listo: ' + codLimpio);
    }
  });
}
