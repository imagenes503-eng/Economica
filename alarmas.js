...
async function _detenerScanInterno() {
  ...
  // Cancelar el loop activo (RAF o setTimeout — ambas llamadas son no-op si no aplican)
  if (_scanRafId !== null) {
...
async function iniciarScanner() {
  ...
  try {
    ...
    if (usaNativo) {
      ...
      // ZXing-JS síncrono — setTimeout a 50ms (~20 fps) para no bloquear el UI thread
      const scan = () => {
        if (!_scannerRunning) return;
        if (video.readyState >= 2 && video.videoWidth > 0) {
          ...
          } catch(_) { /* NotFoundException — sin código visible, completamente normal */ }
        }
        if (_scannerRunning) _scanRafId = setTimeout(scan, 50);
      };
      // Pausa inicial para que el video estabilice el foco y resolución
      _scanRafId = setTimeout(scan, 400);
    }
