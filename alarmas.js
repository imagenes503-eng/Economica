...
async function iniciarScanner() {
  ...
  try {
    ...
    // ── Config: solo cámara, formatos de barras de tienda ───────────────
    const config = { fps: 15, qrbox: { width: 260, height: 110 }, aspectRatio: 1.7 };
    try {
    ...
    // ── Obtener cámara trasera: pedir permiso y leer deviceId ───────────
    // Estrategia en un solo paso: getUserMedia con environment,
    // leer el deviceId exacto, cerrar ese stream, abrir Html5Qrcode con ese ID.
    // Así no hay conflicto de dos streams simultáneos.
    let camConstraint = { facingMode: { exact: 'environment' } };
    ...
    try {
      ...
      // Verificar que sea cámara trasera por facing
      const facing = settings.facingMode || '';
      const esTraser = facing === 'environment' || !facing;

      tmpStream.getTracks().forEach(t => t.stop());
      await new Promise(r => setTimeout(r, 350)); // esperar liberación

      if (deviceId && esTraser) {
        camConstraint = { deviceId: { exact: deviceId } };
      }
    } catch(_) {
      // Sin permiso todavía — Html5Qrcode mostrará el diálogo
    ...
    try {
      ...
      // deviceId exacto puede fallar en algunos Android → fallback a facingMode
      try { _html5Qr.clear(); } catch(_) {}
      _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
      await _html5Qr.start(
