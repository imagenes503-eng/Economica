...
// Motor dual: BarcodeDetector nativo (Chrome) + ZXing-JS puro JS (Firefox/Safari)
// Elimina dependencia de html5-qrcode/WASM que falla silenciosamente en Firefox.
let _scannerRunning = false;
let _lastScanTs     = 0;
let _scannerModo    = 'venta';
let _scanStream     = null;   // MediaStream activo
let _scanRafId      = null;   // handle RAF o setTimeout según motor
let _scanCanvas     = null;   // canvas oculto para captura de frames
let _scanCtx        = null;
let _zxingReader    = null;   // BrowserMultiFormatReader para Firefox/Safari

// Formatos para BarcodeDetector nativo (Chrome/Edge)
const _BD_FORMATS = ['ean_13','ean_8','upc_a','upc_e','code_128','code_39','itf','qr_code'];

// ZXing-JS puro JavaScript — sin WASM, funciona en Firefox y Safari
const _ZXING_CDN = 'https://cdn.jsdelivr.net/npm/@zxing/library@0.19.2/umd/index.min.js';

// Inyectar CSS de animación de línea de escaneo (una sola vez al cargar)
(function() {
...
async function _loadZxing() {
  ...
  return new Promise((resolve, reject) => {
    ...
    s.src = _ZXING_CDN;
    s.onload = resolve;
    s.onerror = () => reject(new Error('No se pudo cargar ZXing-JS'));
    document.head.appendChild(s);
  });
}
...
async function _detenerScanInterno() {
  ...
  // Detener loop de Chrome (RAF) o Firefox (RAF interno de ZXing)
  if (_scanRafId !== null) {
    cancelAnimationFrame(_scanRafId);
    clearTimeout(_scanRafId);
    _scanRafId = null;
  }
  // Detener ZXing BrowserMultiFormatReader (Firefox/Safari)
  if (_zxingReader) {
    try { _zxingReader.reset(); } catch(_) {}
    _zxingReader = null;
  }
  // Liberar stream de cámara (Chrome — Firefox lo libera ZXing internamente)
  if (_scanStream) {
...
async function iniciarScanner() {
  ...
  try {
    ...
    // ── 2. Elemento <video> en #scannerRegion ─────────────────────────
    const video = document.createElement('video');
    video.srcObject = _scanStream;
    video.setAttribute('playsinline', '');
    video.setAttribute('autoplay', '');
    video.muted = true;
    video.style.cssText = 'width:100%;height:260px;object-fit:cover;border-radius:12px;display:block;background:#000;';
    region.appendChild(video);

    // ── 3. Overlay con caja y línea de escaneo ────────────────────────
    const overlay = document.createElement('div');
    overlay.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:260px;display:flex;align-items:center;justify-content:center;pointer-events:none;';
    overlay.innerHTML = `
      <div style="position:relative;width:280px;height:120px;border:2.5px solid #38bdf8;border-radius:10px;box-shadow:0 0 0 2000px rgba(0,0,0,0.45);">
        <div style="position:absolute;top:-3px;left:-3px;width:22px;height:22px;border-top:4px solid #38bdf8;border-left:4px solid #38bdf8;border-radius:5px 0 0 0;"></div>
        <div style="position:absolute;top:-3px;right:-3px;width:22px;height:22px;border-top:4px solid #38bdf8;border-right:4px solid #38bdf8;border-radius:0 5px 0 0;"></div>
        <div style="position:absolute;bottom:-3px;left:-3px;width:22px;height:22px;border-bottom:4px solid #38bdf8;border-left:4px solid #38bdf8;border-radius:0 0 0 5px;"></div>
        <div style="position:absolute;bottom:-3px;right:-3px;width:22px;height:22px;border-bottom:4px solid #38bdf8;border-right:4px solid #38bdf8;border-radius:0 0 5px 0;"></div>
        <div style="position:absolute;left:6px;right:6px;height:2.5px;background:linear-gradient(90deg,transparent,#38bdf8,transparent);animation:_scanAnim 1.8s ease-in-out infinite;"></div>
      </div>`;
    region.appendChild(overlay);

    await video.play();

    // ── 4. Canvas oculto para frames ──────────────────────────────────
    _scanCanvas = document.createElement('canvas');
    _scanCtx    = _scanCanvas.getContext('2d', { willReadFrequently: true });

    // ── 5. Seleccionar motor ──────────────────────────────────────────
    //   Chrome/Edge: BarcodeDetector API nativa — rápida, sin dependencias
    //   Firefox/Safari/otros: ZXing-JS JavaScript puro — sin WASM, universal
    const usaNativo = 'BarcodeDetector' in window;
    let detector    = null;
    let zxingReader = null;

    if (usaNativo) {
      // ── CHROME/EDGE: BarcodeDetector API nativa ──────────────────────
      detector = new BarcodeDetector({ formats: _BD_FORMATS });
    }

    _scannerRunning = true;
    setScannerStatus('📷 Apunta al código de barras…');

    // ── 6. Loop de decodificación según motor ─────────────────────────
    if (usaNativo) {
      // BarcodeDetector es async — flag _detecting evita llamadas simultáneas
      let _detecting = false;
      const scan = async () => {
        if (!_scannerRunning) return;
        if (!_detecting && video.readyState >= 2 && video.videoWidth > 0) {
          _detecting = true;
          try {
            _scanCanvas.width  = video.videoWidth;
            _scanCanvas.height = video.videoHeight;
            _scanCtx.drawImage(video, 0, 0);
            const results = await detector.detect(_scanCanvas);
            if (results.length > 0) onCodigoEscaneado(results[0].rawValue);
          } catch(_) {}
          _detecting = false;
        }
        if (_scannerRunning) _scanRafId = requestAnimationFrame(scan);
      };
      _scanRafId = requestAnimationFrame(scan);

    } else {
      // ── FIREFOX/SAFARI: ZXing.BrowserMultiFormatReader ───────────────
      // Usa la API de ALTO NIVEL de ZXing — maneja el scan loop internamente.
      // Evita la API de bajo nivel (MultiFormatReader + RGBLuminanceSource)
      // que falla silenciosamente en Firefox por exposición incompleta del UMD.
      ...
      // TRY_HARDER: prueba multiples orientaciones, angulos y binarizaciones.
      // Sin esto ZXing solo lee codigos perfectamente centrados y alineados —
      // esta es la diferencia principal con Chrome que lo hace nativamente.
      hints.set(ZXing.DecodeHintType.TRY_HARDER, true);
      // ALSO_INVERTED: decodifica codigos con colores invertidos (fondo oscuro)
      hints.set(ZXing.DecodeHintType.ALSO_INVERTED, true);

      // 100ms entre scans = ~10fps — equilibrio velocidad/CPU para Firefox
      _zxingReader = new ZXing.BrowserMultiFormatReader(hints, 100);

      // decodeFromConstraints: ZXing maneja getUserMedia, stream y scan loop.
      // El video element ya está en el DOM — ZXing lo usa directamente.
      // Nota: NO await — devuelve Promise que nunca resuelve (loop continuo).
      _zxingReader.decodeFromConstraints(
        ...
        (result, error) => {
          ...
          // error = NotFoundException cuando no hay código visible → normal, ignorar
        }
      ).catch(err => {
        // Solo llega aquí si getUserMedia falla (permiso denegado, sin cámara)
        if (!_scannerRunning) return;
        _scannerRunning = false;
        const msg = err ? (err.message || String(err)) : '';
        const esPermiso = /NotAllowed|Permission|denied/i.test(msg);
        region.innerHTML = esPermiso
          ? `<div style="padding:24px;text-align:center;color:#fff;">
               <div style="font-size:40px;margin-bottom:12px;">📵</div>
               <div style="font-size:15px;font-weight:900;margin-bottom:8px;">Permiso de cámara denegado</div>
               <div style="font-size:12px;color:rgba(255,255,255,0.8);line-height:1.8;margin-bottom:14px;">
                 Toca el 🔒 candado → Permisos → Cámara → Permitir, luego recarga.
               </div>
               <button onclick="location.reload()" style="background:#16a34a;color:#fff;border:none;border-radius:10px;padding:10px 22px;font-size:14px;font-weight:900;cursor:pointer;">🔄 Recargar</button>
             </div>`
          : `<div style="padding:24px;text-align:center;color:#fff;">
               <div style="font-size:36px;margin-bottom:10px;">⚠️</div>
               <div style="font-size:14px;font-weight:900;">Error al iniciar cámara</div>
               <div style="font-size:11px;color:rgba(255,255,255,0.6);margin-top:6px;">${msg.slice(0,120)}</div>
             </div>`;
        setScannerStatus(esPermiso ? '⚠ Permiso denegado' : '⚠ ' + msg.slice(0,50));
      });
    ...
    const msg = err ? (err.message || String(err)) : '';
    const esPermiso  = /NotAllowed|Permission|denied/i.test(msg);
    const esNoCamera = /NotFound|DevicesNotFound|Requested device/i.test(msg);
    ...
    region.innerHTML = esPermiso
      ? `<div style="padding:24px;text-align:center;color:#fff;">
           ...
           <div style="font-size:15px;font-weight:900;margin-bottom:8px;">Permiso de cámara denegado</div>
           <div style="font-size:12px;color:rgba(255,255,255,0.8);line-height:1.8;margin-bottom:14px;">
             Toca el 🔒 candado en la barra del navegador<br>→ Permisos → Cámara → Permitir<br>Luego recarga la página.
           </div>
           <button onclick="location.reload()" style="background:#16a34a;color:#fff;border:none;border-radius:10px;padding:10px 22px;font-size:14px;font-weight:900;cursor:pointer;">🔄 Recargar página</button>
         </div>`
      : `<div style="padding:24px;text-align:center;color:#fff;">
           <div style="font-size:36px;margin-bottom:10px;">⚠️</div>
           <div style="font-size:14px;font-weight:900;">${esNoCamera ? 'No se encontró cámara' : 'Error al iniciar cámara'}</div>
           <div style="font-size:11px;color:rgba(255,255,255,0.6);margin-top:6px;">${msg.slice(0,120)}</div>
         </div>`;

    setScannerStatus(esPermiso ? '⚠ Permiso denegado' : esNoCamera ? '⚠ No se encontró cámara' : '⚠ ' + msg.slice(0,50));
  }
}
...
function onCodigoEscaneado(codigo) {
  ...
  const now = Date.now();
  if (now - _lastScanTs < 600) return; // anti-rebote 600ms
  _lastScanTs = now;
