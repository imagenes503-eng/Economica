...
async function iniciarScanner() {
  ...
  try {
    ...
    const config = { fps: 25, qrbox: { width: 320, height: 140 }, aspectRatio: 1.5, rememberLastUsedCamera: false };
    try {
      ...
      try {
        const tmp = await navigator.mediaDevices.getUserMedia({
          video: {
            ...
            width:  { ideal: 1920 },
            height: { ideal: 1080 },
            zoom:   { ideal: 1.5 }   // zoom óptico si el dispositivo lo soporta
          }
        });
    ...
    // ── Optimizar cámara: zoom + foco continuo para lectura a distancia ──
    setTimeout(async () => {
      try {
        ...
        const constraints = {};
        // Foco continuo — esencial para leer desde más lejos
        if (caps.focusMode && caps.focusMode.includes('continuous')) {
          constraints.focusMode = 'continuous';
        }
        // Zoom moderado (1.8x) — más rango sin perder detalle
        if (caps.zoom) {
          const minZ = caps.zoom.min || 1;
          const maxZ = caps.zoom.max || 1;
          constraints.zoom = Math.min(1.8, maxZ);
        }
...
function onCodigoEscaneado(codigo) {
  ...
  if (_scannerModo === 'inventario') {
    // Doble confirmación: mismo código 2 veces = válido
    if (_ultimoCodLeido === codLimpio) {
      ...
      setScannerStatus('✔ Confirmando ' + codLimpio + '…');
    }
    return;
  }

  // ── Doble confirmación anti-lectura-falsa ────────────────────────────
  if (_ultimoCodLeido !== codLimpio) {
    _ultimoCodLeido = codLimpio;
    setScannerStatus('✔ Confirmando ' + codLimpio + '…');
    return;
