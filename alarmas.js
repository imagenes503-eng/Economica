// =====================================================================
//  SCANNER CÓDIGO DE BARRAS — POS Profesional
//  ▸ 17 formatos (todos los estándares de supermercado / retail)
//  ▸ 12 fps ≈ 80 ms/frame  →  velocidad óptima + tiempo para TRY_HARDER
//  ▸ Loop auxiliar ALSO_INVERTED — detecta códigos oscuros/invertidos,
//    con poca luz o impresos sobre fondo negro
//  ▸ Debounce global 500 ms — evita doble-lectura cruzada entre loops
//  ▸ Lectura única en venta (sin doble-confirmación) para máxima rapidez
//  ▸ Doble-confirmación solo en inventario (exactitud > velocidad)
// =====================================================================
let _html5Qr         = null;
let _scannerRunning  = false;
let _lastScanTs      = 0;        // timestamp del último código aceptado
let _scannerCerrando = false;
let _rearDeviceId    = null;
let _ultimoCodLeido  = null;     // inventario: doble-confirmación
let _auxDecodeTimer  = null;     // handle del loop ALSO_INVERTED
let _auxCanvas       = null;     // canvas reutilizable (inversión de píxeles)
let _auxQrInst       = null;     // instancia Html5Qrcode dedicada a decodificar imágenes

// ── Todos los formatos de código de barras usados en supermercados ────
function _buildScannerFormats() {
  if (!window.Html5QrcodeSupportedFormats) return undefined;
  const F = Html5QrcodeSupportedFormats;
  return [
    // ── 1D lineales ──────────────────────────────────────────────────
    F.EAN_13,            // Estándar internacional → la mayoría de productos
    F.EAN_8,             // Versión compacta EAN (envases pequeños)
    F.UPC_A,             // Estándar USA (supermercados, tiendas de conveniencia)
    F.UPC_E,             // Versión compacta UPC (envases pequeños USA)
    F.CODE_128,          // Logística, cajas, etiquetas de envío, lotes
    F.CODE_39,           // Industria / inventario legacy (muy usado en Centroamérica)
    F.CODE_93,           // Retail compacto, almacenes, cadenas de farmacia
    F.CODABAR,           // Farmacias, laboratorios, bancos de sangre, libros
    F.ITF,               // Cajas de cartón GS1 (Interleaved 2 of 5) — almacén
    // ── GS1 DataBar (críticos para supermercados) ─────────────────────
    F.RSS_14,            // Productos frescos con peso variable (carnes, frutas, verduras)
    F.RSS_EXPANDED,      // DataBar expandido: incluye fecha de vencimiento, número de lote
    // ── 2D ───────────────────────────────────────────────────────────
    F.QR_CODE,           // QR universal (pagos, cupones, menús, info del producto)
    F.DATA_MATRIX,       // Medicamentos, envases pequeños, electrónica, semillas
    F.PDF_417,           // Tarjetas de identificación, boarding pass, membresías
    F.AZTEC,             // Retail especializado, transporte público, boletos
    F.MAXICODE,          // Paquetería UPS / FedEx / DHL
    // ── Extensiones EAN/UPC ──────────────────────────────────────────
    F.UPC_EAN_EXTENSION, // Add-on codes (revistas con precio, productos con peso)
  ];
}

// ── Instancia auxiliar para decodificar imágenes sin iniciar cámara ───
// CRÍTICO: debe incluir los mismos 17 formatos que el scanner principal.
// Sin formatsToSupport, html5-qrcode solo decodifica EAN_13 y QR_CODE
// (los dos defaults de ZXing) → todos los demás formatos son invisibles.
function _getAuxQrInst() {
  if (!_auxQrInst && window.Html5Qrcode) {
    let d = document.getElementById('_auxScanDiv');
    if (!d) {
      d = document.createElement('div');
      d.id = '_auxScanDiv';
      d.style.cssText = 'position:fixed;left:-9999px;top:-9999px;width:1px;height:1px;overflow:hidden;pointer-events:none;';
      document.body.appendChild(d);
    }
    try {
      const fmts = _buildScannerFormats();
      const opts = fmts ? { verbose: false, formatsToSupport: fmts } : { verbose: false };
      _auxQrInst = new Html5Qrcode('_auxScanDiv', opts);
    } catch(_) {}
  }
  return _auxQrInst;
}

// Fuerza recreación de la instancia aux (llamar al limpiar el scanner)
function _resetAuxQrInst() {
  if (_auxQrInst) {
    try { _auxQrInst.clear(); } catch(_) {}
    _auxQrInst = null;
  }
}

// ── Loop ALSO_INVERTED: captura frame → invierte píxeles → decodifica ─
async function _startAlsoInvertedLoop() {
  // Esperar a que el video esté estable
  ...
  if (!_auxCanvas) {
    _auxCanvas = document.createElement('canvas');
  }
  const ctx = _auxCanvas.getContext('2d', { willReadFrequently: true });

  async function _tick() {
    if (!_scannerRunning || _scannerCerrando) return;

    if (video.readyState >= 2 && video.videoWidth > 0) {
      try {
        // Capturar frame a resolución reducida (más rápido de procesar)
        const W = Math.min(video.videoWidth,  480);
        const H = Math.min(video.videoHeight, 270);
        if (_auxCanvas.width !== W || _auxCanvas.height !== H) {
          _auxCanvas.width = W; _auxCanvas.height = H;
        }
        ctx.drawImage(video, 0, 0, W, H);

        // Inversión de píxeles — ALSO_INVERTED
        ...
        for (let i = 0; i < d.length; i += 4) {
          ...
          d[i+1] = 255 - d[i+1];
          d[i+2] = 255 - d[i+2];
          // canal alpha sin cambio
        }
        ctx.putImageData(img, 0, 0);

        // Decodificar imagen invertida
        const blob = await new Promise(res => _auxCanvas.toBlob(res, 'image/jpeg', 0.82));
        if (!blob || !_scannerRunning) return;

        const file  = new File([blob], 'inv.jpg', { type: 'image/jpeg' });
        const auxQr = _getAuxQrInst();
        if (!auxQr) return;

        const result = await auxQr.scanFileV2(file, false);
        if (result?.decodedText && _scannerRunning && !_scannerCerrando) {
          onCodigoEscaneado(result.decodedText);
        }
      } catch (_) {
        // No match es lo normal — continuar silenciosamente
      }
    }

    // Próximo frame en 80 ms (≈ 12 fps / TRY_HARDER timing)
    if (_scannerRunning && !_scannerCerrando) {
      _auxDecodeTimer = setTimeout(_tick, 80);
    }
  }
...
function _stopAlsoInvertedLoop() {
  if (_auxDecodeTimer) { clearTimeout(_auxDecodeTimer); _auxDecodeTimer = null; }
}
...
async function iniciarScanner() {
  // ── Limpiar instancia previa y detener loop ALSO_INVERTED ────────────
  _stopAlsoInvertedLoop();
  if (_html5Qr) {
    if (_scannerRunning) { try { await _html5Qr.stop(); } catch(_) {} }
  ...
  try {
    ...
    //   fps: 12  →  ~83 ms/frame: tiempo suficiente para TRY_HARDER
    //   qrbox más ancho: captura códigos largos (CODE_128, PDF_417, RSS)
    const config = {
      fps: 12,
      qrbox: { width: 310, height: 130 },
      aspectRatio: 1.8,
      rememberLastUsedCamera: false,
    };

    // Agregar los 17 formatos de supermercado
    try {
      const fmts = _buildScannerFormats();
      if (fmts) config.formatsToSupport = fmts;
    } catch(_) {}

    // ── Selección de cámara trasera con caché ────────────────────────
    let camConstraint;
    if (_rearDeviceId) {
      camConstraint = { deviceId: { exact: _rearDeviceId } };
    } else {
      try {
        ...
        if (s.deviceId && (s.facingMode === 'environment' || !s.facingMode)) {
          _rearDeviceId = s.deviceId;
          camConstraint = { deviceId: { exact: _rearDeviceId } };
        } else {
          camConstraint = { facingMode: { ideal: 'environment' } };
        }
      } catch(_) { camConstraint = { facingMode: { ideal: 'environment' } }; }
    }

    _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });

    try {
      await _html5Qr.start(camConstraint, config, (c) => onCodigoEscaneado(c), () => {});
    } catch(_e1) {
      // Caché de deviceId falló → limpiar y usar facingMode
      _rearDeviceId = null;
      try { _html5Qr.clear(); } catch(_) {}
      _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
      await _html5Qr.start({ facingMode: { ideal: 'environment' } }, config, (c) => onCodigoEscaneado(c), () => {});
    }

    _scannerRunning = true;
    setScannerStatus('📷 Apunta al código de barras…');

    // Ocultar controles extra inyectados por html5-qrcode
    setTimeout(() => {
      document.querySelectorAll('#scannerRegion button:not([id]), #scannerRegion select, #scannerRegion img[alt="Info"]')
        .forEach(el => { el.style.display = 'none'; });
    }, 900);

    // Iniciar loop ALSO_INVERTED (detecta códigos oscuros/invertidos)
    _startAlsoInvertedLoop();
    ...
    if (esPermiso) {
      ...
      setScannerStatus('⚠ Permiso de cámara bloqueado');
    } else if (esNoCamera) {
      if (region) region.innerHTML = `
        <div style="padding:24px;text-align:center;color:#fff;">
          ...
          <div style="font-size:12px;color:rgba(255,255,255,0.75);margin-top:6px;">Usa el buscador manual o un lector de código de barras físico (USB/Bluetooth).</div>
        </div>`;
...
// ── Callback único para ambos loops (principal + ALSO_INVERTED) ────────
function onCodigoEscaneado(codigo) {
  if (_scannerCerrando) return;
  ...
  // ══ MODO INVENTARIO: lectura única + debounce 800 ms ═══════════════
  // La doble-confirmación fue eliminada porque:
  //   a) El loop ALSO_INVERTED puede leer el mismo código con mínimo ruido
  //      provocando que _ultimoCodLeido nunca coincida → scanner bloqueado.
  //   b) El debounce de 800 ms es suficiente para evitar doble-disparo.
  if (_scannerModo === 'inventario') {
    if (now - _lastScanTs < 800) return;   // debounce anti-doble-disparo
    _lastScanTs = now;
    _ultimoCodLeido = null;
    if (navigator.vibrate) navigator.vibrate([60, 30, 60]);
    onCodigoInventario(codLimpio);         // cerrarScanner lo hace onCodigoInventario
    return;
  }

  // ══ MODO VENTA: lectura inmediata + debounce 500 ms ══════════════════
  // El debounce evita que el loop ALSO_INVERTED y el loop principal
  // disparen el mismo código dos veces en < 500 ms.
  if (now - _lastScanTs < 500) return;
  _lastScanTs = now;

  if (navigator.vibrate) navigator.vibrate([40, 20, 40]);

  // ── Buscar producto — con normalización de ceros iniciales ───────────
  let prod = productos.find(p => (p.cod || '').trim() === codLimpio);
  if (!prod) {
    // EAN/UPC con ceros iniciales distintos al almacenado
    const codNorm = codLimpio.replace(/^0+/, '');
    prod = productos.find(p => {
      const c = (p.cod || '').trim();
      return c && c.replace(/^0+/, '') === codNorm;
    });
  }
  ...
  if (prod) {
    ...
    toast('❌ Código ' + codLimpio + ' no está en el inventario', true);
...
async function cerrarScanner() {
  if (_scannerCerrando) return;
  _scannerCerrando = true;
  _ultimoCodLeido  = null;
  _scannerRunning  = false;

  // Detener loop ALSO_INVERTED de inmediato
  _stopAlsoInvertedLoop();

  // Ocultar modal sin esperar stop() de la cámara
  const modal = document.getElementById('modalScanner');
  if (modal) modal.style.display = 'none';
  ocultarResultadoScanner();

  // Detener cámara en background (no bloquea la UI)
  const inst = _html5Qr;
  _html5Qr = null;
  if (inst) {
  ...
  // Resetear instancia aux para que se recree con los formatos correctos
  // la próxima vez que el loop ALSO_INVERTED la necesite.
  _resetAuxQrInst();
  _scannerModo     = 'venta';
  _scannerCerrando = false;
}
...
function onCodigoInventario(codigo) {
  // NOTA: NO hay guard de _lastScanTs aquí.
  // El debounce de 800 ms ya se aplica en onCodigoEscaneado (inventario).
  // Agregar un guard aquí causaba race condition: _lastScanTs se seteaba
  // en onCodigoEscaneado justo antes de llamar esta función → always return.
  if (navigator.vibrate) navigator.vibrate([60, 30, 60]);
  const codLimpio = (codigo || '''').trim();
