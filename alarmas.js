...
const VOTE_MS        = 500;   // ventana de votación en ms
...
async function iniciarScanner() {
  ...
  try {
    ...
    // SIN qrbox: analiza el frame completo → máximo rango y cobertura
    // fps=20: balance entre velocidad de lectura y consumo de CPU
    const config = {
      fps: 20,
      // qrbox ELIMINADO — el box restringía área de escaneo y cortaba el rango
      aspectRatio: 1.5,
      rememberLastUsedCamera: false
    };
    // Sin restricción de formatos → lee todos los tipos de código de barras

    // ── Cámara trasera con cache para apertura instantánea ───────────────
    let camConstraint;
    if (_rearDeviceId) {
    ...
    try {
      ...
      // deviceId falló → limpiar cache y usar facingMode
      _rearDeviceId = null;
    ...
    // ── Optimizar cámara tras apertura: foco continuo + zoom ───────────
    setTimeout(async () => {
      try {
        const videoEl = document.querySelector('#scannerRegion video');
        if (!videoEl) return;
        ...
        const adv = {};
        // Foco continuo: reenfoca automáticamente a cualquier distancia
        if (caps.focusMode?.includes('continuous')) adv.focusMode = 'continuous';
        // Sin zoom forzado — el sensor usa resolución máxima a cualquier distancia
        // El zoom artificial recorta el sensor y reduce el rango real
    ...
    setTimeout(() => {
      const extras = document.querySelectorAll('#scannerRegion button:not([id]), #scannerRegion select, #scannerRegion img[alt="Info"]');
...
function onCodigoEscaneado(codigo) {
  ...
  if (!_voteActive) {
    // Primera lectura: iniciar ventana de votación
    _voteActive = true;
    setScannerStatus('✔ Leyendo…');
    _voteTimer = setTimeout(_procesarVotoGanador, VOTE_MS);
  } else {
    // Si un código ya tiene 4+ votos antes del timeout, procesar inmediatamente
    if (_voteBuffer[codLimpio] >= 4) {
