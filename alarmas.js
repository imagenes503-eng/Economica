...
async function iniciarScanner() {
  try {
    ...
    // Verificar/solicitar permiso de cámara ANTES de inicializar html5-qrcode
    // Esto muestra el diálogo del navegador de forma limpia sin la advertencia de foto
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' }, audio: false });
      stream.getTracks().forEach(t => t.stop()); // liberar inmediatamente
    } catch(permErr) {
      const e = permErr.toString();
      if (e.includes('NotAllowed') || e.includes('Permission') || e.includes('permission')) {
        throw new Error('PermissionDenied');
      }
      if (e.includes('NotFound') || e.includes('DevicesNotFound')) {
    ...
    const esPermiso  = errStr.includes('Permission') || errStr.includes('NotAllowed') || errStr.includes('permission');
    const esNoCamera = errStr.includes('NotFound') || errStr.includes('DevicesNotFound');

    if (esPermiso) {
      // Mostrar guía detallada para desbloquear cámara
      ...
      if (region) {
        region.innerHTML = `
          <div style="padding:20px;text-align:center;color:#fff;">
            ...
            <div style="font-size:12px;color:rgba(255,255,255,0.75);line-height:1.6;margin-bottom:16px;">
              Tu navegador bloqueó el acceso a la cámara.<br>Para desbloquearlo:
            </div>
            <div style="background:rgba(255,255,255,0.1);border-radius:12px;padding:14px;text-align:left;font-size:12px;color:rgba(255,255,255,0.9);line-height:1.8;">
              <b>Chrome/Android:</b><br>
              🔒 Toca el candado en la barra de dirección<br>
              → Permisos del sitio → Cámara → Permitir<br><br>
              <b>Safari/iPhone:</b><br>
              ⚙️ Ajustes → Safari → Cámara → Permitir<br><br>
              <b>Después:</b> recarga la página
            </div>
      ...
      setScannerStatus('⚠ No se encontró cámara en este dispositivo');
      toast('⚠ No se encontró cámara', true);
    } else {
      setScannerStatus('⚠ Error al iniciar cámara: ' + errStr.slice(0, 60));
