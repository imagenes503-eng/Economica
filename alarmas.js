...
async function iniciarScanner() {
  ...
  try {
    ...
    // ── Config optimizada: fps alto + zona estrecha para precisión ────────
    // qrbox = 85% del ancho de pantalla para capturar desde lejos
    const sw = Math.min(window.innerWidth, 480);
    const config = {
      fps: 25,
      qrbox: { width: Math.round(sw * 0.85), height: Math.round(sw * 0.35) },
      aspectRatio: 1.5,
    ...
    setTimeout(async () => {
      try {
        ...
        const adv = {};
        // Foco continuo: la cámara siempre lista sin reenfocar
        if (caps.focusMode?.includes('continuous')) adv.focusMode = 'continuous';
        // Foco a distancia media-larga
        if (caps.focusDistance) adv.focusDistance = (caps.focusDistance.min + caps.focusDistance.max) * 0.3;
        // Zoom suave: 1.5x para ganar rango sin perder resolución
        if (caps.zoom) adv.zoom = Math.min(1.5, caps.zoom.max || 1.5);
...
function onCodigoEscaneado(codigo) {
  ...
  // ── Debounce: ignorar lecturas repetidas dentro de 800ms ─────────────
  const now = Date.now();
  if (now - _lastScanTs < 800) return;
  _lastScanTs = now;
  ...
  if (_scannerModo === 'inventario') {
    // onCodigoInventario maneja su propio cierre
  ...
  if (!prod) {
    prod = productos.find(p => {
      const c = (p.cod || '').trim();
      return c && c.replace(/^0+/, '') === codLimpio.replace(/^0+/, '');
    });
  }

  // ── Cerrar siempre y notificar ───────────────────────────────────────
  cerrarScanner();
