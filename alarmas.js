...
async function iniciarScanner() {
  ...
  try {
    ...
    // ── Estrategia de cámara: ambiente → usuario → cualquiera ──────────
    // Pedir directamente sin pre-check getUserMedia para evitar el doble
    // handshake que rompe iOS Safari y Firefox
    let camaraOpts;
    try {
      // Intentar cámara trasera (ideal para escaneo)
      camaraOpts = { facingMode: { ideal: 'environment' } };
      await _html5Qr.start(camaraOpts, config,
        (codigo) => onCodigoEscaneado(codigo), () => {});
    } catch(e1) {
      // Fallback 1: cualquier cámara disponible (funciona en desktop y Firefox)
      try {
        if (_html5Qr) { try { _html5Qr.clear(); } catch(_) {} }
        _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
        camaraOpts = { facingMode: 'user' };
        await _html5Qr.start(camaraOpts, config,
          (codigo) => onCodigoEscaneado(codigo), () => {});
      } catch(e2) {
        // Fallback 2: dejar que html5-qrcode elija la cámara (lista de dispositivos)
        if (_html5Qr) { try { _html5Qr.clear(); } catch(_) {} }
        _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
        const cameras = await Html5Qrcode.getCameras();
        if (!cameras || cameras.length === 0) throw new Error('NotFound');
        // Preferir la última cámara (suele ser la trasera en Android)
