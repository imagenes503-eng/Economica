...
async function renderCierreDia(pgId){
  ...
  _cdSaldoAyerCache = saldoAyer; // cache for _cdActualizarStats
      ...
      <div class="cd-cap-wrap">
        <div class="cd-resumen-captura" id="cdResumenCaptura">
          <div class="cd-cap-inner">
            ...
            <div class="cd-cap-row grand" style="margin-top:8px;"><span>🏦 Total neto en caja</span><span id="capTotalGeneral">$0.00</span></div>
...
function _cdActualizarStats(){
  ...
  // Saldo de ayer en captura (Punto 3)
...
async function _cdGenerarPDFMensual(){
  ...
  const saldoTeorico=sIni+totalACaja-totalG;
  ...
  const separator=(cy)=>{
    doc.setDrawColor(3,105,161);doc.setLineWidth(1.2);doc.line(14,cy,w-14,cy);
  };
  ...
  row('Saldo al inicio del mes',    '$'+sIni.toFixed(2),     y,[50,50,50],[22,163,74]); y+=8;
  row('Total ventas brutas',         '$'+totalV.toFixed(2),   y,[50,50,50],[3,105,161]); y+=8;
  row('Menos alquiler apartado',     '-$'+totalAlq.toFixed(2),y,[50,50,50],[180,83,9]);  y+=8;
  ...
  rowTotal('DEBERÍA HABER EN CAJA', '$'+saldoTeorico.toFixed(2), y); y+=14;

  // ── ANÁLISIS ─────────────────────────────────────────────────────────
  sectionHeader('📈 ANÁLISIS FINANCIERO',y,[21,128,61]);y+=6;
  row('Ganancia estimada de inventario', '$'+totalInvG.toFixed(2), y,[50,50,50],[21,128,61]); y+=8;
  row('Saldo inicio + ventas (a caja)',  '$'+(sIni+totalACaja).toFixed(2), y,[50,50,50],[3,105,161]); y+=8;
  row('Menos total gastos',             '-$'+totalG.toFixed(2), y,[50,50,50],[220,38,38]); y+=8;
  rowTotal('SALDO TEÓRICO FINAL', '$'+saldoTeorico.toFixed(2), y,[21,128,61]); y+=16;
  ...
  _cdMesData.ventas.forEach((v,i)=>{
    ...
    doc.text(_cdFmtFecha(v.fecha),14,y);
    doc.setFont('helvetica','bold');doc.setTextColor(3,105,161);
    doc.text('Venta $'+v.total.toFixed(2),60,y);
    doc.setTextColor(180,83,9);
    doc.text('Alquiler $'+(v.alquiler||0).toFixed(2),105,y);
    doc.setTextColor(21,128,61);
    doc.text('→ Caja $'+(v.total-(v.alquiler||0)).toFixed(2),w-14,y,{align:'right'});
    doc.setDrawColor(186,230,253);doc.setLineWidth(0.3);doc.line(14,y+1.5,w-14,y+1.5);
    y+=8;
  });
  if(!_cdMesData.ventas.length){doc.setFontSize(10);doc.setTextColor(150,150,150);doc.text('Sin ventas registradas',w/2,y,{align:'center'});y+=8;}
  rowTotal('TOTAL VENTAS / ALQUILER / A CAJA',
    '$'+totalV.toFixed(2)+' / $'+totalAlq.toFixed(2)+' / $'+totalACaja.toFixed(2), y); y+=14;

  // ── GASTOS DEL MES ────────────────────────────────────────────────────
  checkPage();
  sectionHeader('📤 GASTOS / PAGOS DEL MES ('+_cdMesData.gastos.length+')',y,[220,38,38]);y+=8;
  _cdMesData.gastos.forEach((g,i)=>{
    checkPage();
    if(i%2===0){doc.setFillColor(255,242,242);doc.rect(14,y-5,w-28,7,'F');}
    doc.setFontSize(10);doc.setFont('helvetica','normal');doc.setTextColor(30,30,30);
    doc.text(_cdFmtFecha(g.fecha),14,y);
    // Punto 4: solo "Se pagó $X" sin descripción completa
    const label='Se pagó $'+g.total.toFixed(2)+(g.tipoInv?' (Inventario +$'+g.gananciaInv.toFixed(2)+' ganancia)':'');
    doc.text(label,50,y);
    doc.setFont('helvetica','bold');doc.setTextColor(220,38,38);
    doc.text('$'+g.total.toFixed(2),w-14,y,{align:'right'});
    doc.setDrawColor(252,165,165);doc.setLineWidth(0.3);doc.line(14,y+1.5,w-14,y+1.5);
    y+=8;
  });
