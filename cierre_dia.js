    ...
    <div class="cd-body">
      ...
      <div class="cd-panel">
        <div class="cd-panel-header">
          ...
          <button class="cd-btn-update" onclick="_cdAplicarCambios()" style="background:#f59e0b;color:#fff;border-color:#f59e0b;">🧮 Calcular</button>
        </div>
        <div class="cd-panel-body">
...
function _cdGuardarAyer(){
  // Tomar los datos de Saldo en Caja como cierre del día de hoy
  // Para que mañana aparezcan como "Caja Día de Ayer"
  const S=_cdLeerMontos('cdSaldo');
  const totalSaldo=_cdTotalM(S);
  if(totalSaldo<=0){
    // Si el saldo está vacío, leer los campos manuales de Ayer como respaldo
    ...
    _cdGuardarSaldoHoy(A);
    if(typeof toast==='function')toast(`\u{1F4BE} Caja de ayer guardada \u2014 $${totalAyer.toFixed(2)}`);
  } else {
    // Guardar el saldo actual como cierre del día
    try{ localStorage.setItem('vpos_cajaAyer', JSON.stringify({montos:S,total:totalSaldo,fecha:_cdFecha})); }catch(e){}
    _cdGuardarSaldoHoy(S);
    if(typeof toast==='function')toast(`\u{1F4BE} Saldo en caja guardado como cierre del d\u00EDa \u2014 $${totalSaldo.toFixed(2)}`);
  }
  _cdActualizarStats();
}
...
function _cdAplicarCambios(){
  ...
  movs.forEach(m=>{
    // Solo actualizar Saldo en Caja (redistribución de denominaciones)
    _cdSet('cdSaldo'+m.deId,Math.max(0,_cdV('cdSaldo'+m.deId)-m.monto));
    _cdSet('cdSaldo'+m.haciaId,_cdV('cdSaldo'+m.haciaId)+m.monto);
    // Registrar en historial para imagen descargada
    _cdCambiosAplicados.push({...m,hora:new Date().toLocaleTimeString('es',{hour:'2-digit',minute:'2-digit'})});
  });
  // Limpiar campos de cambios del día
  _CD_DENOMS.forEach(d=>_cdSet('cdCambioSale'+d.id,0,true));
  const t=_cdTotalM(_cdLeerMontos('cdSaldo'));
  if(typeof toast==='function')toast(`\u2713 Cambios aplicados al saldo \u2014 Total caja: $${t.toFixed(2)}`);
  _cdActualizarStats();
  // Refrescar panel de historial visible en pantalla
  const panelCambios=document.getElementById('cdCambioResumen');
  if(panelCambios){
    const histHtml=_cdCambiosAplicados.length
      ?'<div class="cd-sep" style="margin-top:0;">Cambios ya aplicados hoy</div>'+
        _cdCambiosAplicados.map(m=>`<div style="font-size:12px;font-weight:700;color:var(--text-muted);padding:3px 0;font-family:Nunito,sans-serif;">\u2022 ${m.de} \u2212$${m.monto.toFixed(2)} \u2192 ${m.hacia} +$${m.monto.toFixed(2)} (${m.hora||''})</div>`).join('')
      :'';
    panelCambios.innerHTML=histHtml;
  }
}
