...
(function _estilosCierre() {
  ...
  s.textContent = `
    ...
    .cd-print-mode .cd-resumen-captura{width:1080px!important;min-height:1920px!important;border:none!important;margin:0!important;border-radius:0!important;background:#ffffff!important;display:flex!important;flex-direction:column!important;justify-content:center!important;box-shadow:inset 0 0 0 12px #1e40af!important;}
...
function _cdGuardarEstadoAutomatico(){
  try{
    // NOTE: vpos_saldoCaja and vpos_cajaAyer are ONLY updated by explicit user actions
    // (_cdGuardarSaldoCajaExplicit / _cdGuardarAyer). NOT here, to prevent
    // auto-recalc from overwriting good data with zeros.
...
// Called ONLY when user explicitly types in cdSaldo or cdAlquiler inputs
function _cdGuardarSaldoCajaExplicit(){
  try{
    const saldoM=_cdLeerMontos('cdSaldo');
    const alqVal=_cdV('cdAlquiler');
    // Only save if at least one value is non-zero (prevent saving empty state)
    const hasData=_cdTotalM(saldoM)>0||alqVal>0;
    if(hasData){
...
// Called ONLY when user explicitly types in cdAyer inputs or runs guardarAyer
function _cdGuardarCajaAyerExplicit(){
  try{
    const ayerM=_cdLeerMontos('cdAyer');
    const alqAyer=_cdV('cdAyerAlquiler');
    const hasData=_cdTotalM(ayerM)>0||alqAyer>0;
    if(hasData){
      ...
      <div class="cd-panel" style="border:2px solid #15803d;">
        ...
        <div class="cd-panel-body">
          ...
          <div class="cd-field" style="margin-top:12px;">
            ...
            <input class="cd-inp" type="number" id="cdAlquiler" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();_cdGuardarSaldoCajaExplicit()">
          </div>
...
if(etapa==='venta'){
  ...
  // Paso 3: Aplicar Cambios SOLO sobre el saldo actual (una sola vez)
  ...
  _cdGuardarSaldoCajaExplicit(); // persist after calc changes saldo
  _cdActualizarStats();
  _cdCerrarModal();
}
...
function _cdGuardarAyer(){
  ...
  // Persist explicitly (not through _cdGuardarEstadoAutomatico)
  _cdCajaAyerPersist={montos:A,alquiler:alqAcum,fecha:_cdFecha};
  try{
    localStorage.setItem('vpos_cajaAyer', JSON.stringify(_cdCajaAyerPersist));
  }catch(e){}
  _cdSaldoAyerCache = {...A};
...
function _cdAplicarCambios(){
  ...
  // _cdAutoRecalcSaldo() already applied pending cambios to cdSaldo (preview mode).
  // We must UNDO that preview first, then apply for real, to avoid doubling.
  movs.forEach(m=>{
    // Reverse the preview: undo what autorecalc already added
    _cdSet('cdSaldo'+m.deId,   _cdV('cdSaldo'+m.deId)+m.monto);           // restore DE
    _cdSet('cdSaldo'+m.haciaId, Math.max(0,_cdV('cdSaldo'+m.haciaId)-m.monto)); // restore HACIA
  });

  // Now apply once, cleanly
  movs.forEach(m=>{
    _cdSet('cdSaldo'+m.deId,   Math.max(0,_cdV('cdSaldo'+m.deId)-m.monto));
    _cdSet('cdSaldo'+m.haciaId, _cdV('cdSaldo'+m.haciaId)+m.monto);
...
async function _cdTomarCaptura(){
  ...
  try{
    ...
    // Draw captured content — scale to fit width, center vertically if shorter
    const drawW=PX_W;
    const drawH=Math.round(c.height*(PX_W/c.width));
    const dy=Math.max(0,Math.round((PX_H-drawH)/2));
    ctx.drawImage(c,0,dy,drawW,Math.min(drawH,PX_H));
