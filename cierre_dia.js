...
(function _estilosCierre() {
  ...
  s.textContent = `
    ...
    .cd-modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.55);z-index:9000;display:flex;align-items:flex-end;justify-content:center;opacity:0;pointer-events:none;transition:opacity .22s;}
    .cd-modal-overlay.active{opacity:1;pointer-events:all;}
    .cd-modal-sheet{background:var(--surface,#fff);border-radius:22px 22px 0 0;width:100%;max-width:560px;max-height:88vh;overflow-y:auto;-webkit-overflow-scrolling:touch;transform:translateY(100%);transition:transform .28s cubic-bezier(.32,.72,0,1);padding-bottom:env(safe-area-inset-bottom,0);}
    .cd-modal-overlay.active .cd-modal-sheet{transform:translateY(0);}
    .cd-modal-handle{width:40px;height:4px;background:var(--border,#ddd);border-radius:2px;margin:12px auto 8px;display:block;}
    .cd-modal-header{display:flex;align-items:center;gap:10px;padding:10px 16px 14px;border-bottom:1px solid var(--border);}
    .cd-modal-icon{width:32px;height:32px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0;}
    .cd-modal-title{font-size:15px;font-weight:900;color:var(--text);font-family:Nunito,sans-serif;flex:1;}
    .cd-modal-close{background:var(--surface2,#f5f5f5);border:none;border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:18px;cursor:pointer;color:var(--text-muted);flex-shrink:0;line-height:1;}
    .cd-modal-body{padding:16px;}
    .cd-modal-actions{display:flex;gap:8px;padding:0 16px 16px;flex-wrap:wrap;}
...
async function renderCierreDia(pgId){
  ...
  setTimeout(()=>{
    ...
    document.querySelectorAll('.cd-panel').forEach((panel)=>{
      ...
      const actBtns = panel.querySelectorAll('.cd-btn-update,.cd-btn-add');
      ...
      if(header && !header.dataset.acc){
        ...
        header.addEventListener('click',(e)=>{
          ...
          iconEl.style.cssText = icon ? icon.style.cssText : '';
          titleEl.innerHTML = title ? title.innerHTML : '';
          bodyEl.innerHTML  = body  ? body.innerHTML  : '';

          // Rebuild action buttons (top-right buttons of panel) inside modal footer
          actEl.innerHTML = '';
          actBtns.forEach(btn=>{
          ...
          // Reattach event listeners to cloned inputs/buttons in modal body
          bodyEl.querySelectorAll('input,select,textarea').forEach(inp=>{
            // Mirror id back to original DOM so existing JS functions work
            if(inp.id){
              const orig = document.getElementById(inp.id);
              if(orig && orig !== inp){
                // Sync value immediately
                inp.value = orig.value;
                // Forward changes to original
                inp.addEventListener('input', ()=>{ orig.value=inp.value; orig.dispatchEvent(new Event('input',{bubbles:true})); });
                inp.addEventListener('change',()=>{ orig.value=inp.value; orig.dispatchEvent(new Event('change',{bubbles:true})); });
                inp.addEventListener('blur',  ()=>{ orig.value=inp.value; orig.dispatchEvent(new Event('blur',  {bubbles:true})); });
              }
            }
          });
...
// Helper: volver al menú principal
function _cdVolverMenu(){
  _cdCerrarModal();
  if(typeof mostrarPagina==='function') mostrarPagina('pgInicio');
  else if(typeof window.showPage==='function') window.showPage('home');
  else {
    const navHome = document.querySelector('[data-page="pgInicio"],[data-nav="home"],[data-page="home"],#btnInicio,.nav-home');
    if(navHome) navHome.click();
    else { const pg=document.getElementById('pgCierreDia'); if(pg) pg.style.display='none'; }
  }
}

// Helper: cerrar modal popup
...
function _cdActualizarStats(){
  ...
  const totalCambios=_CD_DENOMS.reduce((s,d)=>s+_cdV('cdCambioSale'+d.id),0);
...
window._cdVolverMenu  = _cdVolverMenu;
