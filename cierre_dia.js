...
function _cdRestaurarEstadoAutomatico(){
  ...
  const _aplicar = (saldo, ayer, estado, gastos, cambios) => {
  ...
  setTimeout(()=>{
    _aplicar(saldoLocal, ayerLocal, estLocal, gastosLocal, cambiosLocal);
    _cdRestoring = false;
  }, 60);
  ...
  Promise.all([
  ...
  ]).then(([sbSaldo, sbAyer, sbEstado, sbGastos, sbCambios])=>{
    ...
    if(sbMasReciente){
      _cdRestoring = true;
      setTimeout(()=>{
        _aplicar(saldoFinal, ayerFinal, estFinal, gastosFinal, cambiosFinal);
        _cdRestoring = false;
      }, 60);
    }
