...
function _cdRestaurarEstadoAutomatico(){
  ...
  const _aplicar = (saldo, ayer, estado) => {
    ...
    if(estado && !estado.borrado){
      ...
      if(Array.isArray(estado.gastos)) _cdGastos = estado.gastos;
      if(Array.isArray(estado.cambios)) _cdCambiosAplicados = estado.cambios;
  ...
  setTimeout(()=>{
    _aplicar(saldoLocal, ayerLocal, estLocal);
    _cdRestoring = false;
  }, 60);
  ...
  Promise.all([
    ...
    // Solo actualizar si Supabase tiene datos MÁS RECIENTES que los locales
    ...
    if(sbMasReciente){
      _cdRestoring = true;
      setTimeout(()=>{
        _aplicar(saldoFinal, ayerFinal, estFinal);
        _cdRestoring = false;
      }, 60);
    }
  }).catch(()=>{ /* offline — ya tenemos los datos locales */ });
}
