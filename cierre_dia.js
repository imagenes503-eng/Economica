...
function _cdRestaurarEstadoAutomatico(){
  ...
  Promise.all([
    ...
    // Usar el más reciente entre Supabase y localStorage (por ts)
    const _pickLatest = (a, b) => {
      if (!a) return b; if (!b) return a;
      return ((a.ts||0) >= (b.ts||0)) ? a : b;
    };
    const saldo  = _pickLatest(sbSaldo, lsSaldo) || _cdSaldoCajaPersist;
    const ayer   = _pickLatest(sbAyer, lsAyer)   || _cdCajaAyerPersist;
    // Para estado: si el de Supabase tiene borrado=true, preferir localStorage si es más reciente
    let estado = _pickLatest(sbEstado, lsEst);
    if (estado && estado.borrado) estado = null; // estado borrado = no restaurar
