...
(function _estilosCierre() {
  ...
  s.textContent = `
    ...
    .cd-panel-body{display:block;}
    .cd-panel{overflow:visible;}
    .cd-panel-toggle{display:none;}
    ...
    .cd-popup-fab{
      ...
      font-size:26px;
      cursor:pointer;
      box-shadow:0 10px 30px rgba(3,105,161,.35);
    }
...
.cd-section-btn-desc{
    ...
    .cd-panel.open .cd-panel-toggle{transform:rotate(180deg);}
...
function _cdSet(id,v,clearZero=true){
  const e=document.getElementById(id);
  if(e&&v!=null){
    const n=Number(v)||0;
    e.value=(clearZero&&n===0)?'':n.toFixed(2);
  }
}
...
function _cdGuardarEstadoAutomatico(){
  try{
    const data = {
      ...
      saldo:_cdLeerMontos('cdSaldo'),
      ayer:_cdLeerMontos('cdAyer'),
      gastos:_cdGastos,
...
function _cdRestaurarEstadoAutomatico(){
  try{
    ...
    setTimeout(()=>{
      ...
      _cdSet('cdAlquiler', data.alquiler||0, true);
      _cdSet('cdAyerAlquiler', data.ayerAlquiler||0, true);

      _CD_DENOMS.forEach(d=>{
        _cdSet('cdVenta'+d.id, data.venta?.[d.id]||0, true);
        _cdSet('cdSaldo'+d.id, data.saldo?.[d.id]||0, true);
      });
...
function _cdTogglePopupMenu(){
  document.getElementById('cdPopupItems')?.classList.toggle('active');
}

function _cdScrollTo(id){
  document.getElementById(id)?.scrollIntoView({
    behavior:'smooth',
    block:'start'
  });

  document.getElementById('cdPopupItems')?.classList.remove('active');
}
...
async function renderCierreDia(pgId){
  ...
  pg.innerHTML=`
    ...
    <div class="cd-body">
<div class="cd-section-nav">
  <div class="cd-section-btn" onclick="document.getElementById('secVenta')?.scrollIntoView({behavior:'smooth'})">
    <div class="cd-section-btn-title">💹 Venta</div>
    <div class="cd-section-btn-desc">Ventas y desglose</div>
  </div>

  <div class="cd-section-btn" onclick="document.getElementById('secAyer')?.scrollIntoView({behavior:'smooth'})">
    <div class="cd-section-btn-title">📦 Caja Ayer</div>
    <div class="cd-section-btn-desc">Saldo anterior</div>
  </div>

  <div class="cd-section-btn" onclick="document.getElementById('secGastos')?.scrollIntoView({behavior:'smooth'})">
    <div class="cd-section-btn-title">📤 Gastos</div>
    <div class="cd-section-btn-desc">Pagos y compras</div>
  </div>

  <div class="cd-section-btn" onclick="document.getElementById('secCambios')?.scrollIntoView({behavior:'smooth'})">
    <div class="cd-section-btn-title">🔄 Cambios</div>
    <div class="cd-section-btn-desc">Cambios aplicados</div>
  </div>

  <div class="cd-section-btn" onclick="document.getElementById('secSaldo')?.scrollIntoView({behavior:'smooth'})">
    <div class="cd-section-btn-title">🏦 Saldo</div>
    <div class="cd-section-btn-desc">Caja final</div>
  </div>

  <div class="cd-section-btn" onclick="document.getElementById('secMensual')?.scrollIntoView({behavior:'smooth'})">
    <div class="cd-section-btn-title">📅 Mensual</div>
    <div class="cd-section-btn-desc">Registro mensual</div>
  </div>
</div>


      <!-- VENTA DEL DÍA -->
      <div class="cd-panel" id="secVenta">
      ...
      <div class="cd-panel" id="secAyer">
        ...
        <div class="cd-panel-body">
          ...
          <div class="cd-total-row amber" style="margin-top:8px;"><span>Total caja ayer</span><span id="cdAyerTotal">$0.00</span></div>
        </div>
      </div>

      <!-- GASTOS / PAGOS -->
      <div class="cd-panel" id="secGastos">
        <div class="cd-panel-header">
          <div class="cd-panel-icon" style="background:#fee2e2;">📤</div>
          <div class="cd-panel-title">Gastos / Pagos del Día</div>
          <button class="cd-btn-update" onclick="_cdCalcular('gastos')" style="background:#dc2626;color:#fff;border-color:#dc2626;">🧮 Calcular</button><div class="cd-panel-toggle">⌄</div>
        </div>
        <div class="cd-panel-body">
      ...
      <!-- CAMBIOS DEL DÍA -->
      <div class="cd-panel" id="secCambios">
        <div class="cd-panel-header">
      ...
      <!-- SALDO EN CAJA -->
      <div class="cd-panel" id="secSaldo" style="border:2px solid #15803d;">
        <div class="cd-panel-header" style="background:linear-gradient(135deg,#15803d,#16a34a);">
          ...
          <button class="cd-btn-update" onclick="_cdCalcular('saldo')" style="background:rgba(255,255,255,.2);color:#fff;border-color:rgba(255,255,255,.4);">🧮 Calcular</button><div class="cd-panel-toggle">⌄</div>
        </div>
        <div class="cd-panel-body">
      ...
      <!-- REGISTRO MENSUAL -->
      <div class="cd-panel" id="secMensual">
        <div class="cd-panel-header">
          <div class="cd-panel-icon" style="background:#dbeafe;">📅</div>
      ...
      <div class="cd-cap-wrap">
        <div class="cd-resumen-captura" id="cdResumenCaptura">
          <div class="cd-cap-inner">
            ...
            <!-- 1+2: Venta del Día | Caja Día de Ayer (lado a lado) -->
...
<div class="cd-popup-menu">
  <button class="cd-popup-fab" onclick="_cdTogglePopupMenu()">☰</button>

  <div class="cd-popup-items" id="cdPopupItems">
    <div class="cd-popup-item" onclick="_cdScrollTo('secVenta')">💹 Venta del Día</div>
    <div class="cd-popup-item" onclick="_cdScrollTo('secAyer')">📦 Caja Día de Ayer</div>
    <div class="cd-popup-item" onclick="_cdScrollTo('secGastos')">📤 Gastos / Pagos</div>
    <div class="cd-popup-item" onclick="_cdScrollTo('secCambios')">🔄 Cambios del Día</div>
    <div class="cd-popup-item" onclick="_cdScrollTo('secSaldo')">🏦 Saldo en Caja</div>
    <div class="cd-popup-item" onclick="_cdScrollTo('secMensual')">📅 Registro Mensual</div>
  </div>
</div>
  ...
  // Navegación moderna sin acordeón
...
function _cdAplicarVentaASaldo(){
  ...
  // Ya no limpiar venta automáticamente
...
function _cdAutoRecalcSaldo(){
  ...
  _CD_DENOMS.forEach(d=>{
    ...
    if(d.id==='Billetes'){
      v=Math.max(0,v-alqHoy);
    }

    _cdSet('cdSaldo'+d.id,_cdV('cdSaldo'+d.id)+v);
  });
...
function _cdCalcular(etapa){
  // Paso 1: Saldo = saldo actual + venta del día
  // Caja Día de Ayer ahora es independiente
  const alqHoy=_cdV('cdVentaAlquilerHoy');

  _CD_DENOMS.forEach(d=>{
    let venta = _cdV('cdVenta'+d.id);

    if(d.id==='Billetes'){
      venta=Math.max(0, venta-alqHoy);
    }

    const saldoActual=_cdV('cdSaldo'+d.id);

    _cdSet('cdSaldo'+d.id, saldoActual + venta);
  });
if(etapa==='venta'){
...
function _cdGuardarAyer(){
  ...
  // FIX alquiler acumulado
  const alquilerActual = _cdV('cdAlquiler');
  _cdSet('cdAyerAlquiler', alquilerActual);
...
function _cdActualizarStats(){
  ...
  const totalCambios=_cdCambiosAplicados.reduce((s,c)=>s+Number(c.monto||0),0);
  _cdTxt('capCambiosTotal',$(totalCambios));
  ...
  const cambiosWrap=document.getElementById('capCambiosList');
  if(cambiosWrap){
    if(_cdCambiosAplicados.length){
      cambiosWrap.innerHTML=_cdCambiosAplicados.map(c=>`
        <div class="cd-cap-row">
          <span>${c.de} → ${c.hacia}</span>
          <span class="val-warn">$${Number(c.monto||0).toFixed(2)}</span>
        </div>
      `).join('');
    }else{
...
async function _cdTomarCaptura(){
  ...
  try{
    ...
    const ctx=o.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,1080,1920);
    const scale=Math.min(1,1920/c.height);
    const dw=Math.round(c.width*scale);
    const dh=Math.round(c.height*scale);
    const dx=Math.round((1080-dw)/2);
    const dy=Math.round((1920-dh)/2);
