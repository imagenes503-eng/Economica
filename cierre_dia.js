...
async function renderCierreDia(pgId){
  ...
  // Fix 10: saldo inicial ya incluye producto+inventario; solo se suma la ganancia de cada pago
  const saldoTeorico=(_cdMesData.saldoInicio||0)+totalInvGanancia;
...
async function _cdGenerarPDFMensual(){
  ...
  // Fórmula correcta: saldoInicio + ganancias de inventario = debería haber
  const saldoTeorico=sIni+totalInvG;
  ...
  // ── RESUMEN GENERAL (Fix 10: lógica correcta) ─────────────────────────
  sectionHeader('RESUMEN DEL MES',y);y+=6;
  row('Saldo inicial del mes (incluye producto+inventario)', '$'+sIni.toFixed(2), y,[50,50,50],[22,163,74]); y+=8;
  row('Ganancia de inventario (pagos/gastos con ganancia)', '+$'+totalInvG.toFixed(2), y,[50,50,50],[21,128,61]); y+=8;
  ...
  rowTotal('Total venta del mes + caja existente', '$'+(totalV+sIni).toFixed(2), y,[3,105,161]); y+=16;
...
// Persistir y cargar "Queda en Efectivo" en IDB (sobrevive reinicio)
