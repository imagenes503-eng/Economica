...
// Called when user explicitly changes cdSaldo/cdAlquiler inputs or after Calcular/Aplicar
...
// Called when user explicitly changes cdAyer inputs or runs guardarAyer
...
function _cdActualizarStats(){
  ...
  // Persistir saldo — único punto de verdad
  _cdGuardarSaldoCajaExplicit();
}
...
async function _cdTomarCaptura(){
  ...
  try{
    ...
    // Draw rounded border frame — uniform on all 4 sides
    const BORDER = 18;       // stroke width at 2× scale = 9px visual at 1×
    const RADIUS = 80;       // corner radius at 2× scale = 40px visual
    const B2 = BORDER / 2;
    ctx.strokeStyle = '#1e40af';
    ctx.lineWidth   = BORDER;
    ctx.beginPath();
    ctx.moveTo(B2 + RADIUS, B2);
    ctx.lineTo(PX_W - B2 - RADIUS, B2);
    ctx.arcTo(PX_W - B2, B2, PX_W - B2, B2 + RADIUS, RADIUS);
    ctx.lineTo(PX_W - B2, PX_H - B2 - RADIUS);
    ctx.arcTo(PX_W - B2, PX_H - B2, PX_W - B2 - RADIUS, PX_H - B2, RADIUS);
    ctx.lineTo(B2 + RADIUS, PX_H - B2);
    ctx.arcTo(B2, PX_H - B2, B2, PX_H - B2 - RADIUS, RADIUS);
    ctx.lineTo(B2, B2 + RADIUS);
    ctx.arcTo(B2, B2, B2 + RADIUS, B2, RADIUS);
    ctx.closePath();
    ctx.stroke();
