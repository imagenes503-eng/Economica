...
async function renderCierreDia(pgId){
  ...
  `;
  _cdRenderListas();_cdRestaurarEstadoAutomatico();setTimeout(()=>{_cdAutoRecalcSaldo();},250);
...
function _cdCalcular(etapa){
  // Cambios del día SOLO modifica saldo actual
  if(etapa==='cambios'){
    const movs=_cdCalcularCambiosPendientes();
    if(!movs.length){
    ...
    _cdActualizarStats();
    if(typeof toast==='function')toast('✓ Cambios aplicados correctamente');
    return;
  }
