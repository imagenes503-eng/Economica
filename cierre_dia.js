...
async function renderCierreDia(pgId){
  ...
  const cambioGrid=_CD_DENOMS.map(d=>`
    <div class="cd-cambio-item">
      ...
      <div class="cd-field" style="margin-bottom:6px;"><label>Monto ($)</label><input class="cd-inp" type="number" id="cdCambioSale${d.id}" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();_cdAutoRecalcSaldo()"></div>
  ...
  pg.innerHTML=`
    ...
    <div class="cd-body">
      ...
      <!-- FILA 1: Venta del Día | Caja Día de Ayer -->
      <div class="cd-2col-grid">
        ...
        <div class="cd-panel">
          <div class="cd-panel-header">
            ...
            <button class="cd-btn-update green" onclick="_cdCalcular('venta')">🧮 Calcular</button>
          </div>
          <div class="cd-panel-body">
            <div class="cd-field" style="margin-bottom:10px;">
              <label>Total vendido ($)</label>
              <input class="cd-inp big" type="number" id="cdVentaTotal" min="0" step="0.01" placeholder="0.00" value="${vSug>0?vSug.toFixed(2):''}" oninput="_cdActualizarStats();_cdAutoRecalcSaldo()">
            ...
            <div class="cd-field" style="margin-bottom:10px;">
              ...
              <input class="cd-inp" type="number" id="cdVentaAlquilerHoy" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();_cdAutoRecalcSaldo()">
            </div>
        ...
        <div class="cd-panel">
          <div class="cd-panel-header">
            ...
            <button class="cd-btn-update" onclick="_cdGuardarAyer()">💾 Guardar</button>
          </div>
          <div class="cd-panel-body">
      ...
      <!-- FILA 2: Gastos / Pagos | Cambios del Día -->
      <div class="cd-2col-grid">
        <!-- GASTOS / PAGOS -->
        <div class="cd-panel">
          <div class="cd-panel-header">
            <div class="cd-panel-icon" style="background:#fee2e2;">📤</div>
            <div class="cd-panel-title">Gastos / Pagos</div>
            <button class="cd-btn-update" onclick="_cdCalcular('gastos')" style="background:#dc2626;color:#fff;border-color:#dc2626;">🧮 Calcular</button>
          </div>
          <div class="cd-panel-body">
            <div class="cd-item-list" id="cdGastosList"></div>
            <div style="background:#fef2f2;border:1.5px solid #fca5a5;border-radius:12px;padding:10px;margin-top:4px;">
              <div style="font-size:10px;font-weight:900;color:#dc2626;text-transform:uppercase;letter-spacing:0.4px;margin-bottom:8px;">➕ Registrar gasto</div>
              <div class="cd-field" style="margin-bottom:8px;">
                <label>Descripción</label>
                <input class="cd-inp" type="text" id="cdGastoDesc" placeholder="Ej: Pepsi, Luz…" onkeydown="if(event.key==='Enter')_cdAgregarGasto()">
              </div>
              <div class="cd-sep" style="margin-top:0;">¿Qué se sacó de caja?</div>
              ${_cdBloqueMontosHTML('cdGastoForm')}
              <div style="margin-top:10px;padding:8px 10px;background:#f0fdf4;border:1.5px solid #86efac;border-radius:10px;">
                <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:11px;font-weight:900;color:#15803d;font-family:Nunito,sans-serif;margin-bottom:6px;">
                  <input type="checkbox" id="cdGastoEsInventario" onchange="_cdToggleInvFields()" style="width:14px;height:14px;accent-color:#16a34a;"> 📦 Es para inventario
                </label>
                <div id="cdGastoInvFields" style="display:none;display:grid;grid-template-columns:1fr 1fr;gap:6px;">
                  <div class="cd-field"><label>Costo ($)</label><input class="cd-inp" type="number" id="cdGastoInvCosto" min="0" step="0.01" placeholder="0.00"></div>
                  <div class="cd-field"><label>Valor venta ($)</label><input class="cd-inp" type="number" id="cdGastoInvVenta" min="0" step="0.01" placeholder="0.00" oninput="_cdCalcularGanancia()"></div>
                  <div class="cd-field" style="grid-column:span 2;"><label>Ganancia</label><div id="cdGastoGananciaLbl" style="padding:6px 10px;background:#dcfce7;border-radius:8px;font-size:13px;font-weight:900;color:#15803d;font-family:Nunito,sans-serif;">$0.00</div></div>
                </div>
              </div>
              <button class="cd-btn-add" style="width:100%;margin-top:8px;" onclick="_cdAgregarGasto()">➕ Registrar gasto</button>
            </div>
            <div class="cd-total-row red" style="margin-top:10px;"><span>Total gastos del día</span><span id="cdGastosTotal">$0.00</span></div>
          </div>
        </div>
        ...
        <div class="cd-panel">
          <div class="cd-panel-header">
            ...
            <button class="cd-btn-update" onclick="_cdCalcular('cambios')" style="background:#f59e0b;color:#fff;border-color:#f59e0b;">🧮 Calcular</button>
          </div>
          <div class="cd-panel-body">
            <div style="font-size:10px;color:var(--text-muted);font-weight:700;margin-bottom:10px;">Sale de una denominación → entra en otra. Se actualiza el saldo en tiempo real.</div>
            <div class="cd-cambio-grid">${cambioGrid}</div>
            <div id="cdCambioResumen" style="margin-top:10px;"></div>
            ...
            <div style="margin-top:6px;">
              <div class="cd-sep" style="margin-top:0;">Aplicados hoy</div>
              ${_cdCambiosAplicados.map(m=>`<div style="font-size:11px;font-weight:700;color:var(--text-muted);padding:2px 0;font-family:Nunito,sans-serif;">• ${m.de} −$${m.monto.toFixed(2)} → ${m.hacia} +$${m.monto.toFixed(2)}</div>`).join('')}
            </div>` : ''}
          </div>
        </div>
      </div>
      ...
      <div class="cd-panel">
        <div class="cd-panel-header">
          ...
          <button class="cd-btn-update" onclick="_cdCargarSaldoAyerEnCaja()">⬆ Usar como saldo inicial</button>
        </div>
        <div class="cd-panel-body">
      ...
      <div class="cd-panel" style="border:2px solid #15803d;">
        <div class="cd-panel-header" style="background:linear-gradient(135deg,#15803d,#16a34a);">
          ...
          <button class="cd-btn-update" onclick="_cdCalcular('saldo')" style="background:rgba(255,255,255,.2);color:#fff;border-color:rgba(255,255,255,.4);">🧮 Calcular</button>
        </div>
        <div class="cd-panel-body">
      ...
      <div class="cd-cap-wrap">
        <div class="cd-resumen-captura" id="cdResumenCaptura">
          <div class="cd-cap-inner">
            ...
            <!-- 1. Venta del Día + Caja Día de Ayer -->
            <div class="cd-cap-2col">
              <div class="cd-cap-col">
                ...
                <div class="cd-cap-row total"><span>Total venta</span><span class="val-blue" id="capVentaTotalFinal">$0.00</span></div>
              </div>
            ...
            <!-- 2. Gastos / Pagos del Día — ancho completo -->
            ...
            <!-- 3. Cambios del Día — ancho completo -->
            <div class="cd-cap-col" style="margin-bottom:12px;">
              ...
              <div class="cd-cap-row total"><span>Total cambios</span><span class="val-warn" id="capCambiosTotal">$0.00</span></div>
            </div>
  ...
  `;
  _cdRenderListas();_cdAutoRecalcSaldo();
...
function _cdCalcular(etapa){
  ...
  // Sumar alquiler de hoy al acumulado
  ...
  // Paso 3: Aplicar Cambios (redistribución sobre el saldo actual, sin recalcular)
  const movs=_cdCalcularCambiosPendientes();
  if(etapa==='cambios'){
    if(!movs.length){if(typeof toast==='function')toast('No hay cambios ingresados',true);return;}
    movs.forEach(m=>{
      _cdSet('cdSaldo'+m.deId,   Math.max(0,_cdV('cdSaldo'+m.deId)-m.monto));
      _cdSet('cdSaldo'+m.haciaId, _cdV('cdSaldo'+m.haciaId)+m.monto);
    });
    const t=_cdTotalM(_cdLeerMontos('cdSaldo'));
    if(typeof toast==='function')toast(`✓ Cambios aplicados al saldo — Total: $${t.toFixed(2)}`);
    _cdActualizarStats();
    return;
  }
...
function _cdGuardarAyer(){
  ...
  // Solo guardar en localStorage para que persista aunque se borren los datos del día
  try{ localStorage.setItem('vpos_cajaAyer', JSON.stringify({montos:A,total:totalAyer,fecha:_cdFecha})); }catch(e){}
  if(typeof toast==='function')toast(`💾 Caja de ayer guardada — $${totalAyer.toFixed(2)}`);
  _cdActualizarStats();
}

function _cdBorrarDatosCierre(){
  if(!confirm('¿Borrar todos los datos del cierre de hoy? Esta acción no se puede deshacer.'))return;
  _cdGastos=[];_cdDeudas=[];_cdCambiosAplicados=[];_cdVentaSnapshot=null;
  // Limpiar todos los campos
  const campos=['cdVentaTotal','cdVentaAlquilerHoy','cdAlquiler'];
  campos.forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
  _CD_DENOMS.forEach(d=>{
    ['cdVenta','cdSaldo','cdQueda','cdGastoForm'].forEach(px=>_cdSet(px+d.id,0,true));
    // cdAyer NO se borra — es referencia del día anterior
  });
  renderCierreDia();
  if(typeof toast==='function')toast('🗑 Datos del cierre borrados');
}
