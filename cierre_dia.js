...
function _cdActualizarStats(){
  ...
  if(capGD) capGD.innerHTML=_cdGastos.length
    ?_cdGastos.map(x=>{
        ...
        return`<div class="cd-cap-row" style="font-weight:900;font-size:14px;padding:5px 0 2px;border-bottom:none;"><span>📌 ${x.desc}</span><span class="val-neg">-$${x.total.toFixed(2)}</span></div>${ds}`;
      }).join('')
