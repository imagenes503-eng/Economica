...
async function renderCierreDia(pgId){
  ...
  const capitalInicial = saldoEfec + invIni;           // efectivo + inventario al inicio del mes
  const capitalActual  = cajaFinal + invFinal;         // efectivo + inventario ahora mismo
  const gananciaNetaMes = capitalActual - capitalInicial;
  const metaTotalCapital = capitalInicial + objetivoMeta;
  const progresoPct = objetivoMeta > 0 ? Math.max(0, Math.min(999, (gananciaNetaMes / objetivoMeta) * 100)) : 0;
  const faltaParaMeta = Math.max(0, objetivoMeta - gananciaNetaMes);
  const metaCumplida = gananciaNetaMes >= objetivoMeta;
  ...
  pg.innerHTML=`
    ...
    <div class="cd-body">
      ...
      <!-- ═══════════════════════════════════════════════════════════════
           ...
           con el resto del cierre. Ganancia neta = (Caja + Inventario AHORA)
           menos (Caja + Inventario al INICIO del mes).
      ═══════════════════════════════════════════════════════════════ -->
      <div class="cd-panel" style="border:2px solid #7c3aed;margin-top:4px;">
        ...
        <div class="cd-panel-body">
          <div style="font-size:11px;color:var(--text-muted);font-weight:700;margin-bottom:12px;line-height:1.5;">
            💡 Ganancia neta = todo lo que tienes AHORA (caja + inventario) menos lo que tenías al INICIO del mes.
            Define cuánto necesitas ganar mínimo para cubrir tus gastos operativos.
          </div>

          <div class="cd-field" style="margin-bottom:14px;">
            <label>🎯 Meta de ganancia neta del mes ($)</label>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px;">
            <div style="background:#f5f3ff;border:1.5px solid #ddd6fe;border-radius:10px;padding:10px 12px;">
              <div style="font-size:9px;font-weight:900;color:#6d28d9;text-transform:uppercase;letter-spacing:.4px;">Capital inicio de mes</div>
              <div style="font-size:16px;font-weight:900;color:#6d28d9;margin-top:3px;">$${capitalInicial.toFixed(2)}</div>
              <div style="font-size:9.5px;color:#7c3aed;font-weight:700;margin-top:2px;">Efectivo $${saldoEfec.toFixed(2)} + Inv. $${invIni.toFixed(2)}</div>
            </div>
            <div style="background:${metaCumplida ? '#f0fdf4' : '#fef9c3'};border:1.5px solid ${metaCumplida ? '#86efac' : '#fde68a'};border-radius:10px;padding:10px 12px;">
              <div style="font-size:9px;font-weight:900;color:${metaCumplida ? '#15803d' : '#b45309'};text-transform:uppercase;letter-spacing:.4px;">Capital actual</div>
              <div style="font-size:16px;font-weight:900;color:${metaCumplida ? '#15803d' : '#b45309'};margin-top:3px;">$${capitalActual.toFixed(2)}</div>
              <div style="font-size:9.5px;color:${metaCumplida ? '#15803d' : '#b45309'};font-weight:700;margin-top:2px;">Caja $${cajaFinal.toFixed(2)} + Inv. $${invFinal.toFixed(2)}</div>
            </div>
          </div>
          ...
          <div style="background:${metaCumplida ? '#f0fdf4' : '#fef2f2'};border:1.5px solid ${metaCumplida ? '#86efac' : '#fca5a5'};border-radius:12px;padding:14px 16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
              <span style="font-size:12px;font-weight:800;color:var(--text-muted);">💰 Ganancia neta actual del mes</span>
              <span style="font-size:18px;font-weight:900;color:${gananciaNetaMes >= 0 ? (metaCumplida ? '#15803d' : '#b45309') : '#dc2626'};">$${gananciaNetaMes.toFixed(2)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;border-top:1px solid ${metaCumplida ? '#bbf7d0' : '#fecaca'};padding-top:8px;">
              ...
              <span style="font-size:14px;font-weight:900;color:${metaCumplida ? '#15803d' : '#dc2626'};">
                ${metaCumplida ? '+$'+(gananciaNetaMes - objetivoMeta).toFixed(2)+' extra' : '-$'+faltaParaMeta.toFixed(2)}
              </span>
            </div>
          </div>

          <div style="margin-top:10px;padding:9px 12px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:9px;font-size:10.5px;font-weight:700;color:#1e40af;font-family:Nunito,sans-serif;">
            📌 Para llegar a tu meta necesitas terminar el mes con al menos <b>$${metaTotalCapital.toFixed(2)}</b> entre caja e inventario (tenías $${capitalInicial.toFixed(2)} al inicio + $${objetivoMeta.toFixed(2)} de meta).
          </div>
        </div>
      </div>
