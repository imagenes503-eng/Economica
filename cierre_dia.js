...
function _cdGuardarEstadoAutomatico(){
  try{
    ...
    // También sincronizar a Supabase
    _cdSbSave('estadoDia_'+_cdFecha, data).catch(()=>{});
    if(typeof _broadcast==='function') _broadcast('cierre_estado_actualizado',{fecha:_cdFecha,data});
  }catch(e){}
}
...
function _cdGuardarSaldoCajaExplicit(){
  ...
  try{
    ...
    if(typeof _broadcast==='function') _broadcast('cierre_saldo_actualizado',{fecha:_cdFecha,saldo:_cdSaldoCajaPersist});
  }catch(e){}
}
...
function _cdGuardarCajaAyerExplicit(){
  ...
  try{
    ...
    if(typeof _broadcast==='function') _broadcast('cierre_cajaayer_actualizado',{fecha:_cdFecha,datos:_cdCajaAyerPersist});
  }catch(e){}
}

function _cdRestaurarEstadoAutomatico(){
  _cdRestoring = true;
  // Cargar desde Supabase primero, luego localStorage como fallback
  Promise.all([
    _cdSbLoad('saldoCaja_'+_cdFecha).catch(()=>null),
    _cdSbLoad('cajaAyer_'+_cdFecha).catch(()=>null),
    _cdSbLoad('estadoDia_'+_cdFecha).catch(()=>null)
  ]).then(([sbSaldo, sbAyer, sbEstado])=>{
    // Fusionar con localStorage (lo más reciente gana)
    const lsSaldo = (()=>{try{const r=localStorage.getItem('vpos_saldoCaja');return r?JSON.parse(r):null;}catch(e){return null;}})();
    const lsAyer  = (()=>{try{const r=localStorage.getItem('vpos_cajaAyer'); return r?JSON.parse(r):null;}catch(e){return null;}})();
    const lsEst   = (()=>{try{const r=localStorage.getItem('vpos_cierre_estado');return r?JSON.parse(r):null;}catch(e){return null;}})();
    // Usar Supabase si existe, si no localStorage
    const saldo  = sbSaldo  || lsSaldo  || _cdSaldoCajaPersist;
    const ayer   = sbAyer   || lsAyer   || _cdCajaAyerPersist;
    const estado = sbEstado || lsEst;
    ...
    setTimeout(()=>{
      ...
      // 3. Estado general (gastos, venta, cambios)
      if(estado){
        ...
        _cdGastos=estado.gastos||[];
        _cdCambiosAplicados=estado.cambios||[];
        if(estado.ventaSnapshot) _cdVentaSnapshot=estado.ventaSnapshot;
      }
      _cdRenderListas();
      _cdActualizarStats();
      setTimeout(()=>{ _cdRestoring=false; }, 200);
    },60);
  }).catch(()=>{
    // Si falla Supabase, usar solo localStorage
    setTimeout(()=>{
      ...
      if(raw){
        ...
        if(data.ventaSnapshot) _cdVentaSnapshot=data.ventaSnapshot;
      }
      _cdRenderListas();
      _cdActualizarStats();
      setTimeout(()=>{ _cdRestoring=false; }, 200);
...
function _cdCalcular(etapa){
  ...
  if(etapa==='venta'){
    ...
    _cdVentaSnapshot = {total:_snapVTotal, alqHoy:alqHoy, montos:{..._snapMontos}};
    const t=_cdTotalM(_cdLeerMontos('cdSaldo'));
    const msg=alqHoy>0
...
function _cdGuardarAyer(){
  ...
  try{ localStorage.setItem('vpos_cajaAyer', JSON.stringify(_cdCajaAyerPersist)); }catch(e){}
  _cdSaldoAyerCache = {...A};
  // Supabase + realtime
  _cdSbSave('cajaAyer_'+_cdFecha, _cdCajaAyerPersist).catch(()=>{});
  if(typeof _broadcast==='function') _broadcast('cierre_cajaayer_actualizado',{fecha:_cdFecha,datos:_cdCajaAyerPersist});
...
function _cdBorrarDatosCierre(){
  ...
  // Borrar estado temporal local
  localStorage.removeItem('vpos_cierre_estado');
  // Borrar de Supabase también
  const estadoVacio={fecha:_cdFecha,ventaTotal:0,ventaAlquiler:0,alquiler:0,ayerAlquiler:0,
    venta:{},ventaSnapshot:null,gastos:[],cambios:[],borrado:true,ts:Date.now()};
  _cdSbSave('estadoDia_'+_cdFecha, estadoVacio).catch(()=>{});
  // Notificar al otro dispositivo en tiempo real
  if(typeof _broadcast==='function') _broadcast('cierre_borrado',{fecha:_cdFecha});
  _cdRenderListas();
  _cdActualizarStats();
  if(typeof toast==='function'){ toast('🗑 Datos temporales borrados'); }
}
...
function _cdRegistrarRealtimeListener() {
  if (typeof window === 'undefined') return;
  // Escuchar eventos de broadcast del otro dispositivo
  window.addEventListener('cd_realtime', (e) => {
    const msg = e.detail || {};
    const hoy = new Date().toISOString().split('T')[0];
    if (msg.fecha && msg.fecha !== _cdFecha && msg.fecha !== hoy) return; // diferente fecha, ignorar

    if (msg.tipo === 'cierre_estado_actualizado' && msg.data) {
      const d = msg.data;
      if (_cdRestoring) return;
      _cdRestoring = true;
      setTimeout(() => {
        ...
        _cdGastos = d.gastos || [];
        _cdCambiosAplicados = d.cambios || [];
        if (d.ventaSnapshot) _cdVentaSnapshot = d.ventaSnapshot;
        _cdRenderListas();
        _cdActualizarStats();
        _cdRestoring = false;
        if (typeof toast === 'function') toast('🔄 Cierre actualizado desde otro dispositivo');
      }, 80);
    }

    if (msg.tipo === 'cierre_saldo_actualizado' && msg.saldo) {
      if (_cdRestoring) return;
      _cdRestoring = true;
      setTimeout(() => {
        _cdSaldoCajaPersist = msg.saldo;
        _CD_DENOMS.forEach(dn => _cdSet('cdSaldo'+dn.id, msg.saldo.montos?.[dn.id]||0, true));
        _cdSet('cdAlquiler', msg.saldo.alquiler||0, true);
        _cdActualizarStats();
        _cdRestoring = false;
      }, 80);
    }

    if (msg.tipo === 'cierre_cajaayer_actualizado' && msg.datos) {
      if (_cdRestoring) return;
      _cdRestoring = true;
      setTimeout(() => {
        _cdCajaAyerPersist = msg.datos;
        _CD_DENOMS.forEach(dn => _cdSet('cdAyer'+dn.id, msg.datos.montos?.[dn.id]||0, true));
        if (msg.datos.alquiler != null) _cdSet('cdAyerAlquiler', msg.datos.alquiler, true);
        _cdActualizarStats();
        _cdRestoring = false;
      }, 80);
    }

    if (msg.tipo === 'cierre_borrado') {
      if (_cdRestoring) return;
...
// Registrar cuando supabase_sync recibe un cambio en cierre_diario via Realtime
// supabase_sync debe llamar: window.dispatchEvent(new CustomEvent('cd_realtime', {detail: payload}))
function _cdManejarRealtimeSB(payload) {
  try {
    const datos = typeof payload.new?.datos === 'string' ? JSON.parse(payload.new.datos) : payload.new?.datos;
    if (!datos) return;
    const clave = payload.new?.fecha || '';
    const fecha = _cdFecha;
    const hoy = new Date().toISOString().split('T')[0];

    if (clave.startsWith('estadoDia_')) {
      const f = clave.replace('estadoDia_', '');
      if (f !== fecha && f !== hoy) return;
      const val = datos.valor !== undefined ? datos.valor : datos;
      window.dispatchEvent(new CustomEvent('cd_realtime', {detail: {tipo:'cierre_estado_actualizado', fecha: f, data: val}}));
    } else if (clave.startsWith('saldoCaja_')) {
      const f = clave.replace('saldoCaja_', '');
      if (f !== fecha && f !== hoy) return;
      const val = datos.valor !== undefined ? datos.valor : datos;
      window.dispatchEvent(new CustomEvent('cd_realtime', {detail: {tipo:'cierre_saldo_actualizado', fecha: f, saldo: val}}));
    } else if (clave.startsWith('cajaAyer_')) {
      const f = clave.replace('cajaAyer_', '');
      if (f !== fecha && f !== hoy) return;
      const val = datos.valor !== undefined ? datos.valor : datos;
      window.dispatchEvent(new CustomEvent('cd_realtime', {detail: {tipo:'cierre_cajaayer_actualizado', fecha: f, datos: val}}));
    }
  } catch(e) {}
}

window._cdManejarRealtimeSB = _cdManejarRealtimeSB;

// Inicializar listener cuando el DOM esté listo
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', _cdRegistrarRealtimeListener);
} else {
  _cdRegistrarRealtimeListener();
}
