...
async function _cdCargarMes(){
  ...
  }catch(e){_cdMesData={saldoInicio:0,inventarioInicial:0,ventas:[],gastos:[]};}
}
...
async function renderCierreDia(pgId){
  ...
  // Lógica correcta:
  // saldoInicio = efectivo al inicio del mes
  // inventarioInicial = valor del inventario al inicio del mes
  // Lo que debería haber = saldoEfec + invIni + ganancias de inventario del mes
  const saldoEfec=(_cdMesData.saldoInicio||0);
  const invIni=(_cdMesData.inventarioInicial||0);
  const saldoTeorico=saldoEfec+invIni+totalInvGanancia;
  ...
  pg.innerHTML=`
    ...
    <div class="cd-body">
      ...
      <!-- QUEDA EN EFECTIVO — persiste entre meses, editable -->
      <div class="cd-panel">
        <div class="cd-panel-header">
          <div class="cd-panel-icon" style="background:#fef3c7;">🪙</div>
          <div class="cd-panel-title">Esto Queda en Efectivo</div>
          <button class="cd-btn-update" onclick="_cdAplicarSaldoAQueda()">🔄 Actualizar</button>
          <button class="cd-btn-update green" onclick="_cdGuardarQuedaManual()">💾 Guardar</button>
        </div>
        <div class="cd-panel-body">
          <div style="font-size:11px;color:#b45309;font-weight:700;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:7px 12px;margin-bottom:10px;font-family:Nunito,sans-serif;">
            🔒 Persiste aunque reinicies el mes. Edita cualquier monto y presiona 💾 Guardar para actualizar.
          </div>
        ...
        <div class="cd-panel-body">
          ...
          <div style="background:#f0fdf4;border:1.5px solid #86efac;border-radius:12px;padding:14px 16px;margin-top:8px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:12px;font-family:Nunito,sans-serif;">
              ...
              <div style="color:var(--text-muted);font-weight:700;">Inventario inicial:</div><div style="font-weight:900;color:#0369a1;">$${invIni.toFixed(2)}</div>
              <div style="color:var(--text-muted);font-weight:700;">Ganancia de inventario:</div><div style="font-weight:900;color:#15803d;">+$${totalInvGanancia.toFixed(2)}</div>
              <div style="color:var(--text-muted);font-weight:700;">Total ventas del mes:</div><div style="font-weight:900;color:#0369a1;">$${totalVentasMes.toFixed(2)}</div>
              <div style="color:var(--text-muted);font-weight:700;">Total gastos del mes:</div><div style="font-weight:900;color:#dc2626;">-$${totalGastosMes.toFixed(2)}</div>
      ...
      <div class="cd-cap-wrap">
        <div class="cd-resumen-captura" id="cdResumenCaptura">
          <div class="cd-cap-inner">
            ...
            <!-- Fila 2: Saldo de Ayer + Saldo de Hoy -->
            <div class="cd-cap-2col">
              <div class="cd-cap-col" id="capAyerWrap" style="display:none;">
                <div class="cd-cap-section-title">📅 Saldo de Ayer</div>
                <div id="capAyerFechaRow" class="cd-cap-row" style="font-size:10px;color:#7c3aed;"><span id="capAyerFechaLbl">—</span><span></span></div>
                ...
                <div class="cd-cap-row"><span>🟡 10 cts</span><span id="capAyerC10">$0.00</span></div>
                <div class="cd-cap-row"><span>🟤 5 cts</span><span id="capAyerC05">$0.00</span></div>
                <div class="cd-cap-row"><span>⚪ 1 cto</span><span id="capAyerC01">$0.00</span></div>
                <div class="cd-cap-row total"><span>Total saldo ayer</span><span class="val-purple" id="capAyerTotal">—</span></div>
                <div id="capVentaAyerRow" style="display:none;"><div class="cd-cap-row" style="color:#0369a1;font-size:11px;margin-top:4px;"><span>Venta de ayer</span><span id="capVentaAyerVal">$0.00</span></div></div>
              </div>
              <div class="cd-cap-col">
            ...
            <!-- Fila 4: Queda en Efectivo — ancho completo, todas las denominaciones en 2 columnas -->
...
function _cdAplicarVentaASaldo(){
  ...
  _cdGuardarMes();
  // FIX 8: Guardar permanentemente por fecha (sobrevive reinicio de mes)
  try{ idbSet('vpos_ventaDia_'+_cdFecha,{fecha:_cdFecha,total:ventaTotal,alquiler:alqHoy,montos:{...V}}); }catch(e){}
...
// Cargar venta del día anterior (permanente, sobrevive reinicio de mes) — Fix 4 + Fix 8
async function _cdCargarVentaAyer(){
  try{
    const ayerIso=new Date(new Date(_cdFecha).getTime()-86400000).toISOString().split('T')[0];
    const r=await idbGet('vpos_ventaDia_'+ayerIso);
    return r||null;
  }catch(e){return null;}
}
...
function _cdActualizarStats(){
  ...
  if(ay){
    ...
    const ayWrap=document.getElementById('capAyerWrap');if(ayWrap)ayWrap.style.display='none';
  }
...
async function _cdGenerarPDFMensual(){
  ...
  // Fórmula: saldoEfectivo + inventarioInicial + ganancias de inventario del mes
  const saldoTeorico=sIni+invIniPDF+totalInvG;
  ...
  row('Inventario inicial del mes',          '$'+invIniPDF.toFixed(2), y,[50,50,50],[3,105,161]); y+=8;
  row('Saldo inicial + Inventario inicial',  '$'+(sIni+invIniPDF).toFixed(2), y,[50,50,50],[22,163,74]); y+=8;
  row('Ganancia de inventario del mes',      '+$'+totalInvG.toFixed(2), y,[50,50,50],[21,128,61]); y+=8;
  rowTotal('DEBERIA HABER EN CAJA AHORA',   '$'+saldoTeorico.toFixed(2), y); y+=14;
  ...
  row('Diferencia ventas - gastos', '$'+(totalACaja-totalG).toFixed(2), y,[50,50,50],[21,128,61]); y+=8;
