...
function _cdSaldoChanged(){
  _cdGuardarSaldoCajaExplicit();   // guarda en localStorage al instante
  _cdFlashBtnGuardar();            // feedback visual en botón
}
...
// Guarda saldo en localStorage — SOLO llamar desde acciones del usuario
function _cdGuardarSaldoCajaExplicit(){
  if(_cdRestoring) return; // bloquear durante carga
  try{
    ...
    console.log('[CierreDia] Saldo guardado en localStorage:', _cdSaldoCajaPersist);
  }catch(e){ console.error('[CierreDia] Error guardando saldo:', e); }
}

// Guarda caja ayer en localStorage — SOLO desde acciones del usuario
function _cdGuardarCajaAyerExplicit(){
  ...
  try{
    ...
    console.log('[CierreDia] Caja ayer guardada:', _cdCajaAyerPersist);
  }catch(e){}
}

function _cdRestaurarEstadoAutomatico(){
  _cdRestoring = true; // bloquear guardado durante todo el proceso de restore
  try{
    setTimeout(()=>{
      // 1. Saldo en caja — desde clave propia (nunca se borra)
      if(_cdSaldoCajaPersist){
        ...
        console.log('[CierreDia] Saldo restaurado:', _cdSaldoCajaPersist);
      }
      // 2. Caja día de ayer — desde clave propia
      ...
      _cdRenderListas();
      _cdActualizarStats();
      // Desbloquear DESPUÉS de que todo esté restaurado
      setTimeout(()=>{ _cdRestoring=false; }, 200);
    },60);
  }catch(e){ _cdRestoring=false; }
}
...
function _cdCalcular(etapa){
  // FIX: Cambios del día debe trabajar sobre el saldo actual y no reconstruir la caja.
  if(etapa==='cambios'){
    const movs=_cdCambiosPendientes();
    ...
    if(typeof toast==='function'){
      toast(`✓ Cambios aplicados al saldo actual — Caja: $${t.toFixed(2)}`);
    }
    return;
  }
