...
function _cdRestaurarEstadoAutomatico(){
  ...
  const _aplicar = (saldo, ayer, estado, gastos, cambios, ventaSnap) => {
    ...
    // ventaSnapshot: usar clave propia si es más reciente
    if(ventaSnap?.ventaSnapshot) {
...
function _cdAplicarVentaASaldo(){
  ...
  const snapData={ventaSnapshot:_cdVentaSnapshot,fecha:_cdFecha,ts:ventaData.ts};
  try{localStorage.setItem('vpos_cd_ventaSnap_'+_cdFecha, JSON.stringify({valor:snapData,ts:ventaData.ts}));}catch(e){}
  _cdSbSave('ventaSnap_'+_cdFecha, snapData).catch(()=>{});
