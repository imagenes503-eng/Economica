...
(function _estilosCierre() {
  ...
  s.textContent = `
    ...
    .cd-resumen-captura{background:#fff;border:2px solid #bfdbfe;border-radius:16px;font-family:var(--cd-ff);width:100%;box-sizing:border-box;overflow:hidden;}
    .cd-cap-inner{padding:20px 16px;box-sizing:border-box;}
    .cd-cap-title{font-size:20px;font-weight:800;color:#0f172a;text-align:center;margin-bottom:3px;letter-spacing:-0.5px;}
    .cd-cap-fecha{font-size:12px;font-weight:600;color:#3b82f6;text-align:center;margin-bottom:14px;}
    .cd-cap-divider{height:2px;background:linear-gradient(90deg,transparent,#3b82f6,transparent);border-radius:2px;margin:12px 0;}
    ...
    .cd-cap-col{border:1.5px solid #bfdbfe;border-radius:10px;padding:10px;background:#f8fafc;}
    .cd-cap-section-title{font-size:10px;font-weight:800;color:#fff;background:linear-gradient(135deg,#1e40af,#3b82f6);border-radius:6px;padding:4px 8px;margin-bottom:7px;text-align:center;text-transform:uppercase;letter-spacing:0.5px;}
    .cd-cap-row{display:flex;justify-content:space-between;align-items:center;font-size:11px;font-weight:600;color:#1e3a5f;padding:4px 0;border-bottom:1px solid #e0efff;}
    .cd-cap-row:last-child{border-bottom:none;}
    .cd-cap-row.total{font-size:12px;font-weight:800;border-top:2px solid #3b82f6;border-bottom:none;margin-top:4px;padding-top:6px;background:#eff6ff;border-radius:0 0 6px 6px;padding-left:4px;padding-right:4px;}
    .cd-cap-row.grand{font-size:13px;font-weight:800;color:#0f172a;background:#eff6ff;border-radius:10px;padding:9px 12px;margin-top:6px;border-bottom:none;border:1.5px solid #bfdbfe;}
    /* Print mode */
    .cd-print-mode .cd-resumen-captura{width:1080px!important;border:none!important;margin:0!important;border-radius:0!important;}
    .cd-print-mode .cd-cap-inner{padding:24px 28px 20px!important;}
    .cd-print-mode .cd-cap-title{font-size:28px!important;margin-bottom:4px!important;}
    .cd-print-mode .cd-cap-fecha{font-size:14px!important;margin-bottom:14px!important;}
    .cd-print-mode .cd-cap-2col{gap:12px!important;margin-bottom:12px!important;grid-template-columns:1fr 1fr!important;}
    .cd-print-mode .cd-cap-col{padding:12px 14px!important;border-width:1.5px!important;}
    .cd-print-mode .cd-cap-section-title{font-size:10px!important;padding:5px 8px!important;margin-bottom:8px!important;}
    .cd-print-mode .cd-cap-row{font-size:12px!important;padding:5px 0!important;border-bottom-width:1px!important;}
    .cd-print-mode .cd-cap-row.total{font-size:13px!important;margin-top:4px!important;padding-top:7px!important;}
    .cd-print-mode .cd-cap-row.grand{font-size:14px!important;padding:8px 12px!important;margin-top:6px!important;}
    .cd-print-mode .cd-cap-gastos-row{padding:4px 0!important;border-bottom:1px solid #e0efff!important;}
    .cd-print-mode .cd-cap-gastos-desc,.cd-print-mode .cd-cap-gastos-total{font-size:12px!important;}
    .cd-print-mode .cd-cap-gastos-denoms{font-size:10px!important;}
    /* Util colors */
...
function _cdGuardarEstadoAutomatico(){
  try{
    ...
    // Always update persist globals and dedicated localStorage keys
    _cdSaldoCajaPersist={montos:saldoM,alquiler:alqVal};
    _cdCajaAyerPersist={montos:ayerM,alquiler:alqAyer,fecha:_cdFecha};
    ...
    const data={
      ...
      alquiler:alqVal,ayerAlquiler:alqAyer,
      venta:_cdLeerMontos('cdVenta'),saldo:saldoM,ayer:ayerM,
      gastos:_cdGastos,cambios:_cdCambiosAplicados
    };
...
function _cdRestaurarEstadoAutomatico(){
  try{
    // Use pre-loaded globals (set at module start, survive re-renders)
    setTimeout(()=>{
      // Saldo en caja — always from dedicated key, never wiped by borrar datos
      if(_cdSaldoCajaPersist){
        _CD_DENOMS.forEach(d=>_cdSet('cdSaldo'+d.id,_cdSaldoCajaPersist.montos?.[d.id]||0,true));
        _cdSet('cdAlquiler',_cdSaldoCajaPersist.alquiler||0,true);
      }
      // Caja día de ayer — same
      if(_cdCajaAyerPersist){
        _CD_DENOMS.forEach(d=>_cdSet('cdAyer'+d.id,_cdCajaAyerPersist.montos?.[d.id]||0,true));
        if(_cdCajaAyerPersist.alquiler!=null) _cdSet('cdAyerAlquiler',_cdCajaAyerPersist.alquiler,true);
      }
      // Venta y estado general (sí se borra con borrar datos)
...
async function _cdTomarCaptura(){
  ...
  try{
    ...
    const c=await window.html2canvas(el,{
      ...
      width:OUT_W,height:realH,windowWidth:OUT_W,
      scrollX:0,scrollY:-window.scrollY,logging:false
    });
    document.body.classList.remove('cd-print-mode');
    // Scale captured content to fill 9:16 canvas (fit width, center vertically)
    const outCanvas=document.createElement('canvas');
    outCanvas.width=OUT_W*2; outCanvas.height=OUT_H*2; // 2× final size
    const ctx=outCanvas.getContext('2d');
    // Background gradient matching the app
    const grad=ctx.createLinearGradient(0,0,0,OUT_H*2);
    grad.addColorStop(0,'#f0f9ff');grad.addColorStop(1,'#f8fafc');
    ctx.fillStyle=grad;
    ctx.fillRect(0,0,OUT_W*2,OUT_H*2);
    // Scale content to fit width exactly; position at top
    const scaleX=OUT_W*2/c.width; // should be 1 since we captured at 2×
    const drawW=Math.round(c.width*scaleX);
    const drawH=Math.round(c.height*scaleX);
    const dy=Math.min(0, Math.round((OUT_H*2-drawH)/2)); // top-align if taller than 9:16
    ctx.drawImage(c,0,dy,drawW,drawH);
