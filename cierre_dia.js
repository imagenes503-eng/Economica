...
// Auto-guarda con feedback visual en el botón (se llama desde oninput de cdSaldo)
let _cdAutoGuardarTimer = null;
function _cdAutoGuardarSaldo(){
  // Debounce: esperar 800ms tras el último cambio antes de guardar
  clearTimeout(_cdAutoGuardarTimer);
  _cdAutoGuardarTimer = setTimeout(()=>{
    _cdGuardarSaldoCajaExplicit();
    _cdFlashBtnGuardar();
  }, 800);
}

// Feedback visual: el botón destella verde brevemente
...
function _cdBloqueMontosHTML(px){
  ...
  return`<div class="cd-montos-grid">
    <div class="cd-field"><label>💵 Billetes ($)</label><input class="cd-inp" type="number" id="${px}Billetes" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();if(px==='cdSaldo')_cdAutoGuardarSaldo()"${extraBlur}></div>
    <div class="cd-field"><label>🪙 M. Dólar ($)</label><input class="cd-inp" type="number" id="${px}Monedas" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();if(px==='cdSaldo')_cdAutoGuardarSaldo()"${extraBlur}></div>
    <div class="cd-field"><label>🔵 Coras ($)</label><input class="cd-inp" type="number" id="${px}Coras" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();if(px==='cdSaldo')_cdAutoGuardarSaldo()"${extraBlur}></div>
    <div class="cd-field"><label>🟡 10 centavos ($)</label><input class="cd-inp" type="number" id="${px}C10" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();if(px==='cdSaldo')_cdAutoGuardarSaldo()"${extraBlur}></div>
    <div class="cd-field"><label>🟤 5 centavos ($)</label><input class="cd-inp" type="number" id="${px}C05" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();if(px==='cdSaldo')_cdAutoGuardarSaldo()"${extraBlur}></div>
    <div class="cd-field"><label>⚪ 1 centavo ($)</label><input class="cd-inp" type="number" id="${px}C01" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();if(px==='cdSaldo')_cdAutoGuardarSaldo()"${extraBlur}></div>
  </div>`;
}
      ...
      <div class="cd-panel" style="border:2px solid #15803d;">
        ...
        <div class="cd-panel-body">
          ...
          <div class="cd-field" style="margin-top:12px;">
            ...
            <input class="cd-inp" type="number" id="cdAlquiler" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats();_cdAutoGuardarSaldo()">
          </div>
...
window._cdAutoGuardarSaldo = _cdAutoGuardarSaldo;
