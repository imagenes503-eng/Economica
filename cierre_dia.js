...
(function _estilosCierre() {
  ...
  s.textContent = `
    ...
    .cd-panel-header{cursor:pointer;}
    .cd-panel-body{display:none;}
    .cd-panel-toggle{font-size:18px;font-weight:900;color:var(--text-muted);margin-left:auto;transition:transform .2s;}
    .cd-panel-toggle-icon::after{content:'›';font-size:20px;font-weight:900;color:var(--text-muted);}
    .cd-panel-header { display:flex;align-items:center;gap:9px;padding:12px 16px;background:var(--surface);flex-wrap:wrap;gap:6px; }
    /* ── Pop-up modal overlay ── */
    .cd-modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9000;display:flex;align-items:flex-end;justify-content:center;opacity:0;pointer-events:none;transition:opacity .25s;}
    .cd-modal-overlay.active{opacity:1;pointer-events:all;}
    .cd-modal-sheet{background:var(--surface,#fff);border-radius:20px 20px 0 0;width:100%;max-width:600px;max-height:92vh;display:flex;flex-direction:column;transform:translateY(100%);transition:transform .3s cubic-bezier(.32,.72,0,1);padding-bottom:env(safe-area-inset-bottom,16px);box-shadow:0 -8px 40px rgba(0,0,0,0.18);}
    .cd-modal-overlay.active .cd-modal-sheet{transform:translateY(0);}
    .cd-modal-handle{width:44px;height:5px;background:var(--border,#ddd);border-radius:3px;margin:14px auto 10px;display:block;flex-shrink:0;}
    .cd-modal-header{display:flex;align-items:center;gap:10px;padding:0 16px 14px;border-bottom:1.5px solid var(--border);flex-shrink:0;}
    .cd-modal-icon{width:34px;height:34px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:17px;flex-shrink:0;}
    .cd-modal-title{font-size:16px;font-weight:900;color:var(--text);font-family:Nunito,sans-serif;flex:1;}
    .cd-modal-close{background:var(--surface2,#f0f0f0);border:none;border-radius:50%;width:34px;height:34px;display:flex;align-items:center;justify-content:center;font-size:19px;cursor:pointer;color:var(--text-muted);flex-shrink:0;line-height:1;transition:background .15s;}
    ...
    .cd-modal-actions{display:flex;gap:10px;padding:12px 16px 8px;border-top:1.5px solid var(--border);flex-shrink:0;flex-wrap:wrap;background:var(--surface,#fff);}
    .cd-modal-actions .cd-btn-update{flex:1;min-width:110px;padding:12px 14px;font-size:13px;border-radius:11px;}
    ...
    /* Print mode: 1080px ancho, ajuste exacto al contenido real */
    .cd-print-mode .cd-resumen-captura { width:1080px!important;border:none!important;margin:0!important; }
    .cd-print-mode .cd-cap-inner { padding:20px 22px 16px!important; }
    .cd-print-mode .cd-cap-title { font-size:26px!important;margin-bottom:3px!important; }
    .cd-print-mode .cd-cap-fecha { font-size:13px!important;margin-bottom:12px!important; }
    .cd-print-mode .cd-cap-2col { gap:10px!important;margin-bottom:10px!important;grid-template-columns:1fr 1fr!important; }
    .cd-print-mode .cd-cap-col { padding:10px 12px!important;border-width:1.5px!important; }
    .cd-print-mode .cd-cap-section-title { font-size:10px!important;padding:4px 7px!important;margin-bottom:7px!important; }
    .cd-print-mode .cd-cap-row { font-size:11px!important;padding:4px 0!important;border-bottom-width:1px!important; }
    .cd-print-mode .cd-cap-row.total { font-size:12px!important;border-top-width:1.5px!important;margin-top:3px!important;padding-top:6px!important; }
    .cd-print-mode .cd-cap-row.grand { font-size:13px!important;padding:7px 10px!important;margin-top:5px!important; }
    .cd-print-mode .cd-cap-gastos-row { padding:4px 0!important;border-bottom:1px solid #bae6fd!important; }
    .cd-print-mode .cd-cap-gastos-desc,.cd-print-mode .cd-cap-gastos-total { font-size:11px!important; }
    .cd-print-mode .cd-cap-gastos-denoms { font-size:9px!important; }
...
function _cdGuardarEstadoAutomatico(){
  try{
    ...
    // Persistir saldo en caja y caja ayer en clave propia (no se borra con borrar datos)
    localStorage.setItem('vpos_saldoCaja', JSON.stringify({montos:saldoM,alquiler:alqVal}));
    localStorage.setItem('vpos_cajaAyer', JSON.stringify({montos:ayerM,alquiler:alqAyer,fecha:_cdFecha}));
...
function _cdRestaurarEstadoAutomatico(){
  try{
    const raw = localStorage.getItem('vpos_cierre_estado');
    const rawSaldo = localStorage.getItem('vpos_saldoCaja');
    const rawAyer  = localStorage.getItem('vpos_cajaAyer');

    setTimeout(()=>{
      // Restore saldo en caja (persiste aunque se borre el estado general)
      if(rawSaldo){
        const sd=JSON.parse(rawSaldo);
        _CD_DENOMS.forEach(d=>_cdSet('cdSaldo'+d.id, sd.montos?.[d.id]||0, true));
        _cdSet('cdAlquiler', sd.alquiler||0, true);
      }
      // Restore caja día de ayer
      if(rawAyer){
        const ay=JSON.parse(rawAyer);
        _CD_DENOMS.forEach(d=>_cdSet('cdAyer'+d.id, ay.montos?.[d.id]||0, true));
        if(ay.alquiler!=null) _cdSet('cdAyerAlquiler', ay.alquiler, true);
      }
      // Restore rest of state
      if(raw){
        const data=JSON.parse(raw);
        _cdSet('cdVentaTotal', data.ventaTotal||0, true);
        _cdSet('cdVentaAlquilerHoy', data.ventaAlquiler||0, true);
        _CD_DENOMS.forEach(d=>_cdSet('cdVenta'+d.id, data.venta?.[d.id]||0, true));
        _cdGastos = data.gastos || [];
        _cdCambiosAplicados = data.cambios || [];
      }
      _cdRenderListas();
      _cdActualizarStats();
    },120);
...
function _cdBloqueMontosHTML(px){
  // Para cdSaldo: auto-actualizar queda al cambiar valores
  const extraBlur = px==='cdSaldo' ? ` onblur="_cdAplicarSaldoAQueda()"` : '';
  return`<div class="cd-montos-grid">
    <div class="cd-field"><label>💵 Billetes ($)</label><input class="cd-inp" type="number" id="${px}Billetes" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats()"${extraBlur}></div>
    <div class="cd-field"><label>🪙 M. Dólar ($)</label><input class="cd-inp" type="number" id="${px}Monedas" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats()"${extraBlur}></div>
    <div class="cd-field"><label>🔵 Coras ($)</label><input class="cd-inp" type="number" id="${px}Coras" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats()"${extraBlur}></div>
    <div class="cd-field"><label>🟡 10 centavos ($)</label><input class="cd-inp" type="number" id="${px}C10" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats()"${extraBlur}></div>
    <div class="cd-field"><label>🟤 5 centavos ($)</label><input class="cd-inp" type="number" id="${px}C05" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats()"${extraBlur}></div>
    <div class="cd-field"><label>⚪ 1 centavo ($)</label><input class="cd-inp" type="number" id="${px}C01" min="0" step="0.01" placeholder="0.00" oninput="_cdActualizarStats()"${extraBlur}></div>
  </div>`;
}
...
async function renderCierreDia(pgId){
  ...
  const cambioGrid=_CD_DENOMS.map(d=>`
    <div class="cd-cambio-item">
      ...
      <div class="cd-field" style="margin-bottom:6px;"><label>Monto ($)</label><input class="cd-inp" type="number" id="cdCambioSale${d.id}" min="0" step="0.01" placeholder="0.00" oninput="_cdAutoRecalcSaldo()"></div>
  ...
  pg.innerHTML=`
    ...
    <div class="cd-body">
      ...
      <div class="cd-panel">
        ...
        <div class="cd-panel-body">
          <div style="font-size:11px;color:var(--text-muted);font-weight:700;margin-bottom:12px;">Sale de una denominación → entra en otra. Se actualiza el saldo en tiempo real.</div>
          ...
          <div class="cd-cap-inner">
            ...
            <div class="cd-cap-2col">
              <div class="cd-cap-col" id="capAyerWrap">
                <div class="cd-cap-section-title">📦 Caja Día de Ayer</div>
            ...
            <div class="cd-cap-col" style="margin-bottom:12px;border:2px solid #0369a1;border-radius:10px;overflow:hidden;">
              <div class="cd-cap-section-title" style="background:#0369a1;border-radius:0;margin-bottom:0;padding:8px 12px;">🏦 Saldo en Caja — <span id="capSaldoFecha"></span></div>
              <div style="padding:8px 4px 0;">
                ...
                <div class="cd-cap-row total" style="background:#e0f2fe;"><span style="color:#0369a1;font-weight:900;">💰 Total físico en caja</span><span class="val-pos" id="capSaldoTotal" style="font-size:1.1em;">$0.00</span></div>
              </div>
            </div>
  ...
  `;
  _cdRenderListas();_cdAutoRecalcSaldo();_cdRestaurarEstadoAutomatico();
  ...
  // Restaurar "Caja Día de Ayer" — no se borra con el borrar datos
  try{
    const ayerGuardado = JSON.parse(localStorage.getItem('vpos_cajaAyer')||'null');
    if(ayerGuardado?.montos){
      setTimeout(()=>{
        _CD_DENOMS.forEach(d=>_cdSet('cdAyer'+d.id, ayerGuardado.montos[d.id]||0));
        // FIX: restaurar también el alquiler acumulado ayer
        if(ayerGuardado.alquiler!=null) _cdSet('cdAyerAlquiler', ayerGuardado.alquiler);
        _cdActualizarStats();
      },80);
    }
  }catch(e){}
...
//   cambios → Saldo = Venta + Ayer − Gastos ± Cambios (redistribución)
//   saldo   → igual que cambios (botón en Saldo en Caja)
...
if(etapa==='venta'){
  ...
  // Paso 3: Aplicar Cambios SOLO sobre el saldo actual
  const movs=_cdCalcularCambiosPendientes();
  ...
  if(etapa==='cambios'){
    ...
    // Limpiar cambios después de calcular
    _CD_DENOMS.forEach(d=>{
    ...
    if(typeof toast==='function'){
      toast(`✓ Cambios aplicados correctamente — Caja: $${t.toFixed(2)}`);
    }
  ...
  // saldo (botón en Saldo en Caja): recalculo completo incluyendo cambios
  movs.forEach(m=>{
    _cdSet('cdSaldo'+m.deId,   Math.max(0,_cdV('cdSaldo'+m.deId)-m.monto));
    _cdSet('cdSaldo'+m.haciaId, _cdV('cdSaldo'+m.haciaId)+m.monto);
  });
...
function _cdGuardarAyer(){
  // Guardar automáticamente el saldo actual como caja del día anterior
  const A=_cdLeerMontos('cdSaldo');
  const totalAyer=_cdTotalM(A);
  // FIX: también copiar el alquiler acumulado al campo "Alquiler acumulado ayer"
  const alqAcum=_cdV('cdAlquiler');

  // Reflejar visualmente en Caja Día de Ayer
  _CD_DENOMS.forEach(d=>{
    _cdSet('cdAyer'+d.id, A[d.id]||0);
  });
  // Actualizar el campo de alquiler acumulado ayer
  _cdSet('cdAyerAlquiler', alqAcum);

  try{
    localStorage.setItem('vpos_cajaAyer', JSON.stringify({
      montos:A,
      total:totalAyer,
      fecha:_cdFecha,
      alquiler:alqAcum
  ...
  if(typeof toast==='function'){
    toast(`💾 Caja del día guardada correctamente — $${totalAyer.toFixed(2)}`);
  }
  _cdCerrarModal();
  _cdActualizarStats();
}
...
function _cdBorrarDatosCierre(){
  if(!confirm('¿Borrar datos temporales del cierre?')) return;

  // Mantener saldo en caja y caja día de ayer
  _cdGastos=[];
  _cdDeudas=[];
  _cdCambiosAplicados=[];
  _cdVentaSnapshot=null;

  const campos=['cdVentaTotal','cdVentaAlquilerHoy'];
  ...
  if(typeof toast==='function'){
    toast('🗑 Datos temporales borrados');
  }
}
...
async function _cdTomarCaptura(){
  ...
  try{
    ...
    const c=await window.html2canvas(el,{
      ...
      width:1080,height:realH,windowWidth:1080,
      scrollX:0,scrollY:-window.scrollY,logging:false
    });
    document.body.classList.remove('cd-print-mode');
    // Output = exact content size, no padding/centering
    const o=document.createElement('canvas');
    o.width=c.width;o.height=c.height;
    o.getContext('2d').drawImage(c,0,0);
    const lnk=document.createElement('a');
    lnk.download=`Cierre_${_cdFecha}.png`;lnk.href=o.toDataURL('image/png');
    document.body.appendChild(lnk);lnk.click();document.body.removeChild(lnk);
    if(typeof toast==='function')toast(`📸 Imagen ${c.width}×${c.height}px descargada`);
  }catch(e){
    document.body.classList.remove('cd-print-mode');
    if(typeof toast==='function')toast('⚠ Error: '+e.message,true);
  }
  finally{if(btn){btn.disabled=false;btn.innerHTML='📸 Descargar imagen';}}
}
