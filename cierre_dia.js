// =====================================================================
//  📋 CIERRE DIARIO DE CAJA — Despensa Económica v2
// =====================================================================
...
(function _inyectarEstilosCierre() {
  ...
  s.textContent = `
    ...
    /* Resumen captura */
    .cd-resumen-captura { background:#fff;border:2px solid #0369a1;border-radius:16px;padding:18px;font-family:Nunito,sans-serif; }
    .cd-cap-title { font-size:16px;font-weight:900;color:#0c4a6e;margin-bottom:4px;text-align:center;letter-spacing:0.3px; }
    ...
    .cd-cap-row.total { font-size:15px;font-weight:900;border-top:1.5px solid var(--border);margin-top:4px;padding-top:6px; }
    .val-pos { color:#15803d; }
    .val-neg { color:#dc2626; }
    .val-warn { color:#b45309; }
    .val-blue { color:#0369a1; }
    .val-purple { color:#7c3aed; }
    .cd-cap-divider { height:1px;background:#e0f2fe;margin:10px 0; }
...
function _cdFmtFecha(iso) {
  ...
  return `${dNames[new Date(iso+'T12:00:00').getDay()]} ${d}/${m}/${y.slice(2)}`;
}
...
function _cdLeerMontos(px) {
  return {
    billetes: _cdV(px+'Billetes'), monedas: _cdV(px+'Monedas'), coras: _cdV(px+'Coras'),
...
async function _cdSubirSupabase(cierre) {
  ...
  try {
    await _sbPost('cierre_diario',{
      id: _getTiendaId()+'_'+cierre.fecha, tienda_id:_getTiendaId(),
      fecha:cierre.fecha, datos:JSON.stringify(cierre), updated_at:new Date().toISOString()
    },true);
    return true;
...
// ══ Bloque de inputs de montos (reutilizable) ════════════════════════════
function _cdBloqueMontosHTML(px) {
  return `
    <div class="cd-montos-grid">
...
async function renderCierreDia(pgId) {
  ...
  pg.innerHTML = `
    ...
    <div class="cd-body">
      ...
      <!-- ⑧ RESUMEN PARA CAPTURA DE PANTALLA -->
      <div class="cd-resumen-captura" id="cdResumenCaptura">
        ...
        <div class="cd-cap-section">
          <div class="cd-cap-section-title">📊 Cálculo del Día (billetes)</div>
          <div class="cd-cap-row"><span>  💵 Billetes de la venta</span><span id="capCalcBilletes">$0.00</span></div>
          <div class="cd-cap-row"><span>  − Gastos del día</span><span class="val-neg" id="capCalcGastos">$0.00</span></div>
          <div class="cd-cap-row"><span>  + Billetes previos en caja</span><span id="capCalcSaldoPrev">$0.00</span></div>
          <div class="cd-cap-row total"><span>= Total billetes en caja</span><span class="val-pos" id="capCalcResult">$0.00</span></div>
          <div class="cd-cap-row" style="margin-top:6px;"><span>  🏘 Alquiler acumulado (apartado)</span><span class="val-warn" id="capCalcAlquiler">$0.00</span></div>
        </div>
        ...
        <div class="cd-cap-section">
          ...
          <div id="capGastosList"><div class="cd-cap-row"><span>Sin gastos registrados</span><span>—</span></div></div>
...
function _cdRenderListas() {
  ...
  if (dEl) dEl.innerHTML = _cdDeudas.length
    ? _cdDeudas.map(x=>`<div class="cd-item-row"><span class="cd-item-desc">${x.desc}</span><span class="cd-item-monto pos">$${Number(x.monto).toFixed(2)}</span><button class="cd-item-del" onclick="_cdEliminarDeuda('${x.id}')">✕</button></div>`).join('')
...
function _cdActualizarStats() {
  ...
  // Cálculo principal de billetes
  const calcBill = V.billetes - totalGastos + S.billetes;
  const $ = _cdFmt;
  ...
  // Cálculo billetes
  _cdTxt('capCalcBilletes',  $(V.billetes));
  _cdTxt('capCalcGastos',    $(-totalGastos));
  _cdTxt('capCalcSaldoPrev', $(S.billetes));
  _cdTxt('capCalcResult',    $(calcBill));
  _cdTxt('capCalcAlquiler',  $(alquiler));
  ...
  // Gastos lista
  const capG = document.getElementById('capGastosList');
  if (capG) capG.innerHTML = _cdGastos.length
    ? _cdGastos.map(x=>`<div class="cd-cap-row"><span>  ${x.desc}</span><span class="val-neg">$${Number(x.monto).toFixed(2)}</span></div>`).join('')
    : `<div class="cd-cap-row"><span>Sin gastos registrados</span><span>—</span></div>`;
...
function _cdCargarCierreEnForm(c) {
  ...
  const setSec=(px,obj)=>{ if(!obj)return; _cdSet(px+'Billetes',obj.billetes);_cdSet(px+'Monedas',obj.monedas);_cdSet(px+'Coras',obj.coras);_cdSet(px+'C10',obj.c10);_cdSet(px+'C05',obj.c05);_cdSet(px+'C01',obj.c01); };
  setSec('cdVenta',c.ventaMontos); setSec('cdSaldo',c.saldoMontos); setSec('cdCambio',c.cambioMontos); setSec('cdQueda',c.quedaMontos);
...
async function _cdGuardarCierre() {
  ...
  const V=_cdLeerMontos('cdVenta'), S=_cdLeerMontos('cdSaldo'), C=_cdLeerMontos('cdCambio'), Q=_cdLeerMontos('cdQueda');
  const alquiler=_cdV('cdAlquiler');
  const cierre={
    ...
    ventaMontos:V, saldoMontos:S, cambioMontos:C, quedaMontos:Q, alquiler,
    gastos:[..._cdGastos], deudas:[..._cdDeudas],
    totalGastos:_cdSumArr(_cdGastos), totalCambios:_cdTotalM(C),
    totalDeudas:_cdSumArr(_cdDeudas), totalQueda:_cdTotalM(Q),
    saldoNeto:_cdTotalM(S)+ventaTotal-_cdSumArr(_cdGastos)-_cdTotalM(C),
