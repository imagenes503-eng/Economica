...
function renderDashboardPro() {
  ...
  // ── Ganancia del día ──
  // Usar costoItem si está guardado (ventas recientes), si no, compra unitaria real × cant.
  // NUNCA usar prod.compra directo cuando se vende un paquete, porque compra es precio
  // por unidad y cant puede ser 1 (el paquete entero), lo que infla la ganancia.
  const gananciaHoy = ventasHoy.reduce((acc, v) => {
    return acc + (v.items || []).reduce((a, it) => {
      // FIX: usar totalItem (fuente exacta guardada al vender) para evitar que
      // precio_paquete × unidades_físicas infle la ganancia.
      // Ej: Paquete $7.50 con 100 uds → precio=$7.50, cant=100 → $750 INCORRECTO.
      // totalItem = precio × cantCobrada = $7.50 × 1 = $7.50 CORRECTO.
      let ventaTotal;
      if (it.totalItem !== undefined) {
        ventaTotal = Number(it.totalItem);
      } else if (it.cantCobrada !== undefined) {
        ventaTotal = parseFloat(it.precio || 0) * Number(it.cantCobrada || 1);
      } else {
        ventaTotal = parseFloat(it.precio || 0) * Number(it.cant || 1);
      }
      let costoTotal;
      if (it.costoItem !== undefined) {
        costoTotal = Number(it.costoItem || 0);
      } else {
        const prod = (productos || []).find(p => String(p.id) === String(it.id) || p.nom === it.nom);
        const compraUd = prod ? (typeof _compraUdReal === 'function' ? _compraUdReal(prod) : parseFloat(prod.compra || 0)) : 0;
        costoTotal = compraUd * Number(it.cant || 1);
      }
      return a + (ventaTotal - costoTotal);
    }, 0);
  }, 0);
  ...
  // ── Render ──
  // ── Últimas ventas (para el panel) ──────────────────────────────────────
  const ultimasVentas = ventasHoy.slice().reverse().slice(0,5);
  ...
  cont.innerHTML = `
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:18px;">
      <div>
        ...
        <div style="font-size:12px;color:var(--text-muted);font-weight:700;margin-top:2px;">
          ${hoy.toLocaleDateString('es-SV',{weekday:'long',year:'numeric',month:'long',day:'numeric'})}
        </div>
      </div>
      <div style="display:flex;gap:8px;">
        <button class="btn btn-green" onclick="abrirCambioTurno()" style="padding:9px 14px;font-size:12px;border-radius:10px;">
          🔄 Cambiar turno
        </button>
        <button class="btn btn-ghost" onclick="abrirBackupPanel()" style="padding:9px 14px;font-size:12px;border-radius:10px;">
          💾 Backup
        </button>
      </div>
    </div>
    ...
    <!-- Banner turno activo -->
    ${turnoActual ? `
    <div style="background:linear-gradient(135deg,#1e3a5f,#1d4ed8);border-radius:12px;padding:12px 18px;margin-bottom:16px;display:flex;align-items:center;gap:12px;box-shadow:0 4px 14px rgba(29,78,216,.25);">
      <div style="width:38px;height:38px;background:rgba(255,255,255,.15);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;">👤</div>
      <div style="flex:1;">
        <div style="font-weight:900;font-size:13px;color:#fff;">Turno activo: ${turnoActual.cajero}</div>
        <div style="font-size:11px;color:rgba(255,255,255,.65);margin-top:1px;">Inicio: ${turnoActual.inicio} · ${turnoActual.nota || 'Sin nota'}</div>
      </div>
      <div style="font-family:'Space Mono',monospace;font-size:16px;font-weight:700;color:#93c5fd;background:rgba(255,255,255,.1);padding:4px 10px;border-radius:8px;">${_tiempoTurno(turnoActual.tsInicio)}</div>
    </div>` : ''}

    <!-- ── KPIs principales ──────────────────────────────────────── -->
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin-bottom:16px;">
      <div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:2px solid #86efac;border-radius:14px;padding:16px;position:relative;overflow:hidden;">
        <div style="font-size:22px;margin-bottom:6px;">💰</div>
        <div style="font-family:'Space Mono',monospace;font-size:22px;font-weight:900;color:#15803d;">$${totalHoy.toFixed(2)}</div>
        <div style="font-size:10px;font-weight:900;color:#15803d;text-transform:uppercase;letter-spacing:.5px;margin-top:4px;">Ventas hoy</div>
        <div style="font-size:10px;color:#4ade80;font-weight:700;margin-top:2px;">${numVentas} transacciones</div>
      </div>
      <div style="background:linear-gradient(135deg,#f0fdf4,#ecfdf5);border:2px solid #6ee7b7;border-radius:14px;padding:16px;">
        <div style="font-size:22px;margin-bottom:6px;">✨</div>
        <div style="font-family:'Space Mono',monospace;font-size:22px;font-weight:900;color:#059669;">$${gananciaHoy.toFixed(2)}</div>
        <div style="font-size:10px;font-weight:900;color:#059669;text-transform:uppercase;letter-spacing:.5px;margin-top:4px;">Ganancia</div>
        <div style="font-size:10px;color:#34d399;font-weight:700;margin-top:2px;">Margen ${margenHoy}%</div>
      </div>
      <div style="background:linear-gradient(135deg,#eff6ff,#dbeafe);border:2px solid #93c5fd;border-radius:14px;padding:16px;">
        <div style="font-size:22px;margin-bottom:6px;">🏷️</div>
        <div style="font-family:'Space Mono',monospace;font-size:22px;font-weight:900;color:#1d4ed8;">${numVentas}</div>
        <div style="font-size:10px;font-weight:900;color:#1d4ed8;text-transform:uppercase;letter-spacing:.5px;margin-top:4px;">Transacciones</div>
        <div style="font-size:10px;color:#60a5fa;font-weight:700;margin-top:2px;">${numVentas===0?'Sin ventas aún':'Promedio $'+(numVentas>0?(totalHoy/numVentas).toFixed(2):'0')}</div>
      </div>
      <div style="background:linear-gradient(135deg,#faf5ff,#ede9fe);border:2px solid #c4b5fd;border-radius:14px;padding:16px;">
        <div style="font-size:22px;margin-bottom:6px;">📦</div>
        <div style="font-family:'Space Mono',monospace;font-size:22px;font-weight:900;color:#7c3aed;">${(productos||[]).length}</div>
        <div style="font-size:10px;font-weight:900;color:#7c3aed;text-transform:uppercase;letter-spacing:.5px;margin-top:4px;">Productos</div>
        <div style="font-size:10px;color:${prodsBajos.length>0?'#dc2626':'#a78bfa'};font-weight:700;margin-top:2px;">${prodsBajos.length>0?'⚠️ '+prodsBajos.length+' con stock bajo':'✅ Stock OK'}</div>
      </div>
    </div>

    <!-- ── Gráfica de horas + Stock bajo ────────────────────────── -->
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
      ...
      <div style="background:#fff;border:1.5px solid #e5e7eb;border-radius:14px;overflow:hidden;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid #f3f4f6;">
          <div style="font-size:12px;font-weight:900;color:#111827;display:flex;align-items:center;gap:6px;">⏰ <span>Ventas por hora</span></div>
          <span style="font-size:10px;color:#9ca3af;font-weight:700;">Hoy</span>
        </div>
        <div style="padding:12px 14px;">
          ${Object.keys(porHora).length === 0
            ? '<div style="text-align:center;color:#9ca3af;padding:16px;font-size:12px;">Sin ventas aún hoy</div>'
            : `<div style="display:flex;align-items:flex-end;gap:3px;height:80px;">
                ${Array.from({length:14},(_,i)=>i+7).map(h => {
                  const val = porHora[h] || 0;
                  const pct = Math.max(4,(val/maxHora)*70);
                  return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;" title="$${val.toFixed(0)}">
                    <div style="width:100%;background:${val>0?'#22c55e':'#f3f4f6'};border-radius:3px 3px 0 0;height:${pct}px;transition:height .4s;min-height:4px;"></div>
                    <div style="font-size:7px;font-weight:700;color:#9ca3af;">${h}h</div>
                  </div>`;
                }).join('')}
              </div>`}
        </div>
      ...
      <!-- Stock bajo -->
      <div style="background:#fff;border:1.5px solid ${prodsBajos.length>0?'#fca5a5':'#e5e7eb'};border-radius:14px;overflow:hidden;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid #f3f4f6;">
          <div style="font-size:12px;font-weight:900;color:#111827;display:flex;align-items:center;gap:6px;">⚠️ <span>Stock bajo</span></div>
          <span style="background:${prodsBajos.length>0?'rgba(220,38,38,.1)':'#f0fdf4'};color:${prodsBajos.length>0?'#dc2626':'#15803d'};border-radius:100px;padding:2px 8px;font-size:11px;font-weight:900;">${prodsBajos.length}</span>
        </div>
        <div style="padding:8px 14px;">
          ${prodsBajos.length === 0
            ? '<div style="text-align:center;color:#9ca3af;padding:12px;font-size:12px;">✅ Todo el stock está bien</div>'
            : prodsBajos.slice(0,3).map(p => `
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #f9fafb;">
              <div style="width:28px;height:28px;border-radius:7px;background:${p.stock===0?'#fef2f2':'#fffbeb'};display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;">
                ${p.img?`<img src="${p.img}" style="width:100%;height:100%;object-fit:cover;border-radius:7px;">`:'📦'}
              </div>
              <div style="flex:1;min-width:0;">
                <div style="font-size:11px;font-weight:900;color:#111827;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${p.nom}</div>
              </div>
              <div style="font-family:'Space Mono',monospace;font-size:14px;font-weight:900;color:${p.stock===0?'#dc2626':'#d97706'};">${p.stock}</div>
            </div>`).join('')}
          ${prodsBajos.length>0?`<button onclick="navTo('pgInventario')" style="width:100%;margin-top:8px;background:none;border:1px solid #e5e7eb;border-radius:8px;padding:6px;font-size:11px;color:#6b7280;font-weight:700;cursor:pointer;">Ver inventario →</button>`:''}
        </div>
      </div>
    </div>

    <!-- ── Top del día + Últimas ventas ─────────────────────────── -->
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">

      <!-- Top del día -->
      <div style="background:#fff;border:1.5px solid #fde68a;border-radius:14px;overflow:hidden;">
        <div style="display:flex;align-items:center;padding:12px 14px;border-bottom:1px solid #fef9c3;background:linear-gradient(to right,#fffbeb,#fff);">
          <div style="font-size:12px;font-weight:900;color:#92400e;display:flex;align-items:center;gap:6px;">🏆 <span>Top del día</span></div>
        </div>
        <div style="padding:8px 14px;">
          ${top5.length===0
            ?'<div style="text-align:center;color:#9ca3af;padding:12px;font-size:12px;">Sin ventas aún</div>'
            :top5.map(([nom,cant],i)=>`
            <div style="display:flex;align-items:center;gap:8px;padding:6px 0;${i<top5.length-1?'border-bottom:1px solid #f9fafb':''}">
              <div style="width:22px;height:22px;border-radius:50%;background:${['#f59e0b','#9ca3af','#b45309','#f3f4f6','#f3f4f6'][i]};display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:${i<3?'#fff':'#6b7280'};flex-shrink:0;">${i+1}</div>
              <div style="flex:1;font-size:11px;font-weight:800;color:#111827;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${nom}</div>
              <div style="font-size:11px;font-weight:700;color:#6b7280;">${cant} uds</div>
            </div>`).join('')}
        </div>
      </div>
      ...
      <div style="background:#fff;border:1.5px solid #e5e7eb;border-radius:14px;overflow:hidden;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid #f3f4f6;">
          <div style="font-size:12px;font-weight:900;color:#111827;display:flex;align-items:center;gap:6px;">🕐 <span>Últimas ventas</span></div>
          <button onclick="navTo('pgReportes')" style="background:none;border:1px solid #e5e7eb;border-radius:7px;padding:3px 9px;font-size:10px;color:#6b7280;font-weight:700;cursor:pointer;">Ver todas</button>
        </div>
        <div style="padding:8px 14px;">
          ${ultimasVentas.length===0
            ...
            :ultimasVentas.map(v=>{
              ...
              const items=(v.items||[]).map(i=>i.nom).join(', ').slice(0,28);
              return `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #f9fafb;">
                <div style="font-size:10px;font-family:'Space Mono',monospace;color:#9ca3af;width:36px;flex-shrink:0;">${hora}</div>
                <div style="flex:1;font-size:11px;color:#6b7280;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${items}…</div>
                <div style="font-family:'Space Mono',monospace;font-size:12px;font-weight:900;color:#15803d;">$${parseFloat(v.total).toFixed(2)}</div>
              </div>`;
            }).join('')}
        </div>
      </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(100px,1fr));gap:8px;">
      ${[
        ['VENTA HOY',  '$'+totalHoy.toFixed(2),      '#15803d'],
        ['ÍTEMS HOY',  ventasHoy.reduce((s,v)=>s+(v.items||[]).reduce((a,i)=>a+Number(i.cant||1),0),0), '#0369a1'],
        ['VENTA MES',  '$'+(()=>{const m=(ventasDiarias||[]).filter(v=>{ try{return esMesActual(v.fecha+'T00:00:00')}catch(e){return false} });return m.reduce((s,v)=>s+Number(v.monto||0),0).toFixed(2)})(), '#7c3aed'],
        ['COBROS',     (ventasDiarias||[]).filter(v=>{ try{return esMesActual(v.fecha+'T00:00:00')}catch(e){return false} }).length, '#374151'],
        ['GASTOS MES', '$'+(()=>{const g=pagos.filter(p=>{ try{return esMesActual(p.fechaISO)&&(p.cat==='GASTO'||p.cat==='FACTURA')}catch(e){return false} });return g.reduce((s,p)=>s+Number(p.monto||0),0).toFixed(2)})(), '#dc2626'],
        ['STOCK CRIT.', prodsBajos.length, prodsBajos.length>0?'#dc2626':'#15803d'],
      ].map(([lbl,val,col])=>`
        <div style="background:#fff;border:1.5px solid #f3f4f6;border-radius:10px;padding:10px 12px;text-align:center;">
          <div style="font-family:'Space Mono',monospace;font-size:16px;font-weight:900;color:${col};">${val}</div>
          <div style="font-size:9px;font-weight:900;color:#9ca3af;text-transform:uppercase;letter-spacing:.4px;margin-top:3px;">${lbl}</div>
        </div>`).join('')}
    </div>
  `;
}
