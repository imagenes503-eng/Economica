...
async function verificarMultiTienda() {
  if (typeof _sbGet !== 'function' || !_sesionActiva) return;
  try {
    // Un super admin puede ver todas las tiendas
    if (typeof _esSuperAdmin === 'function' && _esSuperAdmin()) {
      const tiendas = await _sbGet('perfiles', { select: 'tienda_id,tienda_nombre', order: 'tienda_id.asc' }).catch(() => []);
      const unicas  = [...new Map((tiendas||[]).map(t => [t.tienda_id, t])).values()].filter(t => t.tienda_id);
      if (unicas.length > 1) _mostrarSelectorTienda(unicas);
    }
  } catch(e) {}
}
