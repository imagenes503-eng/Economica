...
async function _ejecutarEliminarMemb() {
  ...
  try {
    ...
    // 2. Desactivar perfil del usuario si se proporcionó userId
    if (userId && userId !== 'null' && userId !== '') {
      await _sbPatch('perfiles', { id: 'eq.' + userId }, { activo: false }).catch(() => {});
    }

    // 3. Registrar acción
    if (typeof _registrarAccion === 'function') {
      _registrarAccion('revocar_membresia', `${nombre} (${email}) — membresía eliminada`);
    }

    if (typeof toast === 'function') {
      toast(`✅ Membresía de ${nombre} eliminada. El usuario ya no puede iniciar sesión.`);
    }
