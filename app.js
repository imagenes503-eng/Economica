...
function salvar(doRender = true) {
  ...
  _salvarTimer = setTimeout(() => {
    // ── 1. Supabase primero (fuente de verdad) — 1 segundo de delay ──
    if (typeof syncAhora === 'function') syncAhora('todo');

    // ── 2. IDB en paralelo como caché offline (silencioso) ──
    idbSetMany([
      ...
      console.warn('[IDB caché] Error al guardar caché offline:', err);
    });
  }, 1000); // 1 segundo de delay tras la acción
