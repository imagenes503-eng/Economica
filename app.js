...
function getDB() {
  ...
  return _openDBConReintentos(0);
}

function _openDBConReintentos(intento) {
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = (e) => {
      _db = e.target.result;
      // Cerrar la conexión limpiamente si otra versión necesita actualizarse
      _db.onversionchange = () => { _db.close(); _db = null; };
      resolve(_db);
    };

    req.onerror = (e) => {
      console.warn('[IDB] Error al abrir:', e.target.error);
      if (intento < 3) {
        // Reintentar hasta 3 veces con delay creciente
        setTimeout(() => _openDBConReintentos(intento + 1).then(resolve).catch(reject), 400 * (intento + 1));
      } else {
    ...
    req.onblocked = () => {
      console.warn('[IDB] Bloqueado por otra pestaña/recarga — reintento', intento + 1);
      if (intento < 5) {
        // Esperar a que la conexión anterior se cierre y reintentar
        setTimeout(() => _openDBConReintentos(intento + 1).then(resolve).catch(reject), 600 * (intento + 1));
      } else {
        reject(new Error('IndexedDB bloqueado tras varios intentos'));
      }
    };
  });
}
...
(async function init() {
  try {
    ...
    // Reintentar hasta 4 veces (maneja recargas rápidas y Parallel Space)
    let dbOk = false;
    for (let t = 0; t < 4; t++) {
      try {
        await getDB();
        ...
        console.warn('[Init] IDB intento', t+1, dbErr.message);
        if (t < 3) {
          setLoadingMsg('Iniciando almacenamiento… (intento ' + (t+2) + ')');
          await new Promise(r => setTimeout(r, 700));
        }
      }
    }
    if (!dbOk) throw new Error('IDB no disponible tras 4 intentos');
    setIDBStatus(true);
