...
function renderCajaPanel() {
  ...
  // Al hacer una venta, el dinero pasa de inventario a efectivo pendiente.
  // El capital total NO cambia hasta que se registra en "Ventas por Día".
  // Ganancia = ventasDiariasDelMes - egresos (lo que oficialmente entró a caja)
  ...
  // = (efectivoInicial + ventasDiariasDelMes - egresos + inventarioInicial) - (efectivoInicial + inventarioInicial)
  // = ventasDiariasDelMes - egresos
...
function recalcularInventarioInicialManual() {
  ...
  toast('✅ Inventario inicial actualizado: $' + inventarioInicial.toFixed(2));
}
...
function finalizarVenta() {
  ...
  const venta = {
    ...
    items: carrito.map(i => { const p = productos.find(x => x.id === i.id); const esPromo = !!(i.stockPorCant && i.stockPorCant > 1); return { id: String(i.id), nom: i.nom, cant: i.cant * (i.stockPorCant || 1), cantCobrada: i.cant, precio: i.venta, totalItem: i.venta * i.cant, esPromo, cat: (p && p.cat) ? p.cat : '', paqueteLabel: i.paqueteLabel || null }; }),
