...
function cantTecladoConfirmar() {
  ...
  // Limpiar búsqueda para agilizar siguiente producto
  const busq = document.getElementById('busquedaVenta');
  if (busq) { busq.value = ''; busq.dispatchEvent(new Event('input')); setTimeout(() => busq.focus(), 80); }
...
function generarFacturaPDF() {
  ...
  doc.text('COMPROBANTE DE DESPACHO', W - 14, 20, { align: 'right' });
  ...
  doc.save(`Comprobante_${num}_${cliente.replace(/\s+/g, '_')}.pdf`);
...
function generarPDFInventarioActual() {
  ...
  for (const cat of Object.keys(porCat).sort()) {
    ...
    y = _pdfAutoTable(doc,
      [[{ content: cat, colSpan: 5, styles: { fillColor:[22,163,74], textColor:[255,255,255], fontStyle:'bold', fontSize:9 }}],
       ['Nombre', 'Categoría', 'P. Venta', 'Stock', 'Desglose presentaciones']],
      rows, y, {
        columnStyles: {
          0: { fontStyle: 'bold' },
          2: { halign: 'right' },
          3: { halign: 'center', fontStyle: 'bold', textColor:[22,101,52] },
          4: { fontSize: 7, textColor:[80,80,80] }
        }
      }) + 6;
...
function _pdfBuildReporte(doc, titulo, subtitulo, acumData, catFiltro) {
  ...
  for (const cat of Object.keys(cats).sort()) {
    ...
    y = _pdfAutoTable(doc,
      [[{ content: cat, colSpan: 4, styles:{fillColor:[22,163,74],textColor:[255,255,255],fontStyle:'bold',fontSize:9}}],
...
function generarPDF(tipo) {
  ...
  // Convertir data al formato acum esperado
  const acum = {};
...
function generarPDFRango() {
  ...
  // Acumular ventas del período por producto
  const acum = {};
  historial.forEach(v => {
    ...
    (v.items || []).forEach(item => {
      ...
      if (!acum[key]) acum[key] = { id: item.id||null, nom: item.nom||'—', cat, cant: 0, total: 0 };
  ...
  const totalGlobal = _pdfBuildReporte(doc, 'REPORTE POR PERÍODO', subtitulo, acum, catFiltro);

  const diasRango = Math.max(1, Math.round((hasta - desde) / (1000*60*60*24)) + 1);
  const finalY = doc.lastAutoTable ? doc.lastAutoTable.finalY + 18 : 270;
  if (finalY < 280) {
    doc.setFont('helvetica','normal'); doc.setFontSize(8); doc.setTextColor(100,116,139);
    doc.text(`Período: ${diasRango} día${diasRango!==1?'s':''} · Promedio diario: $${(totalGlobal/diasRango).toFixed(2)}`, 14, finalY);
  }
...
function abrirModal(id) {
  ...
  // No auto-enfocar en modalCantidad para no re-abrir el teclado del teléfono
  if (id !== 'modalCantidad') {
...
function abrirReportePedido() {
  ...
  historial.forEach(v => {
    ...
    (v.items || []).forEach(item => {
      ...
      const prod = item.id ? productos.find(p => String(p.id) === String(item.id)) : null;
      const cat  = (prod && prod.cat) ? prod.cat : (item.cat || 'SIN CATEGORÍA');
      if (!acum[key]) acum[key] = { id: item.id||null, nom: item.nom||'—', cat, cant: 0 };
      acum[key].cant += Number(item.cant || 0);
    });
  });
  ...
  // Lista completa para sección 2 (todos los productos del filtro)
  let listaTodos = (productos || []).filter(p => catFiltro === 'todas' || (p.cat || 'SIN CATEGORÍA') === catFiltro);
  listaTodos = listaTodos.slice().sort((a,b) => (a.cat||'').localeCompare(b.cat||'') || (a.nom||'').localeCompare(b.nom||''));
  ...
  if (tbody1) {
    ...
    if (!listaConVentas.length) {
      rows = `<tr><td colspan="5" style="text-align:center;padding:20px;color:#6b7280;font-weight:700;">Sin ventas registradas en este período para los filtros seleccionados.</td></tr>`;
    } else {
      listaConVentas.forEach(p => {
        const cat = p.cat || 'SIN CATEGORÍA';
        if (cat !== lastCat) {
          rows += `<tr><td colspan="5" style="background:#dcfce7;color:#166534;font-size:11px;font-weight:900;padding:6px 10px;border-bottom:1px solid #86efac;">📦 ${cat}</td></tr>`;
          lastCat = cat;
        }
        ...
        snapFilas.push({ id: String(p.id), nom: p.nom||'—', cat, stock: p.stock||0, vendido, costoTotal, ventaTotal });
      });
    ...
    _rpS1Snapshot = { rangoStr, catLabel, invInicialVenta, invInicialCosto, filas: snapFilas,
      totalCosto: totalCostoS1, totalVenta: totalVentaS1 };
  }
...
function rp_descargarImagenesPedido() {
  ...
  const POR_IMG = 30;
  const totalImgs = Math.ceil(filas.length / POR_IMG);
  toast(`⏳ Generando ${totalImgs} imagen${totalImgs>1?'es':''}...`);

  for (let imgIdx = 0; imgIdx < totalImgs; imgIdx++) {
    const bloque = filas.slice(imgIdx * POR_IMG, imgIdx * POR_IMG + POR_IMG);
    _rpGenerarImagen(bloque, imgIdx + 1, totalImgs);
  }
}

function _rpGenerarImagen(filas, numImg, totalImgs) {
  const FILAS_TOTAL  = 30;
  const SCALE        = 2;
  const COL_NOM      = 460;
  const COL_CANT     = 160;
  const ANCHO_L      = COL_NOM + COL_CANT;
  const ANCHO        = ANCHO_L * SCALE;
  const H_BAND       = 64  * SCALE;
  const H_SUB        = 28  * SCALE;
  const H_HEAD_COL   = 38  * SCALE;
  const H_FILA       = 40  * SCALE;
  const H_FOOT       = 38  * SCALE;
  const ALTO         = H_BAND + H_SUB + H_HEAD_COL + H_FILA * FILAS_TOTAL + H_FOOT;
  ...
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, ANCHO, H_BAND);

  ctx.save();
  ctx.globalAlpha = 0.12;
  ctx.fillStyle = '#6ee7b7';
  ctx.beginPath();
  ctx.arc(ANCHO - 55*SCALE, H_BAND/2, 44*SCALE, 0, Math.PI*2);
  ctx.fill();
  ctx.restore();
  ...
  ctx.fillRect(0, H_BAND, ANCHO, H_SUB);
  ...
  ctx.fillText(`Hoja ${numImg} / ${totalImgs}`, ANCHO - 20*SCALE, H_BAND + 19*SCALE);

  const yCH = H_BAND + H_SUB;
  ctx.fillStyle = '#059669';
  ctx.fillRect(0, yCH, ANCHO, H_HEAD_COL);
  ctx.fillStyle = '#10b981';
  ctx.fillRect(0, yCH + H_HEAD_COL - 3*SCALE, ANCHO, 3*SCALE);

  ctx.fillStyle = '#ffffff';
  ctx.font = `bold ${13*SCALE}px Arial`;
  ctx.textAlign = 'left';
  ctx.fillText('PRODUCTO', 56*SCALE, yCH + 25*SCALE);
  ctx.textAlign = 'center';
  ctx.fillText('CANTIDAD', (COL_NOM + COL_CANT/2)*SCALE, yCH + 25*SCALE);

  ctx.strokeStyle = '#34d399';
  ctx.lineWidth = 2*SCALE;
  ctx.beginPath();
  ctx.moveTo(COL_NOM*SCALE, yCH);
  ctx.lineTo(COL_NOM*SCALE, yCH + H_HEAD_COL);
  ctx.stroke();

  const yFilas = yCH + H_HEAD_COL;
  for (let i = 0; i < FILAS_TOTAL; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(0, y + H_FILA);
    ctx.lineTo(ANCHO, y + H_FILA);
    ctx.stroke();

    ctx.strokeStyle = '#a7f3d0';
    ctx.lineWidth = 1.5*SCALE;
    ctx.beginPath();
    ctx.moveTo(COL_NOM*SCALE, y);
    ctx.lineTo(COL_NOM*SCALE, y + H_FILA);
    ctx.stroke();

    const numLinea = i + 1 + (numImg-1)*FILAS_TOTAL;
    ctx.fillStyle = fila ? '#059669' : '#d1fae5';
    ctx.beginPath();
    ctx.roundRect(10*SCALE, y + 10*SCALE, 32*SCALE, 20*SCALE, 6*SCALE);
    ctx.fill();
    ctx.fillStyle = fila ? '#ffffff' : '#a7f3d0';
    ctx.font = `bold ${10*SCALE}px Arial`;
    ctx.textAlign = 'center';
    ctx.fillText(String(numLinea), 26*SCALE, y + 24*SCALE);
    ...
    if (fila) {
      ...
      ctx.textAlign = 'left';
      let nom = fila.nom;
      const maxW = (COL_NOM - 56) * SCALE;
      while (ctx.measureText(nom).width > maxW && nom.length > 1) nom = nom.slice(0,-1);
      if (nom !== fila.nom) nom += '…';
      ctx.fillText(nom, 52*SCALE, y + H_FILA/2 + 5*SCALE);

      const cantStr = String(fila.cant);
      const cxCant  = (COL_NOM + COL_CANT/2) * SCALE;
      const cyRect  = y + 7*SCALE;
      const rw = 70*SCALE, rh = H_FILA - 14*SCALE;
      const cantGrad = ctx.createLinearGradient(cxCant - rw/2, cyRect, cxCant + rw/2, cyRect + rh);
      cantGrad.addColorStop(0, '#059669');
      cantGrad.addColorStop(1, '#047857');
      ctx.fillStyle = cantGrad;
      ctx.beginPath();
      ctx.roundRect(cxCant - rw/2, cyRect, rw, rh, 10*SCALE);
      ctx.fill();
      ctx.fillStyle = '#ffffff';
      ctx.font = `bold ${16*SCALE}px Arial`;
      ctx.textAlign = 'center';
      ctx.fillText(cantStr, cxCant, cyRect + rh/2 + 6*SCALE);
    }
  }

  const yFoot = yFilas + FILAS_TOTAL * H_FILA;
  const footGrad = ctx.createLinearGradient(0, yFoot, ANCHO, yFoot);
  footGrad.addColorStop(0, '#064e3b');
  footGrad.addColorStop(1, '#047857');
  ctx.fillStyle = footGrad;
  ctx.fillRect(0, yFoot, ANCHO, H_FOOT);

  ctx.fillStyle = '#6ee7b7';
  ctx.font = `${11*SCALE}px Arial`;
  ctx.textAlign = 'center';
  ctx.fillText(`Generado: ${new Date().toLocaleString('es-SV')}  ·  ${filas.length} producto${filas.length!==1?'s':''} en esta hoja  ·  Hoja ${numImg}/${totalImgs}`, ANCHO/2, yFoot + 24*SCALE);
  ...
  link.download = `Pedido_Hoja${numImg}_de${totalImgs}.png`;
  link.click();
  toast(`✓ Imagen ${numImg}/${totalImgs} descargada`);
}
...
function rp_descargarPDFS1() {
  ...
  if (!snap || !snap.filas || !snap.filas.length) {
    toast('Abre el panel de reporte primero para generar el PDF', true);
    return;
  }
  ...
  const filas = snap.filas.map(f => {
    ...
    return {
      ...
      stockAct: f.stock,
      vendido:  sp ? Math.max(0, sp.stock - f.stock) : f.vendido
    };
  });

  const POR_PAG  = 30;
  const totalPag = Math.ceil(filas.length / POR_PAG);
  toast(`⏳ Generando PDF — ${totalPag} página${totalPag>1?'s':''}...`);

  // Usamos canvas igual que las imágenes del pedido, orientación horizontal
  const SCALE   = 2;
  const PW      = 297; // A4 horizontal mm → pixels base
  const PH      = 210;
  const PW_PX   = PW * SCALE * (96/25.4) | 0;  // ~2245px
  const PH_PX   = PH * SCALE * (96/25.4) | 0;  // ~1587px

  const COL_NOM  = Math.round(PW_PX * 0.50);
  const COL_INI  = Math.round(PW_PX * 0.17);
  const COL_ACT  = Math.round(PW_PX * 0.165);
  const COL_VEN  = PW_PX - COL_NOM - COL_INI - COL_ACT;

  const H_BAND   = Math.round(PH_PX * 0.09);
  const H_SUB    = Math.round(PH_PX * 0.045);
  const H_HEAD   = Math.round(PH_PX * 0.06);
  const H_FOOT   = Math.round(PH_PX * 0.045);
  const H_BODY   = PH_PX - H_BAND - H_SUB - H_HEAD - H_FOOT;
  const H_FILA   = Math.floor(H_BODY / POR_PAG);

  const images = [];

  for (let pg = 0; pg < totalPag; pg++) {
    const bloque = filas.slice(pg * POR_PAG, pg * POR_PAG + POR_PAG);
    const canvas = document.createElement('canvas');
    canvas.width  = PW_PX;
    canvas.height = PH_PX;
    const ctx = canvas.getContext('2d');
    ...
    // Banda principal roja
    const grad = ctx.createLinearGradient(0, 0, PW_PX, 0);
    grad.addColorStop(0,   '#7f1d1d');
    grad.addColorStop(0.5, '#991b1b');
    grad.addColorStop(1,   '#b91c1c');
    ctx.fillStyle = grad;
    ctx.fillRect(0, 0, PW_PX, H_BAND);

    // Círculo decorativo
    ctx.save(); ctx.globalAlpha = 0.10;
    ctx.fillStyle = '#fca5a5';
    ctx.beginPath(); ctx.arc(PW_PX - 70*SCALE, H_BAND/2, 55*SCALE, 0, Math.PI*2); ctx.fill();
    ctx.restore();
    ...
    ctx.textAlign = 'left';
    ctx.fillText('📋  REPORTE DE INVENTARIO', 18*SCALE, 36*SCALE);

    // Sub-banda
    ctx.fillStyle = '#450a0a';
    ctx.fillRect(0, H_BAND, PW_PX, H_SUB);
    const hoy = new Date().toLocaleDateString('es-SV', { day:'2-digit', month:'long', year:'numeric' });
    ctx.fillStyle = '#fca5a5';
    ctx.font = `${10*SCALE}px Arial`;
    ctx.textAlign = 'left';
    ctx.fillText(`📅 ${snap.rangoStr}  ·  ${snap.catLabel}  ·  ${hoy}`, 18*SCALE, H_BAND + 17*SCALE);
    ctx.textAlign = 'right';
    ctx.fillStyle = '#fecaca';
    ctx.fillText(`Página ${pg+1} / ${totalPag}`, PW_PX - 18*SCALE, H_BAND + 17*SCALE);

    // Cabecera columnas
    const yH = H_BAND + H_SUB;
    ctx.fillStyle = '#dc2626';
    ctx.fillRect(0, yH, PW_PX, H_HEAD);
    ctx.fillStyle = '#ef4444';
    ctx.fillRect(0, yH + H_HEAD - 3*SCALE, PW_PX, 3*SCALE);

    ctx.fillStyle = '#fff';
    ctx.font = `bold ${12*SCALE}px Arial`;
    ctx.textAlign = 'left';
    ctx.fillText('PRODUCTO', 50*SCALE, yH + Math.round(H_HEAD*0.65));
    ctx.textAlign = 'center';
    ctx.fillText('STOCK INICIAL', (COL_NOM + COL_INI/2), yH + Math.round(H_HEAD*0.65));
    ctx.fillText('STOCK ACTUAL',  (COL_NOM + COL_INI + COL_ACT/2), yH + Math.round(H_HEAD*0.65));
    ctx.fillText('VENDIDO',       (COL_NOM + COL_INI + COL_ACT + COL_VEN/2), yH + Math.round(H_HEAD*0.65));

    // Separadores cabecera
    [COL_NOM, COL_NOM+COL_INI, COL_NOM+COL_INI+COL_ACT].forEach(x => {
      ctx.strokeStyle = '#f87171'; ctx.lineWidth = 1.5*SCALE;
      ctx.beginPath(); ctx.moveTo(x, yH); ctx.lineTo(x, yH+H_HEAD); ctx.stroke();
    });

    // Filas
    const yF = yH + H_HEAD;
    for (let i = 0; i < POR_PAG; i++) {
      const y = yF + i * H_FILA;
      const f = bloque[i] || null;

      ctx.fillStyle = i%2===0 ? '#fff' : '#fff5f5';
      ctx.fillRect(0, y, PW_PX, H_FILA);

      ctx.strokeStyle = '#fecaca'; ctx.lineWidth = 1*SCALE;
      ctx.beginPath(); ctx.moveTo(0, y+H_FILA); ctx.lineTo(PW_PX, y+H_FILA); ctx.stroke();
      ...
      // Número de línea (chip rojo)
      const numL = i + 1 + pg*POR_PAG;
      ctx.fillStyle = f ? '#dc2626' : '#fecaca';
      ctx.beginPath();
      ctx.roundRect(8*SCALE, y+Math.round(H_FILA*0.22), 28*SCALE, Math.round(H_FILA*0.56), 5*SCALE);
      ctx.fill();
      ctx.fillStyle = f ? '#fff' : '#f87171';
      ctx.font = `bold ${9*SCALE}px Arial`;
      ctx.textAlign = 'center';
      ctx.fillText(String(numL), 22*SCALE, y + Math.round(H_FILA*0.62));
      ...
      if (f) {
        ...
        ctx.textAlign = 'left';
        let nom = f.nom;
        const maxW = COL_NOM - 48*SCALE;
        while (ctx.measureText(nom).width > maxW && nom.length > 1) nom = nom.slice(0,-1);
        if (nom !== f.nom) nom += '…';
        ctx.fillText(nom, 42*SCALE, y + Math.round(H_FILA*0.65));

        // Función para dibujar chip de valor
        const chip = (val, cx, color, bg) => {
          const str = val !== null ? String(val) : '—';
          const rw = 60*SCALE, rh = Math.round(H_FILA*0.62);
          const rx = cx - rw/2, ry = y + Math.round(H_FILA*0.18);
          ctx.fillStyle = bg;
          ctx.beginPath(); ctx.roundRect(rx, ry, rw, rh, 8*SCALE); ctx.fill();
          ctx.fillStyle = color;
          ctx.font = `bold ${14*SCALE}px Arial`;
          ctx.textAlign = 'center';
          ctx.fillText(str, cx, ry + Math.round(rh*0.72));
        };

        chip(f.stockIni, COL_NOM + COL_INI/2,    '#7c3aed', '#f5f3ff');
        chip(f.stockAct, COL_NOM+COL_INI + COL_ACT/2, '#15803d', '#f0fdf4');
        const cVen = f.vendido > 0 ? '#dc2626' : '#6b7280';
        const bgVen = f.vendido > 0 ? '#fef2f2' : '#f9fafb';
        chip(f.vendido,  COL_NOM+COL_INI+COL_ACT + COL_VEN/2, cVen, bgVen);
      }
    }

    // Footer
    const yFoot = yF + POR_PAG*H_FILA;
    const footG = ctx.createLinearGradient(0, yFoot, PW_PX, yFoot);
    footG.addColorStop(0, '#7f1d1d'); footG.addColorStop(1, '#b91c1c');
    ctx.fillStyle = footG;
    ctx.fillRect(0, yFoot, PW_PX, H_FOOT);
    ctx.fillStyle = '#fca5a5';
    ctx.font = `${10*SCALE}px Arial`;
    ctx.textAlign = 'center';
    ctx.fillText(`Generado: ${new Date().toLocaleString('es-SV')}  ·  Stock Inicial al recalcular inventario  ·  Pág ${pg+1}/${totalPag}`, PW_PX/2, yFoot + Math.round(H_FOOT*0.65));

    // Borde rojo
    ctx.strokeStyle = '#dc2626'; ctx.lineWidth = 3*SCALE;
    ctx.strokeRect(2*SCALE, 2*SCALE, PW_PX-4*SCALE, PH_PX-4*SCALE);

    images.push(canvas.toDataURL('image/jpeg', 0.92));
  }

  // Construir PDF con jsPDF usando las imágenes, orientación landscape
  if (!window.jspdf) { toast('jsPDF no disponible', true); return; }
  const { jsPDF } = window.jspdf;
  ...
  const fname = `Inventario_${snap.rangoStr.replace(/ /g,'').replace(/\//g,'-')}.pdf`;
  doc.save(fname);
  toast(`✓ PDF descargado — ${totalPag} página${totalPag>1?'s':''}`);
}

// PDF simplificado del período (solo datos solicitados: nombre, cant vendida, costo, venta)
function generarPDFRangoSimple() {
  ...
  doc.text('REPORTE DE PERÍODO', 105, 13, { align: 'center' });
  doc.setFontSize(9); doc.setFont('helvetica', 'normal');
  doc.text(`${rangoStr}  ·  ${catLabel}`, 105, 21, { align: 'center' });
