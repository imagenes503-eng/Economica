...
function _rp_render() {
  ...
  // Todos los productos de la categoría (para calcular el dinero disponible en inventario)
  let listaCategoriaCompleta = (productos || []).filter(p => catFiltro === 'todas' || (p.cat || 'SIN CATEGORÍA') === catFiltro);
  const dineroDisponibleCat = listaCategoriaCompleta.reduce((s,p) => s + (Number(p.stock)||0) * _compraUdReal(p), 0);
  const elDinero = document.getElementById('rp_s1_dineroDisponible');
  if (elDinero) elDinero.textContent = '$' + dineroDisponibleCat.toFixed(2);
