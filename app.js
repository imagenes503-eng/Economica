...
function renderCajaPanelMini() {
  ...
  // FIX: usar ventasMes (reconstruido desde historial) como fuente única de verdad
  // para el monto del mes. Antes usaba ventasDiarias que puede tener entradas
  // duplicadas o desincronizadas causando que Capital muestre $1.00 de más.
  // ventasMes y VENTA MES en el dashboard usan la misma fuente → siempre consistentes.
...
function renderCajaPanel() {
  ...
  // FIX: usar ventasMes (reconstruido desde historial) como fuente única de verdad.
  // ventasDiarias puede tener duplicados o datos desincronizados entre teléfonos.
  // ventasMes = totalReporte(ventasMes) es idéntico a lo que muestra VENTA MES.
  const ventasMesTotal_vd  = totalReporte(ventasMes);
  const entroACaja         = ventasMesTotal_vd;
  const totalVentasDiariasDelMes = ventasMesTotal_vd;

  // Promedio diario basado en días con ventas en ventasDiarias (solo para mostrar promedio)
  ...
  const promedioDiario     = diasConVentas > 0
    ? ventasMesTotal_vd / diasConVentas
    : (ventasMesTotal_vd > 0 && diaActual > 0 ? ventasMesTotal_vd / diaActual : 0);
  ...
  // Caja actual = efectivo inicial + ventas del mes (desde historial/ventasMes) - egresos
