...
function _regPresRender() {
  ...
  cont.innerHTML = _regPres.map(r => {
    ...
    return `<div class="reg-pres-card">
...
function renderInv() {
  ...
  const cards = lista.map(p => {
    ...
    const totalVal    = ((p.venta||0)*(p.stock||0)).toFixed(2);
    const imgHtml     = p.img
      ? `<img src="${p.img}" loading="lazy" class="inv-card-img" style="width:48px;height:48px;">`
      : `<div class="inv-card-img">${(p.cat||'📦').charAt(0)}</div>`;

    const pkgChips = pkgs.length
      ? pkgs.map(pk => `<span class="inv-pkg-chip">📦 ${pk.cant} uds · $${fmtP(pk.precio)}</span>`).join('')
      : `<span class="inv-pkg-chip" style="background:#f8fafc;color:var(--text-muted);">Sin presentaciones</span>`;
    ...
    return `<div class="inv-prod-card${critico?' critico':''}">
      <div class="inv-card-top">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div class="inv-card-cat">${p.cat||''}${p.abrev?' · '+p.abrev:''}</div>
          ${p.cod ? `<div class="inv-card-cod">${p.cod}</div>` : ''}
        </div>
        ${critico ? `<span style="background:#fee2e2;color:#dc2626;border-radius:999px;font-size:11px;font-weight:900;padding:3px 9px;flex-shrink:0;">⚠ Bajo</span>` : ''}
      </div>

      <div class="inv-card-metrics">
        <div class="inv-metric amber">
          <div class="inv-metric-label">Costo ud</div>
          <div class="inv-metric-val">$${fmtP(p.compra||0)}</div>
        </div>
        <div class="inv-metric green">
          <div class="inv-metric-label">Venta ud${margen?` <span style="color:var(--amber);font-size:8px;">+${margen}%</span>`:''}
          </div>
          <div class="inv-metric-val">$${fmtP(p.venta||0)}</div>
        </div>
        <div class="inv-metric${critico?' red':' green'}">
          <div class="inv-metric-label">Stock</div>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
        <span style="font-size:11px;font-weight:800;color:var(--text-muted);">Valor en inventario</span>
        <span style="font-family:'Space Mono',monospace;font-size:16px;font-weight:900;color:var(--green-dark);">$${totalVal}</span>
      </div>

      <div class="inv-card-actions">
        <button class="inv-card-btn inv-btn-stock" onclick="abrirRestock('${p.id}')">+📦 Stock</button>
        <button class="inv-card-btn inv-btn-edit"  onclick="editarProd('${p.id}')">✎ Editar</button>
        <button class="inv-card-btn inv-btn-pkg"   onclick="abrirGestionPaquetes('${p.id}')">📦 Pres.</button>
  ...
  if (totEl) {
    const tv = lista.reduce((s,p)=>s+(p.venta||0)*(p.stock||0),0);
    const tc = lista.reduce((s,p)=>s+(p.compra||0)*(p.stock||0),0);
