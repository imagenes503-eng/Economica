...
function finalizarVenta() {
  ...
  carrito = []; cobroDigits = '';
  // FIX BUG 2: Registrar ventasDiarias ANTES de salvar() para que Supabase
  // reciba el monto correcto. La versión anterior llamaba esto 400ms DESPUES
  // de salvar(), subiendo snapshot con ventasDiarias=[] y causando que Estado
  // de Caja mostrara $0 pese a tener cobros registrados en el POS.
  ...
  // autoRegistrarVentaDiaria ya se llama sincrónicamente antes de salvar() arriba
}
...
function renderFacturaPreview(venta) {
  ...
  let rows = '';
  items.forEach(it => {
    // FIX: usar totalItem para no multiplicar precio_paquete × unidades_físicas
