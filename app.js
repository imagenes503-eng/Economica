...
function abrirReportePedido() {
  ...
  // Construir lista de productos filtrados por categoría
  ...
  if (tbody1) {
    ...
    lista.forEach(p => {
      ...
      rows += `<tr style="border-bottom:1px solid #f0fdf4;${stockBajo?'background:#fef2f2;':''}">
        ...
        <td style="padding:7px 10px;text-align:center;font-weight:700;color:#1d4ed8;">${vendido || 0}</td>
  ...
  if (tbody2) {
    ...
    lista.forEach(p => {
      ...
      let presOptions = `<option value="unidad">Unidad ($${(p.venta||0).toFixed(2)})</option>`;
      pkgs.forEach((pk, idx) => {
        const label = pk.label || ('Paquete ' + pk.cant + ' uds');
        const precioVentaPk = pk.precio || 0;
        presOptions += `<option value="pkg_${idx}">${label} ($${precioVentaPk.toFixed(2)})</option>`;
      });
...
function _rpGenerarImagen(filas, numImg, totalImgs) {
  ...
  const COL_W_NOM  = 420;
  const COL_W_CANT = 160;
  const ANCHO = COL_W_NOM + COL_W_CANT + 3; // +3 border
  const H_HEADER_MAIN = 54;
  const H_HEADER_COL  = 36;
  const H_FILA        = 38;
  const H_FOOTER      = 34;
  const ALTO = H_HEADER_MAIN + H_HEADER_COL + (H_FILA * FILAS_TOTAL) + H_FOOTER + 2;
  ...
  ctx.fillRect(0, 0, ANCHO, H_HEADER_MAIN);
  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 20px Arial';
  ctx.textAlign = 'center';
  ctx.fillText('PEDIDO DE COMPRA', ANCHO / 2, 26);
  ctx.font = '13px Arial';
  const hoy = new Date().toLocaleDateString('es-SV');
  ctx.fillText(`Hoja ${numImg} de ${totalImgs}  ·  ${hoy}`, ANCHO / 2, 46);

  // ---- Encabezado columnas ----
  const yColH = H_HEADER_MAIN;
  ctx.fillStyle = '#2563eb';
  ctx.fillRect(0, yColH, ANCHO, H_HEADER_COL);
  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 14px Arial';
  ctx.textAlign = 'left';
  ctx.fillText('PRODUCTO', 18, yColH + 23);
  ctx.textAlign = 'center';
  ctx.fillText('CANTIDAD', COL_W_NOM + COL_W_CANT / 2, yColH + 23);

  // ---- Filas ----
  for (let i = 0; i < FILAS_TOTAL; i++) {
    const y = H_HEADER_MAIN + H_HEADER_COL + i * H_FILA;
    ...
    // Separador vertical
    ctx.beginPath();
    ctx.moveTo(COL_W_NOM, y);
    ctx.lineTo(COL_W_NOM, y + H_FILA);
    ctx.stroke();
    ...
    ctx.font = '11px Arial';
    ctx.textAlign = 'right';
    ctx.fillText(String(i + 1 + (numImg-1)*FILAS_TOTAL), 28, y + 24);
    ...
    if (fila) {
      ...
      ctx.textAlign = 'left';
      let nom = fila.nom;
      if (ctx.measureText(nom).width > COL_W_NOM - 50) {
        while (ctx.measureText(nom + '…').width > COL_W_NOM - 50 && nom.length > 0) nom = nom.slice(0, -1);
        nom += '…';
      }
      ctx.fillText(nom, 36, y + 24);

      // Cantidad (resaltada)
      ctx.fillStyle = '#1d4ed8';
      ctx.font = 'bold 16px Arial';
      ctx.textAlign = 'center';
      ctx.fillText(String(fila.cant), COL_W_NOM + COL_W_CANT / 2, y + 25);
    }
  }

  // Borde vertical exterior de la col cantidad
  ctx.strokeStyle = '#cbd5e1';
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(COL_W_NOM, H_HEADER_MAIN);
  ctx.lineTo(COL_W_NOM, H_HEADER_MAIN + H_HEADER_COL + FILAS_TOTAL * H_FILA);
  ctx.stroke();

  // ---- Footer ----
  const yFoot = H_HEADER_MAIN + H_HEADER_COL + FILAS_TOTAL * H_FILA;
  ctx.fillStyle = '#f1f5f9';
  ctx.fillRect(0, yFoot, ANCHO, H_FOOTER);
  ctx.strokeStyle = '#cbd5e1';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(0, yFoot);
  ctx.lineTo(ANCHO, yFoot);
  ctx.stroke();
  ctx.fillStyle = '#475569';
  ctx.font = '12px Arial';
  ctx.textAlign = 'center';
  ctx.fillText(`Pedido generado el ${new Date().toLocaleString('es-SV')}  ·  Hoja ${numImg}/${totalImgs}  ·  ${filas.length} productos en esta hoja`, ANCHO/2, yFoot + 22);
