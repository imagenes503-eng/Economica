...
function renderDestacados() {
  ...
  cont.innerHTML = lista.map((p, i) => {
    ...
    const colorGan = p.ganancia > 0 ? 'var(--green)' : p.ganancia < 0 ? '#dc2626' : 'var(--text-muted)';
    return `
      <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid var(--border);border-radius:10px;padding:11px 14px;background:#fff;margin-bottom:7px;">
        <div style="display:flex;align-items:center;gap:10px;min-width:0;">
          <div style="font-size:15px;font-weight:900;color:var(--text-dim);width:26px;flex-shrink:0;text-align:center;">${medalla}</div>
          <div style="min-width:0;">
            <div style="font-weight:900;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${p.nom}</div>
            <div style="font-size:11px;color:var(--text-muted);font-weight:800;">${p.cat} · Costo $${p.costo.toFixed(2)} → Venta $${p.venta.toFixed(2)} · Stock ${p.stock}</div>
          </div>
        </div>
        <div class="mono" style="font-weight:900;font-size:15px;color:${colorGan};white-space:nowrap;flex-shrink:0;margin-left:10px;">$${p.ganancia.toFixed(2)}</div>
      </div>
    `;
  }).join('');
}
...
function filtrarDestVendidos(tipo) {
  ...
  (historial || []).forEach(v => {
    ...
    (v.items || []).forEach(item => {
      ...
      const prod = item.id ? productos.find(p => String(p.id) === String(item.id)) : null;
      const cat  = (prod && prod.cat) ? prod.cat : (item.cat || 'SIN CATEGORÍA');
      if (!acum[key]) acum[key] = { nom: item.nom || '—', cat, cant: 0, total: 0 };
      const cant = Number(item.cant || 0);
      acum[key].cant += cant;
      const totItem = item.totalItem !== undefined ? Number(item.totalItem) : cant * Number(item.precio || 0);
    ...
    return `
      <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid var(--border);border-radius:10px;padding:11px 14px;background:#fff;margin-bottom:7px;">
        <div style="display:flex;align-items:center;gap:10px;min-width:0;">
          ...
          <div style="min-width:0;">
            <div style="font-weight:900;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${p.nom}</div>
            <div style="font-size:11px;color:var(--text-muted);font-weight:800;">${p.cat} · $${p.total.toFixed(2)} vendido</div>
          </div>
        </div>
        <div class="mono" style="font-weight:900;font-size:15px;color:var(--green);white-space:nowrap;flex-shrink:0;margin-left:10px;">${p.cant} uds</div>
      </div>
    `;
  }).join('');
}
