...
function renderInvTotales() {
  ...
  if (peBox && peGrid) {
    if (hayPE) {
      ...
      // Capital actual = caja + inventario a precio VENTA (valor real del negocio)
      ...
      peGrid.innerHTML = `
        ...
        <div class="stat-box" style="border-color:rgba(29,78,216,0.3);background:rgba(29,78,216,0.04);"><div class="s-lbl" style="color:var(--blue);">📊 Capital Actual (venta)</div><div class="s-val" style="color:var(--blue);font-size:18px;">$${capitalActual.toFixed(2)}</div><div style="font-size:11px;color:var(--text-muted);margin-top:2px;">caja + inv. a precio venta</div></div>
...
function buscarV() {
  ...
  m.forEach(p => {
    ...
    const stockBadge = sinStock
      ? `<span class="sug-stock-badge" style="background:rgba(220,38,38,0.7);">❌ Agotado</span>`
      : stockBajo
        ? `<span class="sug-stock-badge" style="background:rgba(217,119,6,0.7);">⚠ ${p.stock}</span>`
        : `<span class="sug-stock-badge">${p.stock} uds</span>`;
    d.innerHTML = `
      ${hasImg
        ? `<div style="position:relative;flex-shrink:0;"><img class="sug-item-img" src="${p.img}" alt="${p.nom}" loading="lazy">${stockBadge}</div>`
        : `<div class="sug-item-ph" style="position:relative;">${p.cat ? p.cat.charAt(0) : '🛒'}${stockBadge}</div>`}
      ...
      ${!sinStock ? `<button class="sug-add-btn">🛒 Agregar</button>` : ''}
    `;
    if (!sinStock) {
      d.querySelector('.sug-add-btn')?.addEventListener('click', (e) => {
        e.stopPropagation();
        document.getElementById('busquedaVenta').value = '';
...
function reiniciarDia() {
  ...
  if (!confirm('¿Reiniciar el reporte del día?\n\nEsto borrará las ventas del día actual del POS y el registro de caja de hoy.')) return;

  const hoyISORD = new Date().toISOString().split('T')[0];

  // 1. Quitar ventas de hoy de ventasMes y ventasSem para evitar doble conteo
  Object.keys(ventasDia || {}).forEach(pid => {
    const v = ventasDia[pid];
    if (ventasMes[pid]) {
      ventasMes[pid].cant  -= Number(v.cant  || 0);
      ventasMes[pid].total -= Number(v.total || 0);
      if (ventasMes[pid].cant <= 0) delete ventasMes[pid];
    }
    if (ventasSem[pid]) {
      ventasSem[pid].cant  -= Number(v.cant  || 0);
      ventasSem[pid].total -= Number(v.total || 0);
      if (ventasSem[pid].cant <= 0) delete ventasSem[pid];
    }
  });
  ...
  // 3. Eliminar el registro de ventasDiarias de hoy para que no se cuente doble en caja
  ventasDiarias = (ventasDiarias || []).filter(v => v.fecha !== hoyISORD);

  salvar();
  if (typeof renderCajaPanel === 'function') renderCajaPanel();
  if (typeof renderVentasDiarias === 'function') renderVentasDiarias();
  if (typeof renderDashboardPro === 'function') renderDashboardPro();
...
function guardarEdicionCobro() {
  ...
  if (typeof syncAhora === 'function') syncAhora('productos');
  // Broadcast del historial actualizado para que otros dispositivos lo reflejen
