...
function generarPDFRangoSimple() {
  ...
  // Acumular VENTAS REALES del rango (no el valor del inventario actual)
  const acum = {};
  historial.forEach(v => {
    const ts = v.ts || (v.fechaISO ? Date.parse(v.fechaISO) : 0);
    if (!ts || new Date(ts) < desde || new Date(ts) > hasta) return;
  ...
  const catLabel = catFiltro === 'todas' ? 'Todas las categorías' : catFiltro;
  const subtitulo = `${rangoStr} · ${catLabel}`;
  const descripcion = 'Este PDF refleja únicamente lo que REALMENTE SE VENDIÓ dentro del rango de fechas '
    + 'seleccionado (no el inventario disponible). Por producto: unidades vendidas, cuánto invertiste '
    + '(costo real de lo vendido) y cuánto ganaste (precio de venta − costo) en ese rango.';

  const { totalInvertidoGlobal, totalGananciaGlobal } =
    _pdfBuildReporte(doc, 'REPORTE DE PERÍODO', subtitulo, acum, catFiltro, { conGanancia: true, descripcion });
