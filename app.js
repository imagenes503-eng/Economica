...
let _formPkgTemp = []; // presentaciones temporales antes de guardar el producto

function formPkgRender() {
  const lista = document.getElementById('formPkgLista');
  if (!lista) return;
  if (!_formPkgTemp.length) {
    lista.innerHTML = `<div style="font-size:12px;color:var(--text-muted);font-weight:700;text-align:center;padding:4px 0;">Sin presentaciones aún — agrégalas abajo</div>`;
    return;
  }
  lista.innerHTML = _formPkgTemp.slice().sort((a,b)=>b.cant-a.cant).map((pk,i)=>{
    const pv  = parseFloat(document.getElementById('inpVenta')?.value) || 0;
    const pud = pv ? (((pv*pk.cant - pk.precio)/(pv*pk.cant))*100).toFixed(0)+'% dto' : '';
    return `<div style="display:flex;align-items:center;justify-content:space-between;background:#fff;border:1.5px solid var(--border-mid);border-radius:9px;padding:9px 13px;">
      <div>
        <span style="background:var(--green);color:#fff;border-radius:6px;padding:2px 9px;font-size:12px;font-weight:900;">${pk.cant} uds</span>
        <span style="font-family:'Space Mono',monospace;font-size:15px;font-weight:900;color:var(--green);margin-left:8px;">$${fmtP(pk.precio)}</span>
        ${pud ? `<span style="font-size:10px;color:var(--amber);font-weight:800;margin-left:6px;">🏷 ${pud}</span>` : ''}
      </div>
      <button type="button" onclick="formPkgEliminar(${i})" style="width:24px;height:24px;border-radius:50%;border:none;background:rgba(220,38,38,0.1);color:#dc2626;font-size:13px;cursor:pointer;font-weight:900;line-height:1;">✕</button>
    </div>`;
  }).join('');
}

function formPkgPreview() {
  const cant   = parseInt(document.getElementById('formPkgCant')?.value)    || 0;
  const precio = parseFloat(document.getElementById('formPkgPrecio')?.value) || 0;
  const prev   = document.getElementById('formPkgPreview');
  if (!prev) return;
  if (cant < 1 || precio <= 0) { prev.style.display = 'none'; return; }
  const pv     = parseFloat(document.getElementById('inpVenta')?.value) || 0;
  const precUd = precio / cant;
  const ahorro = pv > 0 ? (((pv*cant - precio)/(pv*cant))*100).toFixed(0) : null;
  prev.style.display = 'block';
  prev.innerHTML = `📦 ${cant} uds · $${fmtP(precUd)}/ud${ahorro?` · 🏷 ${ahorro}% más barato que unitario`:''}`;
}

function formPkgAgregar() {
  const cant   = parseInt(document.getElementById('formPkgCant')?.value)    || 0;
  const precio = parseFloat(document.getElementById('formPkgPrecio')?.value) || 0;
  if (cant < 1)    { toast('La cantidad debe ser al menos 1', true); return; }
  if (precio <= 0) { toast('El precio debe ser mayor a 0', true);    return; }
  if (_formPkgTemp.find(pk => pk.cant === cant)) { toast('Ya existe una presentación de '+cant+' uds', true); return; }
  _formPkgTemp.push({ id: Date.now(), cant, precio });
  document.getElementById('formPkgCant').value   = '';
  document.getElementById('formPkgPrecio').value = '';
  document.getElementById('formPkgPreview').style.display = 'none';
  formPkgRender();
  toast(`✓ Presentación ${cant} uds por $${fmtP(precio)} lista`);
}

function formPkgEliminar(i) {
  const sorted = _formPkgTemp.slice().sort((a,b)=>b.cant-a.cant);
  const pk = sorted[i];
  if (!pk) return;
  _formPkgTemp = _formPkgTemp.filter(x => x.id !== pk.id);
  formPkgRender();
}
...
function guardarProducto(e) {
  ...
  if (id) {
    ...
    productos = productos.map(x => String(x.id) === String(id) ? { id: Number(id), cod: newCod, nom: newNom, cat: newCat, abrev: newAbrev, compra: newCompra, venta: newVenta, stock: newStock, min: newMin, lotes: lotesActuales, paquetes: _formPkgTemp.length > 0 ? _formPkgTemp : (existing ? (existing.paquetes || []) : []), img: newImg, _ts: Date.now() } : x);
  } else {
    const newPaquetes = typeof _formPkgTemp !== 'undefined' ? (_formPkgTemp || []) : [];
...
function editarProd(id) {
  ...
  // Cargar paquetes existentes en el formulario
  _formPkgTemp = (p.paquetes||[]).map(pk=>({...pk}));
  formPkgRender();
...
function cancelarEdicion() {
  ...
  // Resetear presentaciones temporales del formulario
  _formPkgTemp = [];
  formPkgRender();
}
...
function renderPagina(pgId) {
  ...
  if (pgId === 'pgInvRegistrar') { if (typeof actualizarCats==='function') actualizarCats(); }
}
