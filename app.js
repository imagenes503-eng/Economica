...
function _recalcularReportesDesdeHistorial() {
  ...
  // FIX VENTA MES: excluir ventas de fechas eliminadas (ventasDiariasEliminadas).
  // Sin esto, al reconstruir ventasMes desde historial, los cobros de días
  // borrados con "Eliminar Venta Diaria" seguían sumando al total del mes.
  const _fechasElim = new Set(
  ...
  (historial || []).forEach(v => {
    ...
    // FIX: ignorar ventas de fechas eliminadas para que no sumen a ventasMes/ventasDia
    const fechaStr = (v.fechaISO || v.fecha || '').slice(0, 10); // YYYY-MM-DD
    if (_fechasElim.has(fechaStr)) return;
...
// Usada por _autoCargarDesdeSupa y _aplicarDatos para evitar un loop de sincronización:
...
function _autoRegistrarVentaDiariaSilente() {
  ...
  const idx = (ventasDiarias || []).findIndex(v => v.fecha === hoy);
  if (idx >= 0) {
    if (total <= Number(ventasDiarias[idx].monto || 0)) return; // sin cambio
    ventasDiarias[idx] = { ...ventasDiarias[idx], monto: total, _auto: true };
  } else {
    ventasDiarias.push({ fecha: hoy, monto: total, nota: 'Auto-POS', _auto: true });
  }
