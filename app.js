...
function navTo(pgId, pushHistory = true) {
  ...
  const ids    = ['pgDash', 'pgInventario', 'pgReportes', 'pgDestacados', 'pgVentasDiarias', 'pgSync', 'pgAdmin', 'pgFinanzasMes'];
...
window.addEventListener('popstate', (e) => {
  ...
  const validas = ['pgDash','pgInventario','pgReportes','pgDestacados','pgVentasDiarias','pgSync','pgAdmin','pgFinanzasMes'];
...
function guardarProducto(e) {
  ...
  if (typeof _registrarAccion === 'function') _registrarAccion(id ? 'editar_producto' : 'nuevo_producto', document.getElementById('inpNom')?.value || '');
...
function borrarProd(id) {
  ...
  if (confirm('¿Eliminar este producto?')) {
    ...
    // Borrar en Supabase inmediatamente
...
function finalizarVenta() {
  ...
  // Enviar venta a Supabase en segundo plano (inmediato)
  // items_json guarda el detalle completo para recalcular stock correctamente
  sheetsEnviar('VENTA', {
    id: venta.id, fecha: venta.fechaISO,
    total: venta.total, pago: venta.pago, vuelto: venta.vuelto,
    items: (venta.items||[]).map(i => i.cant+'x '+i.nom).join(' | '),
    items_json: JSON.stringify(venta.items || [])
  }).catch(()=>{});
...
function ejecutarReiniciarMes() {
  ...
  // Subir snapshot limpio a Supabase con solo el inventario (sin ventas del mes borrado)
  // syncAhora('todo') sube el estado actual (ya limpio) — preserva productos en la nube
  if (typeof syncAhora === 'function') syncAhora('todo');
}
...
async function softReload() {
  ...
  const validas = ['pgDash','pgInventario','pgReportes','pgDestacados','pgVentasDiarias','pgSync','pgFinanzasMes'];
