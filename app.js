...
function renderInv() {
  ...
  if (totEl) {
    ...
    // _compraUdReal ahora retorna p.compra (promedio ponderado), que es el mismo
    // valor calculado por el formulario. Esto asegura consistencia total.
    const tc = lista.reduce((s,p) => {
      const costoUd  = _compraUdReal(p);
      const lotStock = (p.lotes || []).reduce((ls, l) => ls + (l.stockRestante || 0), 0);
      const mainStock = Math.max(0, (p.stock || 0) - lotStock);
      let costoP = mainStock * costoUd;
      (p.lotes || []).forEach(l => { costoP += (l.stockRestante || 0) * (l.compra || costoUd); });
      return s + costoP;
    }, 0);
