...
function getDB() {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      // v2: cola de operaciones offline — guarda ventas/pagos/etc. mientras no hay internet
      if (!db.objectStoreNames.contains(OQ_STORE)) {
        const oqStore = db.createObjectStore(OQ_STORE, { keyPath: 'id' });
        oqStore.createIndex('by_ts', 'ts', { unique: false });
      }
    };
...
let cierresMes    = []; // historial de cierres mensuales: [{mes, totalCobros, cobros:[...], pagos:[...]}]
let carrito       = [];
let cobroDigits   = '';
...
async function _cargarCacheIDB() {
  const keys = [
    'vpos_productos','vpos_ventasDia','vpos_ventasSem','vpos_ventasMes',
    'vpos_historial','vpos_pagos','vpos_ventasDiarias','vpos_restockLog',
    'vpos_productosEliminados','vpos_pagosEliminados','vpos_cierresMes'
  ];
  const data = await idbGetMany(keys);
  ...
  cierresMes            = data['vpos_cierresMes']           || [];
  ventasDia = normalizeReport(ventasDia);
  ventasSem = normalizeReport(ventasSem);
  ventasMes = normalizeReport(ventasMes);
...
function renderHistorial() {
  ...
  div.innerHTML = historial.map((v, idx) => {
    // Formatear fecha — si es ISO string, convertir a formato local
    ...
    // Formatear montos con 2 decimales siempre
...
// ===== 18b. CIERRE DE MES — ARCHIVO MENSUAL DE COBROS =====

const MESES_ES = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];

function abrirCierreMes() {
  if (typeof _puedeHacer === 'function' && !_puedeHacer('reiniciar')) { toast('Solo el Admin puede hacer el cierre de mes', true); return; }
  const now = new Date();
  const mesNombre = MESES_ES[now.getMonth()] + ' ' + now.getFullYear();
  const totalCobros = historial.reduce((s, v) => s + parseFloat(v.total || 0), 0);
  const totalPagosMes = pagos.filter(g => esMesActual(g.fechaISO)).reduce((s, g) => s + Number(g.monto || 0), 0);
  const mesClave = now.toISOString().substring(0, 7);
  const yaCerrado = cierresMes.some(c => c.mes === mesClave);
  document.getElementById('modalCierreMesResumen').innerHTML = `
    <div style="background:rgba(22,163,74,0.06);border:1.5px solid rgba(22,163,74,0.3);border-radius:var(--r-sm);padding:14px 16px;margin-bottom:14px;">
      <div style="font-weight:900;color:var(--green-700);font-size:15px;margin-bottom:10px;">📅 ${mesNombre}</div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px;"><span style="color:var(--text-muted);font-size:13px;">Cobros registrados</span><span class="mono" style="font-weight:700;">${historial.length}</span></div>
      <div style="display:flex;justify-content:space-between;margin-bottom:5px;"><span style="color:var(--text-muted);font-size:13px;">Total cobros</span><span class="mono" style="font-weight:700;color:var(--green-600);">$${totalCobros.toFixed(2)}</span></div>
      <div style="display:flex;justify-content:space-between;"><span style="color:var(--text-muted);font-size:13px;">Total gastos</span><span class="mono" style="font-weight:700;color:var(--red);">$${totalPagosMes.toFixed(2)}</span></div>
    </div>
    ${yaCerrado ? `<div style="background:rgba(245,158,11,0.1);border:1px solid rgba(245,158,11,0.3);border-radius:var(--r-sm);padding:10px 12px;font-size:12px;color:#92400e;margin-bottom:12px;">⚠️ Ya existe un cierre para <strong>${mesNombre}</strong>. Se actualizará con los cobros actuales.</div>` : ''}
    ${!historial.length ? `<div style="background:rgba(220,38,38,0.06);border:1px solid rgba(220,38,38,0.2);border-radius:var(--r-sm);padding:10px 12px;font-size:12px;color:var(--red);margin-bottom:12px;">⚠️ No hay cobros en el historial actual para archivar.</div>` : ''}
  `;
  abrirModal('modalCierreMes');
}

function ejecutarCierreMes() {
  if (!historial.length) { toast('No hay cobros para archivar', true); cerrarModal('modalCierreMes'); return; }
  const now = new Date();
  const mesClave = now.toISOString().substring(0, 7); // 'YYYY-MM'
  const mesNombre = MESES_ES[now.getMonth()];
  const anio = now.getFullYear();
  const totalCobros = historial.reduce((s, v) => s + parseFloat(v.total || 0), 0);
  const totalPagosMes = pagos.filter(g => esMesActual(g.fechaISO)).reduce((s, g) => s + Number(g.monto || 0), 0);

  // Construir entrada del cierre
  const cierre = {
    mes: mesClave,
    mesNombre,
    anio,
    fechaCierre: nowISO(),
    totalCobros: parseFloat(totalCobros.toFixed(2)),
    totalGastos: parseFloat(totalPagosMes.toFixed(2)),
    numCobros: historial.length,
    cobros: historial.map(v => ({ ...v, img: undefined }))
  };

  // Reemplazar si ya existe ese mes, si no agregar
  const idx = cierresMes.findIndex(c => c.mes === mesClave);
  if (idx >= 0) { cierresMes[idx] = cierre; } else { cierresMes.push(cierre); }
  // Ordenar de más reciente a más antiguo
  cierresMes.sort((a, b) => b.mes.localeCompare(a.mes));

  // Limpiar historial actual (sin borrar ventasDiarias ni stock)
  historial = [];
  if (typeof _recalcularReportesDesdeHistorial === 'function') _recalcularReportesDesdeHistorial();
  salvar();
  cerrarModal('modalCierreMes');
  toast(`✅ Cierre de ${mesNombre} archivado — $${totalCobros.toFixed(2)}`);
  renderHistorialMeses();

  // Sync al otro teléfono
  if (typeof _broadcast === 'function') {
    _broadcast('cierres_mes_actualizado', { cierresMes });
    _broadcast('historial_actualizado', { historial: [] });
  }
  setTimeout(() => {
    if (typeof _autoEnviarSnapshot === 'function') _autoEnviarSnapshot();
  }, 3000);
...
function renderHistorialMeses() {
  const container = document.getElementById('histMesesList');
  const emptyEl   = document.getElementById('histMesesEmpty');
  const anioEl    = document.getElementById('histMesesAnio');
  if (!container) return;

  if (!cierresMes.length) {
    container.innerHTML = '';
    if (emptyEl) emptyEl.style.display = 'flex';
    if (anioEl) anioEl.textContent = '';
    return;
  }
  ...
  if (anioEl) anioEl.textContent = anios[0] || '';

  container.innerHTML = anios.map(anio => {
    const mesesAnio = porAnio[anio];
    const totalAnio = mesesAnio.reduce((s, c) => s + c.totalCobros, 0);
    // Generar los 12 meses — solo los que tienen cierre, en orden Ene→Dic
    const mesesOrdenados = [...mesesAnio].sort((a, b) => a.mes.localeCompare(b.mes));
    return `
      <div style="padding:10px 18px 6px;background:rgba(22,163,74,.05);border-bottom:1px solid var(--green-100);">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <span style="font-weight:900;font-size:13px;color:var(--green-700);">📆 ${anio}</span>
          <span style="font-family:'Space Mono',monospace;font-weight:700;color:var(--green-600);font-size:13px;">$${totalAnio.toFixed(2)}</span>
        </div>
      </div>
      ${mesesOrdenados.map((c, i) => {
        const abrev = c.mesNombre.substring(0, 3).toUpperCase();
        const rowId = 'mesrow_' + c.mes.replace('-','_');
        return `
        <div class="mes-row">
          <div class="mes-header" onclick="toggleMesRow('${rowId}')">
            <div class="mes-header-left">
              <div class="mes-badge">${abrev}</div>
              <div>
                <div class="mes-nombre">${c.mesNombre} ${c.anio}</div>
                <div class="mes-meta">${c.numCobros} cobro${c.numCobros !== 1 ? 's' : ''} · Gastos $${c.totalGastos.toFixed(2)}</div>
              </div>
            </div>
            <div style="display:flex;align-items:center;gap:10px;">
              <span class="mes-total">$${c.totalCobros.toFixed(2)}</span>
              <span class="mes-chevron" id="${rowId}_chev">▼</span>
            </div>
          </div>
          <div class="mes-body" id="${rowId}">
            ${c.cobros && c.cobros.length ? c.cobros.map(v => {
              const fechaMostrar = v.fechaStr || (v.fechaISO ? new Date(v.fechaISO).toLocaleString('es-SV') : '—');
              const prods = (v.items||[]).map(it => `${it.cant}× ${it.nom}`).join(', ');
              return `<div class="mes-cobro-item">
                <div class="mes-cobro-fecha">📅 ${fechaMostrar}</div>
                <div class="mes-cobro-prods">${prods || '—'}</div>
                <div class="mes-cobro-footer">
                  <span class="mes-cobro-pago">Pagó: $${parseFloat(v.pago||0).toFixed(2)} | Vuelto: $${parseFloat(v.vuelto||0).toFixed(2)}</span>
                  <span class="mes-cobro-total">$${parseFloat(v.total||0).toFixed(2)}</span>
                </div>
              </div>`;
            }).join('') : `<div class="mes-empty-cobros">Sin cobros registrados</div>`}
          </div>
        </div>`;
      }).join('')}`;
  }).join('');
...
function ejecutarReiniciarMes() {
  ...
  // ── AUTO-ARCHIVAR en cierresMes antes de limpiar ──
  if (historial.length > 0) {
    const now = new Date();
    const mesNombreArchivo = MESES_ES[now.getMonth()];
    const anioArchivo = now.getFullYear();
    const totalCobros = historial.reduce((s, v) => s + parseFloat(v.total || 0), 0);
    const totalPagosMes = pagos.filter(g => esMesActual(g.fechaISO)).reduce((s, g) => s + Number(g.monto || 0), 0);
    const cierre = {
      mes: mesClave, mesNombre: mesNombreArchivo, anio: anioArchivo,
      fechaCierre: nowISO(),
      totalCobros: parseFloat(totalCobros.toFixed(2)),
      totalGastos: parseFloat(totalPagosMes.toFixed(2)),
      numCobros: historial.length,
    ...
    const idxC = cierresMes.findIndex(c => c.mes === mesClave);
    if (idxC >= 0) { cierresMes[idxC] = cierre; } else { cierresMes.push(cierre); }
    cierresMes.sort((a, b) => b.mes.localeCompare(a.mes));
  }
...
function actualizarTodo() {
  ...
  if (pgId === 'pgReportes')     { renderVentas(); renderHistorial(); renderHistorialMeses(); renderCritico(); renderPagos(); renderBalance(); }
  if (pgId === 'pgDestacados')   { renderDestacados(); }
...
(async function init() {
  try {
    ...
    await getDB(); // probar conexión IDB
    setIDBStatus(true);
