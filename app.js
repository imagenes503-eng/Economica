...
function guardarProducto(e) {
  ...
  if (typeof syncAhora === 'function') { await syncAhora('historial'); await syncAhora('productos'); await syncAhora('venta_diaria'); }
...
async function guardarEdicionCobro() {
  ...
  if (!activos.length) {
    ...
    if (_cobroId && !cobrosEliminados.includes(_cobroId)) { cobrosEliminados.push(_cobroId); idbSet('vpos_cobrosEliminados', cobrosEliminados).catch(()=>{}); }
  ...
  if (fechaDev && montoDevuelto > 0) {
    ...
    if (vdIdx >= 0) {
      ...
      if (typeof syncAhora === 'function') await syncAhora('venta_diaria');
    }
  }
