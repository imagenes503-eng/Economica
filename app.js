  ...
  if (pkgsOrdenados.length) {
    pkgsOrdenados.forEach(pk => {
      ...
      _regPres.push({
        ...
        costo: pk.precioCompra ? parseFloat((pk.precioCompra * udsXPres).toFixed(6)) : parseFloat((compraUd * udsXPres).toFixed(6)),
        venta: pk.precio || 0
      });
    });
  }
...
function guardarProducto(e) {
  ...
  if (id) {
    ...
    productos = productos.map(x => String(x.id) === String(id)
      ? { id: Number(id), cod: newCod, nom: newNom, cat: newCat, abrev: newAbrev, compra: newCompra, venta: newVenta, stock: newStock, min: newMin, costoTotal: newCostoTotal, lotes: lotesActuales, paquetes: newPaquetes, img: newImg, _ts: Date.now() }
      : x);
  } else {
    productos.push({ id: Date.now(), cod: newCod, nom: newNom, cat: newCat, abrev: newAbrev, compra: newCompra, venta: newVenta, stock: newStock, min: newMin, costoTotal: newCostoTotal, lotes: [], paquetes: newPaquetes, img: newImg, _ts: Date.now() });
  }
