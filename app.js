...
function ejecutarReiniciarMes() {
  if (!confirm('Se descargará el Estado de Resultados (PDF) y un backup JSON, y luego se reiniciará el mes. ¿Continuar?')) return;

  // Generar el PDF profesional ANTES de borrar nada, con los datos tal como están
  try { generarEstadoResultadosPDF(); } catch (e) { console.error('Error generando Estado de Resultados:', e); }
...
function rp_descargarImagenesPedido() {
  ...
  bloques.forEach((bloque, idx) => {
    _rpGenerarImagen(bloque, idx + 1, totalImgs, totalCostoPedido, totalVentaPedido, gananciaPedido);
  });
}

function _rpGenerarImagen(filas, numImg, totalImgs, totalCostoPedido, totalVentaPedido, gananciaPedido) {
  const SCALE    = 2;
  ...
  const H_RESUMEN = 62 * SCALE;   // banda de totales (solo primera imagen)
  const H_GANANCIA = 34 * SCALE;  // fórmula de ganancia (solo última imagen, hasta abajo)
  const H_HEAD   = 38  * SCALE;
  const H_FILA   = 40  * SCALE;
  const H_FOOT   = 38  * SCALE;
  const POR_IMG  = 30;
  const conResumen  = numImg === 1;
  const conGanancia = numImg === totalImgs;
  const ALTO = H_BAND + H_SUB + (conResumen ? H_RESUMEN : 0) + H_HEAD + H_FILA * POR_IMG + H_FOOT + (conGanancia ? H_GANANCIA : 0);
  ...
  // ── Banda de resumen: TOTAL COSTO PEDIDO / TOTAL VENTA PEDIDO (solo imagen 1) ──
  let yResumenEnd = H_BAND + H_SUB;
  if (conResumen) {
    const yRes = H_BAND + H_SUB;
    ctx.fillStyle = '#f0fdf4'; ctx.fillRect(0, yRes, ANCHO, H_RESUMEN);
    ctx.strokeStyle = '#a7f3d0'; ctx.lineWidth = 2*SCALE;
    ctx.beginPath(); ctx.moveTo(0, yRes + H_RESUMEN); ctx.lineTo(ANCHO, yRes + H_RESUMEN); ctx.stroke();
    ...
    yResumenEnd = yRes + H_RESUMEN;
  }

  // Cabecera columnas
  const yCH = yResumenEnd;
  ...
  // ── Fórmula de Ganancia: Ganancia = Venta − Costo = X (hasta abajo, solo última imagen) ──
  if (conGanancia) {
    const yGan = yFoot + H_FOOT;
...
function rp_descargarPDFS1() {
  ...
  const H_BAND  = 64  * SCALE;
  const H_SUB   = 28  * SCALE;
  const H_RESUMEN = 92 * SCALE; // banda de totales + ganancia (solo en la primera imagen)
  const H_HEAD  = 38  * SCALE;
  const H_FILA  = 40  * SCALE;
  const H_FOOT  = 38  * SCALE;

  bloques.forEach((bloque, imgIdx) => {
    const conResumen = imgIdx === 0; // el resumen de totales/ganancia solo va en la primera imagen
    const ALTO = H_BAND + H_SUB + (conResumen ? H_RESUMEN : 0) + H_HEAD + H_FILA * POR_IMG + H_FOOT;
    const canvas = document.createElement('canvas');
    canvas.width  = ANCHO;
    canvas.height = ALTO;
    const ctx = canvas.getContext('2d');
    ...
    // ── Banda de resumen: TOTAL COSTO/VENTA (vendido), Disponible, Ganancia + fórmula (solo imagen 1) ──
    let yResumenEnd = H_BAND + H_SUB;
    if (conResumen) {
      const yRes = H_BAND + H_SUB;
