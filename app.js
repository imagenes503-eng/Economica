...
function _recalcularReportesDesdeHistorial() {
  ...
  (historial || []).forEach(v => {
    ...
    (v.items || []).forEach(it => {
      ...
      // cant = unidades físicas (para paquetes ya incluye stockPorCant)
      // totalItem = monto monetario correcto guardado desde finalizarVenta
      // precio = precio por unidad individual (INCORRECTO para paquetes si cant>1)
      // Solución: usar totalItem si existe, sino cant * precio (ventas antiguas)
