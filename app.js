...
function abrirTecladoCantidad(ctx) {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
    <div class="modal-box modal-sm" style="max-width:310px;border-radius:20px;">
      <div class="modal-header" style="background:linear-gradient(135deg,#1e3a5f,#1d4ed8);border-radius:18px 18px 0 0;">
        <h3 id="teclNomProd" style="color:#fff;font-size:14px;font-weight:900;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:220px;">Cantidad</h3>
        <button class="btn-close" onclick="cerrarModal('modalTecladoCant')" style="color:#fff;flex-shrink:0;">✕</button>
      </div>
      <div class="modal-body" style="padding:16px;">
        <div id="teclDisplay"
          style="text-align:center;font-size:42px;font-weight:900;font-family:'Space Mono',monospace;
          background:#f0f9ff;border:2.5px solid #7dd3fc;border-radius:14px;padding:14px 12px;
          margin-bottom:14px;min-height:74px;display:flex;align-items:center;justify-content:center;
          color:#1e3a5f;letter-spacing:2px;">0</div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:9px;margin-bottom:9px;">
          ${[7,8,9,4,5,6,1,2,3].map(n => `
          <button onclick="tecladoPush('${n}')"
            style="font-size:24px;font-weight:900;padding:16px 8px;border-radius:13px;border:1.5px solid var(--border-mid);
            background:var(--surface2);cursor:pointer;transition:background 0.1s;font-family:'Space Mono',monospace;"
            onmousedown="this.style.background='#dbeafe'" onmouseup="this.style.background='var(--surface2)'">${n}</button>
          `).join('')}
          <button onclick="tecladoClear()"
            style="font-size:14px;font-weight:900;padding:16px 8px;border-radius:13px;border:1.5px solid #fca5a5;
            background:#fee2e2;cursor:pointer;color:#dc2626;">C</button>
          <button onclick="tecladoPush('0')"
            style="font-size:24px;font-weight:900;padding:16px 8px;border-radius:13px;border:1.5px solid var(--border-mid);
            background:var(--surface2);cursor:pointer;font-family:'Space Mono',monospace;">0</button>
          <button onclick="tecladoBack()"
            style="font-size:20px;font-weight:900;padding:16px 8px;border-radius:13px;border:1.5px solid #fcd34d;
            background:#fef3c7;cursor:pointer;color:#d97706;">⌫</button>
        </div>
        <div id="teclPkgRow" style="display:none;margin-bottom:9px;"></div>
        <button onclick="tecladoConfirmar()"
          style="width:100%;padding:17px;border-radius:13px;border:none;background:linear-gradient(135deg,#16a34a,#15803d);
          color:#fff;font-size:18px;font-weight:900;cursor:pointer;letter-spacing:0.5px;">✓ CONFIRMAR</button>
      </div>
    </div>`;
    document.body.appendChild(modal);
  }

  const nomProd = ctx.p?.nom || ctx.nomProd || 'Editar cantidad';
  document.getElementById('teclNomProd').textContent = nomProd;
  tecladoClear();

  // Mostrar botones de paquete si el producto tiene paquetes y mode='add'
  const pkgRow = document.getElementById('teclPkgRow');
  if (ctx.mode === 'add' && ctx.p && (ctx.p.paquetes||[]).length > 0) {
  ...
  setTimeout(() => modal.querySelector && modal.querySelector('#teclDisplay') && null, 0);
}

function tecladoPush(d) {
  if ((window._tecladoValor || '').length >= 4) return;
  window._tecladoValor = (window._tecladoValor || '') + d;
  document.getElementById('teclDisplay').textContent = window._tecladoValor || '0';
}
...
function tecladoBack() {
  ...
  const el = document.getElementById('teclDisplay');
  if (el) el.textContent = window._tecladoValor || '0';
}
