...
function renderInvTotales() {
  ...
  if (peBox && peGrid) {
    if (hayPE) {
      ...
      // Punto equilibrio = capital inicial a precio COSTO (lo que el negocio tenía al empezar)
      const puntoEq       = efectivoInicial + inventarioInicial;
      // Capital actual comparado a COSTO para ser consistente con el punto inicial
      ...
      peGrid.innerHTML = `
        ...
        <div class="stat-box" style="border-color:rgba(29,78,216,0.3);background:rgba(29,78,216,0.04);"><div class="s-lbl" style="color:var(--blue);">📊 Capital Actual (costo)</div><div class="s-val" style="color:var(--blue);font-size:18px;">$${capitalActual.toFixed(2)}</div><div style="font-size:11px;color:var(--text-muted);margin-top:2px;">caja + inv. a precio costo</div></div>
...
function guardarProducto(e) {
  ...
  // Sync img separately and immediately so it doesn't get wiped by batch sync
  if (newImg && typeof syncImgProducto === 'function') {
    const savedProd = productos.find(x => String(x.img) === String(newImg));
    if (savedProd) setTimeout(() => syncImgProducto(savedProd), 500);
  }
}
...
// ── Foto Pro: cargar imagen pendiente desde foto-pro.html ──
async function cargarFotoProPendiente() {
  try {
    const db = await getDB();
    const fotoPro = await new Promise((res, rej) => {
      const req = db.transaction('kv','readonly').objectStore('kv').get('fotopro_pendiente');
      req.onsuccess = () => res(req.result);
      req.onerror   = () => rej(req.error);
    });
    if (!fotoPro || !fotoPro.dataUrl) return;
    // Mostrar aviso
    const banner = document.getElementById('fotoProBanner');
    if (banner) {
      banner.style.display = 'flex';
      banner.querySelector('.fpb-img').src = fotoPro.dataUrl;
      banner.querySelector('.fpb-name').textContent = fotoPro.name || 'Foto Pro';
      banner._dataUrl = fotoPro.dataUrl;
    }
  } catch(e) { console.error('FotoPro pending:', e); }
}

function usarFotoProEnProducto() {
  const banner = document.getElementById('fotoProBanner');
  if (!banner || !banner._dataUrl) return;
  _imagenPendiente = banner._dataUrl;
  const prev = document.getElementById('imgPreview');
  if (prev) {
    prev.innerHTML = `<img src="${banner._dataUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:9px;">`;
    prev.style.border = '2px solid var(--green)';
  }
  ...
  // Limpiar pendiente
  idbSet('fotopro_pendiente', null).catch(console.error);
  toast('✓ Foto Pro cargada');
}
...
function borrarProd(id) {
  ...
  if (confirm('¿Eliminar este producto?')) {
    ...
    // Registrar el ID como eliminado para que no vuelva al fusionar con otros dispositivos
...
function ejecutarReiniciarMes() {
  ...
  // Subir snapshot limpio a Supabase
  if (typeof syncAhora === 'function') syncAhora('todo');
}
...
function renderPagina(pgId) {
  ...
  if (pgId === 'pgInventario')    { renderInv(); actualizarCats(); poblarInvAnCat(); cargarFotoProPendiente(); if (!document.getElementById('invAnDesde')?.value && !document.getElementById('invAnHasta')?.value) { invAnSetRango(30); } else { renderInvAnalisis(); } }
  if (pgId === 'pgReportes')      { renderVentas(); renderHistorial(); renderCritico(); renderPagos(); renderBalance(); }
  if (pgId === 'pgDestacados')    { renderDestacados(); }
  if (pgId === 'pgVentasDiarias') { renderVentasDiarias(); }
  if (pgId === 'pgAdmin' && typeof renderAdminPanel === 'function') renderAdminPanel();
