...
function finalizarVenta() {
  ...
  // ── FIX: Actualizar ventasDiarias automáticamente al cobrar ──────────
  // Esto es lo que alimenta capitalTotal — sin esto el capital no cambia
  (function() {
    const hoyISO = new Date().toISOString().split('T')[0];
    const montoVenta = parseFloat(venta.total) || 0;
    const idxVD = ventasDiarias.findIndex(v => v.fecha === hoyISO);
    if (idxVD >= 0) {
      ventasDiarias[idxVD].monto = (Number(ventasDiarias[idxVD].monto) || 0) + montoVenta;
    } else {
      ventasDiarias.push({ fecha: hoyISO, monto: montoVenta, nota: 'Auto-POS' });
      ventasDiarias.sort((a, b) => a.fecha.localeCompare(b.fecha));
    }
    if (typeof idbSet === 'function') idbSet('vpos_ventasDiarias', ventasDiarias).catch(() => {});
  })();
  // ─────────────────────────────────────────────────────────────────────
