...
function reiniciarDia() {
  ...
  setTimeout(() => { if (typeof _autoEnviarSnapshot === 'function') _autoEnviarSnapshot(); }, 1000);
...
function reiniciarHistorial() {
  ...
  // ── RESTAURAR STOCK: devolver unidades vendidas HOY antes de borrar ──
  // Solo se restauran ventas del día actual (misma fecha calendario).
  // Ventas de días anteriores ya no afectan el stock actual.
  const hoyStr = new Date().toDateString();
  let productosRestaurados = 0;

  (historial || []).forEach(venta => {
    const fechaVenta = venta.fechaISO || venta.fecha;
    if (!fechaVenta) return;
    if (new Date(fechaVenta).toDateString() !== hoyStr) return; // solo ventas de hoy
    ...
    (venta.items || []).forEach(item => {
      ...
      // Buscar producto por id, con fallback por nombre
      const prod = pid
        ? productos.find(p => String(p.id) === pid)
        : productos.find(p => p.nom === item.nom);

      if (prod) {
        prod.stock += cant;

        // Restaurar lotes en orden inverso (último vendido = primero en recuperar)
        if (prod.lotes && prod.lotes.length > 0) {
          let restante = cant;
          for (let i = prod.lotes.length - 1; i >= 0 && restante > 0; i--) {
            const lote = prod.lotes[i];
            const maxDevolver = (lote.stockInicial || 0) - (lote.stockRestante || 0);
            const devolver = Math.min(restante, maxDevolver);
            if (devolver > 0) { lote.stockRestante += devolver; restante -= devolver; }
          }
        }
  ...
  // FIX: también limpiar ventasDiarias para que el capital total no se descuadre entre teléfonos
  historial = []; ventasDiarias = []; salvar();

  const msg = productosRestaurados > 0
    ? `✓ Historial borrado — ${productosRestaurados} producto(s) restaurados al inventario`
    : 'Historial borrado';
  toast(msg, productosRestaurados === 0);

  // 1️⃣ Broadcast instantáneo → teléfonos conectados actualizan en tiempo real
  //    FIX: incluir ventasDiarias:[] en el payload para que el otro teléfono también limpie su capital
  if (typeof _broadcast === 'function') _broadcast('historial_actualizado', { historial: [], ventasDiarias: [] });
  // 2️⃣ Subir a Supabase (borra filas) → teléfonos que inicien sesión no cargan datos viejos
  //    FIX: también sincronizar ventasDiarias vacía y productos actualizados
  if (typeof syncAhora === 'function') { syncAhora('historial'); syncAhora('venta_diaria'); if (productosRestaurados > 0) syncAhora('productos'); }
  // 3️⃣ Push snapshot actualizado para reconexiones
  setTimeout(() => {
    if (typeof _autoEnviarSnapshot === 'function') _autoEnviarSnapshot();
...
function guardarEdicionCobro() {
  ...
  // ── Calcular monto total a devolver (antes de modificar historial) ──
  const montoDevuelto = devolver.reduce((s, i) => s + Number(i.cant||0) * Number(i.precio||0), 0);
  ...
  // ── FIX: Descontar de ventasDiarias (fuente única de verdad del capital) ──
  // Sin esto, el capital no baja al borrar un cobro y la ganancia se descuadra.
  if (montoDevuelto > 0) {
    const fechaVenta = (v.fechaISO || v.fecha || '').substring(0, 10); // YYYY-MM-DD
    const idxVD = ventasDiarias.findIndex(vd => vd.fecha === fechaVenta);
    if (idxVD >= 0) {
      ventasDiarias[idxVD].monto = Math.max(0, Number(ventasDiarias[idxVD].monto || 0) - montoDevuelto);
      if (ventasDiarias[idxVD].monto === 0) ventasDiarias.splice(idxVD, 1);
  ...
  if (typeof syncAhora === 'function') syncAhora('productos');
  // Broadcast con ventasDiarias actualizado para que otros dispositivos reflejen el capital correcto
  if (typeof _broadcast === 'function') _broadcast('historial_actualizado', { historial: historial.map(v => ({...v, img: undefined})), ventasDiarias });
}
