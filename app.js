...
function reiniciarHistorial() {
  ...
  // ── NUEVO: restaurar stock de ventas del día actual ──────────────────
  // Si no se ha reiniciado el día, las ventas de HOY aún tienen efecto
  // en el inventario → devolver esas unidades al stock antes de borrar.
  ...
  (historial || []).forEach(venta => {
    ...
    const esHoy = new Date(fechaVenta).toDateString() === hoyStr;
    if (!esHoy) return; // solo restaurar ventas de hoy
    ...
    (venta.items || []).forEach(item => {
      ...
      // Buscar producto por id (o por nombre como respaldo)
      const prod = pid
      ...
      if (prod) {
        ...
        // También restaurar los lotes si corresponde
        if (prod.lotes && prod.lotes.length > 0) {
          let restante = cant;
          // Restaurar en orden inverso (último lote vendido = primero en devolver)
          for (let i = prod.lotes.length - 1; i >= 0 && restante > 0; i--) {
            const lote = prod.lotes[i];
            const devolver = Math.min(restante, (lote.stockInicial || 0) - (lote.stockRestante || 0));
  ...
  if (typeof _broadcast === 'function') {
    _broadcast('historial_actualizado', { historial: [], ventasDiarias: [] });
    // Notificar el stock actualizado a otros dispositivos
    if (productosRestaurados > 0 && typeof syncAhora === 'function') syncAhora('productos');
  }
...
(async function init() {
  try {
    ...
    // ── FIX: después de iniciarAutoSync ya tenemos datos frescos de Supabase ──
    //         Re-renderizar la página activa para que el historial/venta día
    //         muestren los datos correctos y no los del caché IDB viejo.
    actualizarStats();
    try {
      const pgFresh = document.querySelector('.page.active');
      if (pgFresh && typeof renderPagina === 'function') renderPagina(pgFresh.id);
    } catch(e) {}
