...
// Formatea un valor monetario con hasta 4 decimales (mínimo 2).
// Ej: 77.5→"77.50"  0.7125→"0.7125"  0.71→"0.71"  18.172→"18.172"
function _fmtCosto(v) {
...
function calcValorInventario() {
  ...
  productos.forEach(p => {
    compra += _costoTotalProd(p);
    venta  += _ventaTotalProd(p);
  });
  return { compra, venta };
}
...
function regPresEditar(id, campo, valor) {
  ...
  // AUTO-CÁLCULO DE PRECIOS COSTO
  // Cuando el usuario ingresa/cambia el costo de una presentación, calcular el precio
  // por unidad y propagarlo automáticamente a todas las demás presentaciones.
  // Cuando cambia las uds de una presentación, recalcular su costo desde la referencia.
  if (campo === 'costo') {
    const uds   = Number(r.uds)   || 0;
    const costo = Number(r.costo) || 0;
    if (uds > 0 && costo > 0) {
      const costoUnitario = costo / uds; // precio exacto por unidad, con todos los decimales
      _regPres.forEach(other => {
        if (String(other.id) === String(id)) return; // no tocar la fila editada
        const oUds = Number(other.uds) || 0;
        if (oUds > 0) {
          other.costo = parseFloat((costoUnitario * oUds).toFixed(4));
          // Actualizar el input visual de la otra tarjeta sin perder el foco
          const otherCard = document.querySelector('[data-id="' + String(other.id) + '"].reg-pres-card');
          if (otherCard) {
            const inp = otherCard.querySelector('.reg-pres-field.costo input');
            if (inp) inp.value = other.costo.toFixed(4);
          }
        }
      });
    }
  ...
  if (campo === 'uds') {
    const newUds = Number(r.uds) || 0;
    if (newUds > 0) {
      // Buscar la referencia: primera fila con costo y uds definidos
      const ref = _regPres.find(o => String(o.id) !== String(id) && Number(o.uds) > 0 && Number(o.costo) > 0);
      if (ref) {
        const costoUnitario = Number(ref.costo) / Number(ref.uds);
        r.costo = parseFloat((costoUnitario * newUds).toFixed(4));
        const thisCard = document.querySelector('[data-id="' + String(id) + '"].reg-pres-card');
        if (thisCard) {
          const inp = thisCard.querySelector('.reg-pres-field.costo input');
          if (inp) inp.value = r.costo.toFixed(4);
        }
      }
    }
  }
  ...
  if (card) {
    ...
    if (showSub) {
      const html = '<div class="reg-pres-subtotal">' +
        '<span style="color:var(--text-muted);">📦 ' + (uds*cant) + ' uds · costo <strong style="color:#b45309;">$' + _fmtCosto(costoSub) + '</strong></span>' +
  ...
  cont.innerHTML = _regPres.map(r => {
    ...
    return `<div class="reg-pres-card" data-id="${r.id}">
      ...
      ${showSub ? `<div class="reg-pres-subtotal">
        <span style="color:var(--text-muted);">📦 ${udsTotal} uds · costo <strong style="color:#b45309;">$${_fmtCosto(costoSub)}</strong></span>
...
function _regPresActualizarResumen() {
  ...
  // Costo unitario para p.compra (display en tarjeta de inventario):
  // Prioridad 1: fila Unidad (uds=1) si existe — precio real de la unidad suelta.
  // Prioridad 2: promedio ponderado cuando hay cantidades pero no hay fila Unidad.
  // Prioridad 3: costo derivado del paquete mayor (fallback sin cantidades).
  // _costoTotalProd usa estructura de paquetes para el total, por lo que
  // p.compra puede reflejar el precio real de la unidad sin afectar el inventario.
  const filasConCosto = _regPres.filter(r=>Number(r.uds)>0&&Number(r.costo)>0);
  const filaUnidad = filasConCosto.find(r=>Number(r.uds)===1);
  const filaMaxC   = filasConCosto.slice().sort((a,b)=>Number(b.uds)-Number(a.uds))[0];
  const compraUd = filaUnidad
    ? Number(filaUnidad.costo)
    : (costoPorUd > 0 ? costoPorUd : (filaMaxC ? Number(filaMaxC.costo) / Number(filaMaxC.uds) : 0));
  ...
  if(elCosto)  elCosto.textContent = '$' + _fmtCosto(totalCosto);
