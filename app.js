...
function _pdfBuildReporte(doc, titulo, subtitulo, acumData, catFiltro, opts = {}) {
  const conGanancia = !!opts.conGanancia; // true solo para el Reporte Mensual
...
function generarPDF(tipo) {
  ...
  if (tipo === 'mensual') {
    // ── Reporte Mensual con Invertido/Ganancia por producto ──
    // Se calcula directo del historial real del mes (misma lógica de costo
    // que usa el Reporte por Período), no del acumulado simple ventasMes.
    const acum = {};
    (historial || []).forEach(v => {
      if (!v.fechaISO || (typeof esMesActual === 'function' && !esMesActual(v.fechaISO))) return;
    ...
    _pdfBuildReporte(doc, 'REPORTE MENSUAL', subtitulo, acum, filtro, { conGanancia: true });
    doc.save(`Despensa_mensual_${now.toLocaleDateString('es-SV').replace(/\//g,'-')}.pdf`);
    toast('✓ PDF mensual descargado');
    return;
  }

  // ── Semanal / Diario: sin cambios, igual que siempre ──
  const data = tipo === 'semanal' ? ventasSem : ventasDia;
  const acum = {};
