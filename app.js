...
function renderCajaPanel() {
  ...
  // Capital Total = caja actual + inventario inicial (fijo del mes)
  const capitalTotal       = cajaActual + inventarioInicial;
  ...
  balance.innerHTML = `
    <div>
      ...
      <div style="font-size:11px;color:var(--text-muted);font-weight:700;margin-top:3px;">💵 Caja actual + 📦 Inventario inicial (fijo del mes)</div>
    </div>
    <div class="balance-resultado">
      ${capitalInicial > 0 ? `
      <div style="text-align:center;">
        <div style="font-size:10px;font-weight:900;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px;">📈 Estado de Resultados del Mes</div>
        <div style="display:grid;gap:5px;">
          <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:5px 10px;font-size:11px;font-weight:800;color:#15803d;">
            💰 Ventas: $${totalVentasDiariasDelMes.toFixed(2)}
          </div>
          <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:8px;padding:5px 10px;font-size:11px;font-weight:800;color:#c2410c;">
            📦 − COGS (costo vendido): $${cogsDelMes.toFixed(2)}
          </div>
          <div style="background:${esGB ? '#f0fdf4' : '#fef2f2'};border:1px solid ${esGB ? '#86efac' : '#fca5a5'};border-radius:8px;padding:5px 10px;font-size:12px;font-weight:900;color:${esGB ? '#15803d' : '#dc2626'};">
            = Ganancia Bruta: ${esGB ? '+' : ''}$${gananciaBruta.toFixed(2)}
          </div>
          <div style="background:#fef9f0;border:1px solid #fde68a;border-radius:8px;padding:5px 10px;font-size:11px;font-weight:800;color:#b45309;">
            🧾 − Gastos operativos: $${totalEgresos.toFixed(2)}
          </div>
          <div class="balance-badge ${esGN ? 'pos' : 'neg'}" style="font-size:13px;">
            ${esGN ? '✅' : '🔴'} Ganancia Neta: ${esGN ? '+' : ''}$${gananciaNeta.toFixed(2)}
          </div>
          ${cogsDelMes === 0 ? '<div style="font-size:10px;color:#b45309;font-weight:700;margin-top:2px;">⚠️ COGS en $0 — las ventas antiguas no tienen costo guardado</div>' : ''}
        </div>
  ...
  if (peBox && peGrid) {
    if (hayPE) {
      ...
      peGrid.innerHTML = `
        ...
        <div class="stat-box" style="border-color:#f59e0b;"><div class="s-lbl" style="color:#d97706;">📦 Inv. Inicial (venta)</div><div class="s-val" style="color:#d97706;font-size:18px;">$${inventarioInicial.toFixed(2)}</div><div style="font-size:11px;color:var(--text-muted);margin-top:2px;">stock × precio venta</div></div>
        <div class="stat-box" style="border-color:#0ea5e9;"><div class="s-lbl" style="color:#0369a1;">🏷️ Inv. Inicial (costo)</div><div class="s-val" style="color:#0369a1;font-size:18px;">$${invCostoBase.toFixed(2)}</div><div style="font-size:11px;color:var(--text-muted);margin-top:2px;">stock × precio compra</div></div>
