...
function rp_descargarPDFS1() {
  ...
  doc.text('REPORTE DE INVENTARIO', 105, 12, { align: 'center' });
  doc.setFontSize(8); doc.setFont('helvetica', 'normal');
  doc.text(`${snap.rangoStr}  ·  ${snap.catLabel}`, 105, 20, { align: 'center' });
  doc.setFontSize(7); doc.setTextColor(167, 243, 208);
  doc.text(`Generado: ${new Date().toLocaleString('es-SV')}`, 105, 25, { align: 'center' });

  // Nota al pie del header sobre el stock inicial
  doc.setFillColor(240, 253, 244);
  doc.rect(10, 31, 190, 9, 'F');
  doc.setDrawColor(134, 239, 172); doc.rect(10, 31, 190, 9, 'S');
  doc.setFont('helvetica', 'italic'); doc.setFontSize(7); doc.setTextColor(6, 78, 59);
  doc.text('Stock Inicial = cantidad registrada al presionar "Recalcular inventario inicial"  ·  Stock Actual = existencia actual  ·  Vendido = diferencia', 105, 37, { align: 'center' });

  // Tabla: solo producto, stock inicial, stock actual, vendido
  const stockSnap = window._stockInicialSnap || {};
  const rows = snap.filas.map(f => {
    const snapProd = stockSnap[String(f.id)] || null;
    const stockIni = snapProd ? snapProd.stock : '—';
    const stockAct = f.stock;
    const vendido  = snapProd ? Math.max(0, snapProd.stock - f.stock) : f.vendido;
    return [f.nom, String(stockIni), String(stockAct), String(vendido)];
  });

  if (doc.autoTable) {
    doc.autoTable({
      startY: 43,
      head: [['Producto', 'Stock Inicial', 'Stock Actual', 'Vendido']],
      body: rows,
      ...
      columnStyles: {
        ...
        1: { halign: 'center', cellWidth: 30, textColor: [107, 33, 168] },
        2: { halign: 'center', cellWidth: 30, textColor: [22, 101, 52] },
        3: { halign: 'center', cellWidth: 30, textColor: [29, 78, 216], fontStyle: 'bold' }
      },
      margin: { left: 10, right: 10 }
    });
  }
