...
function actualizarInventarioInicialAuto() {
  ...
  const totalCost = (productos || []).reduce((s, p) => s + (Number(p.stock)||0)*_compraUdReal(p), 0);
...
function _compraUdReal(p) {
  // Precio de compra por unidad real: usa el paquete de mayor volumen si existe,
  // porque ese refleja el precio real pagado al proveedor (más barato por unidad).
  const pkgs = (p.paquetes || []).filter(pk => pk.cant > 1 && pk.precioCompra > 0);
  if (pkgs.length) {
    // precioCompra en el paquete ya es por unidad (se guardó como costo/cant)
    const pkMax = pkgs.slice().sort((a,b) => b.cant - a.cant)[0];
    return pkMax.precioCompra;
  }
  return Number(p.compra) || 0;
}

function calcValorInventario() {
  let compra = 0, venta = 0;
  productos.forEach(p => {
    const compraUd = _compraUdReal(p);
    const lotStock  = (p.lotes || []).reduce((s, l) => s + (l.stockRestante || 0), 0);
    const mainStock = Math.max(0, (p.stock || 0) - lotStock);
    compra += mainStock * compraUd;
...
function renderInvTotales() {
  ...
  if (peBox && peGrid) {
    if (hayPE) {
      ...
      // ── Capital Hoy CORRECTO ──────────────────────────────────────────────────
      // = Caja actual (Ef.ini + ventas − gastos) + Inventario real a costo (tiempo real)
      // totalCapCompra ya calcula esto correctamente:
      //   cajaActual     = Ef.ini + ventasConfirmadas − totalGastosMes
      //   totalInvCompra = valor actual del stock × precio compra (baja cuando vendes)
      // Al vender $10 / costo $8 → caja +$10, inventario −$8 → capital neto +$2 ✓
      // Al comprar inventario → caja −$X, inventario +$X → capital neto 0 ✓
      const capitalActual      = totalCapCompra;  // a precio costo (real)
      const capitalActualVenta = totalCapVenta;   // a precio venta (potencial)
      ...
      peGrid.innerHTML = `
        ...
        <div style="background:rgba(29,78,216,0.04);border:2px solid rgba(29,78,216,0.3);border-radius:10px;padding:12px;">
          <div style="font-size:10px;font-weight:900;color:#1d4ed8;margin-bottom:4px;">💼 CAPITAL HOY (a costo)</div>
          <div style="font-size:20px;font-weight:900;color:#1d4ed8;">$${capitalActual.toFixed(2)}</div>
          <div style="font-size:10px;color:#60a5fa;margin-top:3px;font-weight:700;">Caja + Inv.actual a costo</div>
        ...
        <!-- Fila 4: Capital Hoy a precio VENTA -->
        <div style="grid-column:1/-1;font-size:11px;font-weight:900;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;padding:8px 0 2px;">
          💹 Capital Hoy a Precio Venta
        </div>

        <div style="background:rgba(21,128,61,0.05);border:2px solid rgba(21,128,61,0.35);border-radius:10px;padding:12px;grid-column:1/-1;">
          <div style="font-size:10px;font-weight:900;color:#15803d;margin-bottom:4px;">📈 CAPITAL HOY (a precio venta)</div>
          <div style="font-size:20px;font-weight:900;color:#15803d;">$${capitalActualVenta.toFixed(2)}</div>
          <div style="font-size:10px;color:#4ade80;margin-top:3px;font-weight:700;">Caja + Inv.actual a precio venta · Potencial si vendes todo lo que queda</div>
        </div>
      `;
  ...
  if (pkgsOrdenados.length) {
    pkgsOrdenados.forEach(pk => {
      ...
      _regPres.push({
        ...
        costo: pk.precioCompra ? pk.precioCompra * udsXPres : (compraUd * udsXPres),
        venta: pk.precio || 0
      });
    });
  }
...
function renderInv() {
  ...
  if (totEl) {
    ...
    const tc = lista.reduce((s,p)=>s+_compraUdReal(p)*(p.stock||0),0);
...
function _restockIniciarLineas(p) {
  ...
  pkgs.forEach(pk => {
    _restockLineas.push({
      ...
      label: pk.label || ('Paquete ' + pk.cant + ' uds'),
      uds: pk.cant, cant: '', costo: pk.precio||0, venta: pk.precio||0,
      esNuevo: false
    });
  });
