...
function renderInvTotales() {
  ...
  const ventasConfirmadas  = (ventasDiarias || [])
    .filter(v => esMesActual(v.fecha + 'T00:00:00'))
    .reduce((s, v) => s + Number(v.monto || 0), 0);
  // Compras del mes desde restockLog (dinero que salió de caja para comprar inventario)
  const comprasRestockMes  = (restockLog || [])
    .filter(r => { try { return esMesActual(new Date(r.ts).toISOString()); } catch(e) { return false; } })
    .reduce((s, r) => s + (Number(r.cant||0) * Number(r.precioCompra||0)), 0);
  const cajaActual         = efectivoInicial + ventasConfirmadas - totalGastosMes - comprasRestockMes;
  ...
  if (peBox && peGrid) {
    if (hayPE) {
      ...
      // capitalActual = caja actual (ya incluye -comprasRestock) + inventario actual a costo
      // Así una compra de inventario no genera ganancia ficticia:
      // la caja baja por comprasRestock, pero totalInvCompra sube igual → neto 0
