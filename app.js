...
function _rpGenerarImagen(filas, numImg, totalImgs, totalCostoPedido, totalVentaPedido, gananciaPedido) {
  ...
  const H_RESUMEN = 62 * SCALE;   // banda de totales, AHORA hasta abajo, en TODAS las imágenes
  const H_GANANCIA = 34 * SCALE;  // fórmula de ganancia, hasta abajo, en TODAS las imágenes
  const H_HEAD   = 38  * SCALE;
  const H_FILA   = 40  * SCALE;
  const H_FOOT   = 38  * SCALE;
  const POR_IMG  = 30;
  const ALTO = H_BAND + H_SUB + H_HEAD + H_FILA * POR_IMG + H_FOOT + H_RESUMEN + H_GANANCIA;
  ...
  // ── Banda de resumen: TOTAL COSTO PEDIDO / TOTAL VENTA PEDIDO (totales generales, en TODAS las imágenes) ──
  ...
  // ── Fórmula de Ganancia: Ganancia = Venta − Costo = X (hasta abajo, en TODAS las imágenes) ──
