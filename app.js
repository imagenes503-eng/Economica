...
function _validarFechaReportes() {
  ...
  if (guardado.dia !== hoyStr) {
    ...
    // Limpiar flag de reset manual — ya es otro día
    localStorage.removeItem('vpos_diaResetManual');
    cambio = true;
...
function _recalcularReportesDesdeHistorial() {
  ...
  // Respetar reset manual del día
  const _diaResetManual = localStorage.getItem('vpos_diaResetManual');
  const _ignorarHoyEnDia = (_diaResetManual === hoy);
  ...
  (historial || []).forEach(v => {
    ...
    (v.items || []).forEach(it => {
      ...
      if (esHoy && !_ignorarHoyEnDia) { if (!ventasDia[pid]) ventasDia[pid] = {...base}; ventasDia[pid].cant += cant; ventasDia[pid].total += tot; }
...
let ventasEliminadas    = []; // IDs de ventas/cobros borrados — evita que vuelvan desde Supabase
...
function salvar(doRender = true) {
  ...
  _salvarTimer = setTimeout(() => {
    ...
    idbSetMany([
      ...
      ['vpos_ventasEliminadas',    ventasEliminadas],
    ]).catch(err => {
...
async function _cargarCacheIDB() {
  const keys = [
    'vpos_productos','vpos_ventasDia','vpos_ventasSem','vpos_ventasMes',
    'vpos_historial','vpos_pagos','vpos_ventasDiarias','vpos_restockLog',
    'vpos_productosEliminados','vpos_pagosEliminados','vpos_ventasEliminadas'
  ];
  const data = await idbGetMany(keys);
  ...
  ventasEliminadas      = data['vpos_ventasEliminadas']    || [];
  ventasDia = normalizeReport(ventasDia);
  ventasSem = normalizeReport(ventasSem);
  ventasMes = normalizeReport(ventasMes);
...
function renderCajaPanel() {
  ...
  const capitalTotal       = cajaActual + invVenta;          // 💵 Caja + 📦 Inventario actual (precio venta)
...
function borrarGasto(id) {
  if (confirm('¿Eliminar este registro?')) {
    const idStr = String(id);
    // 1. Marcar como eliminado (lista persistente que sobrevive reinicios)
    if (!pagosEliminados.includes(idStr)) pagosEliminados.push(idStr);
    // 2. Persistir la lista en IDB de inmediato (no esperar a salvar())
    if (typeof idbSet === 'function') idbSet('vpos_pagosEliminados', pagosEliminados).catch(() => {});
    // 3. Quitar del array local (usar String en ambos lados)
    pagos = pagos.filter(g => String(g.id) !== idStr);
    salvar();
    toast('Registro eliminado', true);
    // 4. Borrar en Supabase con reintentos
    if (typeof syncBorrarPago === 'function') syncBorrarPago(idStr);
  }
}
...
function reiniciarDia() {
  ...
  // FLAG: indica que el usuario reinició manualmente el día HOY.
  // _autoCargarDesdeSupa lo respeta y no recalcula ventasDia para hoy.
  const _hoyReset = new Date().toDateString();
  localStorage.setItem('vpos_reporteFechaDia', _hoyReset);
  localStorage.setItem('vpos_diaResetManual', _hoyReset);

  salvar();
  actualizarTodo();
  toast('✓ Reporte del día reiniciado');
...
function abrirModalReiniciarSemana() {
  ...
  btnTodo.onclick = () => { if (confirm('¿Reiniciar TODA la semana?\n\nSolo se borra el contador semanal.\nEl historial de cobros, ventas del mes y pagos NO se tocan.')) { ventasSem = {}; salvar(); cerrarModal('modalReiniciarSem'); toast('✓ Semana reiniciada (cobros y mes intactos)'); } };
...
function guardarEdicionCobro() {
  ...
  if (!activos.length) {
    ...
    const _ventaIdEliminada = historial[_editCobroIdx]?.id;
    if (_ventaIdEliminada) {
      const _vidStr = String(_ventaIdEliminada);
      if (!ventasEliminadas.includes(_vidStr)) ventasEliminadas.push(_vidStr);
      // Persistir en IDB de inmediato para sobrevivir recargas
      if (typeof idbSet === 'function') idbSet('vpos_ventasEliminadas', ventasEliminadas).catch(()=>{});
      if (typeof syncBorrarVenta === 'function') syncBorrarVenta(_vidStr);
    }
