...
function renderCajaPanel() {
  ...
  // 📊 CAPITAL TOTAL = inventarioInicial + entradas − salidas
  // Fórmula: inventarioInicial + ventas_confirmadas_mes − egresos_mes
  // NO usamos invVenta (stock×precio) porque con paquetes el precio de venta
  // por unidad ≠ precio cobrado → produce descuadres.
  // La ganancia real = lo que cobré − lo que costó al inicio.
  //
  // Ventas del mes desde historial (usa totalItem para paquetes):
  const totalVentasHistorialMes = (historial || []).filter(v => esMesActual(v.fechaISO)).reduce((s, v) => {
    return s + (v.items || []).reduce((si, it) => {
      if (it.totalItem !== undefined) return si + Number(it.totalItem);
      if (it.cantCobrada !== undefined) return si + Number(it.cantCobrada) * Number(it.precio || 0);
      if (it.paqueteLabel) return si + Number(it.precio || 0);
      return si + Number(it.cant || 0) * Number(it.precio || 0);
    }, 0);
  }, 0);
  // capitalTotal = lo que había al inicio + lo que entró − lo que salió
  const capitalTotal       = inventarioInicial + efectivoInicial + totalVentasHistorialMes - totalEgresos;
...
function recalcularInventarioInicialManual() {
  if (!confirm('¿Actualizar el inventario inicial con el valor actual?\n\nRecalcula SUM(stock x precio venta). Afecta ganancia/perdida del mes.')) return;
  const totalInv = (productos || []).reduce((s, p) => s + (Number(p.stock)||0)*(Number(p.venta)||0), 0);
  inventarioInicial = parseFloat(totalInv.toFixed(2));
  idbSet('vpos_inventarioInicial', inventarioInicial).catch(console.error);
  const inp = document.getElementById('inpInventarioInicial');
...
function recalcularInventarioInicialManual() {
  // Recalcula inventarioInicial desde el stock actual × precio de venta.
  // Usar cuando se haya modificado el inventario y se quiera actualizar el baseline del mes.
  if (!confirm('¿Actualizar el inventario inicial con el valor actual?\n\nRecalcula SUM(stock × precio venta). Afecta ganancia/pérdida del mes.')) return;
  const totalInv = (productos || []).reduce((sum, p) => {
    return sum + (Number(p.stock) || 0) * (Number(p.venta) || 0);
  }, 0);
  inventarioInicial = parseFloat(totalInv.toFixed(2));
  idbSet('vpos_inventarioInicial', inventarioInicial).catch(console.error);
  const inp = document.getElementById('inpInventarioInicial');
