...
function renderCajaPanel() {
  ...
  // Capital total = efectivo inicial + inventario inicial + ventas confirmadas + compras - gastos y pagos
  // inventarioInicial es fijo (no cambia con el stock actual); las ventas entran como caja
  const comprasMes         = pagos.filter(g => esMesActual(g.fechaISO) && g.cat === 'COMPRA')
                               .reduce((s,g) => s + Number(g.monto||0), 0);
  const capitalTotal       = efectivoInicial + inventarioInicial + entroACaja + comprasMes - totalEgresos;
  const capitalInicial     = efectivoInicial + inventarioInicial;
  const ganancia           = capitalTotal - capitalInicial;  // = ventas + compras - gastos
  ...
  balance.innerHTML = `
    <div>
      ...
      <div style="font-size:11px;color:var(--text-muted);font-weight:700;margin-top:3px;">Ef.Inicial + Inv.Inicial + Ventas − Gastos</div>
    </div>
