...
function renderInvTotales() {
  ...
  if (peBox && peGrid) {
    if (hayPE) {
      ...
      // Compras de inventario del mes (restock) — para neutralizar la ganancia ficticia
      // Cuando compras inventario: invCompra sube, pero ese dinero ya no es tuyo → se resta
      ...
      // capitalActual = caja + invActual - comprasDelMes
      // Así comprar inventario no suma ganancia: invActual sube lo mismo que comprasRestockMes
      const capitalActual = cajaActual + totalInvCompra - comprasRestockMes;
