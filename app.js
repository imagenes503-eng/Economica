...
let _db = null;
let _dbAbriendose = null; // evitar aperturas simultáneas

function getDB() {
  if (_db) return Promise.resolve(_db);
  if (_dbAbriendose) return _dbAbriendose;
  _dbAbriendose = _abrirIDB()
    .then(db => { _dbAbriendose = null; return db; })
    .catch(e  => { _dbAbriendose = null; throw e; });
  return _dbAbriendose;
}

function _abrirIDB(intento = 0) {
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = (e) => {
      _db = e.target.result;
      // Cerrar limpiamente si otra versión necesita actualizarse
    ...
    req.onerror = (e) => {
      if (intento < 3) {
        setTimeout(() => _abrirIDB(intento + 1).then(resolve).catch(reject), 500);
      } else {
        reject(e.target.error);
      }
    };

    // onblocked = otra pestaña/recarga tiene el IDB abierto
    // Esperamos hasta 4s a que se cierre (beforeunload lo cierra)
    req.onblocked = () => {
      if (intento < 8) {
        setTimeout(() => _abrirIDB(intento + 1).then(resolve).catch(reject), 500);
      } else {
...
let _tsUltimoVisible = 0;
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
    _tsUltimoVisible = Date.now();
    return;
  }
  // Volvió al frente
  if (carrito.length) return; // nunca interrumpir una venta activa
  const ausencia = Date.now() - _tsUltimoVisible;

  if (ausencia < 60000) {
    // Menos de 1 minuto: solo redibujar UI — sin tocar IDB ni Supabase
    try { actualizarTodo(); } catch(_) {}
    return;
  }

  // Más de 1 minuto: reconectar y refrescar datos
  _db = null; // forzar reconexión fresca al IDB
  migrateAndLoad()
    .then(async () => {
      actualizarTodo();
      if (typeof _sesionActiva !== 'undefined' && _sesionActiva &&
          typeof _autoCargarDesdeSupa === 'function' && ausencia > 300000) {
        // Bajar Supabase solo si estuvo más de 5 minutos fuera
        await _autoCargarDesdeSupa();
        actualizarTodo();
      }
    })
    .catch(e => console.warn('[visibility] reconexión falló:', e.message));
});
...
(async function init() {
  try {
    ...
    for (let _t = 0; _t < 5; _t++) {
      try { await getDB(); break; }
      catch(e) {
        if (_t === 4) throw e;
        setLoadingMsg('Conectando base de datos… (' + (_t + 2) + '/5)');
        _db = null;
    ...
    window.addEventListener('beforeunload', () => {
      salvarSesion();
      // Cerrar IDB explícitamente para que la próxima carga no quede bloqueada
      if (_db) { try { _db.close(); } catch(_) {} _db = null; }
    });
    setInterval(() => salvarSesion(), 30000);
    window.addEventListener('pageshow', (e) => {
      if (e.persisted) {
        // Página restaurada del bfcache — IDB puede estar en estado inválido
        _db = null;
        softReload();
      }
    });
