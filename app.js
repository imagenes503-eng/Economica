  ...
  if (typeof registrarVentaAtomica === 'function') {
    registrarVentaAtomica(venta).then(res => {
      if (res.ok) {
        ...
        if (typeof _autoEnviarSnapshot === 'function') setTimeout(_autoEnviarSnapshot, 200);
        if (typeof syncAhora === 'function') syncAhora('productos');
        // Refrescar stock calculado desde Supabase para sincronizar ambos dispositivos
        setTimeout(function() {
          try {
            var _tid = typeof _getTiendaId === 'function' ? _getTiendaId() : null;
            if (!_tid || typeof _sbRpc !== 'function') return;
            _sbRpc('get_stock_calculado', {p_tienda_id: _tid}).then(function(_rows) {
              ...
              for (var _i = 0; _i < _rows.length; _i++) {
                var _r = _rows[_i];
                var _raw = String(_r.prod_id || '');
                var _u = _raw.lastIndexOf('_');
                var _num = Number(_u >= 0 ? _raw.slice(_u+1) : _raw);
                _map[isNaN(_num) ? _raw : String(_num)] = Number(_r.stock_actual) || 0;
              }
              var _ch = false;
              for (var _j = 0; _j < productos.length; _j++) {
                var _ns = _map[String(productos[_j].id)];
                if (_ns !== undefined && _ns !== productos[_j].stock) {
                  productos[_j].stock = _ns;
                  if (typeof actualizarStockFila === 'function') actualizarStockFila(productos[_j]);
                  _ch = true;
                }
              }
              if (_ch && typeof idbSet === 'function') idbSet('vpos_productos', productos).catch(function(){});
            }).catch(function(){});
          } catch(_e) {}
        }, 400);
      } else {
