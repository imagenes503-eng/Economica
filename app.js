...
function _recalcularReportesDesdeHistorial() {
  ...
  (historial || []).forEach(v => {
    ...
    (v.items || []).forEach(it => {
      ...
      const cant = Number(it.cantCobrada !== undefined ? it.cantCobrada : it.cant || 0);
      const tot  = Number(it.totalItem !== undefined ? it.totalItem : cant * Number(it.precio || 0));
...
function guardarInventarioInicial() {
  // El inventario inicial se calcula automáticamente desde los productos.
  // Esta función solo se llama si el usuario edita el campo manualmente (no debería).
...
// Calcula el valor total del inventario actual (stock × precio_compra) y lo
// guarda como inventarioInicial. Se llama al entrar a Capital e Inventario
// y cada vez que se guarda un producto.
function actualizarInventarioInicialAuto() {
  // inventarioInicial es el BASELINE del inicio del mes — no cambia con ventas.
  // Solo se auto-calcula si está en 0 (nunca fue configurado este mes).
...
function guardarProducto(e) {
  ...
  // Volver al listado de productos tras guardar/actualizar
  navTo('pgInvProductos');
...
function renderInv() {
  ...
  // Inventario inicial siempre se calcula desde los productos (solo lectura)
...
function editarProd(id) {
  ...
  // Ir directamente a la sub-página de registro para editar
...
function pkgPreview() {
  ...
  const p = productos.find(x => String(x.id) === String(_pkgProdIdActivo));
...
function finalizarVenta() {
  ...
  const venta = {
    ...
    items: carrito.map(i => { const p = productos.find(x => x.id === i.id); return { id: String(i.id), nom: i.nom, cant: i.cant * (i.stockPorCant || 1), cantCobrada: i.cant, precio: i.venta, totalItem: i.venta * i.cant, cat: (p && p.cat) ? p.cat : '', paqueteLabel: i.paqueteLabel || null }; }),
...
function actualizarTodo() {
  ...
  if (pgId === 'pgInvAnalisis')  { renderInvAnalisis(); }
  if (pgId === 'pgInvCapital')   { if (typeof actualizarInventarioInicialAuto === 'function') actualizarInventarioInicialAuto(); renderInvTotales(); }
...
function renderPagina(pgId) {
  ...
  if (pgId === 'pgFinanzasMes' && typeof renderFinanzasMes === 'function') renderFinanzasMes(pgId);
  if (pgId === 'pgCierreDia' && typeof renderCierreDia === 'function') renderCierreDia(pgId);
  // Sub-páginas de inventario
  if (pgId === 'pgInvCapital') actualizarInventarioInicialAuto();
  if (pgId === 'pgInvAnalisis' && typeof renderInvAnalisis === 'function') { poblarInvAnCat(); renderInvAnalisis(); }
  if (pgId === 'pgInvProductos') { renderInv(); actualizarCats(); }
  if (pgId === 'pgInvRegistrar') actualizarCats();
}
