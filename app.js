...
async function guardarEdicionCobro() {
  ...
  if (!activos.length) {
    ...
    if (_cobroId && !cobrosEliminados.includes(_cobroId)) {
      cobrosEliminados.push(_cobroId);
  ...
  if (typeof syncAhora === 'function') {
    await syncAhora('historial');
    await syncAhora('productos');
    await syncAhora('venta_diaria');
  }
