...
function rp_descargarPDFS1() {
  ...
  const filas = snap.filas.map(f => {
    ...
    return { nom: f.nom, stockIni: sp ? sp.stock : null, stockAct: f.stock,
  ...
  toast(`⏳ Generando ${totalImgs} imagen${totalImgs > 1 ? 'es' : ''}...`);
  const SCALE = 2;
  const COL_NOM = 460, COL_INI = 160, COL_ACT = 160, COL_VEN = 160;
  const ANCHO = (COL_NOM + COL_INI + COL_ACT + COL_VEN) * SCALE;
  const H_BAND = 64*SCALE, H_SUB = 28*SCALE, H_HEAD = 38*SCALE;
  const H_FILA = 40*SCALE, H_FOOT = 38*SCALE;
  const ALTO = H_BAND + H_SUB + H_HEAD + H_FILA * POR_IMG + H_FOOT;
  for (let imgIdx = 0; imgIdx < totalImgs; imgIdx++) {
    const bloque = filas.slice(imgIdx * POR_IMG, imgIdx * POR_IMG + POR_IMG);
    const canvas = document.createElement('canvas');
    ...
    ctx.fillText('📋  REPORTE DE INVENTARIO', 20*SCALE, 38*SCALE);
    // Sub-banda
    ...
    const xINI=COL_NOM*SCALE, xACT=(COL_NOM+COL_INI)*SCALE, xVEN=(COL_NOM+COL_INI+COL_ACT)*SCALE;
    ctx.fillStyle='#fff'; ctx.font=`bold ${13*SCALE}px Arial`;
    ctx.textAlign='left'; ctx.fillText('PRODUCTO', 50*SCALE, yCH+25*SCALE);
    ctx.textAlign='center';
    ctx.fillText('STOCK INICIAL', xINI+COL_INI*SCALE/2, yCH+25*SCALE);
    ctx.fillText('STOCK ACTUAL',  xACT+COL_ACT*SCALE/2, yCH+25*SCALE);
    ctx.fillText('VENDIDO',       xVEN+COL_VEN*SCALE/2, yCH+25*SCALE);
    [xINI,xACT,xVEN].forEach(x=>{
    ...
    for (let i = 0; i < POR_IMG; i++) {
      ...
      ctx.fillStyle = i%2===0 ? '#fff' : '#fff5f5'; ctx.fillRect(0, y, ANCHO, H_FILA);
      ...
      if (f) {
        ctx.fillStyle='#1e293b'; ctx.font=`bold ${13*SCALE}px Arial`; ctx.textAlign='left';
        let nom = f.nom; const maxW=(COL_NOM-50)*SCALE;
        while(ctx.measureText(nom).width>maxW&&nom.length>1) nom=nom.slice(0,-1);
        if(nom!==f.nom) nom+='…';
        ctx.fillText(nom, 52*SCALE, y+H_FILA/2+5*SCALE);
        const chip=(val,cx,color,bg)=>{
          ...
          ctx.fillStyle=bg; ctx.beginPath(); ctx.roundRect(rx,ry,rw,rh,10*SCALE); ctx.fill();
          ctx.fillStyle=color; ctx.font=`bold ${15*SCALE}px Arial`; ctx.textAlign='center';
          ctx.fillText(str, cx, ry+rh/2+6*SCALE);
        };
        chip(f.stockIni, xINI+COL_INI*SCALE/2, '#7c3aed', '#f5f3ff');
        chip(f.stockAct, xACT+COL_ACT*SCALE/2, '#15803d', '#f0fdf4');
        chip(f.vendido,  xVEN+COL_VEN*SCALE/2, f.vendido>0?'#dc2626':'#6b7280', f.vendido>0?'#fef2f2':'#f9fafb');
      }
    }
    // Footer
    ...
    const link=document.createElement('a');
    link.href=canvas.toDataURL('image/png');
    link.download=`Inventario_Imagen${imgIdx+1}_de${totalImgs}.png`;
    link.click();
