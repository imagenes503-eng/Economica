...
function _regPresActualizarResumen() {
  ...
  // También calcular costo unitario desde fila con costo definido
  const filaMinC = _regPres.filter(r=>Number(r.uds)>0&&Number(r.costo)>0).sort((a,b)=>Number(a.uds)-Number(b.uds))[0];
  const compraUd = filaMinC ? Number(filaMinC.costo) / Number(filaMinC.uds) : costoPorUd;
  ...
  m.forEach(p => {
    ...
    if (!sinStock) {
      d.addEventListener('click', () => {
        ...
        (p.paquetes||[]).some(pk => pk.precio > 0 && pk.cant > 1) ? abrirPickerPaquetes(p) : addCarrito(p);
      });
    }
    grid.appendChild(d);
  });
...
function abrirPickerPaquetes(p) {
  ...
  // Filtrar paquetes válidos: precio > 0 y stock suficiente, excluyendo cant=1 (ya se agrega como "Unidad")
  const pkgsValidos = (p.paquetes||[])
    .filter(pk => pk.precio > 0 && pk.cant > 1 && (p.stock||0) >= pk.cant)
