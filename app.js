...
function renderInvTotales() {
  ...
  if (peBox && peGrid) {
    if (hayPE) {
      ...
      // capitalActual = caja actual + inventario inicial a costo (fijo)
      // Se usa invCostoBase (fijo al inicio del mes) y NO totalInvCompra (tiempo real)
      // porque comprar inventario NO es ganancia — solo cambia la forma del capital.
      // La ganancia real viene de las ventas: cajaActual sube cuando vendes,
      // mientras invCostoBase queda fijo → la diferencia refleja utilidad real.
      const capitalActual = cajaActual + invCostoBase;
...
function _regPresRender() {
  ...
  if (!_regPres.length) {
    ...
    ['inpCompra','inpVenta','inpStock','inpCostoTotal'].forEach(id => { const el = document.getElementById(id); if(el) el.value=''; });
    return;
  }
...
function _regPresActualizarResumen() {
  ...
  if(elT) elT.value = totalCosto.toFixed(6); // costo total exacto sin dividir (evita pérdida de precisión)
...
function guardarProducto(e) {
  ...
  if (id) {
    ...
    productos = productos.map(x => String(x.id) === String(id)
      ? { id: Number(id), cod: newCod, nom: newNom, cat: newCat, abrev: newAbrev, compra: newCompra, venta: newVenta, stock: newStock, min: newMin, costoTotal: newCostoTotal, costoUnidadEntrado, ventaUnidadEntrada, lotes: lotesActuales, paquetes: newPaquetes, img: newImg, _ts: Date.now() }
      : x);
  } else {
    productos.push({ id: Date.now(), cod: newCod, nom: newNom, cat: newCat, abrev: newAbrev, compra: newCompra, venta: newVenta, stock: newStock, min: newMin, costoTotal: newCostoTotal, costoUnidadEntrado, ventaUnidadEntrada, lotes: [], paquetes: newPaquetes, img: newImg, _ts: Date.now() });
  }
...
function renderInv() {
  ...
  if (totEl) {
    ...
    // Usar p.costoTotal (suma exacta de presentaciones) cuando no hay lotes activos,
    // para evitar pérdida de precisión por división/multiplicación del precio unitario.
    const tc = lista.reduce((s,p) => {
      const activeLotes = (p.lotes || []).filter(l => (l.stockRestante || 0) > 0);
      if (activeLotes.length === 0) {
        // Sin lotes: usar costoTotal exacto guardado en el producto (o fallback a compra×stock)
        return s + (p.costoTotal > 0 ? p.costoTotal : (p.stock || 0) * _compraUdReal(p));
      }
      // Con lotes activos: calcular por lote (compra real de cada batch)
      const costoUd  = _compraUdReal(p);
      const lotStock = activeLotes.reduce((ls, l) => ls + (l.stockRestante || 0), 0);
      const mainStock = Math.max(0, (p.stock || 0) - lotStock);
      let costoP = mainStock * costoUd;
      activeLotes.forEach(l => { costoP += (l.stockRestante || 0) * (l.compra || costoUd); });
      return s + costoP;
    }, 0);
