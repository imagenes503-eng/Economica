...
function renderCajaPanel() {
  ...
  // ── LÓGICA CAPITAL: ventas de HOY entran a caja directamente desde ventasDia ──
  // (se actualiza en tiempo real; reiniciarDia() la pone en 0 automáticamente)
  const ventaDiariaHoy     = (ventasDiarias || []).find(v => v.fecha === hoyISO);
  const hoyYaRegistrado    = !!ventaDiariaHoy;
  // Ventas del mes sin hoy (para estadísticas de flujo)
  ...
  // entroACaja sigue usándose para el panel de flujo del mes (sin cambios)
  ...
  // Caja actual = efectivo inicial + ventas de HOY (en tiempo real desde ventasDia) - egresos
  // Si se hizo reiniciarDia(), ventasHoyTotal = 0 → la caja refleja solo el efectivo inicial − gastos
  const cajaActual         = efectivoInicial + ventasHoyTotal - totalEgresos;

  const salioDeCaja        = totalEgresos;

  const comprasMes         = pagos.filter(g => esMesActual(g.fechaISO) && g.cat === 'COMPRA')
                               .reduce((s,g) => s + Number(g.monto||0), 0);
  // 📊 CAPITAL TOTAL = Caja actual (ef. inicial + ventasDia − egresos) + Inventario a precio venta
  // reiniciarDia() → ventasDia = 0 → cajaActual se recalcula solo
  // reiniciarSemana() → ventasSem = 0 → afecta reportes semanales, no cajaActual directamente
