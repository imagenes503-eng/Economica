...
function getDB() {
  // Si la conexión existente está cerrada (app volvió del fondo), resetear
  if (_db) {
    try {
      // Test rápido: si la conexión está muerta, lanza excepción
      _db.transaction(KV_STORE, 'readonly').abort();
      return Promise.resolve(_db);
    } catch(e) {
      console.warn('[IDB] Conexión muerta detectada — reabriendo...');
      _db = null;
    }
  }
  if (_dbPromise) return _dbPromise;
  _dbPromise = _abrirIDB(0)
    .then(db => { _dbPromise = null; return db; })
    .catch(e  => { _dbPromise = null; throw e; });
  return _dbPromise;
}

function _setupStores(db) {
...
function _abrirIDB(intento) {
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => { try { _setupStores(e.target.result); } catch(_) {} };
    ...
    req.onerror = (e) => {
      if (intento < 4) {
        setTimeout(() => _abrirIDB(intento + 1).then(resolve).catch(reject), 600 * (intento + 1));
      } else {
        // Último recurso: borrar BD corrupta y recrear
    ...
    req.onblocked = () => {
      console.warn('[IDB] Bloqueado — intento', intento + 1);
      if (intento < 6) {
...
let _ultimaVisita = Date.now();
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
    _ultimaVisita = Date.now();
    // Cerrar IDB limpiamente para que el navegador pueda liberar recursos
    if (_db) { try { _db.close(); } catch(_) {} _db = null; }
    return;
  }
  // Al volver: solo recargar si pasaron más de 3 minutos fuera
  const ausencia = Date.now() - _ultimaVisita;
  if (ausencia < 180000) {
    // Menos de 3 min: solo redibujar UI con datos en memoria
    actualizarTodo();
    return;
  }
  if (carrito.length) return; // no interrumpir una venta en curso
  // Más de 3 min: reconectar IDB y bajar datos frescos
...
(async function init() {
  try {
    ...
    for (let t = 0; t < 5; t++) {
      try { _db = null; await getDB(); break; }
      catch(e) {
        if (t === 4) throw e;
        setLoadingMsg('Iniciando almacenamiento… (' + (t + 2) + '/5)');
