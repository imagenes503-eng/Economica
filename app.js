...
function navTo(pgId, pushHistory = true) {
  ...
  const ids    = ['pgDash', 'pgInventario', 'pgReportes', 'pgAdmin', 'pgDash'];
  ...
  if (pgId === 'pgDash' && typeof renderFinanzasMes === 'function') renderFinanzasMes(pgId);
  if (pgId === 'pgDash' && typeof renderCierreDia === 'function') renderCierreDia(pgId);
...
window.addEventListener('popstate', (e) => {
  ...
  const validas = ['pgDash','pgInventario','pgReportes','pgAdmin'];
...
function actualizarSubtituloBackup() {
  const sub   =   const alert = document.getElementById('backupAlert');
  if (!sub) return;
...
async function softReload() {
  ...
  const validas = ['pgDash','pgInventario','pgReportes'];
...
function actualizarTodo() {
  ...
  if (pgId === 'pgDash')         {   if (pgId === 'pgInventario')   { renderInv(); actualizarCats(); }
...
function renderPagina(pgId) {
  if (pgId === 'pgDash')          { if (pgId === 'pgInventario')    { renderInv(); actualizarCats(); poblarInvAnCat(); if (!document.getElementById('invAnDesde')?.value && !document.getElementById('invAnHasta')?.value) { invAnSetRango(30); } else { renderInvAnalisis(); } }
  if (pgId === 'pgReportes')      { renderVentas(); renderHistorial(); renderCritico(); renderPagos(); renderBalance(); }
      if (pgId === 'pgAdmin' && typeof renderAdminPanel === 'function') renderAdminPanel();
  if (pgId === 'pgDash' && typeof renderFinanzasMes === 'function') renderFinanzasMes(pgId);
  if (pgId === 'pgDash' && typeof renderCierreDia === 'function') renderCierreDia(pgId);
