...
async function _cargarCacheIDB() {
  ...
  // ── FIX BUG 1: filtrar pagos eliminados al cargar desde IDB ──
  // Sin esto, los pagos borrados reaparecen cada vez que se recarga la app
  if (pagosEliminados.length) {
    const _setElimP = new Set(pagosEliminados.map(String));
    pagos = pagos.filter(g => !_setElimP.has(String(g.id)));
  }
...
function borrarGasto(id) {
  if (confirm('¿Eliminar este registro?')) {
    ...
    pagos = pagos.filter(g => g.id !== id); salvar(); toast('Registro eliminado', true);
...
function reiniciarDia() {
  ...
  // ── FIX BUG 2: guardar ventasDia vacío directo a IDB sin disparar syncAhora
  // salvar() normal llama syncAhora('todo') que jala Supabase y recalcula
  // ventasDia desde el historial, ignorando el reset. En cambio guardamos
  // solo lo necesario localmente y dejamos que el sync ya no toque ventasDia.
  idbSetMany([
    ['vpos_ventasDia',         ventasDia],
    ['vpos_ventasSem',         ventasSem],
    ['vpos_ventasMes',         ventasMes],
    ...
    ['vpos_pagos',             pagos],
    ['vpos_ventasDiarias',     ventasDiarias],
  ]).catch(err => console.warn('[reiniciarDia IDB]', err));
...
(async function init() {
  try {
    ...
    // ── FIX: si tras restaurar sesión no hay datos ni sesión activa,
    //         abrir login automáticamente para que el usuario recupere sus datos ──
    setTimeout(() => {
      const sinSesion   = typeof _sesionActiva === 'undefined' || !_sesionActiva;
      const sinProductos = !productos || productos.length === 0;
      const hayCredenciales = localStorage.getItem('vpos_sesionActiva') === '1';
      if (sinSesion && sinProductos && !hayCredenciales) {
        // Abrir login automáticamente — datos probablemente en Supabase
        if (typeof _crearModalLogin === 'function') {
          _crearModalLogin();
    ...
    // FIX: siempre ocultar overlay aunque haya error
    try { ocultarOverlay(); } catch(e) {}
    try { setIDBStatus(false); } catch(e) {}
    try {
      ...
      if (overlay) {
        ...
        if (badge) { badge.textContent = '⚠ Error — Reiniciando…'; badge.style.background='rgba(220,38,38,0.12)'; badge.style.color='#dc2626'; }
        const btn = document.createElement('button');
        btn.textContent = '🔄 Reintentar';
        btn.style.cssText = 'margin-top:8px;padding:12px 24px;background:var(--green);color:#fff;border:none;border-radius:10px;font-size:15px;font-weight:900;font-family:Nunito,sans-serif;cursor:pointer;';
        btn.onclick = () => location.reload();
        overlay.appendChild(btn);
        setTimeout(() => { try { ocultarOverlay(); } catch(e) {} }, 4000);
      }
    } catch(e2) {}
  }
