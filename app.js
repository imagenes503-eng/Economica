...
function _recalcularReportesDesdeHistorial() {
  ...
  (historial || []).forEach(v => {
    ...
    (v.items || []).forEach(it => {
      ...
      // totalItem = monto cobrado real (correcto para paquetes).
      // cant*precio solo como fallback para ventas antiguas sin totalItem.
      const tot  = (it.totalItem !== undefined)
        ? Number(it.totalItem)
...
function finalizarVenta() {
  ...
  const venta = {
    ...
    items: carrito.map(i => { const p = productos.find(x => x.id === i.id); return { id: String(i.id), nom: i.nom, cant: i.cant * (i.stockPorCant || 1), precio: i.venta, totalItem: i.venta * i.cant, cat: (p && p.cat) ? p.cat : '', paqueteLabel: i.paqueteLabel || null }; }),
