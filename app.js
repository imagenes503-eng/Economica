...
function renderCajaPanel() {
  ...
  // Capital total
  // FIX: usar invCompra (precio costo) para comparar igual con inventarioInicial (también al costo)
  // invVenta inflaba artificialmente la ganancia por el margen del inventario restante
  ...
  m.forEach(p => {
    ...
    d.innerHTML = `
      ...
      <div class="sug-item-body">
        ...
        ${sinStock ? `<div style="font-size:10px;font-weight:800;color:#dc2626;">Sin stock</div>` : `<div style="font-size:10px;font-weight:700;${stockColor}">${stockTxt}</div>`}
      </div>
      ${!sinStock ? `<button class="sug-add-btn">🛒 Agregar</button>` : '<div style="font-size:10px;font-weight:800;color:#dc2626;padding:6px 0;">Sin stock</div>'}
    `;
    if (!sinStock) {
...
function reiniciarDia() {
  ...
  // FIX: eliminar las ventas de hoy del historial para que el dashboard
  // muestre 0 transacciones y no sume nada en ventas del día/semana
  const hoyStr = new Date().toDateString();
  historial = (historial || []).filter(v =>
    new Date(v.fechaISO || v.ts || 0).toDateString() !== hoyStr
  );

  // Recalcular ventasDia Y ventasSem desde historial actualizado
