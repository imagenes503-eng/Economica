...
// Sync silente: igual que softReload pero sin bloqueo de carrito y sin toast visible
// Se llama automáticamente al abrir la página y tras cada acción de cierre diario
let _autoSyncTimer = null;
async function _autoSyncSilente(delaySeg = 0) {
  ...
  _autoSyncTimer = setTimeout(async () => {
    ...
    try {
      ...
      if (_pg) renderPagina(_pg.id);
      actualizarStats();
      if (typeof _sesionActiva !== 'undefined' && _sesionActiva && typeof _autoCargarDesdeSupa === 'function') {
        await _autoCargarDesdeSupa();
        const _pg2 = document.querySelector('.page.active');
        if (_pg2) renderPagina(_pg2.id);
        actualizarStats();
        // Si estamos en cierre diario, restaurar datos del cierre
        if (typeof renderCierreDia === 'function') {
          const cdPage = document.getElementById('pgCierreDia');
          if (cdPage && cdPage.classList.contains('active')) {
            if (typeof _cdRestaurarEstadoAutomatico === 'function') _cdRestaurarEstadoAutomatico();
          }
        }
      }
    } catch(e) {}
...
window._autoSyncSilente = _autoSyncSilente;

document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'visible') {
    if (carrito.length) return;
    // Al volver a la pestaña: sync silente inmediato
    _autoSyncSilente(0);
  }
});
...
(async function init() {
  try {
    ...
    window.addEventListener('pageshow', (e) => {
      ...
      else _autoSyncSilente(3); // 3s después del load para que sesión y canal estén listos
    });

    ocultarOverlay();
    actualizarBadgeSheets(); // Google Sheets badge
    await iniciarAutoSync();  // Restaurar sesion y rol antes de continuar
    // Sync automático 3s después de que todo inicie
    setTimeout(() => { if (typeof _autoSyncSilente === 'function') _autoSyncSilente(0); }, 3000);
  ...
  if (tbody1) {
    ...
    if (!listaConVentas.length) {
      ...
      listaConVentas.forEach(p => {
        ...
        snapFilas.push({ id: String(p.id), nom: p.nom||'—', cat, stock: p.stock||0, vendido, costoTotal, ventaTotal, costoUd, ventaUd });
      });
...
function rp_descargarImagenesPedido() {
  ...
  bloques.forEach((bloque, idx) => {
    _rpGenerarImagen(bloque, idx + 1, totalImgs);
  });
}
...
function _rpGenerarImagen(filas, numImg, totalImgs) {
  ...
  link.download = `Pedido_Imagen${numImg}_de${totalImgs}.png`;
  link.click();
