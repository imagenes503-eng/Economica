...
function actualizarInventarioInicialAuto() {
  ...
  // Segunda protección: verificar IDB antes de recalcular desde stock actual.
  // Si hay un valor guardado en IDB, cargarlo en lugar de recalcular.
  // Esto evita que ventas pasadas contaminen el baseline cuando inventarioInicial = 0
  // al momento de llamar esta función (ej: justo al arrancar antes de que IDB cargue).
  idbGet('vpos_inventarioInicial').then(idbInv => {
    const idbVal = parseFloat(idbInv) || 0;
    if (idbVal > 0) {
      // Ya había un valor guardado — restaurarlo sin recalcular
      inventarioInicial = idbVal;
      idbGet('vpos_inventarioCosto').then(idbCost => {
        inventarioCosto = parseFloat(idbCost) || 0;
        if (inp)  inp.value  = inventarioInicial.toFixed(2);
        if (inpC) inpC.value = inventarioCosto.toFixed(2);
        if (typeof renderInvTotales === 'function') renderInvTotales();
    ...
    // No hay valor en IDB → primera vez del mes → calcular desde stock actual
    _calcularInventarioInicialDesdeStock();
  }).catch(() => _calcularInventarioInicialDesdeStock());
}

function _calcularInventarioInicialDesdeStock() {
  const inp  = document.getElementById('inpInventarioInicial');
  const inpC = document.getElementById('inpInventarioCosto');
...
function renderInv() {
  ...
  const lista  = productos.filter(p => !filtro || (p.nom||'').toUpperCase().includes(filtro) || (p.cod||'').toUpperCase().includes(filtro) || (p.cat||'').toUpperCase().includes(filtro));
  ...
  const container = document.getElementById('tbodyInv');
  if (!container) return;

  if (!lista.length) {
    container.style.display = 'block';
    container.innerHTML = `<div style="text-align:center;padding:48px 20px;color:#9ca3af;">
      <div style="font-size:48px;margin-bottom:12px;">📦</div>
      <div style="font-size:15px;font-weight:700;">${filtro ? 'Sin resultados para "'+filtro.toLowerCase()+'"' : 'Sin productos registrados'}</div>
      <div style="font-size:12px;margin-top:6px;">Agrega tu primer producto desde el menú de inventario</div>
    </div>`;
  ...
  const cards = lista.map(p => {
    ...
    const lotesAct  = (p.lotes||[]).filter(l => (l.stockRestante||0) > 0);

    const imgHTML = p.img
      ? `<img src="${p.img}" loading="lazy" style="width:100%;height:100%;object-fit:cover;border-radius:10px;">`
      : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:28px;background:linear-gradient(135deg,#f0fdf4,#dcfce7);border-radius:10px;">${(p.cat||'?').charAt(0).toUpperCase()}</div>`;

    const stockColor  = critico ? '#dc2626' : '#15803d';
    const stockBg     = critico ? '#fef2f2' : '#f0fdf4';
    const stockBorder = critico ? '#fca5a5' : '#86efac';

    return `
    <div style="background:#fff;border:1.5px solid #e5e7eb;border-radius:14px;overflow:hidden;display:flex;flex-direction:column;transition:box-shadow .2s,transform .15s;"
      onmouseover="this.style.boxShadow='0 4px 20px rgba(0,0,0,0.1)';this.style.transform='translateY(-2px)'"
      onmouseout="this.style.boxShadow='';this.style.transform=''">

      <!-- Imagen -->
      <div style="height:100px;padding:10px 10px 0;flex-shrink:0;">
        ${imgHTML}
      </div>
      ...
      <div style="padding:10px 12px;flex:1;display:flex;flex-direction:column;gap:4px;">
        <!-- Nombre -->
        <div style="font-size:13px;font-weight:900;color:#111827;line-height:1.3;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;">${p.nom}</div>

        <!-- Categoría + Código -->
        <div style="display:flex;align-items:center;gap:5px;flex-wrap:wrap;">
          ${p.cat ? `<span style="font-size:10px;font-weight:700;color:#6b7280;background:#f3f4f6;border-radius:5px;padding:1px 6px;">${p.cat}</span>` : ''}
          ${p.cod ? `<span style="font-size:9px;font-weight:700;color:#9ca3af;font-family:monospace;">${p.cod}</span>` : ''}
        </div>

        <!-- Precios -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-top:4px;">
          <div style="background:#f9fafb;border-radius:7px;padding:5px 7px;">
            <div style="font-size:9px;font-weight:700;color:#9ca3af;text-transform:uppercase;">Costo</div>
            <div style="font-size:13px;font-weight:900;color:#374151;">$${fmtP(p.compra||0)}</div>
          </div>
          <div style="background:#f0fdf4;border-radius:7px;padding:5px 7px;">
            <div style="font-size:9px;font-weight:700;color:#4ade80;text-transform:uppercase;">Venta</div>
            <div style="font-size:13px;font-weight:900;color:#15803d;">$${fmtP(p.venta||0)}</div>
          </div>
        </div>

        <!-- Margen + Stock -->
        <div style="display:flex;align-items:center;justify-content:space-between;margin-top:4px;">
          <div style="font-size:10px;font-weight:800;color:${margen!==null&&margen>0?'#15803d':margen!==null&&margen<0?'#dc2626':'#9ca3af'};">
            ${margen !== null ? (margen > 0 ? '↑' : margen < 0 ? '↓' : '') + margen + '% margen' : '—'}
          </div>
          <div style="background:${stockBg};border:1.5px solid ${stockBorder};border-radius:8px;padding:3px 9px;font-size:12px;font-weight:900;color:${stockColor};">
            ${critico ? '⚠️ ' : ''}${p.stock||0}
          </div>
        </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;margin-top:2px;">
          <div style="font-size:10px;font-weight:700;color:#9ca3af;">Total</div>
          <div style="font-size:12px;font-weight:900;color:#15803d;font-family:monospace;">$${totalVal}</div>
        </div>

        ${numPkgs > 0 || lotesAct.length > 0 ? `
        <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:2px;">
          ${numPkgs > 0 ? `<span style="font-size:10px;font-weight:700;background:#faf5ff;color:#7c3aed;border:1px solid #ddd6fe;border-radius:6px;padding:1px 6px;">📦 ${numPkgs} paq.</span>` : ''}
          ${lotesAct.length > 0 ? `<span style="font-size:10px;font-weight:700;background:#fffbeb;color:#92400e;border:1px solid #fde68a;border-radius:6px;padding:1px 6px;">🏷 ${lotesAct.length} lote${lotesAct.length>1?'s':''}</span>` : ''}
        </div>` : ''}
      </div>

      <!-- Acciones -->
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;border-top:1px solid #f3f4f6;">
        <button onclick="abrirRestock('${p.id}')" title="Agregar stock"
          style="padding:9px 0;background:none;border:none;border-right:1px solid #f3f4f6;font-size:15px;cursor:pointer;color:#15803d;font-weight:900;"
          onmouseover="this.style.background='#f0fdf4'" onmouseout="this.style.background='none'">+</button>
        <button onclick="abrirGestionPaquetes('${p.id}')" title="Paquetes"
          style="padding:9px 0;background:none;border:none;border-right:1px solid #f3f4f6;font-size:14px;cursor:pointer;"
          onmouseover="this.style.background='#faf5ff'" onmouseout="this.style.background='none'">📦</button>
        <button onclick="editarProd('${p.id}')" title="Editar"
          style="padding:9px 0;background:none;border:none;border-right:1px solid #f3f4f6;font-size:14px;cursor:pointer;color:#0369a1;"
          onmouseover="this.style.background='#eff6ff'" onmouseout="this.style.background='none'">✎</button>
        <button onclick="borrarProd('${p.id}')" title="Eliminar"
          style="padding:9px 0;background:none;border:none;font-size:14px;cursor:pointer;color:#dc2626;"
  ...
  if (tfoot) {
    ...
    tfoot.innerHTML = `
      <div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:1.5px solid #86efac;border-radius:12px;padding:12px 16px;display:flex;align-items:center;justify-content:space-between;">
        <div style="font-size:12px;font-weight:900;color:#15803d;">
          💰 TOTAL${esFiltered ? ' (filtrado)' : ' INVENTARIO'}
          ${esFiltered ? `<span style="font-size:11px;color:#4ade80;margin-left:8px;">· General: $${totalVentaTodos.toFixed(2)}</span>` : ''}
        </div>
        <div style="font-size:18px;font-weight:900;color:#15803d;font-family:'Space Mono',monospace;">$${totalVentaLista.toFixed(2)}</div>
      </div>`;
  }
}
...
function guardarEdicionCobro() {
  ...
  if (typeof _broadcast === 'function') {
    _broadcast('historial_actualizado', {
      ...
      // BUGFIX SYNC: enviar lista de cobros eliminados para que el otro teléfono
      // los guarde y _fusionarDos no los restaure desde snapshots antiguos
...
function confirmarFusion() {
  ...
  // ── 5. VENTAS DIARIAS: unir, si misma fecha tomar el MAYOR monto ─────────
  // autoRegistrarVentaDiaria guarda el ACUMULADO total del día (no incrementos),
  // por eso sumar causaría duplicación. El monto correcto es siempre el mayor.
  (ext.ventasDiarias || []).forEach(vExt => {
    const idx = ventasDiarias.findIndex(vL => vL.fecha === vExt.fecha);
    if (idx >= 0) {
      const montoLocal = Number(ventasDiarias[idx].monto || 0);
      const montoExt   = Number(vExt.monto || 0);
      if (montoExt > montoLocal) {
        // El externo tiene más ventas registradas — usar ese
        ventasDiarias[idx].monto = montoExt;
        ventasDiarias[idx].nota  = vExt.nota || ventasDiarias[idx].nota;
      }
      // Si son iguales o el local es mayor, conservar el local
    } else {
