...
function _regPresCargar(paquetes, stockUd, compraUd, ventaUd) {
  ...
  // Cargar solo paquetes con cant > 1 (los de cant=1 eran bug anterior; fila Unidad se agrega siempre abajo)
  if (paquetes && paquetes.length) {
    paquetes
      .filter(pk => (pk.cant || 1) > 1)
      .slice().sort((a,b) => b.cant - a.cant)
      .forEach(pk => {
        _regPres.push({
  ...
  // Siempre agregar fila Unidad con los precios unitarios del producto
  _regPres.push({
    id:    'unidad_' + Date.now(),
    label: 'Unidad',
    uds:   1,
