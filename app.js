  ...
  if (tbody1) {
    ...
    if (!listaConVentas.length) {
      ...
      listaConVentas.forEach(p => {
        ...
        snapFilas.push({ nom: p.nom||'—', cat, stock: p.stock||0, vendido, costoTotal, ventaTotal });
      });
    ...
    // Mostrar inventario inicial en UI
    const elInvC = document.getElementById('rp_s1_invInicialCosto');
    const elInvV = document.getElementById('rp_s1_invInicialVenta');
    if (elInvC) elInvC.textContent = '$' + invInicialCosto.toFixed(2);
    if (elInvV) elInvV.textContent = '$' + invInicialVenta.toFixed(2);
  }
...
function rp_descargarPDFS1() {
  ...
  doc.text('REPORTE DEL PERÍODO — INVENTARIO & VENTAS', 105, 13, { align: 'center' });
  doc.setFontSize(8); doc.setFont('helvetica', 'normal');
  doc.text(`${snap.rangoStr}  ·  ${snap.catLabel}`, 105, 21, { align: 'center' });

  // Bloque inventario inicial
  doc.setFillColor(240, 253, 244);
  doc.rect(10, 33, 190, 14, 'F');
  doc.setDrawColor(134, 239, 172);
  doc.rect(10, 33, 190, 14, 'S');
  doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(6, 78, 59);
  doc.text('INVENTARIO INICIAL (al presionar Recalcular):', 14, 40);
  doc.setTextColor(124, 58, 237);
  doc.text(`Costo: $${snap.invInicialCosto.toFixed(2)}`, 120, 40);
  doc.setTextColor(22, 101, 52);
  doc.text(`Venta: $${snap.invInicialVenta.toFixed(2)}`, 165, 40);
  ...
  const rows = snap.filas.map(f => [
    ...
    '$' + f.ventaTotal.toFixed(2)
  ]);

  if (doc.autoTable) {
    doc.autoTable({
      startY: 51,
      head: [['Producto', 'Categoría', 'Stock actual', 'Cant. vendida', 'Total costo', 'Total venta']],
      body: rows,
      ...
      columnStyles: {
        ...
        3: { halign: 'center', textColor: [29, 78, 216], cellWidth: 22 },
        4: { halign: 'right', textColor: [124, 58, 237], cellWidth: 28 },
        5: { halign: 'right', textColor: [22, 101, 52], cellWidth: 28 }
      },
      margin: { left: 10, right: 10 }
    });

    const finalY = doc.lastAutoTable ? doc.lastAutoTable.finalY + 5 : 270;
    // Fila de totales
    doc.setFillColor(6, 78, 59);
    ...
    doc.text('Costo: $' + snap.totalCosto.toFixed(2), 112, finalY + 8);
    doc.setTextColor(134, 239, 172);
    doc.text('Venta: $' + snap.totalVenta.toFixed(2), 158, finalY + 8);
  }

  const fname = `ReporteS1_${snap.rangoStr.replace(/ /g,'').replace(/\//g,'-')}.pdf`;
  doc.save(fname);
  toast('✓ PDF Sección 1 descargado');
}
