...
function calcValorInventario() {
  ...
  // FIX FLOAT: redondear la aportación de cada producto a 4 decimales antes de
  // acumular, para que la suma total sea idéntica en todos los teléfonos.
  productos.forEach(p => {
    compra += Math.round(_costoTotalProd(p) * 10000) / 10000;
    venta  += Math.round(_ventaTotalProd(p) * 10000) / 10000;
  });
  return { compra, venta };
}
...
function renderInv() {
  ...
  if (tfoot) {
    const r4tf = v => Math.round(v * 10000) / 10000;
    const totalVentaLista = lista.reduce((s,p) => s + r4tf(_ventaTotalProd(p)), 0);
    tfoot.innerHTML = '';
  }
  ...
  if (totEl) {
    // FIX FLOAT: redondear la aportación de CADA producto a 4 decimales antes
    // de acumular, igual que calcValorInventario(). Sin este paso, los residuos
    // de punto flotante se acumulan y el total difiere en $0.01 entre teléfonos.
    const r4 = v => Math.round(v * 10000) / 10000;
    // FIX DISPLAY: usar Math.round con EPSILON para el redondeo final a 2 decimales.
    // toFixed(2) es INCONSISTENTE entre motores JS (Chrome/Safari/Android) en valores
    // borderline (ej. 105.245 → "105.24" en un motor, "105.25" en otro).
    // Math.round((v + ε) * 100) / 100 siempre redondea igual en todos los dispositivos.
    const r2 = v => Math.round((v + Number.EPSILON) * 100) / 100;
    const tv = r2(lista.reduce((s,p) => s + r4(_ventaTotalProd(p)), 0));
    const tc = r2(lista.reduce((s,p) => s + r4(_costoTotalProd(p)), 0));
    const ganancia = r2(tv - tc);
    totEl.innerHTML = `
