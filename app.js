...
function guardarEdicionCobro() {
  ...
  if (diaYaRegistrado) {
    ...
    if (!activos.length) {
      // BUGFIX SYNC: registrar ID como eliminado antes de hacer splice
      const _cobroIdDia = String(v.id || '');
      if (_cobroIdDia && !cobrosEliminados.includes(_cobroIdDia)) cobrosEliminados.push(_cobroIdDia);
    ...
    // Restar el monto devuelto del día en ventasDiarias (aunque sea día cerrado)
    const _fechaDev2   = v.fechaISO ? v.fechaISO.split('T')[0] : null;
    const _montoDev2   = devolver.reduce((s, it) => s + Number(it.cant||0) * Number(it.precio||0), 0);
    if (_fechaDev2 && _montoDev2 > 0) {
      const _vdIdx2 = (ventasDiarias || []).findIndex(vd => vd.fecha === _fechaDev2);
      if (_vdIdx2 >= 0) {
        ventasDiarias[_vdIdx2] = { ...ventasDiarias[_vdIdx2],
          monto: Math.max(0, Number(ventasDiarias[_vdIdx2].monto || 0) - _montoDev2), _auto: true };
        idbSetMany([['vpos_ventasDiarias', ventasDiarias]]).catch(console.error);
        if (typeof syncAhora === 'function') syncAhora('venta_diaria');
      }
    }
    salvar(); cerrarModal('modalEditarCobro'); toast('✓ Cobro corregido — ventas del día ajustadas', false, true);
    if (typeof syncAhora === 'function') syncAhora('productos');
    if (typeof _broadcast === 'function') _broadcast('historial_actualizado', {
