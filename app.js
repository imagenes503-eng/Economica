...
function salvar(doRender = true) {
  ...
  _salvarTimer = setTimeout(() => {
    idbSetMany([
      ...
      // También sincronizar con Supabase para que los datos no se pierdan al recargar
...
function guardarProducto(e) {
  ...
  if (typeof _subirStockBase === 'function') _subirStockBase().catch(()=>{});
}
...
function finalizarVenta() {
  ...
  // Timestamp con milisegundos para que dos ventas simultáneas en distintos
  // dispositivos nunca compartan el mismo fecha_iso aunque ocurran casi al mismo tiempo.
  const ahora   = new Date();
  const fechaISO = ahora.toISOString(); // ej. "2026-04-07T18:23:45.123Z" — incluye ms
  const ts       = ahora.getTime();     // ms desde epoch

  const venta = {
    id: uid(), ts, fechaISO,
    fechaStr: ahora.toLocaleString('es-SV'),
  ...
  // Subir venta a Supabase inmediatamente (upsert directo, incluye ms en fecha_iso)
  if (typeof sheetsEnviar === 'function') {
    sheetsEnviar('VENTA', {
  ...
  // Subir stock actualizado de todos los productos (descontado por esta venta)
  if (typeof syncAhora === 'function') syncAhora('productos');
