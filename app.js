...
function reiniciarHistorial() {
  ...
  // 3️⃣ FIX BUG 2: esperar 5s para que los DELETE en Supabase terminen ANTES de hacer
  //    snapshot_push. Con 1.5s había race condition: el otro teléfono llamaba
  //    _autoCargarDesdeSupa() y Supabase aún tenía los datos viejos → historial volvía.
  setTimeout(async () => {
    if (typeof _autoEnviarSnapshot === 'function') await _autoEnviarSnapshot();
    if (typeof _broadcast === 'function') _broadcast('snapshot_push', { tienda: typeof _getTiendaId === 'function' ? _getTiendaId() : '' });
  }, 5000);
}
...
function guardarVentaDiaria() {
  ...
  // FIX BUG 1: broadcast instantáneo → el otro teléfono ve la venta del día al instante
  if (typeof _broadcast === 'function') _broadcast('ventas_dia_guardada', { fecha, monto, nota });
...
function eliminarVentaDiaria(fecha) {
  ...
  // FIX BUG 5: subir snapshot actualizado para teléfonos desconectados (sin esto el día vuelve al reconectar)
  setTimeout(() => { if (typeof _autoEnviarSnapshot === 'function') _autoEnviarSnapshot(); }, 1200);
}
