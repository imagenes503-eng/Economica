...
// Calcula el valor de venta real del inventario de un producto considerando
// los precios de CADA presentación (paquete × su precio de venta + unidades sueltas).
// NUNCA usar p.venta * p.stock porque p.venta es el precio de unidad suelta,
// y los paquetes se venden a precio de paquete, no al precio de unidad × uds.
function _ventaTotalProd(p) {
  if (p.ventaTotal > 0) return p.ventaTotal; // valor exacto guardado (nuevo flujo)

  // Reconstruir desde paquetes guardados
  const paquetes = (p.paquetes || []).filter(pk => Number(pk.cant) > 1 && Number(pk.precio) > 0);
  if (!paquetes.length) return (p.stock || 0) * (p.venta || 0); // producto sin paquetes
  ...
  total += stockRest * (p.venta || 0); // unidades sueltas al precio unitario
  return total;
}

// ─── FUENTE ÚNICA DE VERDAD: COSTO TOTAL DEL PRODUCTO ────────────────────────
// Usa p.costoTotal (suma exacta guardada) cuando está disponible.
// Fallback: reconstruye desde lotes y precio por unidad.
function _costoTotalProd(p) {
  // Sin lotes activos: usar costoTotal exacto guardado
  const activeLotes = (p.lotes || []).filter(l => (l.stockRestante || 0) > 0);
  if (activeLotes.length === 0) {
    return p.costoTotal > 0 ? p.costoTotal : (p.stock || 0) * _compraUdReal(p);
  }
  // Con lotes: calcular por lote
