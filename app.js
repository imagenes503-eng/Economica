...
function _recalcularReportesDesdeHistorial() {
  ...
  // FIX: excluir cobros de fechas eliminadas (ventasDiariasEliminadas).
  // eliminarVentaDiaria borra la fila de ventas_diarias en Supabase pero NO el cobro
  // de la tabla ventas. Sin este filtro, _recalcularReportesDesdeHistorial incluye ese
  // cobro en ventasMes, y Caja/Capital muestran $1-$2 de más en ambos teléfonos.
  const _elim = new Set(
    (typeof ventasDiariasEliminadas !== 'undefined' ? ventasDiariasEliminadas : [])
  );
  ventasDia = {}; ventasSem = {}; ventasMes = {};
  (historial || []).forEach(v => {
    if (!v.fechaISO && !v.fecha) return;
    const fecha    = new Date(v.fechaISO || v.fecha);
    const fechaStr = (v.fechaISO || v.fecha || '').slice(0, 10);
    // Ignorar cobros cuya fecha fue eliminada de ventasDiarias
    if (_elim.has(fechaStr)) return;
...
function renderCajaPanelMini() {
  ...
  // FIX: usar ventasMes (historial filtrado por ventasDiariasEliminadas) para Caja.
  // ventasDiarias puede tener filas obsoletas de Supabase; ventasMes es la fuente confiable.
  const mes = typeof totalReporte === 'function' ? totalReporte(ventasMes || {}) : 0;
...
function renderCajaPanel() {
  ...
  // FIX: ventasMes (historial filtrado) como fuente para Caja, no ventasDiarias.
  const totalVentasDiariasDelMes = typeof totalReporte === 'function' ? totalReporte(ventasMes || {}) : 0;
  const entroACaja         = totalVentasDiariasDelMes;
  const diasConVentas      = (ventasDiarias || []).filter(v => esMesActual(v.fecha + 'T00:00:00') && Number(v.monto||0) > 0).length;
  const promedioDiario     = diasConVentas > 0
    ? totalVentasDiariasDelMes / diasConVentas
  ...
  const cogsDelMes = (historial || [])
    ...
    .reduce((total, venta) => {
      return total + (venta.items || []).reduce((s, it) => {
        if (it.costoItem !== undefined) return s + Math.round(Number(it.costoItem || 0) * 100) / 100;
...
function actualizarInventarioInicialAuto() {
  ...
  const _r4inv = v => Math.round(v * 100) / 100;
  const totalInv  = (productos || []).reduce((s, p) => s + _r4inv(_ventaTotalProd(p)), 0);
  const totalCost = (productos || []).reduce((s, p) => s + _r4inv(_costoTotalProd(p)), 0);
...
function _compraUdReal(p) {
  ...
  if (Number(p.compra) > 0) return Number(p.compra);
...
function _ventaTotalProd(p) {
  // FIX FLOAT: redondear cada multiplicación a 4 decimales para evitar residuos
  // de punto flotante que hagan diferir el total de inventario entre teléfonos.
  const r4 = v => Math.round(v * 10000) / 10000;
  const paquetes = (p.paquetes || []).filter(pk => Number(pk.cant) > 1 && Number(pk.precio) > 0);
  if (!paquetes.length) return r4((p.stock || 0) * (p.venta || 0));
  ...
  pkgsOrdenados.forEach(pk => {
    ...
    total    += r4(cantPres * Number(pk.precio));
    stockRest = stockRest - cantPres * udsXPres;
  });
  total += r4(stockRest * (p.venta || 0));
  return r4(total);
}
...
function _costoTotalProd(p) {
  ...
  function _costoDeStock(stock) {
    // FIX FLOAT: helper para redondear cada multiplicación a 4 decimales y evitar
    // que errores de punto flotante (ej. 5 × 0.1425 = 0.7124999… en lugar de 0.7125)
    // se acumulen y hagan que el total de inventario difiera en $0.01 entre teléfonos.
    const r4 = v => Math.round(v * 10000) / 10000;
    const paquetes = (p.paquetes || []).filter(pk =>
      Number(pk.cant) > 1 && (Number(pk.costoPresEntrado) > 0 || Number(pk.precioCompra) > 0)
    );
    if (!paquetes.length) return r4(stock * costoUd);
    const pkgsOrdenados = paquetes.slice().sort((a, b) => b.cant - a.cant);
    ...
    total += r4(stockRest * costoSuelto);
    // Redondear el total del producto a 4 decimales antes de devolverlo, para que
    // al sumar todos los productos en calcValorInventario no se acumulen residuos.
    return r4(total);
  }
  ...
  activeLotes.forEach(l => { costo += (l.stockRestante || 0) * (l.compra || costoUd); });
  return costo;
}

function calcValorInventario() {
  let compra = 0, venta = 0;
  // FIX FLOAT: redondear la aportación de CADA PRODUCTO a 2 decimales (centavos)
  // antes de acumular. Esto garantiza que el total sea idéntico en todos los
  // teléfonos y en todas las vistas (Inventario, Estado de Caja, Capital).
  // Antes se usaba 4 decimales (10000) pero eso no era suficiente para eliminar
  // el residuo flotante de valores como 5×0.1425=0.7124999… que causaba que
  // calcValorInventario devolviera 105.25 mientras renderInv (con r2) mostraba 105.24.
  const r2 = v => Math.round(v * 100) / 100;
  productos.forEach(p => {
    compra += r2(_costoTotalProd(p));
    venta  += r2(_ventaTotalProd(p));
  });
  return { compra, venta };
}
...
function guardarProducto(e) {
  ...
  const costoUnidadEntrado = filaUnidad ? Number(filaUnidad.costo) : 0;
  const ventaUnidadEntrada = filaUnidad ? Number(filaUnidad.venta) : 0;
  ...
  const newPaquetes = _regPres
    ...
    .map(r => ({
      ...
      costoPresEntrado: Number(r.costo), // valor exacto ingresado por el usuario (sin recalcular)
      label:           r.label || ''
    }));
...
function renderInv() {
  ...
  if (tfoot) {
    const _r4 = v => Math.round(v * 100) / 100;
    const totalVentaLista = lista.reduce((s,p)=>s+_r4(_ventaTotalProd(p)),0);
    tfoot.innerHTML = '';
  }
  ...
  if (totEl) {
    const _r4b = v => Math.round(v * 100) / 100;
    const tv = lista.reduce((s,p)=>s+_r4b(_ventaTotalProd(p)),0);
    ...
    const tc = lista.reduce((s,p) => s + _r4b(_costoTotalProd(p)), 0);
...
function finalizarVenta() {
  ...
  const venta = {
    ...
    items: carrito.map(i => { const p = productos.find(x => x.id === i.id); const esPromo = !!(i.stockPorCant && i.stockPorCant > 1); return { id: String(i.id), nom: i.nom, cant: i.cant * (i.stockPorCant || 1), cantCobrada: i.cant, precio: i.venta, compra: p ? (Number(p.compra) || 0) : 0, totalItem: i.venta * i.cant, costoItem: Math.round((p ? (Number(p.compra) || 0) : 0) * i.cant * (i.stockPorCant || 1) * 10000) / 10000, esPromo, cat: (p && p.cat) ? p.cat : '', paqueteLabel: i.paqueteLabel || null }; }),
...
function generarPDFInventarioActual() {
  ...
  const _r4pdf = v => Math.round(v * 100) / 100;
  const totalVenta = lista.reduce((s,p)=>s+_r4pdf(_ventaTotalProd(p)),0);
  ...
  for (const cat of Object.keys(cats).sort()) {
    ...
    y = _pdfAutoTable(doc,
      ...
      rows, y, {
        columnStyles: {
          0: { fontStyle: 'bold' },
          1: { halign: 'center', fontStyle: 'bold', textColor:[22,101,52] },
          2: { fontSize: 7, textColor:[80,80,80] },
...
function autoRegistrarVentaDiaria() {
  ...
  // FIX RACE CONDITION: deduplicar ventasDiarias por fecha antes de buscar.
  const _vdMapA = {};
  (ventasDiarias || []).forEach(v => {
    if (!_vdMapA[v.fecha] || Number(v.monto) > Number(_vdMapA[v.fecha].monto)) _vdMapA[v.fecha] = v;
  });
  ventasDiarias = Object.values(_vdMapA).sort((a, b) => a.fecha.localeCompare(b.fecha));
...
// Versión silenciosa: solo actualiza ventasDiarias localmente sin broadcast ni sync.
// Usada por _autoCargarDesdeSupa para evitar el loop de sincronización:
// autoRegistrarVentaDiaria → syncAhora/broadcast → _autoCargarDesdeSupa → autoRegistrar → …
function _autoRegistrarVentaDiariaSilente() {
  const hoy   = new Date().toISOString().split('T')[0];
  const total = typeof totalReporte === 'function' ? totalReporte(ventasDia || {}) : 0;
  if (total <= 0) return;
  // FIX RACE CONDITION: deduplicar por fecha antes de buscar el índice.
  // Si una condición de carrera dejó dos entradas para la misma fecha,
  // consolidarlas aquí evita que Caja cuente el doble.
  const _vdMap = {};
  (ventasDiarias || []).forEach(v => {
    if (!_vdMap[v.fecha] || Number(v.monto) > Number(_vdMap[v.fecha].monto)) {
      _vdMap[v.fecha] = v;
    }
  });
  ventasDiarias = Object.values(_vdMap).sort((a, b) => a.fecha.localeCompare(b.fecha));
  const idx = ventasDiarias.findIndex(v => v.fecha === hoy);
  if (idx >= 0) {
    if (total <= Number(ventasDiarias[idx].monto || 0)) {
      idbSetMany([['vpos_ventasDiarias', ventasDiarias]]).catch(console.error);
      return;
    }
    ventasDiarias[idx] = { ...ventasDiarias[idx], monto: total, _auto: true };
  ...
  // Solo persiste en IDB — sin salvar(), syncAhora() ni _broadcast() para evitar loops
  idbSetMany([['vpos_ventasDiarias', ventasDiarias]]).catch(console.error);
}
...
let _actualizarTodoTimer = null;
function actualizarTodo() {
  // FIX PARPADEO: debounce de 80ms para que llamadas rápidas consecutivas
  // (desde sync, broadcasts, etc.) solo ejecuten un render al final.
  if (_actualizarTodoTimer) clearTimeout(_actualizarTodoTimer);
  _actualizarTodoTimer = setTimeout(() => {
    _actualizarTodoTimer = null;
