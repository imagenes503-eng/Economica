...
function ejecutarReiniciarMes() {
  if (!confirm('Se descargará el Estado de Resultados (PDF + imagen) y luego se reiniciará el mes. ¿Continuar?')) return;

  // Generar el PDF profesional ANTES de borrar nada, con los datos tal como están
  try { generarEstadoResultadosPDF(); } catch (e) { console.error('Error generando Estado de Resultados PDF:', e); }

  // Generar también la imagen 4K (9:16) con el mismo resumen, en vez del backup JSON
