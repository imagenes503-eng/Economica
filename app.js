...
function actualizarInventarioInicialAuto() {
  ...
  const totalInv  = (productos || []).reduce((s, p) => s + (Number(p.stock)||0)*(Number(p.venta)||0), 0);
  const totalCost = (productos || []).reduce((s, p) => s + (Number(p.stock)||0)*(Number(p.compra)||0), 0);
...
function renderInvTotales() {
  ...
  if (peBox && peGrid) {
    if (hayPE) {
      ...
      // Capital actual: efectivo inicial + inv.costo + ventas confirmadas + recompras - gastos
      const comprasMesInv  = pagos.filter(g => esMesActual(g.fechaISO) && g.cat === 'COMPRA')
                               .reduce((s,g) => s + Number(g.monto||0), 0);
      const capitalActual = efectivoInicial + invCostoBase + ventasConfirmadas + comprasMesInv - totalGastosMes;
      ...
      peGrid.innerHTML = `
        ...
        <div style="background:rgba(29,78,216,0.04);border:2px solid rgba(29,78,216,0.3);border-radius:10px;padding:12px;">
          ...
          <div style="font-size:10px;color:#60a5fa;margin-top:3px;font-weight:700;">Ef.ini + Inv.costo + Ventas − Gastos</div>
...
function _regPresRender() {
  ...
  if (!cont) return;
  if (!_regPres.length) {
    cont.innerHTML = `<div style="text-align:center;padding:14px;font-size:13px;color:var(--text-muted);font-weight:700;background:#fafafa;border-radius:9px;border:1.5px dashed var(--border-mid);">
      Sin presentaciones — agrega una abajo 👇
    </div>`;
    document.getElementById('regResumen').style.display = 'none';
    // Limpiar hidden inputs
    ['inpCompra','inpVenta','inpStock'].forEach(id => { const el = document.getElementById(id); if(el) el.value=''; });
    return;
  }
  cont.innerHTML = _regPres.map(r => `
    <div class="reg-pres-row" data-id="${r.id}">
      <input class="reg-pres-inp" style="font-family:'Nunito',sans-serif;font-size:12px;text-align:left;padding:6px 8px;" 
        type="text" value="${r.label}" placeholder="Paquete / Unidad…"
        onchange="regPresEditar('${r.id}','label',this.value)" oninput="regPresEditar('${r.id}','label',this.value)">
      <input class="reg-pres-inp" type="number" min="1" step="1" value="${r.uds||''}" placeholder="50"
        oninput="regPresEditar('${r.id}','uds',this.value)" title="Unidades por presentación">
      <input class="reg-pres-inp" type="number" min="0" step="0.5" value="${r.cant||''}" placeholder="0"
        oninput="regPresEditar('${r.id}','cant',this.value)" title="Cantidad comprada"
        style="border-color:var(--green);background:var(--green-light);">
      <input class="reg-pres-inp" type="number" min="0" step="any" value="${r.costo||''}" placeholder="0.00"
        oninput="regPresEditar('${r.id}','costo',this.value)" title="Precio costo por presentación"
        style="border-color:#fcd34d;background:#fffbeb;">
      <input class="reg-pres-inp" type="number" min="0" step="any" value="${r.venta||''}" placeholder="0.00"
        oninput="regPresEditar('${r.id}','venta',this.value)" title="Precio venta por presentación"
        style="border-color:#86efac;background:#f0fdf4;">
      <button type="button" class="reg-pres-del" onclick="regPresEliminar('${r.id}')">✕</button>
    </div>`).join('');
  _regPresActualizarResumen();
}
...
function _regPresActualizarResumen() {
  ...
  if (totalUds <= 0 && totalCosto <= 0) { box.style.display = 'none'; return; }
  box.style.display = '';
  ...
  // Actualizar campos ocultos de compatibilidad
  const costoPorUd = totalUds > 0 ? totalCosto / totalUds : 0;
  // precio de venta = el de la fila con uds=1 (unidades sueltas) o la más pequeña
  const filaMin  = _regPres.filter(r=>Number(r.uds)>0&&Number(r.venta)>0).sort((a,b)=>a.uds-b.uds)[0];
  ...
  if(elC) elC.value = costoPorUd.toFixed(4);
...
function _regPresCargar(paquetes, stockUd, compraUd, ventaUd) {
  _regPres = [];
  if (paquetes && paquetes.length) {
    paquetes.slice().sort((a,b) => b.cant - a.cant).forEach(pk => {
      _regPres.push({
        id:     pk.id || Date.now() + Math.random(),
        label:  pk.label || (pk.cant === 1 ? 'Unidad' : 'Paquete ' + pk.cant + ' uds'),
        uds:    pk.cant   || 1,
        cant:   0,           // cantidad comprada no se carga (es histórica)
        costo:  pk.precioCompra ? pk.precioCompra * pk.cant : (compraUd * pk.cant),
        venta:  pk.precio || 0
      });
    });
  }
  // Si no tiene presentaciones, crear una fila de unidad por defecto
  if (!_regPres.length) {
    _regPres.push({ id: Date.now(), label: 'Unidad', uds: 1, cant: stockUd || 0, costo: compraUd || 0, venta: ventaUd || 0 });
  }
  _regPresRender();
}
...
function guardarProducto(e) {
  ...
  // Validar que hay presentaciones registradas
  if (_regPres.length === 0) { toast('Agrega al menos una presentación al producto', true); return; }
  ...
  const newPaquetes = _regPres
    ...
    .map(r => ({
      ...
      precioCompra: Number(r.uds) > 0 ? (Number(r.costo) / Number(r.uds)) : 0,
      label:        r.label || ''
    }));
  ...
  if (id) {
    ...
    productos = productos.map(x => String(x.id) === String(id)
      ? { id: Number(id), cod: newCod, nom: newNom, cat: newCat, abrev: newAbrev, compra: newCompra, venta: newVenta, stock: newStock, min: newMin, lotes: lotesActuales, paquetes: newPaquetes, img: newImg, _ts: Date.now() }
      : x);
  } else {
    productos.push({ id: Date.now(), cod: newCod, nom: newNom, cat: newCat, abrev: newAbrev, compra: newCompra, venta: newVenta, stock: newStock, min: newMin, lotes: [], paquetes: newPaquetes, img: newImg, _ts: Date.now() });
  }
...
function editarProd(id) {
  ...
  _regPresCargar(p.paquetes, p.stock, p.compra, p.venta);
...
function abrirRestock(id) {
  ...
  setTimeout(() => document.getElementById('restockCantidad').focus(), 80);
}
...
function _restockIniciarLineas(p) {
  ...
  pkgs.forEach(pk => {
    _restockLineas.push({ id: uid(), tipo: 'pkg_'+pk.id, uds: pk.cant, cant: 1, precio: pk.precio, label: pk.cant+' uds' });
  });
  _restockLineas.push({ id: uid(), tipo: 'unidad', uds: 1, cant: 0, precio: p.compra||0, label: 'Unidades sueltas' });
}
...
function _restockRenderLineas(p) {
  ...
  cont.innerHTML = _restockLineas.map((ln, i) => {
    const opts = [
      ...pkgs.map(pk => `<option value="pkg_${pk.id}" ${ln.tipo==='pkg_'+pk.id?'selected':''}>📦 Paquete de ${pk.cant} uds — $${fmtP(pk.precio)}/paq</option>`),
      `<option value="unidad" ${ln.tipo==='unidad'?'selected':''}>🔹 Unidades sueltas</option>`,
      `<option value="custom" ${ln.tipo==='custom'?'selected':''}>✏️ Personalizado</option>`
    ].join('');
    const isCustom = ln.tipo === 'custom';
    const isPkg    = ln.tipo.startsWith('pkg_');
    const udsLinea = isPkg
      ? ((pkgs.find(pk=>'pkg_'+pk.id===ln.tipo)||{}).cant||1) * (Number(ln.cant)||0)
      : (Number(ln.uds)||1) * (Number(ln.cant)||0);
    const subtotal = (Number(ln.cant)||0) * (Number(ln.precio)||0);
    return `
    <div style="background:#fff;border:1.5px solid var(--border-mid);border-radius:11px;padding:11px 13px;position:relative;">
      <button onclick="restockEliminarLinea('${ln.id}')" style="position:absolute;top:8px;right:8px;width:22px;height:22px;border-radius:50%;border:none;background:rgba(220,38,38,0.1);color:#dc2626;font-size:13px;cursor:pointer;line-height:1;font-weight:900;">✕</button>
      <div style="margin-bottom:8px;">
        <label style="font-size:10px;font-weight:900;color:var(--text-muted);text-transform:uppercase;letter-spacing:.4px;">Tipo de presentación</label>
        <select onchange="restockCambiarTipo('${ln.id}', this.value)" style="width:100%;margin-top:4px;padding:7px 10px;border:1.5px solid var(--border-mid);border-radius:8px;font-family:'Nunito',sans-serif;font-size:13px;font-weight:800;background:#fff;color:var(--text);">${opts}</select>
      </div>
      <div style="display:grid;grid-template-columns:${isCustom?'1fr 1fr 1fr':'1fr 1fr'};gap:8px;align-items:end;">
        ${isCustom ? `<div>
          <label style="font-size:10px;font-weight:900;color:var(--text-muted);text-transform:uppercase;letter-spacing:.4px;">Uds por presentación</label>
          <input type="number" min="1" value="${ln.uds||''}" placeholder="50" oninput="restockEditarLinea('${ln.id}','uds',this.value)" style="width:100%;margin-top:4px;padding:8px 10px;border:1.5px solid var(--border-mid);border-radius:8px;font-family:'Space Mono',monospace;font-size:14px;font-weight:700;text-align:center;">
        </div>` : ''}
        <div>
          <label style="font-size:10px;font-weight:900;color:var(--text-muted);text-transform:uppercase;letter-spacing:.4px;">Cantidad comprada</label>
          <input type="number" min="0" step="0.5" value="${ln.cant||''}" placeholder="0" oninput="restockEditarLinea('${ln.id}','cant',this.value)" style="width:100%;margin-top:4px;padding:8px 10px;border:1.5px solid var(--green);border-radius:8px;font-family:'Space Mono',monospace;font-size:16px;font-weight:900;text-align:center;background:var(--green-light);">
        </div>
        <div>
          <label style="font-size:10px;font-weight:900;color:var(--text-muted);text-transform:uppercase;letter-spacing:.4px;">Precio pagado $</label>
          <input type="number" step="any" min="0" value="${ln.precio||''}" placeholder="0.00" oninput="restockEditarLinea('${ln.id}','precio',this.value)" style="width:100%;margin-top:4px;padding:8px 10px;border:1.5px solid var(--border-mid);border-radius:8px;font-family:'Space Mono',monospace;font-size:14px;font-weight:700;text-align:center;">
        </div>
      </div>
      ${(Number(ln.cant)>0 && Number(ln.precio)>0) ? `
      <div style="margin-top:8px;display:flex;justify-content:space-between;align-items:center;background:var(--green-light);border-radius:7px;padding:5px 10px;">
        <span style="font-size:11px;font-weight:800;color:var(--text-muted);">📦 ${udsLinea} uds</span>
        <span style="font-family:'Space Mono',monospace;font-size:13px;font-weight:900;color:var(--green-dark);">$${subtotal.toFixed(2)}</span>
      </div>` : ''}
    </div>`;
  }).join('');
...
function restockAgregarLinea() {
  const pid = document.getElementById('restockProdId').value;
  const p   = productos.find(x=>String(x.id)===String(pid));
  _restockLineas.push({ id: uid(), tipo: 'unidad', uds: 1, cant: 0, precio: (p&&p.compra)||0 });
  _restockRenderLineas(p);
}
...
function restockCambiarTipo(id, valor) {
  ...
  const p   = productos.find(x=>String(x.id)===String(pid));
  ln.tipo = valor;
  if (valor.startsWith('pkg_')) {
    const pk = (p&&p.paquetes||[]).find(pk=>String(pk.id)===valor.replace('pkg_',''));
    if (pk) { ln.uds = pk.cant; ln.precio = pk.precio; }
  } else if (valor === 'unidad') {
    ln.uds = 1; ln.precio = (p&&p.compra)||0;
  } else { ln.uds = ''; ln.precio = ''; }
  _restockRenderLineas(p);
}

function restockEditarLinea(id, campo, valor) {
  const ln = _restockLineas.find(l=>l.id===id); if (!ln) return;
  ln[campo] = (parseFloat(valor)||0);
  const pid = document.getElementById('restockProdId').value;
  _restockRenderLineas(productos.find(x=>String(x.id)===String(pid)));
}
...
function _restockActualizarResumenPres(p) {
  ...
  _restockLineas.forEach(ln => {
    const cant = Number(ln.cant)||0; const precio = Number(ln.precio)||0;
    if (cant <= 0) return;
    let uds = ln.tipo.startsWith('pkg_')
      ? ((pkgs.find(pk=>'pkg_'+pk.id===ln.tipo)||{}).cant||1)*cant
      : (Number(ln.uds)||1)*cant;
    totalUds += uds; totalInv += precio*cant;
    lineasValidas.push({ ln, uds, sub: precio*cant });
  });
  ...
  if(boxL)boxL.innerHTML = lineasValidas.map(({ln,uds,sub}) => {
    const lbl = ln.tipo.startsWith('pkg_') ? `${ln.cant} paquete(s) → ${uds} uds`
      : ln.tipo==='unidad' ? `${uds} unidades sueltas` : `${ln.cant}×${ln.uds} uds`;
...
function confirmarRestock() {
  ...
  if (_restockModo === 'simple') {
    ...
    const pkgs = (p.paquetes||[]);
    let totalInv = 0;
    _restockLineas.forEach(ln => {
      const cant = Number(ln.cant)||0; const prec = Number(ln.precio)||0;
      if (cant <= 0) return;
      let uds = ln.tipo.startsWith('pkg_')
        ? ((pkgs.find(pk=>'pkg_'+pk.id===ln.tipo)||{}).cant||1)*cant
        : (Number(ln.uds)||1)*cant;
      cantTotal += uds; totalInv += prec*cant;
    });
...
function cancelarEdicion() {
  ...
  // Resetear presentaciones del formulario
  _regPres = [];
  _regPresRender();
}
...
function finalizarVenta() {
  ...
  // Auto-actualizar ventasDiarias con el total acumulado del POS hoy
  setTimeout(() => autoRegistrarVentaDiaria(), 400);
}
...
function guardarEdicionCobro() {
  ...
  const fechaDev     = v.fechaISO ? v.fechaISO.split('T')[0] : null;
  const montoDevuelto = devolver.reduce((s, it) => s + Number(it.cant||0) * Number(it.precio||0), 0);
  ...
  if (typeof _broadcast === 'function') {
    // NOTA: productos_devolucion ya viene incluido en historial_actualizado.
    // El receptor aplica el stock restaurado desde ahí. No emitir _broadcast('producto')
    // adicionalmente porque causaría que el otro dispositivo aplique el stock DOS veces.
...
function autoRegistrarVentaDiaria() {
  ...
  if (total <= 0) return; // nada vendido hoy, no registrar

  const idx = (ventasDiarias || []).findIndex(v => v.fecha === hoy);
  if (idx >= 0) {
    // Solo actualizar si el total POS es mayor (nunca bajar una venta confirmada)
    if (total <= Number(ventasDiarias[idx].monto || 0)) return;
    ventasDiarias[idx] = { ...ventasDiarias[idx], monto: total, _auto: true };
  } else {
    ventasDiarias.push({ fecha: hoy, monto: total, nota: 'Auto-POS', _auto: true });
  }
  ventasDiarias.sort((a, b) => a.fecha.localeCompare(b.fecha));
  salvar(false);
  idbSetMany([['vpos_ventasDiarias', ventasDiarias]]).catch(console.error);
...
function renderPagina(pgId) {
  ...
  if (pgId === 'pgInvRegistrar') {
    ...
    // Si no hay presentaciones (formulario en blanco), agregar fila unidad por defecto
