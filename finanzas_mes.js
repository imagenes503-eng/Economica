...
(function _estilosAgendaProv() {
  ...
  s.textContent = `
    ...
    .ap-pedidos-title {
      font-size:15px; font-weight:900; color:#fff; font-family:Nunito,sans-serif;
    }
    .ap-pedidos-sub {
      font-size:11px; font-weight:700;
      color:rgba(255,255,255,.55); font-family:Nunito,sans-serif; margin-top:2px;
    }
    .ap-pedidos-counter { display:flex; flex-direction:column; align-items:center; gap:1px; }
    .ap-pedidos-counter-num {
      font-size:22px; font-weight:900; color:#fff; font-family:Nunito,sans-serif; line-height:1;
    }
    .ap-pedidos-counter-lbl {
      font-size:9px; font-weight:900; color:rgba(255,255,255,.5);
      text-transform:uppercase; letter-spacing:.8px; font-family:Nunito,sans-serif;
    }
    .ap-progress-wrap { padding:13px 17px 0; }
    .ap-progress-meta {
      display:flex; justify-content:space-between;
      font-size:11px; font-weight:900; font-family:Nunito,sans-serif;
      color:var(--text-muted,#64748b); margin-bottom:6px;
    }
    .ap-progress-meta span:last-child { color:#16a34a; }
    .ap-progress-track {
      width:100%; height:8px; background:#e2e8f0; border-radius:20px; overflow:hidden;
    }
    .ap-progress-fill {
    ...
    .ap-pedidos-list { padding:12px 17px 17px; display:flex; flex-direction:column; gap:9px; }
    .ap-pedido-item {
      display:flex; align-items:center; gap:12px;
      padding:13px 14px; border-radius:14px; border:1.5px solid; transition:all .2s;
    }
    .ap-pedido-item.pendiente {
      background:#fff; border-color:#e2e8f0; box-shadow:0 1px 4px rgba(0,0,0,.05);
    }
    .ap-pedido-item.pendiente:hover { border-color:#bbf7d0; box-shadow:0 2px 10px rgba(22,163,74,.1); }
    .ap-pedido-item.recibido { background:#f0fdf4; border-color:#bbf7d0; }
    .ap-pedido-avatar {
    ...
    .ap-pedido-avatar.pendiente { background:linear-gradient(135deg,#3b82f6,#1d4ed8); }
    .ap-pedido-avatar.recibido  { background:linear-gradient(135deg,#16a34a,#059669); }
    .ap-pedido-info { flex:1; min-width:0; }
    .ap-pedido-nombre {
      font-size:14px; font-weight:900; font-family:Nunito,sans-serif; transition:color .2s;
    }
    .ap-pedido-nombre.pendiente { color:var(--text,#0f172a); }
    .ap-pedido-nombre.recibido  { color:#15803d; }
    .ap-pedido-meta {
      font-size:11px; font-weight:700; font-family:Nunito,sans-serif; margin-top:2px;
    }
    .ap-pedido-meta.pendiente { color:var(--text-muted,#94a3b8); }
    .ap-pedido-meta.recibido  { color:#16a34a; }
    .ap-pedido-status {
      padding:4px 11px; border-radius:20px;
      font-size:11px; font-weight:900; font-family:Nunito,sans-serif; white-space:nowrap;
    }
    .ap-pedido-status.pendiente { background:#fef9c3; color:#854d0e; border:1px solid #fde68a; }
    .ap-pedido-status.recibido  { background:#dcfce7; color:#15803d; border:1px solid #bbf7d0; }
    .ap-btn-recibir {
      padding:8px 14px; border-radius:11px;
      font-size:12px; font-weight:900; font-family:Nunito,sans-serif; cursor:pointer;
      border:1.5px solid #bfdbfe; background:#eff6ff; color:#1d4ed8;
      transition:all .15s; white-space:nowrap; flex-shrink:0;
    }
    .ap-btn-recibir:hover {
      background:linear-gradient(135deg,#16a34a,#15803d);
      border-color:#15803d; color:#fff;
      box-shadow:0 3px 10px rgba(22,163,74,.3); transform:translateY(-1px);
    }
    .ap-btn-recibir:active { transform:translateY(0); }
    .ap-pedidos-empty { padding:28px 20px; text-align:center; font-family:Nunito,sans-serif; }
    .ap-pedidos-empty-ico { font-size:36px; margin-bottom:8px; }
    .ap-pedidos-empty-txt {
      font-size:13px; font-weight:900; color:var(--text-muted,#64748b);
    }
    .ap-pedidos-empty-sub {
      font-size:11px; font-weight:700; color:var(--text-muted,#94a3b8); margin-top:4px;
    }
  `;
...
function _apRender() {
  _apRenderForm(); _apRenderTabla(); _apRenderHoy(); _apRenderStats(); _apRenderPedidosHoy();
}

// ══════════════════════════════════════════════════════════════════════
//  PEDIDOS DE HOY
//  Muestra los proveedores con entrega programada para el día actual.
//  Estado "recibido" se guarda en localStorage por fecha → se resetea
//  automáticamente al siguiente día.
// ══════════════════════════════════════════════════════════════════════

const _apRecibidosKey = () => 'vpos_recibidos_' + new Date().toISOString().split('T')[0];

function _apCargarRecibidos() {
  try { return new Set(JSON.parse(localStorage.getItem(_apRecibidosKey())||'[]')); }
  catch { return new Set(); }
}
function _apGuardarRecibidos(set) {
  localStorage.setItem(_apRecibidosKey(), JSON.stringify([...set]));
}

function _apMarcarRecibido(id) {
  const set  = _apCargarRecibidos();
  const prov = _apLista.find(p => p.id === id);
  if (set.has(id)) {
    set.delete(id);
    if (typeof toast === 'function') toast(`↩ ${prov?.nombre} marcado como pendiente`);
  } else {
    set.add(id);
    if (typeof toast === 'function') toast(`✅ ${prov?.nombre} recibido`);
    // micro-animación
    const item = document.getElementById('apPedItem_' + id);
    if (item) { item.style.transform='scale(1.02)'; setTimeout(()=>{item.style.transform='';},200); }
  }
  _apGuardarRecibidos(set);
  _apRenderPedidosHoy();
}

function _apRenderPedidosHoy() {
  const wrap = document.getElementById('apPedidosWrap');
  if (!wrap) return;
  const diaHoy    = _apHoy();
  const recibidos = _apCargarRecibidos();
  const pedidos   = _apLista.filter(p => p.entrega?.includes(diaHoy));
  const totalRec  = pedidos.filter(p => recibidos.has(p.id)).length;

  // Actualizar counter en header
  const cnt = document.getElementById('apPedidosCnt');
  if (cnt) {
    cnt.innerHTML = `
      <div class="ap-pedidos-counter-num">${pedidos.length}</div>
      <div class="ap-pedidos-counter-lbl">${totalRec}/${pedidos.length}&nbsp;✓</div>`;
  }

  if (!pedidos.length) {
    wrap.innerHTML = `
      <div class="ap-pedidos-empty">
        <div class="ap-pedidos-empty-ico">🎉</div>
        <div class="ap-pedidos-empty-txt">Sin entregas programadas para hoy</div>
        <div class="ap-pedidos-empty-sub">${diaHoy} no tiene proveedores asignados</div>
      </div>`;
    return;
  }

  const pct = Math.round((totalRec / pedidos.length) * 100);
  // pendientes primero, recibidos al final
  const ordenados = [
    ...pedidos.filter(p => !recibidos.has(p.id)),
    ...pedidos.filter(p =>  recibidos.has(p.id)),
  ];
  ...
  wrap.innerHTML = `
    <div class="ap-progress-wrap">
      <div class="ap-progress-meta">
        <span>Pendientes: ${pedidos.length - totalRec}</span>
        <span>Recibidos: ${totalRec} / ${pedidos.length}</span>
      </div>
      <div class="ap-progress-track">
        <div class="ap-progress-fill" style="width:${pct}%"></div>
      </div>
    </div>
    <div class="ap-pedidos-list">
      ${ordenados.map(p => {
        const rec = recibidos.has(p.id);
        const cls = rec ? 'recibido' : 'pendiente';
        return `
          <div class="ap-pedido-item ${cls}" id="apPedItem_${p.id}" style="transition:all .2s,transform .2s;">
            <div class="ap-pedido-avatar ${cls}">${_apInit(p.nombre)}</div>
            <div class="ap-pedido-info">
              <div class="ap-pedido-nombre ${cls}">${p.nombre}</div>
              <div class="ap-pedido-meta ${cls}">
                ${rec ? '✓ Recibido hoy · ' + diaHoy : 'Entrega esperada · ' + diaHoy}
              </div>
            </div>
            <span class="ap-pedido-status ${cls}">${rec ? '✅ Recibido' : '⏳ Pendiente'}</span>
            <button class="ap-btn-recibir" onclick="_apMarcarRecibido('${p.id}')">
              ${rec ? '↩ Deshacer' : '✓ Recibido'}
            </button>
          </div>`;
      }).join('')}
    </div>`;
...
function renderFinanzasMes(pgId) {
  ...
  pg.innerHTML = `
    ...
    <div style="padding:0 14px; display:flex; flex-direction:column; gap:20px;">
      ...
      <!-- ══ PEDIDOS DE HOY ══════════════════════════════════════════ -->
      <div class="ap-pedidos-panel">
        <div class="ap-pedidos-hdr">
          <div class="ap-pedidos-ico">🚚</div>
          <div class="ap-pedidos-txt">
            <div class="ap-pedidos-title">Entregas de Hoy</div>
            <div class="ap-pedidos-sub" id="apPedidosDia">${_apHoy()} — productos que llegan hoy</div>
          </div>
          <div class="ap-pedidos-counter" id="apPedidosCnt">
            <div class="ap-pedidos-counter-num">—</div>
            <div class="ap-pedidos-counter-lbl">0/0 ✓</div>
          </div>
        </div>
        <div id="apPedidosWrap"></div>
      </div>
