...
let _reconcTimer = null; // timer de reconciliación directa contra tabla ventas
...
function _iniciarPolling() {
  ...
  // BUGFIX SYNC: reconciliación dura cada 90s — lee la tabla ventas directamente.
  // Es el último recurso: aunque snapshot y broadcast fallen, la venta aparece en <90s.
  if (!_reconcTimer) {
    _reconcTimer = setInterval(() => {
      if (_sesionActiva && _sbUrl() && _sbKey()) {
        _autoCargarDesdeSupa().catch(e => console.warn('[Reconc]', e.message));
      }
    }, 90000);
  }
  console.log('[Sync] Realtime iniciado, polling 30s + reconciliación 90s');
}
