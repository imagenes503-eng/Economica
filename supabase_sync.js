...
async function intentarLogin() {
  ...
  try {
    if (esRegistrar) {
      ...
      // Crear perfil (tienda_nombre se guarda en localStorage, no en perfiles)
      ...
      // Guardar nombre de tienda localmente
      localStorage.setItem('vpos_tiendaNombre_' + tiendaId, nombreTienda);
...
function _actualizarNombreTienda() {
  ...
  const nombre = esSA
    ? 'Despensa Económica'
    : (localStorage.getItem('vpos_tiendaNombre_' + _getTiendaId()) || _usuarioActual?.tienda_nombre || _getTiendaId() || 'Mi Tienda');
...
async function activarMembresiaManual(email, tipoMembresia) {
  ...
  try {
    ...
    if (pendientes && pendientes.length > 0) {
      ...
      // Buscar perfil por email (puede no existir si el registro falló antes)
      const users = await _sbGet('perfiles', { select: 'id,nombre', email: 'eq.' + email }).catch(() => []);
      const userId = users && users.length > 0 ? users[0].id : ('manual_' + Date.now());
      const nombreUsuario = users && users.length > 0 ? (users[0].nombre || '') : '';
      // Crear membresía aunque no haya perfil — el email es suficiente para verificarla al login
      await _sbPost('membresias', {
        id: 'mem_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6),
        user_id: userId,
        email: email,
