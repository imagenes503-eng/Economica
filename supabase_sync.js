...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    if (ventas && ventas.length > 0) {
      ...
      // ── FILTRAR ventas que el usuario eliminó localmente ──────────────
      // ventasEliminadas se guarda en IDB y sobrevive reinicios
      const _veSet = new Set(
        (typeof ventasEliminadas !== 'undefined' ? ventasEliminadas : []).map(String)
      );
      if (_veSet.size > 0) {
        // Re-intentar DELETE en Supabase para las que pudieron quedar por offline
        for (const vid of _veSet) {
          _sbDeleteFiltro('ventas', { id: 'eq.' + vid }).catch(() => {});
        }
        historial = historial.filter(v => !_veSet.has(String(v.id)));
        console.log('[AutoCarga] Ventas eliminadas filtradas:', _veSet.size);
      }

      idbSet('vpos_historial', historial).catch(() => {});
      console.log('[AutoCarga] Ventas:', ventas.length, '→ tras filtro:', historial.length);

      // ── RECALCULAR ventasDia / ventasSem / ventasMes desde historial ──
      // Si el usuario reinició el día manualmente, respetar ese reset:
      // no incluir ventas de hoy en ventasDia (sí en sem y mes).
      const hoy    = new Date().toDateString();
      const lunes  = _lunesDeLaSemana();
      const ahora  = new Date();
      const _diaResetManual = localStorage.getItem('vpos_diaResetManual');
      const _ignorarHoyEnDia = (_diaResetManual === hoy); // true = respeta el reset
      ...
      historial.forEach(v => {
        ...
        (v.items || []).forEach(it => {
          ...
          // ventasDia: omitir ventas de HOY si el usuario reinició manualmente
      ...
      if (_ignorarHoyEnDia) console.log('[AutoCarga] ⚡ Día reiniciado manualmente — ventasDia en 0');
      ...
      // Si el usuario reinició el día manualmente HOY, forzar ventasDia={} en IDB
      const _hoyAutoC = new Date().toDateString();
      const _flagResetHoy = localStorage.getItem('vpos_diaResetManual') === _hoyAutoC;
      if (_flagResetHoy) ventasDia = {};
    ...
    if (pagosData && pagosData.length > 0) {
      ...
      // Normalizar IDs a String (local usa Number=Date.now(), Supabase usa String)
      const eliminadosPagos = new Set(
        (typeof pagosEliminados !== 'undefined' ? pagosEliminados : []).map(String)
      );
      // Re-intentar DELETE en Supabase para los eliminados offline
      if (eliminadosPagos.size > 0) {
        for (const pid of eliminadosPagos) {
          _sbDeleteFiltro('pagos', { id: 'eq.' + pid }).catch(() => {});
        }
      }
      pagos = pagosData
        .filter(p => !eliminadosPagos.has(String(p.id).trim()))
        .map(p => ({
          id: String(p.id),   // SIEMPRE string — evita mismatch number vs string en comparaciones
          ...
          cat: p.cat || '', nom: p.nom || p.concepto || '', nota: p.nota || ''
        }));
...
async function syncBorrarPago(id){
  if(!_sbUrl()||!_sbKey())return;
  const _pid=String(id);
  for(let t=1;t<=3;t++){
    try{
      await _sbDeleteFiltro('pagos',{id:'eq.'+_pid});
      await _sbPost('deleted_log',{id:'del_pago_'+_pid+'_'+Date.now(),tabla:'pagos',registro_id:_pid,deleted_at:new Date().toISOString()},true);
      return;
    }catch(e){
      console.warn('[syncBorrarPago] intento',t,e.message);
      if(t<3) await new Promise(r=>setTimeout(r,800*t));
    }
  }
}

async function syncBorrarVenta(id){
  if(!_sbUrl()||!_sbKey())return;
  const idStr = String(id);
  // Reintentar hasta 3 veces con backoff corto para garantizar el DELETE
  for(let intento = 1; intento <= 3; intento++){
    try{
      await _sbDeleteFiltro('ventas',{id:'eq.'+idStr});
      await _sbPost('deleted_log',{
        id:'del_venta_'+idStr+'_'+Date.now(),
        tabla:'ventas', registro_id:idStr,
        deleted_at:new Date().toISOString()
      },true);
      return; // éxito
    }catch(e){
      console.warn('[syncBorrarVenta] intento',intento,'fallido:',e.message);
