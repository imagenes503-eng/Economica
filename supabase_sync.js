    ...
    if (prods && prods.length > 0) {
      ...
      const merged = sbProds.map(sp => {
        ...
        if (local) {
          ...
          if (localInvalido) {
            // Datos locales inválidos (sin nombre/precio) → Supabase es fuente de verdad.
            // stock viene directo de Supabase; NO usar Math.max() que restauraría el stock
            // pre-venta del caché local en el segundo dispositivo.
            return {
              ...sp,
          ...
          if (sbTs > localTs) {
            // IMPORTANTE: stock viene directo de Supabase — NO usar Math.max(sp.stock, local.stock)
            // porque eso restauraría el stock sin descontar del caché local del dispositivo offline,
            // ignorando las ventas que hizo el otro dispositivo mientras éste tenía el navegador cerrado.
            return {
              ...sp,
              stock:    Number(sp.stock) || 0,
              img:      local.img || sp.img || null,
