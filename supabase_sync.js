  ...
  try {
    ...
    if (prods && prods.length > 0) {
      const sbProds = prods.map(p => ({
        ...
        // Preservar precios de unidad suelta — NUNCA sobrescribir con compra/uds
        costoUnidadEntrado: Number(p.costoUnidadEntrado) || 0,
        ventaUnidadEntrada: Number(p.ventaUnidadEntrada) || 0,
...
async function _subirStockBase(){
  ...
  const rowsCompleto = productos.map(p=>({
    ...
    // Precios de unidad suelta ingresados manualmente — NUNCA recalcular desde compra/uds
    costoUnidadEntrado: Number(Number(p.costoUnidadEntrado||0).toFixed(2)),
    ventaUnidadEntrada: Number(Number(p.ventaUnidadEntrada||0).toFixed(2)),
