...
function _aplicarVentaRealtime(v) {
  ...
  // ── FIX: Actualizar ventasDiarias al recibir venta del otro teléfono ──
  // Sin esto el capitalTotal no cambia cuando el otro dispositivo cobra
  (function() {
    const fechaVentaISO = fechaISO.split('T')[0];
    const montoVenta = Number(nv.total) || 0;
    if (!ventasDiarias) ventasDiarias = [];
    const idxVD = ventasDiarias.findIndex(v => v.fecha === fechaVentaISO);
    if (idxVD >= 0) {
      ventasDiarias[idxVD].monto = (Number(ventasDiarias[idxVD].monto) || 0) + montoVenta;
    } else {
      ventasDiarias.push({ fecha: fechaVentaISO, monto: montoVenta, nota: 'Auto-Sync' });
