...
async function sbRequest(tabla, method="GET", data=null){
  ...
  if(!res.ok){
    console.error("SB ERROR", await res.text());
    throw new Error("SB ERROR");
  }
  return res.json();
}

async function guardarProductoSupabase(p){
  return sbRequest("productos","POST",{
    id:String(p.id),
    nom:p.nom||"",
    cat:p.cat||"",
    compra:p.compra||0,
...
async function eliminarProductoSupabase(id){
  const url = `${SUPABASE_URL}/rest/v1/productos?id=eq.${id}`;
  await fetch(url,{
    method:"DELETE",
    headers:{
      "apikey":SUPABASE_KEY,
...
// Hook global (sobrescribe funciones si existen)
window._guardarProductoSupabase = guardarProductoSupabase;
window._eliminarProductoSupabase = eliminarProductoSupabase;
