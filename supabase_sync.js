  ...
  try {
    ...
    if (prods && prods.length > 0) {
      ...
      const merged = sbProds.map(sp => {
        ...
        // El stock SIEMPRE viene de Supabase.
        // registrar_venta_atomica actualiza productos.stock directamente en BD
        // pero NO actualiza _ts, por eso el timestamp no sirve para resolver stock.
        // Supabase es la fuente de verdad para stock; local es fuente de verdad para
        // metadatos editados localmente (nom, precio) cuando _ts local > _ts Supabase.
        const stockSb = Number(sp.stock) || 0;
        if (local) {
          ...
          if (sbTs > localTs) {
            // Supabase tiene metadatos más nuevos (nombre/precio editado en otro dispositivo)
            return {
              ...sp,
              stock: stockSb,
              img:      local.img || sp.img || null,
          ...
          // Metadatos locales son más recientes (nombre/precio editado aquí)
          // PERO el stock siempre viene de Supabase
          return {
            ...local,
            stock: stockSb,
            img:      local.img      || sp.img      || null,
