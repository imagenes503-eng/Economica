    ...
    <div class="login-card">
      ...
      <div class="login-body">
        ...
        <div id="campoTienda" class="login-field" style="display:none;">
          ...
          <div class="field-hint">Usa el mismo ID en todos tus dispositivos</div>
        </div>
...
async function _finalizarLogin() {
  ...
  _actualizarTabAdmin(); // mostrar/ocultar tab Admin según rol
...
async function restaurarSesion() {
  ...
  try {
    ...
    _actualizarBadgeLogin();
    _aplicarRestriccionesPorRol();
    _actualizarTabAdmin(); // mostrar tab Admin si es admin
    // Si sesión restaurada correctamente, ocultar login
...
// Llama a esta función desde consola o desde un panel admin:
// activarMembresiaManual('email@ejemplo.com', 'mensual')
async function activarMembresiaManual(email, tipoMembresia) {
  if (!_puedeHacer('config')) { toast('Solo el Admin puede activar membresías', true); return; }
  ...
  try {
    ...
    // Buscar solicitudes pendientes de ese email
    const pendientes = await _sbGet('membresias', { select: '*', email: 'eq.' + email, pago: 'eq.efectivo_pendiente' });
    if (pendientes && pendientes.length > 0) {
      ...
      const users = await _sbGet('perfiles', { select: 'id', email: 'eq.' + email });
  ...
  if (!_puedeHacer('config')) {
    document.getElementById('pgAdmin').innerHTML = `
      <div style="text-align:center;padding:60px 20px;">
        ...
        <div style="font-size:14px;color:var(--text-muted);font-weight:700;margin-top:8px;">Solo el Admin puede ver este panel</div>
      </div>`;
    return;
  }
  adminCargarStats();
...
async function adminRechazarMembresia(memId, email) {
  if (!confirm(`¿Rechazar la solicitud de ${email}? Se eliminará de la lista.`)) return;
  try {
    await _sbPost('membresias', { id: memId, pago: 'rechazado', activa: false }, true);
    _registrarAccion('rechazar_membresia', email);
    toast('Solicitud rechazada', true);
    adminCargarPendientes();
    adminCargarStats();
  } catch(e) { toast('Error: ' + e.message, true); }
}
...
async function adminCargarUsuarios() {
  ...
  try {
    ...
    container.innerHTML = `<div class="admin-users-list">${usuarios.map(u => {
      const esExento = _EMAILS_EXENTOS.includes((u.email||'').toLowerCase());
      const mem = memMap[u.id];
      const rolClass = u.rol === 'admin' ? '' : 'cajero';
      const memTag = esExento
      ...
      return `
        <div class="admin-user-row">
          ...
          ${memTag}
          <span class="aur-role ${rolClass}">${u.rol === 'admin' ? '👑 Admin' : '🏷 Cajero'}</span>
          ${u.id !== _usuarioActual?.id ? `
            <select onchange="cambiarRolUsuario('${u.id}',this.value)"
              style="padding:5px 8px;border-radius:7px;border:1px solid var(--border-mid);font-family:'Nunito',sans-serif;font-weight:700;font-size:11px;background:var(--surface2);">
              <option value="admin" ${u.rol==='admin'?'selected':''}>Admin</option>
              <option value="cajero" ${u.rol==='cajero'?'selected':''}>Cajero</option>
            </select>` : '<span style="font-size:11px;color:var(--text-muted);font-weight:700;">(tú)</span>'}
        </div>`;
    }).join('')}</div>`;
  } catch(e) {
    container.innerHTML = `<div style="color:var(--red);padding:16px;font-size:13px;font-weight:700;">Error: ${e.message}</div>`;
...
async function _verificarNuevasSolicitudes() {
  ...
  try {
    ...
    // Buscar la primera solicitud que no haya sido vista
...
function adminCerrarNotif() {
  ...
  if (_notifPendienteActual) {
    ...
    localStorage.setItem('vpos_memVistas', JSON.stringify([..._memPendientesVistas]));
  }
}
