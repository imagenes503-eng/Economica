      ...
      const merged = sbProds.map(sp => {
        ...
        if (local) {
          ...
          const mejoresLotes = localLtLen > 0
            ...
            : (sbLtLen > 0 ? sp.lotes : []);
          if (sbTs > localTs) {
            // Supabase es más reciente: usar su stock directamente.
            // Math.max estaba incorrecto: si el local tiene 118 (sin la venta)
            // y Supabase tiene 113 (post-venta), Math.max devuelve 118 borrando la venta.
            return {
              ...sp,
