...
async function _autoCargarDesdeSupa() {
  ...
  if (Date.now() < _autoCargarBloqueadoHasta) return; // bloqueado tras guardar producto
  try {
    _dot('yellow');
      ...
      const merged = sbProds.map(sp => {
        ...
        if (local) {
          ...
          if (localInvalido) {
            return {
              ...sp,
              stock: Number(sp.stock) || 0,   // Supabase es fuente de verdad para producto corrupto
          ...
          // FIX: paquetes/lotes — usar el que tenga más datos.
          // Si ambos tienen, gana el más reciente por _ts.
          // Nunca sobreescribir datos reales con array vacío.
          const localPkLen = (local.paquetes && local.paquetes.length) ? local.paquetes.length : 0;
          const sbPkLen    = (sp.paquetes    && sp.paquetes.length)    ? sp.paquetes.length    : 0;
          const mejoresPaquetes = localPkLen > 0
            ? (sbPkLen > localPkLen && sbTs > localTs ? sp.paquetes : local.paquetes)
            : (sbPkLen > 0 ? sp.paquetes : []);
          const localLtLen = (local.lotes && local.lotes.length) ? local.lotes.length : 0;
          const sbLtLen    = (sp.lotes    && sp.lotes.length)    ? sp.lotes.length    : 0;
          const mejoresLotes = localLtLen > 0
            ? (sbLtLen > localLtLen && sbTs > localTs ? sp.lotes : local.lotes)
            : (sbLtLen > 0 ? sp.lotes : []);
          if (sbTs > localTs) {
            // Supabase es más reciente (_ts actualizado por venta en otro dispositivo).
            // Usar sp.stock directamente — es el stock post-venta correcto.
            return {
              ...sp,
              stock:    Number(sp.stock) || 0,
              img:      local.img || sp.img || null,
              paquetes: mejoresPaquetes,
              lotes:    mejoresLotes,
            };
          }
    ...
    if (ventas && ventas.length > 0) {
      historial = ventas.map(v => {
        ...
        // Normalizar ID: el trigger set_ventas_tienda_id() prefija el id con tiendaId+'_'
        // Ej: Supabase guarda 'despensa1_1234567890', pero el snapshot local tiene '1234567890'
        // Si no normalizamos, _fusionarDos no puede deduplicar → historial doble → totales dobles
        const _sbVentaId = String(v.id || '');
        const _tid = _getTiendaId() || '';
        const _localVentaId = (_tid && _sbVentaId.startsWith(_tid + '_'))
          ? _sbVentaId.slice(_tid.length + 1)
          : _sbVentaId;
        return {
          id: _localVentaId,
          fecha: fechaISO,
      ...
      // Sin esto, al recargar la página el historial borrado reaparecía desde Supabase.
      const _wipeTs = localStorage.getItem('vpos_historialWipeTs');
      if (_wipeTs) {
        const _wipeMs = Date.parse(_wipeTs);
        historial = historial.filter(v => (v.ts || 0) >= _wipeMs);
      }
      // BUGFIX SYNC: filtrar cobros eliminados individualmente (por devolución)
      // que podrían volver a aparecer al recargar desde Supabase.
    ...
    // ── Ventas diarias ─────────────────────────────────────────────────
    // FIX OFFLINE-SYNC: leer deleted_log de Supabase ANTES de procesar ventasDiarias.
    // Cuando un teléfono borra una ventaDiaria mientras otro está offline, el teléfono
    // offline nunca recibe el broadcast en tiempo real. Al reconectarse, sin este fix,
    // la venta eliminada permanece porque la fila ya no existe en Supabase (fue borrada)
    // pero el teléfono offline sigue teniéndola localmente y no sabe que fue eliminada.
    // Solución: leer deleted_log (tabla de auditoría de borrados) y aplicar esos
    // tombstones localmente antes de mergear con Supabase.
    try {
      const delLog = await _sbGet('deleted_log', {
        select: 'registro_id,deleted_at',
        tabla: 'eq.ventas_diarias',
        tienda_id: 'eq.' + _getTiendaId(),
        order: 'deleted_at.desc',
        limit: 500
      }).catch(() => null);
      if (delLog && delLog.length > 0) {
        if (typeof ventasDiariasEliminadas === 'undefined') window.ventasDiariasEliminadas = [];
        let cambioTombstone = false;
        delLog.forEach(row => {
          const fecha = row.registro_id; // formato "YYYY-MM-DD" guardado en syncBorrarVentaDiaria
          if (!fecha || !/^\d{4}-\d{2}-\d{2}$/.test(fecha)) return;
          if (!ventasDiariasEliminadas.includes(fecha)) {
            ventasDiariasEliminadas.push(fecha);
            cambioTombstone = true;
          }
        });
        if (cambioTombstone) {
          // Eliminar del array local las fechas que ya fueron borradas en otro dispositivo
          const antes = (ventasDiarias || []).length;
          ventasDiarias = (ventasDiarias || []).filter(v => !ventasDiariasEliminadas.includes(v.fecha));
          ...
          if (ventasDiarias.length !== antes) {
            console.log('[AutoCarga] 🗑 Aplicados', antes - ventasDiarias.length, 'borrado(s) de ventasDiarias desde deleted_log');
    ...
    } catch(_e) { console.warn('[AutoCarga] deleted_log ventas_diarias:', _e.message); }
    ...
    if (vd && vd.length > 0) {
      ...
      vd.forEach(row => {
        ...
        // BUGFIX SYNC: ignorar fechas que fueron eliminadas (tombstone local o de deleted_log)
    ...
    // ── Recalcular inventarioCosto e inventarioInicial desde los productos actuales ──
    // Crítico después de un restock: productos llegan con stock nuevo pero
    // inventarioCosto queda desactualizado sin este paso.
    if (typeof actualizarInventarioInicialAuto === 'function') {
      actualizarInventarioInicialAuto();
    } else if (typeof productos !== 'undefined' && productos.length > 0) {
      let totalCosto = 0;
      productos.forEach(p => { totalCosto += (Number(p.stock)||0) * (Number(p.compra)||0); });
      if (typeof inventarioCosto !== 'undefined' && totalCosto > 0) {
        inventarioCosto = parseFloat(totalCosto.toFixed(2));
        idbSet('vpos_inventarioCosto', inventarioCosto).catch(() => {});
        const elC = document.getElementById('inpInventarioCosto');
        if (elC) elC.value = inventarioCosto.toFixed(2);
      }
    }

    if (typeof actualizarTodo === 'function') actualizarTodo();
    // FIX OFFLINE-SYNC: llamar autoRegistrarVentaDiaria() después de reconstruir
    // historial y ventasDia desde Supabase. Sin esto, el teléfono que estaba offline
    // recalcula ventasDia correctamente pero nunca actualiza ventasDiarias (el array
    // de totales diarios usado por renderCajaPanelMini para calcular Caja y Capital).
    // Resultado: Capital mostraba $1.00 menos que el correcto porque la venta recibida
    // estaba en ventasDia pero no en ventasDiarias.
    // Usamos la versión SILENTE para evitar el loop: salvar→sync→broadcast→autoCargar→…
    if (typeof _autoRegistrarVentaDiariaSilente === 'function') {
      setTimeout(() => {
        _autoRegistrarVentaDiariaSilente();
        if (typeof renderCajaPanelMini === 'function') renderCajaPanelMini();
      }, 200);
    }
    // renderCajaPanel siempre — actualiza capital total aunque no sea la página activa
    if (typeof renderCajaPanel === 'function') setTimeout(renderCajaPanel, 100);
    _dot('green');
...
function _fusionarDos(local, ext) {
  ...
  // Siempre preservar paquetes: usar los que tengan datos reales, priorizando el más reciente.
  local.productos = local.productos.map(lp => {
    ...
    // Paquetes: nunca sobreescribir datos reales con array vacío.
    // FIX SYNC COSTO: usar >= en la comparación de longitud para que cuando ambos
    // tienen igual cantidad de paquetes también gane el más reciente (antes solo
    // ganaba externo si tenía MÁS paquetes, lo que ignoraba ediciones de precio
    // sin cambio de cantidad de presentaciones).
    const lpPkLen  = (lp.paquetes  && lp.paquetes.length)  ? lp.paquetes.length  : 0;
    const extPkLen = (extP.paquetes && extP.paquetes.length) ? extP.paquetes.length : 0;
    const tsLocal = Number(lp._ts  || 0);
    const tsExt   = Number(extP._ts || 0);
    const extEsMasReciente = tsExt > tsLocal;
    const mejoresPaquetes = lpPkLen > 0
      ? (extPkLen >= lpPkLen && extEsMasReciente ? extP.paquetes : lp.paquetes)
      : (extPkLen > 0 ? extP.paquetes : []);
    // FIX SYNC COSTO: si el producto externo es más reciente, adoptar TODOS sus
    // campos de costo/precio (compra, venta, costoUnidadEntrado, costoPresEntrado,
    // ventaUnidadEntrada, costoTotal, ventaTotal, etc.) para que ambos teléfonos
    // calculen exactamente el mismo _costoTotalProd y _ventaTotalProd.
    // El stock NO se toca aquí porque se recalcula correctamente más abajo.
    if (extEsMasReciente) {
      return {
        ...lp,
        ...extP,
        stock: lp.stock,  // se recalcula en el bloque de stock fusion más abajo
        img: lp.img || extP.img || null,
        paquetes: mejoresPaquetes,
      };
    }
    // Producto local es más reciente o igual: preservar campos locales, solo
    // actualizar imagen si local no tiene y paquetes si corresponde.
    if (!lp.img && extP.img) return { ...lp, img: extP.img, paquetes: mejoresPaquetes };
    return { ...lp, paquetes: mejoresPaquetes };
  });
  ...
  local.historial.forEach(v=>{
    ...
    (v.items||[]).forEach(it=>{
      ...
      const cant=Number(it.cant||0);
      // Usar totalItem cuando existe — igual que _recalcularReportesDesdeHistorial
      const total = it.totalItem !== undefined ? Number(it.totalItem)
        : it.cantCobrada !== undefined ? Number(it.cantCobrada)*Number(it.precio||0)
        : cant*Number(it.precio||0);
      const base={id:pid,nom:it.nom||'',cat:it.cat||'',cant:0,total:0};
  ...
  // inventarioInicial se recalcula automáticamente desde los productos (SUM stock×venta)
  // NO usar Math.max: después de una venta el stock baja y el valor correcto es el menor.
  // La recalculación la hace actualizarInventarioInicialAuto() justo después de aplicar datos.
  return local;
}
...
async function _aplicarDatos(datos) {
  ...
  // Aplicar datos en memoria.
  // IMPORTANTE: ventasDia/ventasSem/ventasMes NO se aplican desde el snapshot.
  // Son valores DERIVADOS del historial. Siempre se recalculan desde _recalcularReportesDesdeHistorial
  // para evitar doble conteo cuando el snapshot y el historial de Supabase tienen la misma venta.
  // Merge paquetes/lotes al aplicar snapshot: nunca sobreescribir datos reales con vacío.
  // Los paquetes del snapshot pueden estar desactualizados si vienen de otro dispositivo
  // o fueron guardados antes de que se registraran las presentaciones.
  const _pkgPorId  = {};
  const _tsPorId   = {};
  (typeof productos !== 'undefined' ? productos : []).forEach(p => {
    if (p.paquetes && p.paquetes.length) _pkgPorId[String(p.id)] = p.paquetes;
    if (p._ts) _tsPorId[String(p.id)] = Number(p._ts);
  });
  productos = (datos.productos || []).map(p => {
    const snapPk   = (p.paquetes && p.paquetes.length) ? p.paquetes : null;
    const localPk  = _pkgPorId[String(p.id)] || null;
    const tsSnap   = Number(p._ts || 0);
    const tsLocal  = _tsPorId[String(p.id)] || 0;
    // FIX SYNC COSTO: el snapshot (que viene del fusionado) siempre es la fuente
    // de verdad para los campos de precio/costo cuando es más reciente que local.
    // Antes se preferían los paquetes locales por ser "más recientes en este
    // dispositivo", lo que causaba que ediciones de precio en otro teléfono no se
    // propagaran y los totales de inventario diferían en $0.01 entre dispositivos.
    const snapEsMasReciente = tsSnap > tsLocal;
    let mejoresPk = localPk;
    if (snapPk && (!localPk || snapPk.length > localPk.length || (snapPk.length >= localPk.length && snapEsMasReciente))) {
      mejoresPk = snapPk;
    }
    return { ...p, paquetes: mejoresPk || [] };
  });
  historial=datos.historial||[]; pagos=datos.pagos||[];
  // FIX: Merge ventasDiarias en lugar de reemplazar. Un snapshot antiguo
  // podía sobrescribir ventas recientes con un array vacío.
  // Regla: conservar siempre el monto mayor entre local y snapshot.
  { const _vdSnap = datos.ventasDiarias || [];
    if (!ventasDiarias) ventasDiarias = [];
    // FIX OFFLINE-SYNC: aplicar tombstones de ventasDiarias del snapshot ANTES de mergear.
    // Si el snapshot viene de un dispositivo que borró una ventaDiaria mientras este
    // estaba offline, los tombstones llegan aquí y deben eliminarse localmente.
    if (datos.ventasDiariasEliminadas && datos.ventasDiariasEliminadas.length) {
      if (typeof ventasDiariasEliminadas === 'undefined') window.ventasDiariasEliminadas = [];
      const _snTomb = new Set(datos.ventasDiariasEliminadas);
      let _tombCambio = false;
      datos.ventasDiariasEliminadas.forEach(f => {
        if (!ventasDiariasEliminadas.includes(f)) { ventasDiariasEliminadas.push(f); _tombCambio = true; }
      });
      const _antesVD = ventasDiarias.length;
      ventasDiarias = ventasDiarias.filter(v => !_snTomb.has(v.fecha));
      if (_tombCambio || ventasDiarias.length !== _antesVD) {
        idbSet('vpos_ventasDiariasElim', ventasDiariasEliminadas).catch(() => {});
        console.log('[AplicarDatos] 🗑 Tombstones aplicados: eliminadas', _antesVD - ventasDiarias.length, 'ventasDiarias del snapshot');
      }
    }
    _vdSnap.forEach(vExt => {
      // No agregar fechas que fueron eliminadas (tombstone)
      if (typeof ventasDiariasEliminadas !== 'undefined' && ventasDiariasEliminadas.includes(vExt.fecha)) return;
      const idx = ventasDiarias.findIndex(v => v.fecha === vExt.fecha);
      if (idx >= 0) {
        if (Number(vExt.monto) > Number(ventasDiarias[idx].monto || 0)) {
          ventasDiarias[idx].monto = Number(vExt.monto);
          if (vExt.nota) ventasDiarias[idx].nota = vExt.nota;
        }
      } else {
  ...
  // Recalcular reportes desde el historial (fuente de verdad)
  if (typeof _recalcularReportesDesdeHistorial === 'function') {
    _recalcularReportesDesdeHistorial();
  } else {
    ventasDia=datos.ventasDia||{}; ventasSem=datos.ventasSem||{}; ventasMes=datos.ventasMes||{};
  }
  // inventarioInicial no se recalcula aquí — es un baseline fijo del mes
  ...
  // ── Sincronizar inventarioInicial e inventarioCosto desde el snapshot ──────
  // Son valores fijos del mes — si el otro teléfono los tiene, adoptarlos.
  if (datos.inventarioInicial !== undefined && datos.inventarioInicial > 0) {
    inventarioInicial = parseFloat(datos.inventarioInicial) || 0;
    idbSet('vpos_inventarioInicial', inventarioInicial).catch(()=>{});
    const elInv = document.getElementById('inpInventarioInicial');
    if (elInv) elInv.value = inventarioInicial.toFixed(2);
  } else {
    const el = document.getElementById('inpInventarioInicial');
    if (el && typeof inventarioInicial !== 'undefined') el.value = inventarioInicial > 0 ? inventarioInicial.toFixed(2) : '';
  }
  if (datos.inventarioCosto !== undefined && datos.inventarioCosto > 0) {
    if (typeof inventarioCosto !== 'undefined') {
      inventarioCosto = parseFloat(datos.inventarioCosto) || 0;
      idbSet('vpos_inventarioCosto', inventarioCosto).catch(()=>{});
      const elC = document.getElementById('inpInventarioCosto');
      if (elC) elC.value = inventarioCosto.toFixed(2);
    }
  ...
  actualizarTodo();
  // ── Refrescar paneles de caja e inventario con los datos recién sincronizados ──
  setTimeout(() => {
    // FIX OFFLINE-SYNC: actualizar ventasDiarias desde ventasDia reconstruido.
    // Sin esto, Capital en Estado de Caja muestra $1.00 menos después de fusionar
    // porque la venta del otro teléfono llega en historial/ventasDia pero no en
    // ventasDiarias, que es lo que usa renderCajaPanelMini para calcular Caja.
    // Versión SILENTE para evitar el loop broadcast→sync→_aplicarDatos→broadcast→…
    if (typeof _autoRegistrarVentaDiariaSilente === 'function') _autoRegistrarVentaDiariaSilente();
    if (typeof renderCajaPanelMini === 'function') renderCajaPanelMini();
    if (typeof renderCajaPanel     === 'function') {
      const onSub = document.getElementById('pgEstadoCaja')?.classList?.contains('active');
      if (onSub) renderCajaPanel();
    }
    if (typeof renderInvTotales    === 'function') {
      const onInv = document.getElementById('pgInvCapital')?.classList?.contains('active');
      if (onInv) renderInvTotales();
    }
    if (typeof renderDashboardPro  === 'function') {
      const onDash = document.getElementById('pgDash')?.classList?.contains('active');
      if (onDash) renderDashboardPro();
    }
  }, 300);
...
let _autoCargarBloqueadoHasta = 0; // ms timestamp — bloquea _autoCargarDesdeSupa tras guardar producto
function _bloquearAutoCargar(ms) { _autoCargarBloqueadoHasta = Date.now() + (ms || 5000); }
...
window._bloquearAutoCargar=_bloquearAutoCargar;
  ...
  try{
    for(const tipo of queue){
      if(tipo==='venta'&&_lastVenta){
        ...
        const itemsJson=v.items_json||(itemsArr.length?JSON.stringify(itemsArr):null);
        await _sbPost('ventas',{id:v.id,fecha_iso:v.fechaISO||v.fecha||new Date().toISOString(),
          ...
          tienda_id:_getTiendaId(),
          items:itemsStr,items_json:itemsJson},true);
      }
...
async function _autoEnviarSnapshot() {
  ...
  const snap = {
    ...
    datos: JSON.stringify({
      ...
      efectivoInicial:   typeof efectivoInicial   !== 'undefined' ? efectivoInicial   : 0,
      inventarioInicial: typeof inventarioInicial !== 'undefined' ? inventarioInicial : 0,
      inventarioCosto:   typeof inventarioCosto   !== 'undefined' ? inventarioCosto   : 0,
      // Strip img from snapshot to avoid Supabase row size limits (images sent via productos table separately)
      productos: (productos||[]).map(p=>({...p, img: null})),
      ventasDia, ventasSem, ventasMes,
      ...
      // BUGFIX SYNC: tombstone para borrado de historial
      historialWipeTs: localStorage.getItem('vpos_historialWipeTs') || null,
      // BUGFIX SYNC: IDs de cobros eliminados por devolución — evita que vuelvan al fusionar
      cobrosEliminados: typeof cobrosEliminados !== 'undefined' ? (cobrosEliminados || []) : [],
      // BUGFIX SYNC: fechas de ventas diarias eliminadas — evita que vuelvan al fusionar
      ventasDiariasEliminadas: typeof ventasDiariasEliminadas !== 'undefined' ? (ventasDiariasEliminadas || []) : []
    }),
    created_at: new Date().toISOString()
  };
...
async function _autoFusionar() {
  ...
  try {
    ...
    const miSnap = {
      ...
      historial, pagos, ventasDiarias, restockLog: restockLog || [],
      // FIX OFFLINE-SYNC: incluir tombstones de ventasDiarias eliminadas para que
      // _fusionarDos los propague a dispositivos que estaban offline y no recibieron
      // el broadcast en tiempo real del borrado.
      ventasDiariasEliminadas: typeof ventasDiariasEliminadas !== 'undefined' ? (ventasDiariasEliminadas || []) : [],
      historialEliminado: typeof historialEliminado !== 'undefined' ? (historialEliminado || []) : [],
      productosEliminados: typeof productosEliminados !== 'undefined' ? (productosEliminados || []) : []
    };
    ...
    _realtimeChannel = client
      ...
      .on('broadcast', { event: 'producto' }, ({ payload }) => {
        ...
        if (idx >= 0) {
          ...
          // Preservar paquetes locales si el broadcast no trae datos reales
          const pkLocal = productos[idx].paquetes;
          Object.assign(productos[idx], payload);
          if (!payload.img && imgLocal) productos[idx].img = imgLocal;
          if (!(payload.paquetes && payload.paquetes.length) && pkLocal && pkLocal.length)
            productos[idx].paquetes = pkLocal;
        } else {
      ...
      // BROADCAST: pago/gasto/factura AGREGADO — sincroniza creación a otros dispositivos en tiempo real
      .on('broadcast', { event: 'pago_agregado' }, ({ payload }) => {
        if (!payload || !payload.pago) return;
        const p    = payload.pago;
        const idStr = String(p.id);
        // Ignorar si ya fue marcado como eliminado en este dispositivo
        if ((pagosEliminados||[]).includes(idStr)) return;
        // Ignorar si ya existe localmente (deduplicación)
        if ((pagos||[]).some(x => String(x.id) === idStr)) return;
        if (typeof pagos !== 'undefined') pagos = [p, ...pagos];
        idbSet('vpos_pagos', typeof pagos !== 'undefined' ? pagos : []).catch(() => {});
        if (typeof actualizarTodo === 'function') actualizarTodo();
        _dot('green');
        console.log('[Realtime] 💸 Pago agregado desde otro dispositivo:', p.cat, '$' + p.monto);
      })

      // BROADCAST: cierre_dia_actualizado — datos de cierre de día/mes cambiaron en otro dispositivo
      .on('broadcast', { event: 'cierre_dia_actualizado' }, ({ payload }) => {
        if (!payload) return;
        // Re-renderizar la sección si está abierta para mostrar los datos más frescos
        if (typeof renderCierreDia === 'function') {
          // Recargar datos frescos de Supabase/IDB antes de renderizar
          setTimeout(() => renderCierreDia(), 300);
        }
        _dot('green');
        console.log('[Realtime] 📋 Cierre día/mes actualizado desde otro dispositivo');
      })

      // BROADCAST: finanzas_mes_actualizado — finanzas del mes cambiaron en otro dispositivo
      .on('broadcast', { event: 'finanzas_mes_actualizado' }, ({ payload }) => {
        if (!payload) return;
        // Re-renderizar la sección si está abierta
        if (typeof renderFinanzasMes === 'function') {
          setTimeout(() => renderFinanzasMes(), 300);
        }
        _dot('green');
        console.log('[Realtime] 📊 Finanzas mes actualizadas desde otro dispositivo');
      })

      // POSTGRES CHANGES: pago/gasto nuevo o editado en Supabase (respaldo cuando el WebSocket reconecta)
      // Cubre el caso: dispositivo A offline → agrega gasto → vuelve online → dispositivo B lo recibe
      .on('postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'pagos',
          filter: 'tienda_id=eq.' + tiendaId },
        (change) => {
          if (!change.new || !change.new.id) return;
          const row   = change.new;
          const idStr = String(row.id).replace(/^.*_/, ''); // quitar prefijo tienda si existe
          // Ignorar si ya fue marcado como eliminado
          if ((pagosEliminados||[]).includes(idStr) || (pagosEliminados||[]).includes(String(row.id))) return;
          // Ignorar si ya existe (evitar duplicados con el broadcast que llega primero)
          if ((pagos||[]).some(x => String(x.id) === idStr || String(x.id) === String(row.id))) return;
          const nuevoPago = {
            id:        idStr,
            concepto:  row.nom   || '',
            cat:       row.cat   || 'GASTO',
            monto:     Number(row.monto) || 0,
            fechaISO:  row.fecha_iso || new Date().toISOString(),
            fechaStr:  row.fecha_iso ? new Date(row.fecha_iso).toLocaleString('es-SV') : '',
            ts:        row.updated_at ? new Date(row.updated_at).getTime() : Date.now()
          };
          if (typeof pagos !== 'undefined') pagos = [nuevoPago, ...pagos];
          idbSet('vpos_pagos', typeof pagos !== 'undefined' ? pagos : []).catch(() => {});
          if (typeof actualizarTodo === 'function') actualizarTodo();
          _dot('green');
          console.log('[PG-Change] 💸 Pago INSERT desde Supabase:', nuevoPago.cat, '$' + nuevoPago.monto);
        }
      )
      // Más confiable que broadcast — funciona aunque el WebSocket se corte y reconecte
      .on('postgres_changes',
        ...
        (change) => {
          if (!change.new || !change.new.id) return;
          const row = change.new;
          // Extraer id numérico quitando el prefijo de tienda (ej: "tienda123_456" → 456)
          const rawId = String(row.id || '');
          const u = rawId.lastIndexOf('_');
          const numId = u >= 0 ? rawId.slice(u + 1) : rawId;
          const n = Number(numId);
          const localId = isNaN(n) ? rawId : n;
          const idx = (productos||[]).findIndex(x => String(x.id) === String(localId));
          if (idx < 0) return; // producto nuevo — se cargará en el próximo sync completo
          // Solo actualizar campos de metadatos (nombre, precio, cat) y stock.
          // No sobreescribir imagen local.
          const imgLocal = productos[idx].img;
          if (row.nom)    productos[idx].nom    = row.nom;
          if (row.cat)    productos[idx].cat    = row.cat;
          if (row.abrev !== undefined) productos[idx].abrev = row.abrev;
          if (row.cod  !== undefined) productos[idx].cod   = row.cod;
          if (row.compra !== undefined) productos[idx].compra = Number(row.compra) || 0;
          if (row.venta  !== undefined) productos[idx].venta  = Number(row.venta)  || 0;
          if (row.min    !== undefined) productos[idx].min    = Number(row.min)    || 0;
          if (row.stock  !== undefined) productos[idx].stock  = Number(row.stock)  || 0;
          if (row.paquetes && row.paquetes.length) productos[idx].paquetes = row.paquetes; // solo sobreescribir si hay datos reales
          if (row.img && !imgLocal) productos[idx].img = row.img;
          idbSet('vpos_productos', productos).catch(() => {});
          // Recalcular inventarioCosto tras actualización de stock (restock desde otro teléfono)
          if (typeof actualizarInventarioInicialAuto === 'function') actualizarInventarioInicialAuto();
          if (typeof actualizarTodo === 'function') actualizarTodo();
          if (typeof actualizarStockFila === 'function') actualizarStockFila(productos[idx]);
          console.log('[PG-Change] 🔄 Producto actualizado:', productos[idx].nom);
        }
      )

      // BROADCAST: imagen de producto
      // Recibir actualización de inventarioInicial desde el otro teléfono
      .on('broadcast', { event: 'config_actualizada' }, ({ payload }) => {
        if (!payload) return;
        if (payload.inventarioInicial !== undefined && typeof inventarioInicial !== 'undefined') {
          inventarioInicial = Number(payload.inventarioInicial) || 0;
          idbSet('vpos_inventarioInicial', inventarioInicial).catch(() => {});
          const inp = document.getElementById('inpInventarioInicial');
          if (inp) inp.value = inventarioInicial.toFixed(2);
        }
        // ── NUEVO: sincronizar inventarioCosto al teléfono offline ──────────
        if (payload.inventarioCosto !== undefined && typeof inventarioCosto !== 'undefined') {
          inventarioCosto = Number(payload.inventarioCosto) || 0;
          idbSet('vpos_inventarioCosto', inventarioCosto).catch(() => {});
          const inpC = document.getElementById('inpInventarioCosto');
          if (inpC) inpC.value = inventarioCosto.toFixed(2);
        }
        if (payload.efectivoInicial !== undefined && typeof efectivoInicial !== 'undefined') {
          efectivoInicial = Number(payload.efectivoInicial) || 0;
          idbSet('vpos_efectivoInicial', efectivoInicial).catch(() => {});
          const inp2 = document.getElementById('inpEfectivoInicial');
          if (inp2) inp2.value = efectivoInicial.toFixed(2);
        }
        if (typeof renderCajaPanel === 'function') renderCajaPanel();
        if (typeof renderInvTotales === 'function') renderInvTotales();
        console.log('[Config] config actualizada desde otro dispositivo — inventarioCosto:', inventarioCosto);
      })
      ...
      .on('broadcast', { event: 'historial_actualizado' }, ({ payload }) => {
        ...
        // BUGFIX SYNC: si el broadcast trae historialWipeTs, guardarlo en localStorage
        // para que este teléfono también ignore ventas antiguas en futuras fusiones.
        if (payload.historialWipeTs) {
          const localWipe = localStorage.getItem('vpos_historialWipeTs') || '';
          if (payload.historialWipeTs > localWipe)
            localStorage.setItem('vpos_historialWipeTs', payload.historialWipeTs);
        }
        // BUGFIX SYNC: si el broadcast trae cobrosEliminados, fusionarlos con los locales
        // para que _fusionarDos tampoco restaure esos cobros desde snapshots del otro dispositivo.
        if (payload.cobrosEliminados && Array.isArray(payload.cobrosEliminados)) {
          if (typeof cobrosEliminados === 'undefined') window.cobrosEliminados = [];
          payload.cobrosEliminados.forEach(id => {
            if (!cobrosEliminados.includes(id)) cobrosEliminados.push(id);
          });
          idbSet('vpos_cobrosEliminados', cobrosEliminados).catch(() => {});
        }
      ...
      .on('broadcast', { event: 'ventas_dia_eliminada' }, ({ payload }) => {
        ...
        // BUGFIX SYNC: guardar tombstone recibido para que _fusionarDos tampoco restaure esa fecha
        if (typeof ventasDiariasEliminadas !== 'undefined') {
          (payload.ventasDiariasEliminadas || [payload.fecha]).forEach(f => {
            if (!ventasDiariasEliminadas.includes(f)) ventasDiariasEliminadas.push(f);
          });
          idbSet('vpos_ventasDiariasElim', ventasDiariasEliminadas).catch(() => {});
        }
        const antes = (ventasDiarias||[]).length;
...
function _aplicarVentaRealtime(v) {
  ...
  // BUG FIX: generar fechaStr correcta al recibir venta por broadcast.
  // Sin esto, normalizeHistorial usaba v.fecha (timestamp ISO crudo) como fechaStr.
  const _fdRt = new Date(fechaISO);
  const fechaStr = v.fechaStr || (isNaN(_fdRt.getTime()) ? '—' : _fdRt.toLocaleString('es-SV'));
  const nv = { id: v.id, fecha: fechaISO, fechaISO, fechaStr, ts: Date.parse(fechaISO)||0,
  ...
  (function() {
    ...
    items.forEach(item => {
      ...
      if (p && typeof p.stock === 'number') {
        ...
        p._ts = Date.now(); // marca como modificado — evita que _autoCargarDesdeSupa revierta con stock viejo
        stockCambio = true;
  ...
  items.forEach(it => {
    const pid = String(it.id||''); if (!pid || pid === 'null') return;
    const cant = Number(it.cant||0);
    // CRÍTICO: usar totalItem (precio facturado exacto) cuando existe.
    // it.cant = unidades físicas del paquete (ej: 5), it.precio = precio del paquete ($1.00).
    // cant × precio = 5 × $1.00 = $5.00 INCORRECTO.
    // totalItem = $1.00 (lo que realmente se cobró) CORRECTO.
    let tot;
    if (it.totalItem !== undefined) {
      tot = Number(it.totalItem);
    } else if (it.cantCobrada !== undefined) {
      tot = Number(it.cantCobrada) * Number(it.precio||0);
    } else {
      // Fallback para ventas antiguas sin totalItem ni cantCobrada
      tot = cant * Number(it.precio||0);
    }
    const base = { id: pid, nom: it.nom||'', cat: it.cat||'', cant: 0, total: 0 };
    if (esHoy) { if (!ventasDia[pid]) ventasDia[pid]={...base}; ventasDia[pid].cant+=cant; ventasDia[pid].total+=tot; }
    if (esSem) { if (!ventasSem[pid]) ventasSem[pid]={...base}; ventasSem[pid].cant+=cant; ventasSem[pid].total+=tot; }
  ...
  idbSet('vpos_ventasMes', ventasMes).catch(()=>{});
  if (typeof actualizarTodo === 'function') actualizarTodo();
  // ── Actualizar paneles en tiempo real en el teléfono receptor ──────────
  setTimeout(() => {
    if (typeof renderCajaPanelMini   === 'function') renderCajaPanelMini();
    if (typeof renderCajaPanel       === 'function') {
      const onSub = document.getElementById('pgEstadoCaja')?.classList?.contains('active');
    ...
    // auto-registrar venta diaria en el teléfono receptor
  ...
  try{
    for(let i=0;i<historial.length;i+=50){
      ...
      const rows=historial.slice(i,i+50).map(v=>({
        ...
        items_json:v.items_json||JSON.stringify(v.items||[])
      }));
...
async function _subirStockBase(){
  ...
  for(let i=0;i<rowsCompleto.length;i+=50){
    try{
      if(modoBase) throw new Error('ya en modo base');
      await _sbPost('productos',rowsCompleto.slice(i,i+50),true);
    }catch(e1){
      // Columnas extra no existen → reintentar con columnas mínimas
      console.warn('[Sync productos] Modo completo falló:',e1.message,'— reintentando con columnas base');
      modoBase = true;
      try{
        await _sbPost('productos',rowsBase.slice(i,i+50),true);
      }catch(e2){
        console.error('[Sync productos] ❌ Error crítico en lote',i,':',e2.message);
        if(typeof toast==='function') toast('Error al sincronizar productos: ' + e2.message, true);
        throw e2; // propagar para que _ejecutarSync lo capture
      }
    }
  }
  ...
  for(const p of productos){
    ...
    try{
      const compImg = p.img.startsWith('https://') ? p.img : await _comprimirImg(p.img, 380, 0.82);
...
async function syncImgProducto(prod) {
  ...
  try {
    if (prod.img) {
      // Si la imagen ya es una URL pública (Supabase Storage u otra), guardarla directamente
      // sin recomprimir a base64 — así se respeta la URL original y se evita CORS en canvas.
      const compImg = prod.img.startsWith('https://') ? prod.img : await _comprimirImg(prod.img, 380, 0.82);
      // Asegurarse que el producto existe en Supabase antes de subir la img
      ...
      if (typeof _broadcast === 'function') _broadcast('producto_img', { id: String(prod.id), img: compImg });
      console.log('[imgSync] ✅ img subida para prod', prod.id, '— tienda:', tid);
      // Éxito — sacar de pendientes si estaba
...
async function _subirVentasDiarias(){
  ...
  // FIX: NO borrar Supabase cuando el array local está vacío.
  // Vacío puede significar "aún no cargado" (race condition), no "sin ventas".
  // Borrar aquí destruía registros reales en Supabase.
