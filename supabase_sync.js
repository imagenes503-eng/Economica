...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    if (prods && prods.length > 0) {
      ...
      // MERGE: use Supabase data but preserve local img if Supabase has none
      const localById = {};
      ...
      const merged = sbProds.map(sp => {
        ...
        return {
          ...sp,
          // Keep local img if Supabase img is null (not yet synced)
          img: sp.img || (local ? local.img : null) || null,
          // Keep local paquetes/lotes if Supabase has empty arrays
          paquetes: (sp.paquetes && sp.paquetes.length) ? sp.paquetes : (local ? local.paquetes||[] : []),
          lotes:    (sp.lotes    && sp.lotes.length)    ? sp.lotes    : (local ? local.lotes||[]    : []),
        };
      });
      // ── FIX: también conservar productos que solo existen en local (no sincronizados aún) ──
      const sbIds = new Set(sbProds.map(p => String(p.id)));
      const soloLocales = (productos||[]).filter(lp => !sbIds.has(String(lp.id)));
      productos = [...merged, ...soloLocales];
      ...
      console.log('[AutoCarga] Productos:', productos.length, '(Supabase:', prods.length, '| solo local:', soloLocales.length + ')');
  ...
  try {
    _realtimeChannel = client
      ...
      .on('postgres_changes', {
        ...
        // Ignorar nuestros propios cambios y el registro de fusión
        if (rec.dispositivo_id === _dispositivoId) return;
        if (rec.dispositivo_id === 'fusion') return;
        console.log('[Realtime] 📡 Push de', rec.dispositivo_id, '→ fusionando en 300ms');
        _dot('yellow');
      ...
      .subscribe((status, err) => {
        if (status === 'SUBSCRIBED') {
          ...
          // Cancelar polling agresivo si estaba activo por error previo
          // (el polling de 15s de seguridad se mantiene siempre)
          console.log('[Realtime] ✅ WebSocket activo — sync instantáneo entre cajas');
          _dot('green');
        } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          _realtimeActivo = false;
          _realtimeChannel = null; // permitir reconexión
          console.warn('[Realtime] ⚠', status, '— fallback polling 3s, reintento en 10s');
          if (!_pollingTimer) _pollingTimer = setInterval(_autoFusionar, 3000);
          // Reintentar conexión WebSocket en 10 segundos
          setTimeout(() => {
            if (_sesionActiva && !_realtimeActivo) _iniciarRealtime();
          }, 10000);
        } else if (status === 'CLOSED') {
          _realtimeActivo = false;
          _realtimeChannel = null;
          console.log('[Realtime] WebSocket cerrado');
          // Reconectar si la sesión sigue activa
          if (_sesionActiva) {
            setTimeout(() => {
              if (_sesionActiva && !_realtimeActivo) _iniciarRealtime();
            }, 5000);
...
async function syncImgProducto(prod) {
  if (!prod || !prod.img || !_sbUrl() || !_sbKey()) return;
  const tid = _getTiendaId() || 'sin_tienda';
  const rid = tid + '_' + String(prod.id);
  try {
    const compImg = await _comprimirImg(prod.img, 380, 0.82);
    await _sbPost('productos', { id: rid, tienda_id: tid, img: compImg }, true);
    console.log('[imgSync] ✅ img sincronizada para prod', prod.id, '~'+ Math.round(compImg.length*0.75/1024)+'KB');
