...
function _iniciarRealtime() {
  ...
  try {
    _realtimeChannel = client
      ...
      // cierre_diario: cambios en tiempo real → cierre_dia.js
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'cierre_diario',
        filter: 'tienda_id=eq.' + tiendaId
      }, (payload) => {
        if (typeof window._cdManejarRealtimeSB === 'function') {
          window._cdManejarRealtimeSB(payload);
        }
      })

      // cierre_mes: cambios en tiempo real → Registro Mensual
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'cierre_mes',
        filter: 'tienda_id=eq.' + tiendaId
      }, (payload) => {
        if (typeof window._cdManejarRealtimeSB === 'function') {
          window._cdManejarRealtimeSB(payload);
