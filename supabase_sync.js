...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    // ── Productos ──────────────────────────────────────────────────────
    const prods = await _sbGet('productos', { select: '*', limit: 2000 }).catch(() => null);
    if (prods && prods.length > 0) {
      ...
      // MERGE: FIX — datos locales ganan (son más recientes que Supabase)
      const localById = {};
      (productos||[]).forEach(lp => { localById[String(lp.id)] = lp; });
      const merged = sbProds.map(sp => {
        const local = localById[String(sp.id)];
        if (local) {
          // conservar todos los datos locales, Supabase aporta img si local no tiene
          return {
            ...local,
    ...
    // ── Ventas / Historial ─────────────────────────────────────────────
    const ventas = await _sbGet('ventas', { select: '*', order: 'fecha_iso.desc', limit: 1000 }).catch(() => null);
    ...
    // ── Ventas diarias ─────────────────────────────────────────────────
    const vd = await _sbGet('ventas_diarias', { select: '*', order: 'fecha.desc', limit: 365 }).catch(() => null);
    if (vd && vd.length > 0) {
      ventasDiarias = vd.map(v => ({ fecha: v.fecha, monto: Number(v.monto) || 0, nota: v.nota || '' }));
      idbSet('vpos_ventasDiarias', ventasDiarias).catch(() => {});
    }

    // ── Config ─────────────────────────────────────────────────────────
    const cfg = await _sbGet('config', { select: '*' }).catch(() => null);
    if (cfg && cfg.length > 0) {
      cfg.forEach(row => {
...
function _fusionarDos(local, ext) {
  ...
  // FIX BUG 2: para productos que ya están en local, preservar su imagen
  // (el snapshot externo tiene img:null porque se omite del snapshot para ahorrar tamaño)
  local.productos = local.productos.map(lp => {
    const extP = (ext.productos||[]).find(ep => String(ep.id) === String(lp.id));
    if (extP && !lp.img && extP.img) return { ...lp, img: extP.img };
    return lp; // mantener imagen local si ya la tiene
  });
...
// =====================================================================
//  ⚡ SUPABASE REALTIME — WebSocket push instantáneo entre dispositivos
//  Fallback automático a polling cada 3s si Realtime no está disponible
// =====================================================================
let _realtimeChannel  = null;
let _realtimeActivo   = false;
let _pollingTimer     = null;   // fallback si Realtime falla
let _supabaseClient   = null;   // instancia del SDK

function _getSupabaseClient() {
  if (_supabaseClient) return _supabaseClient;
  // El SDK global lo expone como window.supabase (UMD build)
  ...
  try {
    ...
    console.warn('[Realtime] No se pudo crear cliente Supabase:', e.message);
    return null;
  }
}

// ── Iniciar Realtime (con fallback a polling) ─────────────────────────
function _iniciarPolling() {
  // Intentar Realtime primero
  _iniciarRealtime();
  // Polling como red de seguridad: corre cada 15s solo si Realtime no está activo
  // (Realtime cubre el caso normal; el polling cubre fallos de WebSocket)
  if (_pollingTimer) return;
  _pollingTimer = setInterval(() => {
    if (!_realtimeActivo) {
      // Realtime no funciona → polling activo
      _autoFusionar();
    }
    // Si Realtime está activo el polling no hace nada (no desperdicia recursos)
  }, 15000);
  console.log('[Sync] Realtime + polling fallback (15s) iniciados');
}
...
function _detenerPolling() {
  ...
  console.log('[Sync] Realtime + polling detenidos');
}

// ── Suscripción Realtime a sync_snapshots ────────────────────────────
function _iniciarRealtime() {
  if (_realtimeChannel) return; // ya suscrito
  const client = _getSupabaseClient();
  if (!client) {
    console.warn('[Realtime] SDK no disponible — usando solo polling');
  ...
  try {
    _realtimeChannel = client
      .channel('sync_' + tiendaId, {
        config: { broadcast: { self: false } }
      })
      // ── 1. Snapshots: cambios de dispositivos completos ──────────────
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'sync_snapshots',
        filter: 'tienda_id=eq.' + tiendaId
      }, (payload) => {
        const rec = payload.new || payload.old || {};
        if (rec.dispositivo_id === _dispositivoId) return;
        if (rec.dispositivo_id === 'fusion') return;
        console.log('[Realtime] 📡 Snapshot de', rec.dispositivo_id, '→ fusionando');
        _dot('yellow');
        setTimeout(_autoFusionar, 200);
      })
      // ── 2. Ventas: nueva venta de otra caja → aplicar al instante ───
      .on('postgres_changes', {
        event: 'INSERT', schema: 'public', table: 'ventas'
      }, (payload) => {
        const v = payload.new;
        if (!v) return;
        // Ignorar si ya la tenemos (nuestra propia venta)
        if ((historial||[]).some(h => String(h.id) === String(v.id))) return;

        // Parsear items primero para verificar si esta venta es de nuestra tienda
        let items = [];
        if (v.items_json) {
        ...
        // Filtrar ventas de otras tiendas: si NINGÚN item coincide con nuestros productos → ignorar
        // (ventas de otras tiendas tendrán IDs de productos que no existen en nuestro inventario)
        const nuestrosIds = new Set((productos||[]).map(p => String(p.id)));
        const esDeNuestraTienda = items.length === 0 || items.some(it => it.id && nuestrosIds.has(String(it.id)));
        if (!esDeNuestraTienda) return;

        console.log('[Realtime] 🧾 Venta nueva de otra caja:', v.id, '$'+v.total);
        _dot('yellow');

        const fechaISO = v.fecha_iso || new Date().toISOString();
        const nuevaVenta = { id: v.id, fecha: fechaISO, fechaISO, ts: Date.parse(fechaISO)||0,
          total: Number(v.total)||0, pago: Number(v.pago)||0, vuelto: Number(v.vuelto)||0,
          items, items_json: v.items_json||null };
        historial = [nuevaVenta, ...(historial||[])];

        // Recalcular reportes con la nueva venta
      ...
      // ── 3. Productos: stock o imagen cambiada por otra caja ─────────
      .on('postgres_changes', {
        ...
        if (!sp) return;
        // Extraer id local (quitar prefijo tienda)
        const raw = String(sp.id||'');
        const ul = raw.lastIndexOf('_');
        const localId = ul >= 0 ? (Number(raw.slice(ul+1)) || raw.slice(ul+1)) : raw;
        const p = (productos||[]).find(x => String(x.id) === String(localId));
        if (!p) return;
        let changed = false;
        if (typeof sp.stock === 'number' && sp.stock !== p.stock) { p.stock = sp.stock; changed = true; }
        // Sync de imagen: si viene con img (puede ser null para borrado)
        if ('img' in sp && sp.img !== p.img) {
          p.img = sp.img || null;
          changed = true;
          console.log('[Realtime] 🖼 Imagen actualizada para prod', localId);
        }
        if (changed) {
          idbSet('vpos_productos', productos).catch(()=>{});
          if (typeof actualizarTodo === 'function') actualizarTodo();
      ...
      .subscribe((status, err) => {
        if (status === 'SUBSCRIBED') {
          ...
          console.log('[Realtime] ✅ WebSocket activo — sync instantáneo entre cajas');
          _dot('green');
        } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          _realtimeActivo = false;
          _realtimeChannel = null;
          console.warn('[Realtime] ⚠', status, '— fallback polling 3s, reintento en 10s');
          if (!_pollingTimer) _pollingTimer = setInterval(_autoFusionar, 3000);
          setTimeout(() => { if (_sesionActiva && !_realtimeActivo) _iniciarRealtime(); }, 10000);
        } else if (status === 'CLOSED') {
          _realtimeActivo = false;
          _realtimeChannel = null;
          if (_sesionActiva) setTimeout(() => { if (_sesionActiva && !_realtimeActivo) _iniciarRealtime(); }, 5000);
        }
      });
  } catch(e) {
    console.warn('[Realtime] Error al suscribir:', e.message);
    _realtimeChannel = null;
    if (!_pollingTimer) _pollingTimer = setInterval(_autoFusionar, 3000);
  }
}

async function _pollNuevosDatos() { await _autoFusionar(); } // alias por compatibilidad
...
async function _subirStockBase(){
  ...
  // Send products WITHOUT img first (fast batch), then img individually (avoids size issues)
  const rows=productos.map(p=>({
    ...
    cod:p.cod||'',abrev:p.abrev||'',
    // img intentionally OMITTED from batch — Supabase upsert preserves existing img value when field is absent
    paquetes:p.paquetes||[],lotes:p.lotes||[]
  }));
  for(let i=0;i<rows.length;i+=50) await _sbPost('productos',rows.slice(i,i+50),true);
  // Send img individually for products that have one (avoids batch size limits)
  for(const p of productos){
    if(!p.img) continue;
    const rid=tid+'_'+String(p.id);
    try{
      const compImg = await _comprimirImg(p.img, 380, 0.82);
      await _sbPost('productos',{id:rid,tienda_id:tid,img:compImg},true);
    }catch(e){console.warn('[imgSync] prod '+p.id+':',e.message);}
  }
}

// ── Sync just the img of ONE product immediately after save ──
async function syncImgProducto(prod) {
  if (!prod || !_sbUrl() || !_sbKey()) return;
  const tid = _getTiendaId() || 'sin_tienda';
  const rid = tid + '_' + String(prod.id);
  try {
    if (prod.img) {
      // Tiene imagen → subir comprimida
      const compImg = await _comprimirImg(prod.img, 380, 0.82);
      await _sbPost('productos', { id: rid, tienda_id: tid, img: compImg }, true);
      console.log('[imgSync] ✅ img subida para prod', prod.id, '~'+ Math.round(compImg.length*0.75/1024)+'KB');
    } else {
      // Imagen borrada → poner null en Supabase para que otros dispositivos no la traigan
      await _sbPatch('productos', { id: 'eq.' + rid }, { img: null });
      console.log('[imgSync] 🗑 img borrada en Supabase para prod', prod.id);
    }
  } catch(e) { console.warn('[imgSync] ❌ prod', prod.id, ':', e.message); }
}
