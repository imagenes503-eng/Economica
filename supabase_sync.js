...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    } else if (ventas !== null && ventas.length === 0) {
      // FIX BUG 3: Supabase devolvió array vacío → el historial fue borrado en otro teléfono.
      // Limpiar local para que ambos teléfonos queden sincronizados.
      historial = []; ventasDia = {}; ventasSem = {}; ventasMes = {};
      idbSet('vpos_historial', historial).catch(() => {});
      idbSet('vpos_ventasDia', ventasDia).catch(() => {});
      idbSet('vpos_ventasSem', ventasSem).catch(() => {});
      idbSet('vpos_ventasMes', ventasMes).catch(() => {});
      console.log('[AutoCarga] 🗑 Historial vacío en Supabase → local limpiado');
    }
    ...
    if (vd && vd.length > 0) {
      ...
      // FIX BUG 4: Supabase devolvió array vacío → ventas_diarias fueron borradas en otro teléfono.
      ventasDiarias = [];
      idbSet('vpos_ventasDiarias', ventasDiarias).catch(() => {});
      console.log('[AutoCarga] 🗑 ventasDiarias vacías en Supabase → local limpiado');
    }
...
function _iniciarRealtime() {
  ...
  try {
    _realtimeChannel = client
      ...
      // BROADCAST: ventas_dia_guardada — FIX BUG 1: una venta diaria fue registrada/actualizada
      .on('broadcast', { event: 'ventas_dia_guardada' }, ({ payload }) => {
        if (!payload || !payload.fecha) return;
        const idx = (ventasDiarias||[]).findIndex(v => v.fecha === payload.fecha);
        if (idx >= 0) {
          ventasDiarias[idx].monto = Number(payload.monto) || 0;
          if (payload.nota !== undefined) ventasDiarias[idx].nota = payload.nota || '';
        } else {
          if (!ventasDiarias) ventasDiarias = [];
          ventasDiarias.push({ fecha: payload.fecha, monto: Number(payload.monto) || 0, nota: payload.nota || '' });
        }
        ventasDiarias.sort((a, b) => a.fecha.localeCompare(b.fecha));
        idbSet('vpos_ventasDiarias', ventasDiarias).catch(()=>{});
        if (typeof _recalcularReportesDesdeHistorial === 'function') _recalcularReportesDesdeHistorial();
        ...
        if (typeof actualizarTodo === 'function') actualizarTodo();
        _dot('green');
        console.log('[Realtime] 📅 Venta diaria guardada desde otro teléfono:', payload.fecha, '$'+payload.monto);
      })
