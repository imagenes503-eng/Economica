...
async function _subirStockBase(){
  ...
  const rows=productos.map(p=>({
    ...
    cod:p.cod||'',abrev:p.abrev||'',img:null, // img sent separately below
    paquetes:p.paquetes||[],lotes:p.lotes||[]
  }));
  for(let i=0;i<rows.length;i+=50) await _sbPost('productos',rows.slice(i,i+50),true);
  // Now send img for each product that has one — one by one to avoid batch size issues
  for(const p of productos){
    if(!p.img) continue;
    const rid=tid+'_'+String(p.id);
    try{
      // Compress img to max 100KB base64 before sending
      const compImg = await _comprimirImg(p.img, 350, 0.78);
