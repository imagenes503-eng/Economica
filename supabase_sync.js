...
async function intentarLogin() {
  ...
  try {
    ...
    if (usRows && usRows.length > 0) {
      // Usuario existe — verificar PIN y leer rol fresco de Supabase
      if (usRows[0].pin !== pin) { _mostrarLoginError('PIN incorrecto'); btn.disabled = false; btn.textContent = 'Entrar'; return; }
      usuarioData = usRows[0]; // rol viene directo de Supabase — siempre actualizado
    } else {
    ...
    // Guardar sesión — siempre sobreescribir localStorage con datos frescos de Supabase
    ...
    console.log('[Login] Usuario:', usuarioData.nombre, '| Rol:', usuarioData.rol);

    // Si la tienda tiene URL+Key guardadas en Supabase, cargarlas automáticamente
    // Útil para teléfonos nuevos que solo ingresaron ID y PIN
    try {
      const sesRows = await _sbGet('sesiones', { select: 'sb_url,sb_key', tienda_id: 'eq.' + tiendaId });
      if (sesRows && sesRows.length > 0 && sesRows[0].sb_url && sesRows[0].sb_key) {
        const sbUrl = sesRows[0].sb_url;
        const sbKey = sesRows[0].sb_key;
        // Solo guardar si no hay URL configurada ya (no sobreescribir configuración existente)
        if (!localStorage.getItem('vpos_supabaseUrl')) {
          localStorage.setItem('vpos_supabaseUrl', sbUrl);
          localStorage.setItem('vpos_supabaseKey', sbKey);
          actualizarBadgeSheets();
        }
      }
    } catch(e) { console.warn('[loadSbConfig]', e.message); }
...
function _actualizarBadgeLogin() {
  ...
  // El botón de sesión siempre muestra "Sync: tienda" cuando hay sesión activa
...
async function restaurarSesion() {
  if (localStorage.getItem('vpos_sesionActiva') === '1') {
    ...
    _sesionActiva = true;
    const usuarioGuardado = localStorage.getItem('vpos_usuario');
    // SIEMPRE leer el rol fresco desde Supabase — no confiar en localStorage
    if (usuarioGuardado && _sbUrl() && _sbKey()) {
      try {
        const rows = await _sbGet('usuarios', {
          select: '*', tienda_id: 'eq.' + _tiendaId,
          usuario: 'eq.' + usuarioGuardado, activo: 'eq.true'
        });
        if (rows && rows.length > 0) {
          _usuarioActual = rows[0];
        ...
        if (savedUser) { try { _usuarioActual = JSON.parse(savedUser); } catch(e2) {} }
      }
    } else {
      const savedUser = localStorage.getItem('vpos_usuarioData');
      if (savedUser) { try { _usuarioActual = JSON.parse(savedUser); } catch(e) {} }
    }
    _actualizarBadgeLogin();
...
async function _refrescarRolDesdeSupabase() {
  if (!_sbUrl() || !_sbKey() || !_tiendaId || !_usuarioActual) return;
  try {
    const rows = await _sbGet('usuarios', {
      select: '*',
      tienda_id: 'eq.' + _tiendaId,
      usuario: 'eq.' + _usuarioActual.usuario,
      activo: 'eq.true'
    });
    if (rows && rows.length > 0) {
      const rolAnterior = _usuarioActual.rol;
      _usuarioActual = rows[0];
      localStorage.setItem('vpos_usuarioData', JSON.stringify(_usuarioActual));
      if (rolAnterior !== _usuarioActual.rol) {
        // El rol cambió — re-aplicar restricciones
        _quitarRestriccionesPorRol();
        _aplicarRestriccionesPorRol();
        _actualizarBadgeLogin();
        toast('Rol actualizado: ' + (ROLES[_usuarioActual.rol]?.label || _usuarioActual.rol));
      }
    }
  } catch(e) { console.warn('[refrescarRol]', e.message); }
}
...
function _aplicarRestriccionesPorRol() {
  ...
  console.log('[Rol] Aplicando restricciones para rol:', _usuarioActual.rol, 'usuario:', _usuarioActual.nombre);
...
function _mostrarBadgeRol() {
  // Badge removido — el rol ya se muestra en el botón de sesión
}
...
function guardarConfigSheets() {
  ...
  // Guardar URL+Key en Supabase para que otros telefonos no tengan que ingresarlas
  if (url && key && _getTiendaId()) {
    _sbPost('sesiones', { tienda_id: _getTiendaId(), sb_url: url, sb_key: key }, true).catch(()=>{});
  }
}
...
async function iniciarAutoSync() { await restaurarSesion(); }  // Sin auto-poll — todo es manual
