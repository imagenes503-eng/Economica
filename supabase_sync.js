...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    if (prods && prods.length > 0) {
      ...
      // STOCK CALCULADO: llamar RPC de Supabase para obtener stock real
      // Formula: stock_base - SUM(ventas de todos los dispositivos)
      (async function() {
        try {
          var _tid = _getTiendaId();
          if (!_tid || !_hayConexion()) return;
          var _stockRows = await _sbRpc('get_stock_calculado', {p_tienda_id: _tid});
          if (!_stockRows || !_stockRows.length) return;
          var _stockMap = {};
          for (var _si = 0; _si < _stockRows.length; _si++) {
            var _row = _stockRows[_si];
            var _rawId = String(_row.prod_id || '');
            var _u = _rawId.lastIndexOf('_');
            var _numId = _u >= 0 ? _rawId.slice(_u + 1) : _rawId;
            var _n = Number(_numId);
            _stockMap[isNaN(_n) ? _rawId : String(_n)] = Number(_row.stock_actual) || 0;
          }
          var _changed = false;
          for (var _pi = 0; _pi < productos.length; _pi++) {
            var _p = productos[_pi];
            var _ns = _stockMap[String(_p.id)];
            if (_ns !== undefined && _ns !== _p.stock) {
              _p.stock = _ns;
              _changed = true;
              if (typeof actualizarStockFila === 'function') actualizarStockFila(_p);
            }
          }
          if (_changed) {
            idbSet('vpos_productos', productos).catch(function(){});
            console.log('[StockCalc] Stock actualizado desde Supabase');
          }
        } catch(_e) {
          console.warn('[StockCalc] Error:', _e.message);
        }
      })();
...
function _aplicarVentaRealtime(v) {
  ...
  (function() {
    ...
    if (stockCambio) idbSet('vpos_productos', productos).catch(function(){});
    // Confirmar con RPC de Supabase (stock real = stock_base - todas las ventas)
    setTimeout(function() {
      try {
        var _tid = _getTiendaId();
        if (!_tid || !_hayConexion()) return;
        _sbRpc('get_stock_calculado', {p_tienda_id: _tid}).then(function(_rows) {
          if (!_rows) return;
          var _map = {};
          for (var _i = 0; _i < _rows.length; _i++) {
            var _raw = String(_rows[_i].prod_id || '');
            var _u = _raw.lastIndexOf('_');
            var _n = Number(_u >= 0 ? _raw.slice(_u+1) : _raw);
            _map[isNaN(_n) ? _raw : String(_n)] = Number(_rows[_i].stock_actual) || 0;
          }
          for (var _j = 0; _j < productos.length; _j++) {
            var _ns = _map[String(productos[_j].id)];
            if (_ns !== undefined) { productos[_j].stock = _ns; if (typeof actualizarStockFila === 'function') actualizarStockFila(productos[_j]); }
          }
          idbSet('vpos_productos', productos).catch(function(){});
        }).catch(function(){});
      } catch(_e) {}
...
function _calcStockBase(p){const pid=String(p.id);let v=0;(historial||[]).forEach(h=>{(h.items||[]).forEach(it=>{if(String(it.id)===pid)v+=Number(it.cant||0);});});var calc=(p.stock||0)+v;return Math.max(calc,Number(p.stock_base)||0);}
