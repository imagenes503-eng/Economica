  ...
  try {
    ...
    if (cfg && cfg.length > 0) {
      cfg.forEach(row => {
        ...
        if (row.clave === 'inventarioCosto') {
          inventarioCosto = parseFloat(row.valor) || 0;
          idbSet('vpos_inventarioCosto', inventarioCosto).catch(() => {});
          const elC = document.getElementById('inpInventarioCosto');
          if (elC) elC.value = inventarioCosto > 0 ? inventarioCosto.toFixed(2) : '';
        }
        if (row.clave === 'stockInicialSnap' && row.valor) {
          try {
            const snap = JSON.parse(row.valor);
            window._stockInicialSnap = snap;
            idbSet('vpos_stockInicialSnap', snap).catch(() => {});
            if (typeof piRender === 'function') piRender();
          } catch(e) {}
        }
      });
    }
...
async function _aplicarDatos(datos) {
  ...
  // Sincronizar stockInicialSnap si viene en el snapshot
  if (datos.stockInicialSnap && typeof datos.stockInicialSnap === 'object') {
    window._stockInicialSnap = datos.stockInicialSnap;
    idbSet('vpos_stockInicialSnap', datos.stockInicialSnap).catch(()=>{});
    if (typeof piRender === 'function') piRender();
  }
...
async function _ejecutarSync(){
  ...
  try{
    for(const tipo of queue){
      ...
      if(tipo==='stockInicial'||tipo==='todo') await _subirStockInicial();
    }
    localStorage.setItem('vpos_ultimoSync',new Date().toISOString());
...
async function _autoEnviarSnapshot() {
  ...
  const snap = {
    ...
    datos: JSON.stringify({
      ...
      stockInicialSnap: window._stockInicialSnap || null
    }),
    created_at: new Date().toISOString()
  };
...
let _broadcastQueue = []; // cola para cuando el canal no está listo
function _broadcast(evento, datos) {
  if (!_realtimeChannel || !_realtimeActivo) {
    // Guardar en cola, máximo 20 mensajes, deduplicar por evento+fecha
    const key = evento + (datos?.fecha||'');
    _broadcastQueue = _broadcastQueue.filter(q => (q.evento + (q.datos?.fecha||'')) !== key);
    _broadcastQueue.push({ evento, datos, ts: Date.now() });
    if (_broadcastQueue.length > 20) _broadcastQueue.shift();
    return;
  }
  // Vaciar cola primero si hay mensajes pendientes
  if (_broadcastQueue.length > 0) {
    const pending = _broadcastQueue.splice(0);
    pending.forEach(q => {
      try { _realtimeChannel.send({ type: 'broadcast', event: q.evento, payload: q.datos }); } catch(e) {}
    });
  }
  try {
    ...
    _realtimeChannel = client
      ...
      .on('postgres_changes',
        ...
        (change) => {
          ...
          if (row.compra !== undefined) productos[idx].compra = Number(Number(row.compra).toFixed(2)) || 0;
          if (row.venta  !== undefined) productos[idx].venta  = Number(Number(row.venta).toFixed(2))  || 0;
      ...
      // cierre_estado: gastos, venta, cambios del día
      .on('broadcast', { event: 'cierre_estado' }, ({ payload }) => {
        if (typeof window._cdRecibirBroadcast === 'function')
          window._cdRecibirBroadcast('cierre_estado', payload);
      })

      // cierre_saldo: saldo en caja
      .on('broadcast', { event: 'cierre_saldo' }, ({ payload }) => {
        if (typeof window._cdRecibirBroadcast === 'function')
          window._cdRecibirBroadcast('cierre_saldo', payload);
      })

      // cierre_cajaayer: caja día de ayer
      .on('broadcast', { event: 'cierre_cajaayer' }, ({ payload }) => {
        if (typeof window._cdRecibirBroadcast === 'function')
          window._cdRecibirBroadcast('cierre_cajaayer', payload);
      })

      // cierre_venta: venta del día aplicada
      .on('broadcast', { event: 'cierre_venta' }, ({ payload }) => {
        if (typeof window._cdRecibirBroadcast === 'function')
          window._cdRecibirBroadcast('cierre_venta', payload);
      })

      // cierre_gastos: gastos del día
      .on('broadcast', { event: 'cierre_gastos' }, ({ payload }) => {
        if (typeof window._cdRecibirBroadcast === 'function')
          window._cdRecibirBroadcast('cierre_gastos', payload);
      })

      // cierre_cambios: cambios del día
      .on('broadcast', { event: 'cierre_cambios' }, ({ payload }) => {
        if (typeof window._cdRecibirBroadcast === 'function')
          window._cdRecibirBroadcast('cierre_cambios', payload);
      })

      // cierre_borrado: borrar datos del cierre
      .on('broadcast', { event: 'cierre_borrado' }, ({ payload }) => {
        if (typeof window._cdRecibirBroadcast === 'function')
          window._cdRecibirBroadcast('cierre_borrado', payload);
      })
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          _dot('green');
          // Vaciar cola de broadcasts pendientes que se acumularon antes de conectar
          if (_broadcastQueue.length > 0) {
            const pending = _broadcastQueue.splice(0);
            setTimeout(() => {
              pending.forEach(q => {
...
async function _subirStockBase(){
  ...
  const rowsCompleto = productos.map(p=>({
    ...
    nom:p.nom||'',cat:p.cat||'',
    compra:Number(Number(p.compra).toFixed(2))||0,venta:Number(Number(p.venta).toFixed(2))||0,
    stock_base:_calcStockBase(p),stock:Number(p.stock)||0,min:Number(p.min)||0,
    cod:p.cod||'',abrev:p.abrev||'',
    paquetes:p.paquetes||[],lotes:p.lotes||[],
    costoUnidadEntrado:Number(Number(p.costoUnidadEntrado||0).toFixed(2))||0,ventaUnidadEntrada:Number(Number(p.ventaUnidadEntrada||0).toFixed(2))||0,
    _ts:Number(p._ts)||0
  }));
  ...
  try {
    if (prod.img) {
      ...
      await _sbPost('productos', {
        ...
        compra: Number(Number(prod.compra).toFixed(2)) || 0,
        venta:  Number(Number(prod.venta).toFixed(2))  || 0,
...
async function _subirStockInicial(){
  ...
  try{
    await _sbPost('config',{
      clave:'stockInicialSnap',
      tienda_id:tid,
      valor:JSON.stringify(snap),
      updated_at:new Date().toISOString()
    },true);
  }catch(e){console.warn('[StockInicial]',e.message);}
}
async function _bajarStockInicial(){
  const tid=_getTiendaId();if(!tid)return;
  try{
    const rows=await _sbGet('config',{select:'valor',clave:'eq.stockInicialSnap',tienda_id:'eq.'+tid});
    if(rows&&rows.length&&rows[0].valor){
      const snap=JSON.parse(rows[0].valor);
      window._stockInicialSnap=snap;
      if(typeof idbSet==='function') idbSet('vpos_stockInicialSnap',snap).catch(()=>{});
      if(typeof piRender==='function') piRender();
    }
  }catch(e){console.warn('[StockInicial bajar]',e.message);}
}
