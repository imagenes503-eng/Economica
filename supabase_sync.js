...
function _aplicarRestriccionesPorRol() {
  ...
  // SUPER ADMIN: ve absolutamente todo sin restricciones
  if (esSuperAdmin) {
    _actualizarBadgeRol();
    return;
  }
  ...
  // SECCIONES OCULTAS PARA TODOS (excepto Super Admin)
  const pgsRestringidas = ['pgDestacados', 'pgSync', 'pgFinanzasMes', 'pgCierreDia'];
  pgsRestringidas.forEach(id => {
    const el = document.getElementById(id);
    if (el) el.style.display = 'none';
  });
  document.querySelectorAll('.nav-tab, .drawer-nav-item').forEach(tab => {
    const onclick = tab.getAttribute('onclick') || '';
    if (pgsRestringidas.some(p => onclick.includes(p))) tab.style.display = 'none';
  });

  // Ocultar Backup, Restaurar, Fusionar para todos los no-superadmin
  document.querySelectorAll('.btn-backup, .btn-restore').forEach(btn => {
    const oc  = btn.getAttribute('onclick') || '';
    const txt = (btn.textContent || '').trim();
    if (
      oc.includes('abrirBackupPanel') ||
      oc.includes('inputImportar')    ||
      oc.includes('inputFusionar')    ||
      txt.includes('Backup')          ||
      txt.includes('Restaurar')       ||
      txt.includes('Fusionar')
    ) {
      btn.style.display = 'none';
    }
  });
  ['inputImportar', 'inputFusionar'].forEach(id => {
    const el = document.getElementById(id); if (el) el.style.display = 'none';
  });

  // CAJERO: restricciones adicionales
  if (esCajero) {
    ['pgReportes', 'pgInventario'].forEach(id => {
      const el = document.getElementById(id); if (el) el.style.display = 'none';
    });
    document.querySelectorAll('.nav-tab, .drawer-nav-item').forEach(tab => {
      const onclick = tab.getAttribute('onclick') || '';
      if (['pgReportes','pgInventario'].some(p => onclick.includes(p))) tab.style.display = 'none';
    });
...
async function renderAdminPanel() {
  // Si aún no hay usuario cargado, esperar hasta 2s y reintentar
  if (!_usuarioActual) {
    let intentos = 0;
    await new Promise(resolve => {
      const t = setInterval(() => {
        intentos++;
        if (_usuarioActual || intentos > 20) { clearInterval(t); resolve(); }
      }, 100);
    });
  }
  if (!_esSuperAdmin()) {
