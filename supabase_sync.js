      ...
      const merged = sbProds.map(sp => {
        ...
        if (local) {
          ...
          if (sbTs > localTs) {
            return {
              ...sp,
              // Preservar cod de Supabase (puede haber sido actualizado en otro dispositivo)
              cod:      sp.cod      || local.cod || '',
              stock:    Math.max(Number(sp.stock) || 0, Number(local.stock) || 0),
              img:      local.img || sp.img || null,
...
function _iniciarRealtime() {
  ...
  if (!client) {
    console.warn('[Realtime] SDK no disponible — polling 30s');
    if (!_pollingTimer) _pollingTimer = setInterval(_autoFusionar, 30000);
    return;
  }
