...
async function _sbGet(tabla, params = {}) {
  ...
  if (!url || !key) throw new Error('Sin configuración');

  const tiendaId = _getTiendaId();

  // 🔴 FORZAR tienda_id SIEMPRE
  if (tabla !== 'config' && tabla !== 'deleted_log') {
    params.tienda_id = 'eq.' + tiendaId;
  }
...
async function _sbDeleteFiltro(tabla, filtro) {
  ...
  filtro.tienda_id = 'eq.' + tiendaId;

  const qs = '?' + new URLSearchParams(filtro).toString();

  await fetch(_sbUrl() + '/rest/v1/' + tabla + qs, {
    method: 'DELETE',
    headers: _headers()
  });
}
...
async function intentarLogin() {
  ...
  try {
    if (esRegistrar) {
      ...
      // ── Verificar si el correo ya tiene un perfil activo ──
      ...
      if (perfilesExistentes && perfilesExistentes.length > 0) {
        ...
        // Perfil inactivo — borrarlo para permitir re-registro limpio
        for (const p of perfilesExistentes) {
          await _sbDeleteFiltro('perfiles', { id: 'eq.' + p.id }).catch(() => {});
        }
      }

      let authData;
      try {
        authData = await _authSignUp(email, password);
      } catch(signUpErr) {
        // Si el error es que el email ya existe en Auth pero no en perfiles,
        // intentar login directo para recuperar el userId y continuar
        const msg = (signUpErr.message || '').toLowerCase();
        if (msg.includes('already registered') || msg.includes('already been registered') || msg.includes('user already')) {
          try {
            authData = await _authSignIn(email, password);
          } catch(signInErr) {
            _mostrarLoginError('Este correo ya está registrado pero la contraseña no coincide. Usá "Entrar" o contactá al administrador.');
            btn.disabled = false; btn.textContent = '✅ Crear cuenta'; return;
          }
        } else {
          throw signUpErr;
      ...
      const userId   = authData.user?.id || authData.access_token && (await _sbGet('perfiles',{select:'id',email:'eq.'+email}).catch(()=>[]))[0]?.id || authData.id;
