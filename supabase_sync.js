...
let _reconcTimer = null; // timer de pull de ventas nuevas

// ── Pull de ventas nuevas (el motor de sincronización más confiable) ──────
// Corre cada 20s. Solo lee las 50 ventas más recientes de Supabase y añade
// las que aún no están en el historial local. No reemplaza datos, solo suma.
// Respeta los tombstones (cobrosEliminados, historialWipeTs).
async function _pullNuevasVentas() {
  if (!_sesionActiva || !_sbUrl() || !_sbKey()) return;
  try {
    const ventas = await _sbGet('ventas', {
    ...
    const _wipeMs = localStorage.getItem('vpos_historialWipeTs')
      ? Date.parse(localStorage.getItem('vpos_historialWipeTs')) : 0;
    const _elimSet = new Set((typeof cobrosEliminados !== 'undefined' ? cobrosEliminados : []).map(String));
    const _localIds = new Set((historial || []).map(v => String(v.id)));

    let _nuevos = 0;
    ventas.forEach(v => {
      const id = String(v.id);
      if (_localIds.has(id)) return;      // ya existe localmente
      if (_elimSet.has(id)) return;       // fue eliminado por devolución
      const ts = Date.parse(v.fecha_iso || '') || 0;
      if (_wipeMs > 0 && ts < _wipeMs) return; // anterior al borrado masivo

      // Parsear items
      let items = [];
      if (v.items_json) {
        try {
          const p = typeof v.items_json === 'string' ? JSON.parse(v.items_json) : v.items_json;
      ...
      if (!items.length && v.items && typeof v.items === 'string') {
        items = v.items.split('|').map(s => s.trim()).filter(Boolean).map(s => {
          ...
          if (!m) return null;
          const nom = m[2].trim();
          const prod = (typeof productos !== 'undefined' ? productos : []).find(x => x.nom === nom);
          return { id: prod ? String(prod.id) : null, nom, cant: Number(m[1]) || 1,
                   precio: prod ? prod.venta : 0, cat: prod ? prod.cat : '' };
        }).filter(Boolean);
      }

      const fechaISO = v.fecha_iso || new Date().toISOString();
      const fd = new Date(fechaISO);
      const fechaStr = isNaN(fd.getTime()) ? '—' : fd.toLocaleString('es-SV');

      historial.push({
        id: v.id, fecha: fechaISO, fechaISO, fechaStr, ts,
    ...
    if (_nuevos > 0) {
      ...
      if (typeof actualizarTodo === 'function') actualizarTodo();
      _dot('green');
      console.log('[PullVentas] ✅', _nuevos, 'venta(s) nueva(s) recibida(s) desde Supabase');
    }
  } catch(e) {
    console.warn('[PullVentas]', e.message);
  }
}
window._pullNuevasVentas = _pullNuevasVentas;

function _iniciarPolling() {
  _iniciarRealtime();
  // FIX: NO hacer return temprano — siempre verificar AMBOS timers independientemente
  if (!_pollingTimer) {
    _pollingTimer = setInterval(_autoFusionar, 30000);
    console.log('[Sync] Polling snapshots iniciado (30s)');
  }
  if (!_reconcTimer) {
    // Pull directo de tabla ventas cada 20s — el mecanismo más confiable
    _reconcTimer = setInterval(_pullNuevasVentas, 20000);
    // Ejecutar inmediatamente también (sin esperar 20s)
    setTimeout(_pullNuevasVentas, 2000);
    console.log('[Sync] Pull ventas iniciado (20s) + ejecución inmediata en 2s');
  }
}
...
function _detenerPolling() {
  ...
  if (_reconcTimer)  { clearInterval(_reconcTimer);  _reconcTimer  = null; }
}
...
function _iniciarRealtime() {
  ...
  try {
    _realtimeChannel = client
      ...
      // BROADCAST: snapshot_push — señal para jalar ventas nuevas de Supabase
      .on('broadcast', { event: 'snapshot_push' }, ({ payload }) => {
        if (!payload || payload.tienda !== _getTiendaId()) return;
        console.log('[Realtime] 📥 snapshot_push → jalando ventas nuevas de Supabase');
        // BUGFIX: usar _pullNuevasVentas (liviano) en vez de _autoCargarDesdeSupa (pesado)
        setTimeout(() => _pullNuevasVentas(), 400);
      })

      // postgres_changes: snapshot completo al reconectar (respaldo)
      // ── postgres_changes: venta nueva insertada en Supabase ──────────
      // Se dispara cuando registrarVentaAtomica() hace el INSERT en la tabla ventas.
      // Es el mecanismo más confiable: no depende del broadcast (que puede perderse).
      // REQUISITO: la tabla 'ventas' debe estar en la publicación de Supabase Realtime.
      // En el dashboard de Supabase: Database → Replication → supabase_realtime → añadir 'ventas'
      .on('postgres_changes', {
        ...
        if (!row.id) return;
        // Ignorar si es una venta que YA tenemos localmente (la hicimos nosotros)
        if ((historial||[]).some(h => String(h.id) === String(row.id))) return;
        console.log('[Realtime] 📥 postgres_changes INSERT ventas:', row.id, '$'+row.total);
        // Construir objeto venta desde la fila de Supabase
        let items = [];
        if (row.items_json) {
          try { const p = typeof row.items_json === 'string' ? JSON.parse(row.items_json) : row.items_json; if (Array.isArray(p)) items = p; } catch(e) {}
        }
        const ventaDB = {
          id: row.id,
          fechaISO: row.fecha_iso || new Date().toISOString(),
          items_json: row.items_json || null,
          total: row.total, pago: row.pago, vuelto: row.vuelto,
          items
        };
        _aplicarVentaRealtime(ventaDB);
      })
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          _dot('green');
          // Al conectar: fusionar snapshots Y jalar ventas nuevas inmediatamente
          setTimeout(_autoFusionar, 600);
          setTimeout(_pullNuevasVentas, 1500);
