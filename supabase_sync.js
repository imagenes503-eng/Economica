...
function _fusionarDos(local, ext) {
  ...
  // Filter out local historial entries older than the wipe timestamp
  if (_wipeMax > 0) local.historial = (local.historial||[]).filter(v => (v.ts||0) >= _wipeMax);
  const seenH=new Set((local.historial||[]).map(v=>v.id));
  // Only add external entries that postdate the wipe timestamp
  (ext.historial||[]).forEach(v=>{
    if (!seenH.has(v.id) && (_wipeMax === 0 || (v.ts||0) >= _wipeMax)) {
