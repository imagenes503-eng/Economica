...
async function _completarLogin(email, password, tiendaIdOverride) {
  ...
  if (!perfiles || !perfiles.length) throw new Error('Tu cuenta ha sido desactivada o no existe. Contacta al administrador.');
...
async function restaurarSesion() {
  ...
  try {
    ...
    _actualizarNombreTienda(); // restaurar nombre de la tienda
    // Si sesión restaurada correctamente, ocultar login
    const ml = document.getElementById('modalLogin');
    if (ml) ml.style.display = 'none';
    // Descargar datos actualizados de Supabase para que no se pierdan al recargar
    setTimeout(() => descargarDatosActualizados(), 800);
  } catch(e) {
