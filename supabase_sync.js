  ...
  try {
    ...
    if (prods && prods.length > 0) {
      ...
      const merged = sbProds.map(sp => {
        ...
        if (local) {
          // Si el producto local tiene datos básicos vacíos (nombre, precio),
          // significa que vino de un snapshot corrupto — usar datos de Supabase como base
          const localInvalido = !(local.nom || '').trim() || (!(local.venta) && !(local.compra));
          if (localInvalido) {
            // Producto en Supabase tiene datos reales — usarlos, preservar stock local si es mayor
            return {
              ...sp,
          ...
          // Datos locales válidos — conservarlos, Supabase aporta img si local no tiene
          return {
            ...local,
            img:      local.img      || sp.img      || null,
