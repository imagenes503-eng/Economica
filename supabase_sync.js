...
async function restaurarSesion() {
  ...
  try {
    ...
    // ── FIX: await para garantizar que la UI se renderiza con datos actualizados
    //         antes de que el usuario pueda interactuar con historial/venta día ──
    try {
      await _autoCargarDesdeSupa();
    } catch(eSupa) {
      console.warn('[restaurarSesion] _autoCargarDesdeSupa falló, usando caché:', eSupa.message);
    }
    // Re-renderizar la página activa con los datos frescos de Supabase
    if (typeof actualizarTodo === 'function') actualizarTodo();
    try {
      const pgActiva = document.querySelector('.page.active');
      if (pgActiva && typeof renderPagina === 'function') renderPagina(pgActiva.id);
    } catch(eRender) { /* ignorar errores de render */ }
