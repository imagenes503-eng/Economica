...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    if (prods && prods.length > 0) {
      ...
      // Recalcular inventarioInicial con los productos frescos de Supabase
      if (typeof actualizarInventarioInicialAuto === 'function') {
        actualizarInventarioInicialAuto();
      } else {
        // Fallback inline si la función no está disponible aún
        var _invSum = 0;
        for (var _ii = 0; _ii < productos.length; _ii++) {
          _invSum += (Number(productos[_ii].stock)||0) * (Number(productos[_ii].venta)||0);
        }
        if (typeof inventarioInicial !== 'undefined') {
          inventarioInicial = parseFloat(_invSum.toFixed(2));
          idbSet('vpos_inventarioInicial', inventarioInicial).catch(function(){});
        }
      }
...
async function _aplicarDatos(datos) {
  ...
  // Recalcular inventarioInicial desde productos frescos (SUM stock×precio_venta)
  if (typeof productos !== 'undefined' && productos.length) {
    var _invAuto = 0;
    for (var _i = 0; _i < productos.length; _i++) {
      _invAuto += (Number(productos[_i].stock) || 0) * (Number(productos[_i].venta) || 0);
    }
    if (typeof inventarioInicial !== 'undefined') {
      inventarioInicial = parseFloat(_invAuto.toFixed(2));
      if (typeof idbSet === 'function') idbSet('vpos_inventarioInicial', inventarioInicial).catch(function(){});
    }
  }
