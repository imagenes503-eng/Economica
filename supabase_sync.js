    ...
    if (prods && prods.length > 0) {
      ...
      const merged = sbProds.map(sp => {
        ...
        if (local) {
          ...
          // FIX: paquetes/lotes siempre se toman del que tenga datos,
          // priorizando LOCAL para evitar que las presentaciones desaparezcan
          // por race condition entre el upsert y la recarga inmediata.
          const mejoresPaquetes = (local.paquetes && local.paquetes.length)
            ? local.paquetes
            : (sp.paquetes && sp.paquetes.length ? sp.paquetes : []);
          const mejoresLotes = (local.lotes && local.lotes.length)
            ? local.lotes
            : (sp.lotes && sp.lotes.length ? sp.lotes : []);
          if (sbTs > localTs) {
            return {
              ...sp,
              stock:    Math.max(Number(sp.stock) || 0, Number(local.stock) || 0),
...
async function _aplicarDatos(datos) {
  ...
  productos=datos.productos||[]; historial=datos.historial||[]; pagos=datos.pagos||[];
    ...
    _realtimeChannel = client
      ...
      .on('postgres_changes',
        ...
        (change) => {
          ...
          if (row.paquetes) productos[idx].paquetes = row.paquetes;
