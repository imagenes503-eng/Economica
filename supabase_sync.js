...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    if (prods && prods.length > 0) {
      // Convertir formato Supabase → formato app
    ...
    _dot('green');
    console.log('[AutoCarga] Datos cargados desde Supabase:', { prods: prods?.length, ventas: ventas?.length });
  } catch(e) {
    console.warn('[AutoCarga] Error cargando de Supabase:', e.message);
  }
}
