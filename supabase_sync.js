...
function _iniciarRealtime() {
  ...
  if (!client) {
    ...
    // Activar polling agresivo como fallback
  ...
  try {
    _realtimeChannel = client
      ...
      .on('postgres_changes', {
        ...
        // Ignorar cambios que vienen de nuestro propio dispositivo
        const rec = payload.new || payload.old || {};
        if (rec.dispositivo_id === _dispositivoId) return;
        if (rec.dispositivo_id === 'fusion') return;
        console.log('[Realtime] 📡 Cambio recibido de', rec.dispositivo_id, '— fusionando…');
        _dot('yellow');
        // Pequeño delay para que Supabase termine la escritura antes de leer
        setTimeout(_autoFusionar, 300);
      })
      .subscribe((status, err) => {
        if (status === 'SUBSCRIBED') {
          _realtimeActivo = true;
          console.log('[Realtime] ✅ WebSocket conectado — sync instantáneo activo');
          _dot('green');
        } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          _realtimeActivo = false;
          console.warn('[Realtime] ⚠ Error WebSocket (' + status + ') — fallback a polling 3s');
          // Activar polling agresivo como fallback
          if (!_pollingTimer) _pollingTimer = setInterval(_autoFusionar, 3000);
