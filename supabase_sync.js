...
function _fusionarDos(local, ext) {
  ...
  // 2. Reportes: sumar SOLO las ventas que son exclusivas del externo
  // (ventas que no existen en el local — para no duplicar)
  // Construir reporte solo con cobros exclusivos del externo
  const repExtExclusivo = { dia: {}, sem: {}, mes: {} };
  (ext.historial || []).forEach(v => {
    if (!idsCobrosLocal.has(v.id)) {
      // Esta venta es exclusiva del externo — agregarla al reporte
      (v.items || []).forEach(it => {
        const pid = String(it.id || '');
        const total = Number(it.cant || 0) * Number(it.precio || 0);
        const cant  = Number(it.cant || 0);
        ['dia','sem','mes'].forEach(key => {
          if (!repExtExclusivo[key][pid]) repExtExclusivo[key][pid] = { id: pid, nom: it.nom||'', cat: it.cat||'', cant: 0, total: 0 };
          repExtExclusivo[key][pid].cant  += cant;
          repExtExclusivo[key][pid].total += total;
        });
      });
    }
  });
  local.ventasDia = _mergeRep(local.ventasDia || {}, repExtExclusivo.dia);
  local.ventasSem = _mergeRep(local.ventasSem || {}, repExtExclusivo.sem);
  local.ventasMes = _mergeRep(local.ventasMes || {}, repExtExclusivo.mes);
  ...
  // 5. Ventas diarias: unir, si misma fecha sumar
