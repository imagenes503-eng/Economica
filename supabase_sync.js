  ...
  try {
    ...
    if (prods && prods.length > 0) {
      const sbProds = prods.map(p => {
        ...
        const id = (()=>{ const raw=String(p.id||''); const u=raw.lastIndexOf('_'); const n=Number(u>=0?raw.slice(u+1):raw); return isNaN(n)?raw:n; })();
        const stock    = Number(p.stock)  || 0;
        const paquetes = p.paquetes || [];
        // CORRECCIÓN PRECISIÓN: la columna 'compra' en Supabase puede ser numeric/float
        // y truncar decimales (ej: 0.0775 → 0.078). Reconstruir desde costoTotal/stock
        // para recuperar el valor exacto cuando detectamos pérdida de precisión.
        let compra = Number(p.compra) || 0;
        if (p.costoTotal && stock > 0) {
          const compraRecalc = Number(p.costoTotal) / stock;
          if (Math.abs(compraRecalc - compra) > 0.0001) compra = compraRecalc;
        }
        return {
          id, nom: p.nom || '', cat: p.cat || '',
          compra, venta: Number(p.venta) || 0,
          stock, min: Number(p.min) || 0,
          cod: p.cod || '', abrev: p.abrev || '',
          img: p.img || null, paquetes, lotes: p.lotes || [],
...
function _fusionarDos(local, ext) {
  ...
  local.productos = local.productos.map(lp => {
    ...
    const mejoresPaquetes = lpPkLen > 0
      ? (extPkLen > lpPkLen && Number(extP._ts||0) > Number(lp._ts||0) ? extP.paquetes : lp.paquetes)
...
async function _aplicarDatos(datos) {
  ...
  productos = (datos.productos || []).map(p => {
    ...
    // Ganar el que tenga más paquetes; si igual, preferir local (más reciente en este dispositivo)
    let mejoresPk = localPk;
    if (snapPk && (!localPk || snapPk.length > localPk.length)) mejoresPk = snapPk;
