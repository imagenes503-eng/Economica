...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    if (prods && prods.length > 0) {
      // Hay datos en Supabase → son la fuente de verdad
      ...
      productos = prodsApp;
      // Actualizar caché IDB en paralelo (silencioso)
      idbSet('vpos_productos', productos).catch(() => {});
      console.log('[AutoCarga] Cargados', prods.length, 'productos de Supabase → caché IDB actualizado');
    } else if (typeof productos !== 'undefined' && productos && productos.length > 0) {
      // No hay datos en Supabase pero sí en caché local → subirlos como fuente de verdad
      console.log('[AutoCarga] Supabase vacío, subiendo', productos.length, 'productos desde caché local...');
      await _subirStockBase();
      console.log('[AutoCarga] Productos subidos a Supabase como fuente de verdad');
    }
    ...
    // Cargar restock log desde Supabase
    ...
    // Cargar ventas diarias desde Supabase
    ...
    // Cargar config (efectivoInicial, inventarioInicial) desde Supabase
    ...
    _dot('green');
    console.log('[AutoCarga] Supabase → fuente de verdad cargada. IDB actualizado como caché.');
  } catch(e) {
    console.warn('[AutoCarga] Sin conexión a Supabase, usando caché IDB:', e.message);
    _dot('red');
    // El caché IDB ya fue cargado por cargarDatos() al inicio — no hacer nada más
  }
}
  ...
  try{
    for(const tipo of queue){
      if(tipo==='venta'&&_lastVenta){
        ...
        await _sbPost('ventas',{id:v.id,fecha_iso:v.fecha||new Date().toISOString(),
          total:parseFloat(v.total)||0,pago:parseFloat(v.pago)||0,vuelto:parseFloat(v.vuelto)||0,
          items:v.items||'',items_json:v.items_json||null},true);
      }
