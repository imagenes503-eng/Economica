...
async function _finalizarLogin() {
  ...
  // Iniciar polling automático para recibir cambios de otros dispositivos
  _iniciarPolling();
}
...
async function cerrarSesion() {
  await _authSignOut();
  _detenerPolling(); // detener sincronización automática
...
async function restaurarSesion() {
  ...
  try {
    ...
    // Descargar datos frescos de Supabase al restaurar sesión (otro dispositivo puede tener cambios más recientes)
    _autoCargarDesdeSupa();
    // Iniciar polling automático para recibir cambios de otros dispositivos en tiempo real
    _iniciarPolling();
  } catch(e) {
...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    if (prods && prods.length > 0) {
      ...
      console.log('[AutoCarga] Productos:', prods.length);
...
async function _aplicarDatos(datos) {
  ...
  idbSetMany([['vpos_productos',productos],['vpos_ventasDia',ventasDia],['vpos_ventasSem',ventasSem],['vpos_ventasMes',ventasMes],['vpos_historial',historial],['vpos_pagos',pagos],['vpos_ventasDiarias',ventasDiarias],['vpos_restockLog',restockLog]]).catch(()=>{});
  actualizarTodo();
  ...
  try{
    for(const tipo of queue){
      if(tipo==='venta'&&_lastVenta){
        ...
        // Arreglar items: si es array, convertir a string y guardar items_json correctamente
    ...
    console.warn('[Sync]',e.message);_dot('red');
  }finally{
    _syncRunning=false;
...
// =====================================================================
//  🔄 POLLING AUTOMÁTICO — sincroniza con otros dispositivos cada 15s
//  Descarga solo ventas/pagos nuevos desde la última sincronización
// =====================================================================
let _pollingTimer = null;
let _ultimaFechaVenta = null;   // ISO — la venta más reciente que tenemos
let _ultimaFechaPago  = null;   // ISO — el pago más reciente que tenemos

function _iniciarPolling() {
  if (_pollingTimer) return; // ya corriendo
  _pollingTimer = setInterval(_pollNuevosDatos, 15000); // cada 15 segundos
  console.log('[Polling] Iniciado — sincronización automática cada 15s');
}
...
async function _pollNuevosDatos() {
  if (!_sesionActiva || !_sbUrl() || !_sbKey()) return;
  if (typeof _syncRunning !== 'undefined' && _syncRunning) return; // esperamos si hay un sync en curso
  try {
    let hayNuevos = false;

    // ── Ventas nuevas desde el último timestamp que tenemos ────────────
    const fechaCorteVenta = _ultimaFechaVenta ||
      (historial && historial.length ? (historial[0].fechaISO || historial[0].fecha) : null) ||
      new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(); // último día como fallback

    const params = { select: '*', order: 'fecha_iso.desc', limit: 50 };
    if (fechaCorteVenta) params['fecha_iso'] = 'gt.' + fechaCorteVenta;

    const nuevasVentas = await _sbGet('ventas', params).catch(() => null);
    if (nuevasVentas && nuevasVentas.length > 0) {
      const idsLocales = new Set((historial || []).map(v => String(v.id)));
      const ventasExternas = nuevasVentas.filter(v => !idsLocales.has(String(v.id)));

      if (ventasExternas.length > 0) {
        console.log('[Polling] 🆕', ventasExternas.length, 'ventas nuevas de otro dispositivo');
        ventasExternas.forEach(v => {
          // Parsear items_json
          let items = [];
          if (v.items_json) {
          ...
          const nuevaVenta = {
            id: v.id, fecha: fechaISO, fechaISO, ts: Date.parse(fechaISO) || 0,
          ...
          // Actualizar reportes con esta venta
        ...
        historial.sort((a, b) => (b.ts || 0) - (a.ts || 0));
        _ultimaFechaVenta = historial[0].fechaISO || historial[0].fecha;
        idbSet('vpos_historial', historial).catch(() => {});
        idbSet('vpos_ventasDia', ventasDia).catch(() => {});
        idbSet('vpos_ventasSem', ventasSem).catch(() => {});
        idbSet('vpos_ventasMes', ventasMes).catch(() => {});
        ...
        // Actualizar el corte aunque no haya nuevas
        if (nuevasVentas[0]) _ultimaFechaVenta = nuevasVentas[0].fecha_iso;
      }
    }

    // ── Pagos nuevos ───────────────────────────────────────────────────
    const fechaCortePago = _ultimaFechaPago ||
      (pagos && pagos.length ? (pagos[0].fechaISO || pagos[0].fecha) : null) ||
      new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();

    const paramsPagos = { select: '*', order: 'fecha_iso.desc', limit: 20 };
    if (fechaCortePago) paramsPagos['fecha_iso'] = 'gt.' + fechaCortePago;

    const nuevosPagos = await _sbGet('pagos', paramsPagos).catch(() => null);
    if (nuevosPagos && nuevosPagos.length > 0) {
      const idsLocales = new Set((pagos || []).map(p => String(p.id)));
      const pagosExternos = nuevosPagos.filter(p => !idsLocales.has(String(p.id)));
      if (pagosExternos.length > 0) {
        pagosExternos.forEach(p => {
          const fechaISO = p.fecha_iso || new Date().toISOString();
          pagos.unshift({ id: p.id, fechaISO, fecha: fechaISO, ts: Date.parse(fechaISO) || 0, monto: Number(p.monto) || 0, cat: p.cat || '', nom: p.nom || '', nota: p.nota || '' });
        });
        pagos.sort((a, b) => (b.ts || 0) - (a.ts || 0));
        _ultimaFechaPago = pagos[0].fechaISO;
        idbSet('vpos_pagos', pagos).catch(() => {});
        hayNuevos = true;
      }
      if (nuevosPagos[0]) _ultimaFechaPago = nuevosPagos[0].fecha_iso;
    }

    // ── Productos (stock actualizado por otro dispositivo) ─────────────
    const prodsActualizados = await _sbGet('productos', { select: 'id,stock,nom', limit: 2000 }).catch(() => null);
    if (prodsActualizados && prodsActualizados.length > 0) {
      let cambioStock = false;
      prodsActualizados.forEach(sp => {
        const localId = Number(sp.id) || sp.id;
        // El id en Supabase es tienda_id + '_' + producto_id
        const parts = String(sp.id).split('_');
        const prodId = parts.length > 1 ? (Number(parts[parts.length - 1]) || parts[parts.length - 1]) : localId;
        const p = (productos || []).find(x => String(x.id) === String(prodId));
        if (p && Number(sp.stock) !== p.stock) {
          p.stock = Number(sp.stock);
          cambioStock = true;
        }
      });
      if (cambioStock) {
    ...
    if (hayNuevos && typeof actualizarTodo === 'function') {
      actualizarTodo();
      _dot('green');
      console.log('[Polling] ✅ Datos nuevos aplicados desde otro dispositivo');
    }
  } catch(e) {
