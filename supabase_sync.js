...
function _iniciarPolling() {
  ...
  // BUGFIX SYNC: siempre correr _autoFusionar cada 30s aunque Realtime esté activo.
  // Si un broadcast de venta se pierde (background del navegador, micro-corte de red),
  // la fusión de snapshot recupera la venta en máximo 30 segundos.
  _pollingTimer = setInterval(() => {
    _autoFusionar();
  }, 30000);
  // BUGFIX SYNC: pull de ventas nuevas cada 20s — simple y enfocado.
  // Solo lee la tabla ventas y añade las que no existen localmente.
  // Es el último recurso si broadcast y snapshot fallan.
  if (!_reconcTimer) {
    _reconcTimer = setInterval(_pullNuevasVentas, 20000);
  }
  console.log('[Sync] Realtime iniciado, polling snapshots 30s + pull ventas 20s');
}
