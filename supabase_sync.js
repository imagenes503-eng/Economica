...
async function _autoCargarDesdeSupa() {
  ...
  try {
    ...
    // BUG PREVIO: el fetch filtraba por tienda_id pero deleted_log no tiene esa columna
    // → siempre devolvía vacío → tombstones nunca se aplicaban al teléfono offline
    // → el teléfono offline descargaba la fila $2 de Supabase → Caja mostraba $102.
    try {
      ...
      if (delLog && delLog.length > 0) {
        ...
        if (cambioTombstone) {
          ...
          console.log('[AutoCarga] 🗑 Tombstones desde deleted_log:', ventasDiariasEliminadas);
        }
      }
