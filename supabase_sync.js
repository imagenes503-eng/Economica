  ...
  try {
    ...
    if (prods && prods.length > 0) {
      const sbProds = prods.map(p => ({
        ...
        // Preservar stock_base de Supabase para que _calcStockBase nunca lo disminuya
        stock_base: Number(p.stock_base) || 0,
        _ts: Number(p._ts) || 0
      }));
      ...
      const merged = sbProds.map(sp => {
        ...
        if (local) {
          ...
          if (localInvalido) {
            // Datos locales inválidos → usar Supabase.
            // NO usar Math.max: el stock vendrá de get_stock_calculado inmediatamente después.
            return {
              ...sp,
          ...
          if (sbTs > localTs) {
            // CRÍTICO: NO usar Math.max para el stock. Supabase tiene el stock correcto.
            // get_stock_calculado (llamado abajo) sobreescribirá con el valor definitivo.
            return {
              ...sp,
              stock:    Number(sp.stock) || 0,
              img:      local.img || sp.img || null,
      ...
      // ── STOCK CALCULADO: pedir a Supabase el stock real (stock_base - ventas) ──
      // Esto garantiza que ambos dispositivos siempre tengan el stock correcto
      // independientemente de cuándo abrieron el navegador por última vez.
      _aplicarStockCalculado().catch(e => console.warn('[StockCalc] Error:', e.message));
...
function _aplicarVentaRealtime(v) {
  ...
  // STOCK: al recibir venta de otro dispositivo, aplicar descuento optimista en UI
  // y luego confirmar con el stock calculado por Supabase.
  (function() {
    ...
    // Confirmar con el stock calculado por Supabase (fuente de verdad)
    // Se ejecuta en 500ms para dar tiempo a que la venta esté insertada en BD
    setTimeout(() => {
      _aplicarStockCalculado().catch(() => {});
    }, 500);
  })();
...
function _calcStockBase(p) {
  // Suma todas las ventas locales conocidas para reconstruir el stock original.
  // El resultado = stock_actual + ventas_locales = "stock si nunca se hubiera vendido nada".
  // Este valor se sube a Supabase como stock_base para que la función SQL pueda calcular:
  //   stock_real = stock_base - SUM(todas_las_ventas_en_supabase)
  const pid = String(p.id);
  let vendidoLocal = 0;
  (historial || []).forEach(h => {
    (h.items || []).forEach(it => {
  ...
  const calculado = (p.stock || 0) + vendidoLocal;
  // CRÍTICO: nunca disminuir el stock_base que ya existe en Supabase.
  // Si otro dispositivo hizo un restock, su stock_base será mayor — respetar ese valor.
  return Math.max(calculado, Number(p.stock_base) || 0);
}
// =====================================================================
//  📊 STOCK CALCULADO DESDE SUPABASE
//  Llama a la función SQL get_stock_calculado que devuelve:
//    stock_real = stock_base - SUM(ventas en Supabase)
//  Esto garantiza stock correcto sin importar desde qué dispositivo
//  o cuándo se abrió el navegador por última vez.
// =====================================================================
async function _aplicarStockCalculado() {
  if (!_hayConexion() || !_sbUrl() || !_sbKey()) return;
  const tiendaId = _getTiendaId();
  if (!tiendaId || !productos || !productos.length) return;
  try {
    const resultado = await _sbRpc('get_stock_calculado', { p_tienda_id: tiendaId });
    if (!resultado || !resultado.length) return;

    // Resultado: [{ prod_id: 'tiendaId_12345', stock_actual: 47 }, ...]
    const stockMap = {};
    resultado.forEach(row => {
      // prod_id en el resultado es el ID completo de Supabase (con prefijo de tienda)
      // Necesitamos extraer el ID numérico que usa el frontend
      const rawId = String(row.prod_id || '');
      const u = rawId.lastIndexOf('_');
      const numId = u >= 0 ? rawId.slice(u + 1) : rawId;
      const n = Number(numId);
      const key = isNaN(n) ? rawId : n;
      stockMap[String(key)] = Number(row.stock_actual) || 0;
    });

    let cambios = 0;
    productos.forEach(p => {
      const nuevoStock = stockMap[String(p.id)];
      if (nuevoStock !== undefined && nuevoStock !== p.stock) {
        p.stock = nuevoStock;
        if (typeof actualizarStockFila === 'function') actualizarStockFila(p);
        cambios++;
      }
    ...
    if (cambios > 0) {
      idbSet('vpos_productos', productos).catch(() => {});
      if (typeof actualizarTodo === 'function') actualizarTodo();
      console.log('[StockCalc] ✅', cambios, 'producto(s) actualizados desde Supabase');
    } else {
      console.log('[StockCalc] ✓ Stock ya estaba al día');
    }
  } catch(e) {
    console.warn('[StockCalc] Error al calcular stock:', e.message);
  }
}
window._aplicarStockCalculado = _aplicarStockCalculado;
