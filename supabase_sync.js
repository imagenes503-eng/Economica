...
async function enviarDatosNube() {
  ...
  try {
    const snap = { version: typeof APP_SCHEMA_VERSION!=='undefined'?APP_SCHEMA_VERSION:4, exportado: new Date().toISOString(),
      ...
      productos, ventasDia, ventasSem, ventasMes, historial, pagos, ventasDiarias, restockLog: restockLog||[], cierresMes: typeof cierresMes!="undefined"?cierresMes:[] };
...
function _fusionarDos(local, ext) {
  ...
  // Merge cierresMes — unir historial de cierres de ambos dispositivos
  if (!local.cierresMes) local.cierresMes = [];
  (ext.cierresMes || []).forEach(cExt => {
    const idx = local.cierresMes.findIndex(c => c.mes === cExt.mes);
    if (idx >= 0) {
      // Conservar el más reciente (mayor fechaCierre)
      if ((cExt.fechaCierre || '') > (local.cierresMes[idx].fechaCierre || '')) local.cierresMes[idx] = cExt;
    } else {
      local.cierresMes.push(cExt);
    }
  });
  local.cierresMes.sort((a, b) => b.mes.localeCompare(a.mes));
  return local;
}
...
async function _aplicarDatos(datos) {
  ...
  // Merge cierresMes desde el snapshot (no reemplazar, hacer merge por mes)
  if (Array.isArray(datos.cierresMes) && datos.cierresMes.length) {
    if (typeof cierresMes === 'undefined') cierresMes = [];
    datos.cierresMes.forEach(cExt => {
      const idx = cierresMes.findIndex(c => c.mes === cExt.mes);
      if (idx >= 0) {
        if ((cExt.fechaCierre || '') > (cierresMes[idx].fechaCierre || '')) cierresMes[idx] = cExt;
      } else { cierresMes.push(cExt); }
    });
    cierresMes.sort((a, b) => b.mes.localeCompare(a.mes));
    idbSet('vpos_cierresMes', cierresMes).catch(() => {});
  }
...
async function _autoEnviarSnapshot() {
  ...
  const snap = {
    ...
    datos: JSON.stringify({
      ...
      cierresMes: typeof cierresMes !== "undefined" ? cierresMes : [],
...
function _iniciarRealtime() {
  ...
  try {
    _realtimeChannel = client
      ...
      // BROADCAST: cierres_mes_actualizado — un teléfono hizo cierre de mes
      .on('broadcast', { event: 'cierres_mes_actualizado' }, ({ payload }) => {
        if (!payload || !Array.isArray(payload.cierresMes)) return;
        // Merge: preservar cierres locales que no vengan en el payload
        (payload.cierresMes || []).forEach(cExt => {
          const idx = (cierresMes||[]).findIndex(c => c.mes === cExt.mes);
          if (idx >= 0) { cierresMes[idx] = cExt; } else { cierresMes.push(cExt); }
        });
        cierresMes.sort((a, b) => b.mes.localeCompare(a.mes));
        idbSet('vpos_cierresMes', cierresMes).catch(() => {});
        if (typeof renderHistorialMeses === 'function') renderHistorialMeses();
        _dot('green');
        console.log('[Realtime] 📦 Cierres de mes recibidos:', cierresMes.length);
      })
...
async function _subirVentasDiarias(){
  if(!ventasDiarias||!ventasDiarias.length){
    try{ await _sbDeleteFiltro('ventas_diarias',{fecha:'not.is.null'}); }catch(e){ console.warn('[subirVD] delete vacío:',e.message); }
    return;
  }
...
async function _subirPagos(){
  if(!pagos||!pagos.length){
    try{ await _sbDeleteFiltro('pagos',{id:'not.is.null'}); }catch(e){ console.warn('[subirPagos] delete vacío:',e.message); }
    return;
  }
