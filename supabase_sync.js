...
async function restaurarSesion() {
  ...
  try {
    ...
    // CRÍTICO: await aquí para que la UI muestre datos actualizados desde el primer
    // momento, sin necesitar recargar la página. Sin await, el polling y _autoFusionar
    // pueden correr con datos viejos del caché antes de que Supabase responda.
    await _autoCargarDesdeSupa();
    // Iniciar Realtime (WebSocket) + polling fallback DESPUÉS de tener datos frescos
    _iniciarPolling();
    // Check de fusión DESPUÉS de cargar datos frescos (no antes)
    setTimeout(_autoFusionar, 800);
  } catch(e) {
    ...
    if (prods && prods.length > 0) {
      ...
      const merged = sbProds.map(sp => {
        ...
        if (local) {
          ...
          // Datos locales son más recientes (nombre/precio editado aquí)
          // El stock siempre viene de Supabase — es la fuente de verdad para inventario
          return {
            ...local,
            stock: Number(sp.stock) || 0,
            img:      local.img      || sp.img      || null,
