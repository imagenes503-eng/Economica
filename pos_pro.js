...
// ── Atajos de teclado y scanner HID (USB/Bluetooth) ──────────────────
(function initKeyboardPOS() {
  let _scanBuffer = '';
  let _scanTimer  = null;

  // ── Parámetros del scanner HID ──────────────────────────────────────
  // SCAN_TIMEOUT: tiempo máximo entre caracteres consecutivos del scanner.
  // Los scanners Honeywell/Zebra/Datalogic envían chars cada 1-20 ms,
  // muy por debajo de los 150 ms que un humano tardaría tecleando.
  // 150 ms da margen para scanners lentos o conexiones Bluetooth con latencia.
  const SCAN_TIMEOUT = 150;
  const SCAN_MIN_LEN = 3;   // mínimo de caracteres para considerar un código válido
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (document.getElementById('modalCobro')?.classList.contains('open')) {
      if (((e.key >= '0' && e.key <= '9') || e.key === '.') && !enInput) {
        e.preventDefault(); posProAgregarDigito(e.key); return;
      }
    ...
    // ══ SCANNER HID — lector de código de barras USB/Bluetooth ════════
    // Los scanners envían ráfagas de caracteres (< 20 ms entre chars)
    // seguidas de Enter o Tab como terminador.
    // Capturar en: sin input activo | busquedaVenta | busquedaInventario
    const esScanTarget = !enInput ||
      activeId === 'busquedaVenta' ||
      activeId === 'busquedaInventario';

    if (esScanTarget && e.key.length === 1 && !e.ctrlKey && !e.altKey && !e.metaKey) {
      // Solo caracteres imprimibles (filtra STX, GS, RS que algunos scanners insertan)
      if (e.key >= ' ') {
        _scanBuffer += e.key;
    ...
    // Terminadores: Enter (estándar) o Tab (Honeywell, Zebra en modo HID)
    const esTerminador = e.key === 'Enter' || e.key === 'Tab';

    if (esTerminador) {
      // ── Caso A: scanner llenó el campo inpCod (inventario) ──────────
      // El browser ya volcó los chars al input — leer valor y avanzar foco
      if (activeId === 'inpCod') {
        const val = (document.getElementById('inpCod')?.value || '').trim();
        if (val.length >= SCAN_MIN_LEN) {
          e.preventDefault();
          _scanBuffer = ''; clearTimeout(_scanTimer);
          // Avanzar al campo Nombre si está vacío (flujo de alta rápida)
          const inpNom = document.getElementById('inpNom');
          if (inpNom) {
            inpNom.focus();
            if (typeof toast === 'function') toast('✓ Código escaneado: ' + val);
          }
          return;
        }
      }

      // ── Caso B: buffer acumulado en venta / búsqueda ─────────────────
      if (esScanTarget && _scanBuffer.length >= SCAN_MIN_LEN) {
        // Eliminar caracteres de control que algunos modelos de scanner insertan
        const cod = _scanBuffer.replace(/[^\x20-\x7E\xA0-\xFF]/g, '').trim();
        _scanBuffer = ''; clearTimeout(_scanTimer);
        if (cod.length >= SCAN_MIN_LEN) {
          e.preventDefault();
      ...
      // Limpiar buffer en cualquier Enter/Tab para evitar acumulaciones
      _scanBuffer = '';
    }
  });
})();
...
// ── Escanear código de barras (hardware scanner o cámara) ─────────────
function posProEscanearCodigo(cod) {
  if (typeof productos === 'undefined') return;

  const codClean = (cod || '').trim();
  const codLower = codClean.toLowerCase();
  const codNorm  = codClean.replace(/^0+/, ''); // EAN con ceros iniciales distintos

  // Buscar por código exacto → sin ceros → abreviación → id
  const prod = productos.find(p =>
    (p.cod   && p.cod.toLowerCase()           === codLower)  ||
    (p.cod   && p.cod.replace(/^0+/, '')      === codNorm && codNorm) ||
  ...
  if (prod) {
    ...
    toast(`⚠ Código no encontrado: ${codClean}`, true);
    posProFlashRed();
  }
}
