...
async function abrirScannerVenta() {
  try {
    ...
    if (!modal) { alert('Error: modal del scanner no encontrado'); return; }
    modal.style.display = 'flex';
    setScannerStatus('Iniciando cámara…');
    await iniciarScanner();
  } catch(e) {
    setScannerStatus('⚠ Error al abrir: ' + (e.message || e));
    console.error('[Scanner] abrirScannerVenta:', e);
  }
}
...
async function _loadHtml5Qrcode() {
  ...
  return new Promise((resolve, reject) => {
    // Timeout de 8s por si el CDN falla
    const timeout = setTimeout(() => reject(new Error('Timeout cargando librería de scanner')), 8000);
    const s = document.createElement('script');
    // Intentar CDN principal, con fallback a unpkg
    ...
    s.onerror = () => {
      ...
      s2.onload = () => { clearTimeout(timeout); resolve(); };
      s2.onerror = () => { clearTimeout(timeout); reject(new Error('No se pudo cargar el scanner')); };
...
async function iniciarScanner() {
  ...
  try {
    ...
    // Config simplificada — sin enums que pueden no estar disponibles
    const config = {
      fps: 15,
    ...
    // Agregar formatos solo si los enums están disponibles
    try {
    ...
    // ── 2. Intentar con cámara trasera directamente (sin preStream) ──────
    // No pedimos getUserMedia antes — Html5Qrcode lo maneja internamente
    // y evitamos la condición de carrera de liberar/pedir la cámara dos veces
    _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });

    let iniciado = false;

    // Intento 1: facingMode environment (trasera)
    try {
      ...
      console.warn('[Scanner] environment falló:', e1.message);
    }

    // Intento 2: facingMode user (frontal) como fallback
    if (!iniciado) {
      try {
        try { _html5Qr.clear(); } catch(_) {}
        _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
        ...
        console.warn('[Scanner] user falló:', e2.message);
        throw e2; // propagar al catch externo
      }
    }
    ...
    // Ocultar botones extra que inyecta html5-qrcode
    setTimeout(() => {
...
async function abrirScannerInventario() {
  try {
    ...
    setScannerStatus('⚠ Error al abrir: ' + (e.message || e));
    console.error('[Scanner] abrirScannerInventario:', e);
  }
}
