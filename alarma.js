...
async function iniciarScanner() {
  ...
  try {
    ...
    // Config con FPS alto para escaneo veloz
    const config = {
      fps: 30,
    ...
    // ── FIX BUG 3: Pedir permiso de cámara ANTES de listar dispositivos ──
    // En Android/Chrome, getCameras() devuelve lista vacía si no hay permiso previo.
    // Pedimos acceso directo a la cámara trasera primero; eso fuerza el diálogo de
    // permiso y luego getCameras() ya puede ver todos los dispositivos disponibles.
    let _preStream = null;
    try {
      _preStream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: { ideal: 'environment' } }
      });
    } catch(_) { /* si el usuario niega, el error se maneja abajo en el catch global */ }

    // ── Estrategia: listar cámaras y priorizar la trasera ──────────────
    let camId = null;
    try {
      const cameras = await Html5Qrcode.getCameras();
      if (cameras && cameras.length > 0) {
        // Buscar por label: "back", "environment", "rear", "posterior", "trasera"
        const rear = cameras.find(c =>
          /back|environ|rear|poste|trasera/i.test(c.label || '')
        );
        // Si no hay match por nombre, elegir la ÚLTIMA cámara (en móviles = trasera)
        camId = rear ? rear.id : cameras[cameras.length - 1].id;
      }
    } catch(_) { /* si falla, caer a facingMode */ }

    // Liberar el stream previo ANTES de iniciar Html5Qrcode (evita conflicto)
    if (_preStream) {
      _preStream.getTracks().forEach(t => t.stop());
      _preStream = null;
      await new Promise(r => setTimeout(r, 150)); // pausa para liberar la cámara
    }

    _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });

    if (camId) {
      // Usar ID específico de la cámara trasera — más confiable que facingMode
      await _html5Qr.start(camId, config,
        (codigo) => onCodigoEscaneado(codigo), () => {});
    } else {
      // Fallback: facingMode environment → user
      try {
        await _html5Qr.start({ facingMode: { ideal: 'environment' } }, config,
          (codigo) => onCodigoEscaneado(codigo), () => {});
      } catch(_) {
        try { _html5Qr.clear(); } catch(__) {}
        _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
        await _html5Qr.start({ facingMode: 'user' }, config,
          (codigo) => onCodigoEscaneado(codigo), () => {});
      }
    }
