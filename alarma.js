...
async function iniciarScanner() {
  ...
  try {
    ...
    // ── Pedir permiso y obtener ID de cámara trasera en UN solo paso ──
    // Estrategia: usar getUserMedia directamente sin liberar el stream,
    // luego obtener el deviceId del track activo — sin condición de carrera.
    let camConstraint = { facingMode: { ideal: 'environment' } };

    try {
      // Paso 1: pedir permiso y obtener stream
      const tempStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: 'environment' } } });
      // Paso 2: leer el deviceId del track activo
      const track = tempStream.getVideoTracks()[0];
      const deviceId = track?.getSettings()?.deviceId;
      // Paso 3: cerrar el stream temporal
      tempStream.getTracks().forEach(t => t.stop());
      // Paso 4: esperar que el navegador libere la cámara completamente
      await new Promise(r => setTimeout(r, 400));
      // Paso 5: usar deviceId exacto → sin ambigüedad
      if (deviceId) camConstraint = { deviceId: { exact: deviceId } };
    } catch(_) {
      // Si falla el permiso, el catch global lo captura
    }

    // ── Iniciar Html5Qrcode con la constraint obtenida ─────────────────
    _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });

    try {
      await _html5Qr.start(camConstraint, config,
        (codigo) => onCodigoEscaneado(codigo), () => {});
    } catch(e1) {
      // Fallback 1: facingMode genérico
      try {
        try { _html5Qr.clear(); } catch(_) {}
        _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
        await _html5Qr.start({ facingMode: 'environment' }, config,
          (codigo) => onCodigoEscaneado(codigo), () => {});
      } catch(e2) {
        // Fallback 2: cámara frontal
        try { _html5Qr.clear(); } catch(_) {}
        _html5Qr = new Html5Qrcode('scannerRegion', { verbose: false });
